...
//				echo("data[$j][$i]=$data[$j][$i]<br>");
	...
	for($i=0; $i<count($arr_categories); $i++) {
		...
		if ($rand_dot==1) { // default dot
			for($idot=0; $idot<count($data[$i]);$idot++) {
				...
				$mydot->tooltip("$arr_categories[$i]<br>ค่า:#val#");
			}
		...
		$line[$i]->set_key( $arr_categories[$i], 10 );
		$rand_anime = rand(0,5);
	...
	$x_labels->rotate(20);
	$x_labels->set_labels($arr_series_caption);

	for($j=0; $j < count($arr_series_caption); $j++) {
		if ($j % 2 == 0) {
			$arr_series_caption[$j]="<br>$arr_series_caption[$j]";
		} elseif ($j % 3 == 0) {
			$arr_series_caption[$j]="<br><br>$arr_series_caption[$j]";
		}
	}
...
//	$x_axis->set_3d( 1 );
	...
	$x_axis->stroke(2);
	$x_axis->set_labels_from_array($arr_series_caption);
