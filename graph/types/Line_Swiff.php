...
//	  $chart->SetTTFontsDir("/var/www/html/ttfonts");
	  ...
	  $categories = $categories_list;//echo($categories."<BR>");
	  $chart->SetCategoriesFromString($categories);

	  $arr_series_caption = explode(";", $series_caption_list);//print_r($arr_series_caption);
	  $arr_series = explode("|", $series_list);//print_r($arr_series);
	  ...
	  if(isset($setAxisYMin) && $setAxisYMin != ""){
	  	$chart->SetAxisMinValue(1, $setAxisYMin);
		}
	  if(isset($setAxisYMax) && $setAxisYMax != ""){
	  	$chart->SetAxisMaxValue(1, $setAxisYMax);
		}
