<?php 
 ...
 if ($categories_list) {
	...
	for($i=0; $i < count($arr_series_caption); $i++) {
		$title[$i] = new title($chart_title."-".$arr_series_caption[$i]);
	}	
	$ii=0;
	for($i=0; $i<count($arr_series); $i++){
		...
		if (is_null($dmax[$ii])) { $dmax[$ii] = 0; }
		$tmp = new pie_value($dmax[$ii],"");
		$categories_val=substr($arr_categories[$maxidx[$i]],0,30);
		$tmp->set_label("BIG-$categories_val-$dmax[$ii]","#FF653F",20);
		$data[$ii][$maxidx[$ii]]=$tmp;
		if (!$notnum) $ii++;
	} //for($i=0; $i<count($arr_series); $i++){
