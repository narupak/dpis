...
//	$head_map = array("เลขที่","ชื่อ","นามสกุล","Email","สถานะ","เพศ","วันเกิด","","");
	...
	$column_map[] = "running";	// 0-running number SAH_ID
	$column_map[] = "sql-n-d-select PER_ID from PER_PERSONAL where PER_CARDNO='@1' or '@2' like '%'||PER_NAME||' '||PER_SURNAME^NOTNULL";		// 1- ดึงข้อมูลจาก sql เป็นข้อมูลประเภท s = string (หรือ n = number)
																																											// 		field ที่แสดง ให้เป็นแบบ d = disabled หรือ e = enabled
																																											// 		มีค่า = PER_ID ที่มี PER_CARDNO = ค่าใน text file column ที่ {1}
	$column_map[] = "screen-s-e-SAH_EFFECTIVEDATE|save_date";		// 2-map กับ column 2 ใน text หรือ excel  SAH_EFFECTIVEDATE
	$column_map[] = "php-s-e-MOV_CODE";		// 3-map กับ ตัวแปร ในส่วนของ php MOV_CODE
	$column_map[] = "php-s-e-SAH_SALARY";		// 4-map กับ column 15 ใน text หรือ excel SAH_SALARY
	$column_map[] = "screen-s-e-SAH_DOCNO";		// 5-map กับ ตัวแปรในจอ SAH_DOCNO
	$column_map[] = "screen-s-e-SAH_DOCDATE|save_date";	// 6-map กับ ตัวแปรในจอ  SAH_DOCDATE = save_date(SAH_DOCDATE)
	$column_map[] = "screen-s-e-SAH_ENDDATE|save_date";	// 7-map กับ ตัวแปรในจอ SAH_ENDDATE = save_date(SAH_ENDDATE)
	$column_map[] = "update_user";	// 8-map กับ UPDATE_USER จากระบบ
	$column_map[] = "update_date";	// 9-map กับ UPDATE_DATE จากระบบ
	$column_map[] = "php-s-e-PER_CARDNO";		// 10-map กับ column 1 ใน text หรือ excel  PER_CARDNO
	$column_map[] = "10";	// 11-map กับ column 10 ใน text หรือ excel  SAH_PERCENT_UP
	$column_map[] = "12";	// 12-map กับ column 12 ใน text หรือ excel  SAH_SALARY_UP
	$column_map[] = "13";	// 13-map กับ column 13 ใน text หรือ excel  SAH_SALARY_EXTRA
	$column_map[] = "php-s-e-SAH_SEQ_NO";	// 14-map ตัวแปร ในส่วน php SAH_SEQ_NO
	$column_map[] = "17";	// 15-map กับ column 17 ใน text หรือ excel  SAH_REMARK
	$column_map[] = "php-s-e-LEVEL_NO";	// 16-map ตัวแปร ในส่วน php LEVEL_NO
	$column_map[] = "3";	// 17-map กับ column 3 ใน text หรือ excel  SAH_POS_NO
	$column_map[] = "4";	// 18-map กับ column 4 ใน text หรือ excel  SAH_POSITION
	$column_map[] = "php-s-e-SAH_ORG";	// 19-map ตัวแปร ในส่วน php SAH_ORG
	$column_map[] = "php-s-e-EX_CODE";	// 20-map ตัวแปร ในส่วน php EX_CODE
	$column_map[] = "3";	// 21-map กับ column 3 (SAH_POS_NO) ใน text หรือ excel  SAH_PAY_NO
	$column_map[] = "8";	// 22-map กับ column 8 ใน text หรือ excel SAH_SALARY_MIDPOINT
	$column_map[] = "php-s-e-SAH_KF_YEAR";	// 23-map ตัวแปร ในส่วน php SAH_KF_YEAR
	$column_map[] = "php-s-e-SAH_KF_CYCLE";	// 24-map ตัวแปร ในส่วน php SAH_KF_CYCLE
	$column_map[] = "9";	// 25-map กับ column 9 ใน text หรือ excel SAH_TOTAL_SCORE
	$column_map[] = "php-s-e-SAH_LAST_SALARY";	// 26-map ตัวแปร ในส่วน php SAH_LAST_SALARY
	$column_map[] = "php-s-e-SM_CODE";	// 27-map ตัวแปร ในส่วน php SM_CODE
	if ($SESS_DEPARTMENT_NAME=="กรมพินิจและคุ้มครองเด็กและเยาวชน" || $SESS_DEPARTMENT_NAME=="กรมสรรพสามิต") {
		$column_map[] = "";		// 28-map space SAH_ORG_DOPA_CODE
		$column_map[] = "6";		// 29-map space SAH_OLD_SALARY
		$column_map[] = "php-s-e-SAH_CMD_SEQ";		// 30-map ตัวแปร ในส่วน php SAH_CMD_SEQ
	} else {
		$column_map[] = "php-s-e-SAH_CMD_SEQ";		// 28-map ตัวแปร ในส่วน php SAH_CMD_SEQ
		$column_map[] = "";		// 29-map space SAH_ORG_DOPA_CODE
		$column_map[] = "6";		// 30-map space SAH_OLD_SALARY
	}
...
//			$SAH_ENDDATE =  save_date($SAH_ENDDATE);
				...
				if (!$SAH_SALARY) $SAH_SALARY = $SAH_OLD_SALARY + $SAH_SALARY_UP;
