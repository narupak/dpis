...
//		var PROVINCE_CODE = "<?=($CTRL_TYPE==2 || $SESS_USERGROUP_LEVEL==2)?'$PROVINCE_CODE':''?>";
		...
		if (!window.showModalDialog) {
			...
			data_pass = document.formDialog.data_pass;
			f_fromDialog = document.formDialog.f_fromDialog;
			dataitem = window.open(call_file + "?MENU_ID_LV0="+form1.MENU_ID_LV0.value+"&MENU_ID_LV1="+form1.MENU_ID_LV1.value+"&MENU_ID_LV2="+form1.MENU_ID_LV2.value+"&MENU_ID_LV3="+form1.MENU_ID_LV3.value+parameter,"","dialogHeight: 600px; dialogWidth: 800px; status: No; resizable: No; help: No; statusbar: No;"); 
			dataitem.data_pass = data_pass;
			dataitem.f_fromDialog = f_fromDialog;
		} else {
...
//		var PROVINCE_CODE = "<?=($CTRL_TYPE==2 || $SESS_USERGROUP_LEVEL==2)?'$PROVINCE_CODE':''?>";
		...
		if(MINISTRY_ID != ""){
			...
			if (!window.showModalDialog) {
				...
				data_pass = document.formDialog.data_pass;
				f_fromDialog = document.formDialog.f_fromDialog;
				dataitem = window.open(call_file + "?MENU_ID_LV0="+form1.MENU_ID_LV0.value+"&MENU_ID_LV1="+form1.MENU_ID_LV1.value+"&MENU_ID_LV2="+form1.MENU_ID_LV2.value+"&MENU_ID_LV3="+form1.MENU_ID_LV3.value+parameter,"","dialogHeight: 600px; dialogWidth: 800px; status: No; resizable: No; help: No; statusbar: Yes;"); 
				dataitem.data_pass = data_pass;
				dataitem.f_fromDialog = f_fromDialog;
			} else {
				childReturn = window.showModalDialog(call_file + "?MENU_ID_LV0="+form1.MENU_ID_LV0.value+"&MENU_ID_LV1="+form1.MENU_ID_LV1.value+"&MENU_ID_LV2="+form1.MENU_ID_LV2.value+"&MENU_ID_LV3="+form1.MENU_ID_LV3.value+parameter,"","dialogHeight: 600px; dialogWidth: 800px; status: No; resizable: No; help: No; statusbar: No;");
...
//		var DEPARTMENT_ID = ((ctrl_type==4 || sess_usergroup_level==4)?(form1.DEPARTMENT_ID?form1.DEPARTMENT_ID.value:""):"");
	...
	function returnFromWindowOpen_00(){
		data_pass = document.formDialog.data_pass;
		f_fromDialog = document.formDialog.f_fromDialog;
//		if (f_fromDialog.value) alert("returnFromWindowOpen_00...returnValue="+data_pass.value+" , f_fromDialog ="+f_fromDialog.value);
		childReturn = data_pass.value;
		if (f_fromDialog.value=="search_ministry") {
//			alert("returnFromWindowOpen...returnValue="+data_pass+" , f_fromDialog ="+f_fromDialog);
		...
		} else if (f_fromDialog.value=="search_department") {
			if(childReturn){
				arrValue = childReturn.split("<::>");
				if (form1.DEPARTMENT_ID) form1.DEPARTMENT_ID.value = arrValue[0];
				if (form1.DEPARTMENT_NAME) form1.DEPARTMENT_NAME.value = arrValue[1];
		...
		} else if (f_fromDialog.value=="search_org") {
			if(childReturn){
				arrValue = childReturn.split("<::>");
		...
		} else if (f_fromDialog.value=="search_org_1") {
			if(childReturn){
				arrValue = childReturn.split("<::>");
				if(form1.select_org_structure){
					if(form1.select_org_structure[0].checked){
		...
		} else if (f_fromDialog.value=="search_org_2") {
			if(childReturn){
				arrValue = childReturn.split("<::>");
				if(form1.select_org_structure){
					if(form1.select_org_structure[0].checked){
