...
// holiday=ชุดของวันสำคัญจาก database, f_no_weekholi=ส่งเข้ามาเพื่อให้ check เตือนวันหยุดหรือไม่
...
//		alert("holiday:"+holiday);
		if (holiday && holiday != "undefined") {
			...
			for(i = 0; i < arr_buff.length; i++) {
				...
				hdate = new Date(Number(df[0]), Number(df[1])-1, Number(df[2])); // จาก database มา java month ต้อง -1
...
//				cal.holiday_name.push(buff[1]);	// array of holiday name
				cal.holiday_name.push("หยุดราชการ");	// array of holiday name
			}
		} else {
...
//		alert("date="+el.caldate+"/"+cal.date.getMonth()+"/"+cal.date.getFullYear()+" , "+cal.f_no_weekholi);
		if (cal.f_no_weekholi.length > 0) {	// ถ้าส่งค่าเข้ามา "confirm", "noconfirm"
			...
			var wstr = "";
			if (SAT==wday) { wstr = "เสาร์"; }
			else if (SUN == wday) { wstr = "อาทิตย์"; }
...
//			alert(cal.holiday_date.toString()+">>>>"+rundatestr+"(sat:"+SAT+", sun:"+SUN+", wday:"+wday+")");
			...
			var w_h_alert = "";
			if (wstr) {
				w_h_confirm = "เป็นวัน "+wstr+" ต้องการเลือกวันนี้แน่นอน?..";
				w_h_alert = "เป็นวัน "+wstr+" ไม่สามารถเลือกได้..";
			} else if (idxholiday > -1) {
				w_h_confirm = "เป็นวัน "+cal.holiday_name[idxholiday]+" ต้องการเลือกวันนี้แน่นอน?..";
				w_h_alert = "เป็นวัน "+cal.holiday_name[idxholiday]+" ไม่สามารถเลือกได้..";
			}
			if (w_h_confirm) {
