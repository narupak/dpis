<?	
	...
	if($command=="ADD" && $PJ_YEAR && trim($NEW_PJ_NAME) && $NEW_PJ_PER_ID && $NEW_PFR_ID){
		...
		if(!$NEW_PJ_WEIGHT) $NEW_PJ_WEIGHT = "NULL";
		if(!$NEW_PJ_TARGET_LEVEL1) $NEW_PJ_TARGET_LEVEL1 = "NULL";
		if(!$NEW_PJ_TARGET_LEVEL2) $NEW_PJ_TARGET_LEVEL2 = "NULL";
		if(!$NEW_PJ_TARGET_LEVEL3) $NEW_PJ_TARGET_LEVEL3 = "NULL";
		if(!$NEW_PJ_TARGET_LEVEL4) $NEW_PJ_TARGET_LEVEL4 = "NULL";
		if(!$NEW_PJ_TARGET_LEVEL5) $NEW_PJ_TARGET_LEVEL5 = "NULL";
		if(!$NEW_PJ_EVALUATE1) $NEW_PJ_EVALUATE1 = "NULL";
		if(!$NEW_PJ_EVALUATE2) $NEW_PJ_EVALUATE2 = "NULL";
		if(!$NEW_PJ_EVALUATE3) $NEW_PJ_EVALUATE3 = "NULL";
		if(!$NEW_PJ_EVALUATE) $NEW_PJ_EVALUATE = "NULL";
		
		if (!get_magic_quotes_gpc()) {
			$NEW_PJ_TARGET_LEVEL1_DESC = addslashes(str_replace('"', "&quot;", trim($NEW_PJ_TARGET_LEVEL1_DESC)));
			$NEW_PJ_TARGET_LEVEL2_DESC = addslashes(str_replace('"', "&quot;", trim($NEW_PJ_TARGET_LEVEL2_DESC)));
			$NEW_PJ_TARGET_LEVEL3_DESC = addslashes(str_replace('"', "&quot;", trim($NEW_PJ_TARGET_LEVEL3_DESC)));
			$NEW_PJ_TARGET_LEVEL4_DESC = addslashes(str_replace('"', "&quot;", trim($NEW_PJ_TARGET_LEVEL4_DESC)));
			$NEW_PJ_TARGET_LEVEL5_DESC = addslashes(str_replace('"', "&quot;", trim($NEW_PJ_TARGET_LEVEL5_DESC)));
		}else{
			$NEW_PJ_TARGET_LEVEL1_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($NEW_PJ_TARGET_LEVEL1_DESC))));
			$NEW_PJ_TARGET_LEVEL2_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($NEW_PJ_TARGET_LEVEL2_DESC))));
			$NEW_PJ_TARGET_LEVEL3_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($NEW_PJ_TARGET_LEVEL3_DESC))));
			$NEW_PJ_TARGET_LEVEL4_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($NEW_PJ_TARGET_LEVEL4_DESC))));
			$NEW_PJ_TARGET_LEVEL5_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($NEW_PJ_TARGET_LEVEL5_DESC))));
		} // end if
		...
		$cmd = " insert into PER_PROJECT
							(PJ_ID, PJ_NAME, PJ_WEIGHT, PJ_MEASURE, PJ_PER_ID,ORG_NAME, UNDER_ORG_NAME1, PFR_ID,
							 PJ_TARGET_LEVEL1, PJ_TARGET_LEVEL2, PJ_TARGET_LEVEL3, PJ_TARGET_LEVEL4, PJ_TARGET_LEVEL5,
							 PJ_TARGET_LEVEL1_DESC, PJ_TARGET_LEVEL2_DESC, PJ_TARGET_LEVEL3_DESC, PJ_TARGET_LEVEL4_DESC, 
							 PJ_TARGET_LEVEL5_DESC, PJ_EVALUATE,
							 PJ_ID_REF, PJ_YEAR, DEPARTMENT_ID, UPDATE_USER, UPDATE_DATE, PJ_RESULT, ORG_ID)
						values
							($PJ_ID, '$NEW_PJ_NAME', $NEW_PJ_WEIGHT, '$NEW_PJ_MEASURE', $NEW_PJ_PER_ID, '$NEW_ORG_NAME', '$NEW_UNDER_ORG_NAME1', $NEW_PFR_ID,
							 $NEW_PJ_TARGET_LEVEL1, $NEW_PJ_TARGET_LEVEL2, $NEW_PJ_TARGET_LEVEL3, $NEW_PJ_TARGET_LEVEL4, 
							 $NEW_PJ_TARGET_LEVEL5,	'$NEW_PJ_TARGET_LEVEL1_DESC', '$NEW_PJ_TARGET_LEVEL2_DESC', 
							 '$NEW_PJ_TARGET_LEVEL3_DESC', '$NEW_PJ_TARGET_LEVEL4_DESC', '$NEW_PJ_TARGET_LEVEL5_DESC', 
							 $NEW_PJ_EVALUATE, $PJ_ID_REF, '$PJ_YEAR', $DEPARTMENT_ID, $SESS_USERID, '$UPDATE_DATE', '$PJ_RESULT', $ORG_ID) ";
...
//	$db_dpis->show_error();
	...
	if($command=="UPDATE" && $PJ_ID && $PJ_YEAR && trim($PJ_NAME) && $PJ_PER_ID && $PFR_ID){
		if(!$PJ_WEIGHT) $PJ_WEIGHT = "NULL";
		if(!$PJ_TARGET_LEVEL1) $PJ_TARGET_LEVEL1 = "NULL";
		if(!$PJ_TARGET_LEVEL1) $PJ_TARGET_LEVEL1 = "NULL";
		if(!$PJ_TARGET_LEVEL2) $PJ_TARGET_LEVEL2 = "NULL";
		if(!$PJ_TARGET_LEVEL3) $PJ_TARGET_LEVEL3 = "NULL";
		if(!$PJ_TARGET_LEVEL4) $PJ_TARGET_LEVEL4 = "NULL";
		if(!$PJ_TARGET_LEVEL5) $PJ_TARGET_LEVEL5 = "NULL";
		if(!$PJ_EVALUATE1) $PJ_EVALUATE1 = "NULL";
		if(!$PJ_EVALUATE2) $PJ_EVALUATE2 = "NULL";
		if(!$PJ_EVALUATE3) $PJ_EVALUATE3 = "NULL";
		if(!$PJ_EVALUATE) $PJ_EVALUATE = "NULL";
		if(!$ORG_ID) $ORG_ID = "NULL";
		if (!get_magic_quotes_gpc()) {
			$PJ_WEIGHT= addslashes(str_replace('"', "&quot;", trim($PJ_WEIGHT)));
			$PJ_EVALUATE= addslashes(str_replace('"', "&quot;", trim($PJ_EVALUATE)));
			$PJ_TARGET_LEVEL1_DESC = addslashes(str_replace('"', "&quot;", trim($PJ_TARGET_LEVEL1_DESC)));
			$PJ_TARGET_LEVEL2_DESC = addslashes(str_replace('"', "&quot;", trim($PJ_TARGET_LEVEL2_DESC)));
			$PJ_TARGET_LEVEL3_DESC = addslashes(str_replace('"', "&quot;", trim($PJ_TARGET_LEVEL3_DESC)));
			$PJ_TARGET_LEVEL4_DESC = addslashes(str_replace('"', "&quot;", trim($PJ_TARGET_LEVEL4_DESC)));
			$PJ_TARGET_LEVEL5_DESC = addslashes(str_replace('"', "&quot;", trim($PJ_TARGET_LEVEL5_DESC)));
		}else{
			$PJ_WEIGHT = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_WEIGHT))));
			$PJ_EVALUATE = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_EVALUATE))));
			$PJ_TARGET_LEVEL1_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_TARGET_LEVEL1_DESC))));
			$PJ_TARGET_LEVEL2_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_TARGET_LEVEL2_DESC))));
			$PJ_TARGET_LEVEL3_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_TARGET_LEVEL3_DESC))));
			$PJ_TARGET_LEVEL4_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_TARGET_LEVEL4_DESC))));
			$PJ_TARGET_LEVEL5_DESC = addslashes(str_replace('"', "&quot;", stripslashes(trim($PJ_TARGET_LEVEL5_DESC))));
		} // end if
		...
		$cmd = " update PER_PROJECT set
							PJ_NAME='$PJ_NAME', 
							PJ_WEIGHT=$PJ_WEIGHT,
							PJ_MEASURE='$PJ_MEASURE',
							PJ_PER_ID='$PJ_PER_ID',
							ORG_ID='$ORG_ID',
							ORG_NAME='$UNDER_ORG_NAME',
							UNDER_ORG_NAME1='$UNDER_ORG_NAME1',
							PFR_ID='$PFR_ID',
							PJ_TARGET_LEVEL1=$PJ_TARGET_LEVEL1,
							PJ_TARGET_LEVEL2=$PJ_TARGET_LEVEL2,
							PJ_TARGET_LEVEL3=$PJ_TARGET_LEVEL3,
							PJ_TARGET_LEVEL4=$PJ_TARGET_LEVEL4,
							PJ_TARGET_LEVEL5=$PJ_TARGET_LEVEL5,
							PJ_TARGET_LEVEL1_DESC='$PJ_TARGET_LEVEL1_DESC',
							PJ_TARGET_LEVEL2_DESC='$PJ_TARGET_LEVEL2_DESC',
							PJ_TARGET_LEVEL3_DESC='$PJ_TARGET_LEVEL3_DESC',
							PJ_TARGET_LEVEL4_DESC='$PJ_TARGET_LEVEL4_DESC',
							PJ_TARGET_LEVEL5_DESC='$PJ_TARGET_LEVEL5_DESC',
							PJ_EVALUATE = $PJ_EVALUATE,
							PJ_RESULT='$PJ_RESULT', 
...
//		echo "Data :: <pre>"; print_r($data); echo "</pre>";
	...
	function list_tree_pj ($pre_image, $pj_parent, $sel_pj_id, $tree_depth) {
		...
		if ($BKK_FLAG==1) $order_str = "ORG_ID, PJ_TYPE, PJ_NAME";
		else $order_str = "ORG_ID, PJ_NAME";
