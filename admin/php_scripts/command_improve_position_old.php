...
//		foreach($list_confirm_id as $ORD_ID){
			...
			$cmd = " select 		POS_ID_OLD, ORD_POS_NO, OT_CODE, PL_CODE, PM_CODE, CL_NAME,  PT_CODE, SKILL_CODE, PC_CODE, 
			...
			while($data = $db_dpis->get_array()){
				...
				$cmd = " insert into PER_POS_MOVE	(POS_ID, POS_DATE, OT_CODE, PL_CODE, PM_CODE, CL_NAME, PT_CODE, 
								 ...
								 values ($POS_ID_OLD, '$POS_DATE', '$PREV_OT_CODE', '$PREV_PL_CODE', '$PREV_PM_CODE', '$PREV_CL_NAME', '$PREV_PT_CODE',
				...
				insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > บันทึกประวัติตำแหน่ง [". $POS_ID_OLD ." : ".trim($ORD_POS_NO)." : ".$PREV_PL_CODE."]");
				...
				$cmd = " update PER_POSITION set 
								 ...
								 where POS_ID=$POS_ID_OLD and trim(POS_NO) = '$ORD_POS_NO' ";
				$db_dpis2->send_cmd($cmd);
				//$db_dpis2->show_error();

				insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ปรับปรุงตำแหน่ง [". $POS_ID_OLD ." : ".trim($ORD_POS_NO)." : ".$PL_CODE."]");

				echo "<script language='JavaScript'>";
				echo "parent.refresh_opener('1<::>!<::>!<::>1<::><::>')";
				echo "</script>";
			} // end while
...
//		$db_dpis->show_error();
		...
		if($ORD_CONFIRM == 1){
			// ยืนยันคำสั่ง
			$cmd = " select 		POS_ID_OLD, ORD_POS_NO, OT_CODE, PL_CODE, PM_CODE, CL_NAME,  PT_CODE, SKILL_CODE, PC_CODE, 
												ORD_CONDITION, ORD_SALARY, ORD_MGTSALARY, ORG_ID, ORG_ID_1, ORG_ID_2, ORG_ID_3, ORG_ID_4, 
												ORG_ID_5, LEVEL_NO, ORD_REMARK
							 from 			PER_ORDER_DTL 
							 where 		ORD_ID=$ORD_ID and ORD_RESULT=1
							 order by 	ORD_SEQ	  ";
			$db_dpis->send_cmd($cmd);
...
//		$db_dpis->show_error();
	...
	if($UPD || $VIEW){
		$cmd = " select 		ORD_YEAR, ORD_NO, ORD_TITLE, ORD_DATE, ORD_CONFIRM, DEPARTMENT_ID 
				   from 		PER_ORDER 
				   where 	ORD_ID=$ORD_ID ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		...
		$cmd = " select 	ORD_POS_NO, ORD_EFF_DATE, ORD_RESULT 
						 from 		PER_ORDER_DTL 
						 where 	ORD_ID=$ORD_ID ";
		$db_dpis->send_cmd($cmd);
		while($data = $db_dpis->get_array()){
			if(trim($data[ORD_EFF_DATE]) == ""){
				$ARR_DETAIL_ERR[] = "เลขที่ตำแหน่ง $data[ORD_POS_NO] ยังไม่ได้ระบุวันที่มีผลบังคับใช้";
			}elseif(trim($data[ORD_RESULT]) == ""){
				$ARR_DETAIL_ERR[] = "เลขที่ตำแหน่ง $data[ORD_POS_NO] ยังไม่ได้ระบุผลการตรวจสอบคำสั่ง";
			} // end if
		} // end while
	} // end if
