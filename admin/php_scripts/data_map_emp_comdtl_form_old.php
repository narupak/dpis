<?	
	...
	if($command=="ADD"){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > $MENU_TITLE_LV2 > เพิ่มข้อมูลลูกจ้างแนบท้ายบัญชีคำสั่งจัดระบบตำแหน่งลูกจ้างประจำ [ $COM_ID : $PER_ID ]");
	...
	if($command=="UPDATE" && $COM_ID){
		...
		//$db_dpis->show_error();
		$UPD=1;
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > $MENU_TITLE_LV2 > แก้ไขข้อมูลลูกจ้างแนบท้ายบัญชีคำสั่งจัดระบบตำแหน่งลูกจ้างประจำ [ $COM_ID : $PER_ID ]");
	} // end if
	...
	if($command=="DELETE" && $COM_ID){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > $MENU_TITLE_LV2 > ลบข้อมูลลูกจ้างแนบท้ายบัญชีคำสั่งจัดระบบตำแหน่งลูกจ้างประจำ [ $COM_ID : $PER_ID ]");		
	} // end if
	...
	if( (($PAGE_AUTH["add"] && trim($CMD_SEQ_LAST)>0) || $UPD || $VIEW) && trim($COM_ID) && trim($CMD_SEQ) ){
		...
		if($UPD || $VIEW){		//เฉพาะ ดู และแก้ไข
			...
			$db_dpis1->send_cmd($cmd);
			$data1 = $db_dpis1->get_array();
			$PL_PN_NAME_ASSIGN = trim($data1[PN_NAME]);
