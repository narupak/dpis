...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับวุฒิการศึกษา [$EN_CODE : $EN_NAME]");
	} // end if
...
//$db_dpis->show_error(); echo "<br>";
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($EN_SEQ_NO==''){
			...
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > $MENU_TITLE_LV2 > $menu_detail เพิ่มข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[3]."]");
			$success_sql = "เพิ่มข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[3]."]เรียบร้อยแล้ว"; //กำหนดในส่วนของ php
