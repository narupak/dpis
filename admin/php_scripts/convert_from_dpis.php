...
//  	if(strtoupper($dpisdb_user)=="KP" || strtoupper($dpisdb_user)=="EXCISE") 
  	if(strtoupper($dpisdb_user)=="HIPPS") 
		$where = "(PER_HIP_FLAG like '%1%')";
  	elseif(strtoupper($dpisdb_user)=="HISDB" || strtoupper($dpisdb_user)=="SES") 
		$where = "(LEVEL_NO IN ('09','10','11','O4','K4','K5','D1','D2','M1','M2'))";
  	elseif(strtoupper($dpisdb_user)=="STOU") 
		$where = "(1 = 1)";
...
// หมู่โลหิต บัญชีอัตราเงินเดือนลูกจ้างประจำ 
...
// บัญชีอัตราเงินเดือนข้าราชการใหม่ ชื่อตำแหน่งพนักงานราชการ
	if( $command=='ORG' ) { // โครงสร้างส่วนราชการ  
		...
		$cmd = " SELECT ORG_ID, ORG_CODE, ORG_NAME, ORG_SHORT, OL_CODE, OT_CODE, OP_CODE, OS_CODE, 
						...
						FROM PER_ORG  WHERE DEPARTMENT_ID = $DEPT_ID ORDER BY ORG_ID ";
		$db_dpis35->send_cmd($cmd);
		$db_dpis35->show_error();
		while($data = $db_dpis35->get_array()){
			$PER_ORG++;
		...
		$cmd = " SELECT ORG_ID, ORG_CODE, ORG_NAME, ORG_SHORT, OL_CODE, OT_CODE, OP_CODE, OS_CODE, 
						ORG_ADDR1, ORG_ADDR2, ORG_ADDR3, AP_CODE, PV_CODE, CT_CODE, ORG_DATE, ORG_JOB, ORG_ID_REF, 
						ORG_ACTIVE, UPDATE_USER, UPDATE_DATE, ORG_WEBSITE, ORG_SEQ_NO 
						FROM PER_ORG_ASS  WHERE DEPARTMENT_ID = $DEPT_ID ORDER BY ORG_ID ";
		$db_dpis35->send_cmd($cmd);
		$db_dpis35->show_error();
		while($data = $db_dpis35->get_array()){
			$PER_ORG_ASS++;
...
//			$db_dpis2->show_error();
		...
		while($data = $db_dpis35->get_array()){
			...
			$cmd = " SELECT POS_NO FROM PER_POSITION WHERE POS_NO = $POS_NO ";
	...
	if( $command=='PERSONAL' ) { // ข้อมูลข้าราชการ
		...
		$cmd = " SELECT PER_ID, PER_TYPE, OT_CODE, PN_CODE, PER_NAME, PER_SURNAME, PER_ENG_NAME,
						...
						FROM PER_PERSONAL WHERE DEPARTMENT_ID = $DEPT_ID AND $where ORDER BY PER_ID ";
		$db_dpis35->send_cmd($cmd);
		//$db_dpis35->show_error();
		while($data = $db_dpis35->get_array()){
			$PER_PERSONAL++;
...
//			$db_dpis2->show_error();
			...
			$db_dpis2->show_error();
			$data2 = $db_dpis2->get_array();
			if ($data2[NEW_CODE]) $ORG_ID = $data2[NEW_CODE] + 0;

			$cmd = " select NEW_CODE from PER_MAP_CODE where MAP_CODE = 'PER_ORG' AND OLD_CODE = $ORG_ID_ASSIGN ";
			$db_dpis2->send_cmd($cmd);
			$db_dpis2->show_error();
			$data2 = $db_dpis2->get_array();
			if ($data2[NEW_CODE]) $ORG_ID_ASSIGN = $data2[NEW_CODE] + 0;
