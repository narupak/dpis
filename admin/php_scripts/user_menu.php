...
//		echo $auth_list."<br>";
		...
		for($i=0; $i<count($arr_auth); $i++){
			...
			$cmd = " select group_id from user_privilege where group_id=$group_id and page_id=$auth_detail[1] and menu_id_lv0=$auth_detail[2] and 
							menu_id_lv1=$auth_detail[3] and menu_id_lv2=$auth_detail[4] and menu_id_lv3=$auth_detail[5] ";
			...
			if(!$count_data) {
				$cmd = " insert into user_privilege (group_id, page_id, menu_id_lv0, menu_id_lv1, menu_id_lv2, menu_id_lv3, 
								 ...
								 values ($group_id, $auth_detail[1], $auth_detail[2], $auth_detail[3], $auth_detail[4], $auth_detail[5], '$auth_detail[6]', 
								 '$auth_detail[7]', '$auth_detail[8]', '$auth_detail[9]', '$auth_detail[10]', '$auth_detail[11]', '$auth_detail[12]', '$auth_detail[13]') ";
				$db->send_cmd($cmd);
			} else {
				$cmd = " update user_privilege 
								 ...
								 where group_id=$group_id and page_id=$auth_detail[1] and menu_id_lv0=$auth_detail[2] and 
								 menu_id_lv1=$auth_detail[3] and menu_id_lv2=$auth_detail[4] and menu_id_lv3=$auth_detail[5] ";
				$db->send_cmd($cmd);
				//$db->show_error();
			}
...
//		echo $auth_list."<br>";
		...
		for($j=0; $j < count($ARRSELECTED_PER_ID); $j++) {
			...
			for($i=0; $i<count($arr_auth); $i++){
				$auth_detail = explode(",", $arr_auth[$i]);
				$cmd = " select group_id from user_privilege where group_id=$ARRSELECTED_PER_ID[$j] and page_id=$auth_detail[1] and menu_id_lv0=$auth_detail[2] and 
								menu_id_lv1=$auth_detail[3] and menu_id_lv2=$auth_detail[4] and menu_id_lv3=$auth_detail[5] ";
				if(!$db->send_cmd($cmd)) :
					$cmd = " insert into user_privilege (group_id, page_id, menu_id_lv0, menu_id_lv1, menu_id_lv2, menu_id_lv3, 
									 can_add, can_edit, can_del, can_inq, can_print, can_confirm, can_audit, can_attach)
									 values ($ARRSELECTED_PER_ID[$j], $auth_detail[1], $auth_detail[2], $auth_detail[3], $auth_detail[4], $auth_detail[5], '$auth_detail[6]', 
									 '$auth_detail[7]', '$auth_detail[8]', '$auth_detail[9]', '$auth_detail[10]', '$auth_detail[11]', '$auth_detail[12]', '$auth_detail[13]') ";
					$db->send_cmd($cmd);
				else :
					$cmd = " update user_privilege 
									 set	can_add = '$auth_detail[6]',
											...
											can_audit = '$auth_detail[12]',
											can_attach = '$auth_detail[13]'
									 where group_id=$ARRSELECTED_PER_ID[$j] and page_id=$auth_detail[1] and menu_id_lv0=$auth_detail[2] and 
									 menu_id_lv1=$auth_detail[3] and menu_id_lv2=$auth_detail[4] and menu_id_lv3=$auth_detail[5] ";
					$db->send_cmd($cmd);
				endif;
//				$db->show_error();
...
//						$db->show_error();
						...
						$auth_list .= ",".(($data[can_audit])?$data[can_audit]:"N");
						$auth_list .= ",".(($data[can_attach])?$data[can_attach]:"N");
...
//	echo $init_auth."<br>".$auth_list;
	...
	function list_program($page_id, $table1, $table2, $table3, $table4, $flag_show, $menu_depth){
		...
		global $group_open_depth;

//		echo "group_open_depth=$group_open_depth ";
...
//			$db_lv0->show_error();
			...
			while( $data_lv0=$db_lv0->get_array() ) :
				...
				$init_auth .= $group_id.",".$page_id.",".$data_lv0[menu_id].",0,0,0,N,N,N,N,N,N,N,N";
				...
				if($menu_depth>=2){
					...
					$count1 = $db_lv1->send_cmd($cmd);
//					$db_lv1->show_error();
					$key_openlist = "1-".$data_lv0[menu_id];
//					echo "[key $key_openlist in array =".array_search($key_openlist, $arr_open_list)."] ";
					...
					while( $data_lv1=$db_lv1->get_array() ){
						...
						if($init_auth) $init_auth .= "|";
						$init_auth .= $group_id.",".$page_id.",".$data_lv0[menu_id].",".$data_lv1[menu_id].",0,0,N,N,N,N,N,N,N,N";
						...
						if($menu_depth>=3){
							...
							$count2 = $db_lv2->send_cmd($cmd);
//							$db_lv2->show_error();
							$k = -1;
							$key_openlist1 = "2-".$data_lv1[menu_id];
//							echo "[key $key_openlist in array =".array_search($key_openlist1, $arr_open_list)."] ";
							if ($count2 && array_search($key_openlist1, $arr_open_list)!==false) {
								$arr_program_lv1[$page_id][$data_lv0[menu_id]][$j]["flag"] = "<font color='#FF0000'><B>-</B></font>";
							} else if ($count2) {
								$arr_program_lv1[$page_id][$data_lv0[menu_id]][$j]["flag"] = "<font color='#0000FF'><B>+</B></font>";
							} else {
								$arr_program_lv1[$page_id][$data_lv0[menu_id]][$j]["flag"] = "<font color='#CCCCCC'><B>x</B></font>";
							} // end if ($count2 && array_search($key_openlist1, $arr_open_list)!==false)
							while( $data_lv2=$db_lv2->get_array() ) :
								...
								if($init_auth) $init_auth .= "|";
								$init_auth .= $group_id.",".$page_id.",".$data_lv0[menu_id].",".$data_lv1[menu_id].",".$data_lv2[menu_id].",0,N,N,N,N,N,N,N,N";
						...
						//			echo $cmd."<br>";
									$count3 = $db_lv3->send_cmd($cmd);
//									$db_lv3->show_error();
									$l = -1;
									$key_openlist2 = "3-".$data_lv2[menu_id];
//									echo "[key $key_openlist in array =".array_search($key_openlist2, $arr_open_list)."] ";
									if ($count3 = array_search($key_openlist2, $arr_open_list)!==false) {
										$arr_program_lv2[$page_id][$data_lv0[menu_id]][$data_lv1[menu_id]][$k]["flag"] = "<font color='#FF0000'><B>-</B></font>";
									} else if ($count3) {
										$arr_program_lv2[$page_id][$data_lv0[menu_id]][$data_lv1[menu_id]][$k]["flag"] = "<font color='#0000FF'><B>+</B></font>";
									} else {
										$arr_program_lv2[$page_id][$data_lv0[menu_id]][$data_lv1[menu_id]][$k]["flag"] = "<font color='#CCCCCC'><B>x</B></font>";
									} // endif ($count3 && array_search($key_openlist1, $arr_open_list)!==false)
									while( $data_lv3=$db_lv3->get_array() ){
										...
										if($init_auth) $init_auth .= "|";
										$init_auth .= $group_id.",".$page_id.",".$data_lv0[menu_id].",".$data_lv1[menu_id].",".$data_lv2[menu_id].",".$data_lv3[menu_id].",N,N,N,N,N,N,N,N";
									} // end while
								} // end if menu_depth >= 4
							endwhile;
						} // end if menu_depth >= 3
