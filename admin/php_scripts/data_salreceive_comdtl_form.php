<?	
	...
	if($command=="ADD" || $command=="UPDATE"){
		...
		$CMD_RCV_SALARY = str_replace(",", "", $CMD_RCV_SALARY) + 0;	
...
//	echo ">>((".$PAGE_AUTH["add"]." && ".trim($CMD_SEQ_LAST).") || ".$UPD_PER." || ".$VIEW_PER.") && ".trim($COM_ID)." && ".trim($CMD_SEQ)."<br>";
	if((($PAGE_AUTH["add"]=="Y" && trim($CMD_SEQ_LAST)>0) || $UPD_PER || $VIEW_PER) && trim($COM_ID) && trim($CMD_SEQ)){
		...
		$cmd = "	select	CMD_SEQ, PER_ID, EN_CODE, CMD_DATE, CMD_POSITION, 
							CMD_LEVEL, CMD_ORG1, CMD_ORG2, CMD_ORG3, CMD_ORG4, CMD_ORG5, 
							CMD_OLD_SALARY, PL_CODE, PN_CODE, EP_CODE, CMD_AC_NO, CMD_ACCOUNT, 
...
//		echo "cmd=$cmd<br>";
		...
		if($UPD_PER || $VIEW_PER){		//เฉพาะ ดู และแก้ไข		
			...
			$CMD_RCV_SALARY = number_format(trim($data[CMD_SALARY]), 2, '.', ','); 
...
//				$cmd = " select LEVEL_NO from PER_LEVEL where LEVEL_NO < '$CMD_LEVEL_full' order by LEVEL_NO desc ";
			...
			//ข้อมุลการศึกษา
			$cmd = "select EN_NAME, EN_SHORTNAME,b.INS_CODE, b.EDU_ENDDATE
							from 	PER_EDUCNAME a, PER_EDUCATE b 
							where trim(a.EN_CODE)='$EN_CODE' and 
										PER_ID='$PER_ID' and
										a.EN_CODE=b.EN_CODE ";		
			$db_dpis1->send_cmd($cmd);
			...
			//วันสำเร็จการศึกษา
			$EDU_ENDDATE = show_date_format($data_dpis1[EDU_ENDDATE], 1);
			//หาชื่อสถานศึกษา
			$INS_CODE = trim($data_dpis1[INS_CODE]);
			if($INS_CODE){
			...
			if ($COM_PER_TYPE == 2) {
				...
				$cmd = " 	select 	POEM_NO_NAME, POEM_NO, a.PN_CODE, PN_NAME, ORG_ID, ORG_ID_1, ORG_ID_2, a.DEPARTMENT_ID 
			...
			if ($COM_PER_TYPE == 3) {
				...
				$cmd = " 	select 	POEMS_NO_NAME, POEMS_NO, a.EP_CODE, EP_NAME, ORG_ID, ORG_ID_1, ORG_ID_2, a.DEPARTMENT_ID 
