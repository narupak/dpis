...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $DC_CODE => $DC_SEQ_NO){
		if($DC_SEQ_NO=="") { $cmd = " update PER_DECORATION set DC_SEQ_NO='' where DC_CODE='$DC_CODE' "; }
		else { $cmd = " update PER_DECORATION set DC_SEQ_NO=$DC_SEQ_NO where DC_CODE='$DC_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับชั้นเครื่องราชอิสริยาภรณ์ [$DC_CODE : $DC_NAME]");
	} // end if
...
//$db_dpis->show_error(); echo "<br>";
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		$cmd = " select $arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4], $arr_fields[5], $arr_fields[8]
		...
		if($DC_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', '".$$arr_fields[1]."', '".$$arr_fields[2]."', ".$$arr_fields[3].", ".$$arr_fields[4].", ".$$arr_fields[5].", $SESS_USERID, '$UPDATE_DATE', '".$$arr_fields[8]."',$DC_SEQ_NO, '".$$arr_fields[10]."', ".$$arr_fields[11].") ";
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
			...
			$success_sql = "เพิ่มข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[2]."]เรียบร้อยแล้ว"; //กำหนดในส่วนของ php
...
//		$db_dpis->show_error();
	...
	if($UPD){
		$cmd = " select $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4], $arr_fields[5], $arr_fields[8], $arr_fields[10], $arr_fields[11], UPDATE_USER, UPDATE_DATE 
