...
function thaiWrap($dataIn){
	...
	if (preg_match("/(([ก-ฮ])([่|้|๊|๋|็])?อก)/",$dataIn,$match)) { // กอก ออก ยอก นอก บอก ศอก ร๊อก ช๊อก วอก ลอก บ๊อก
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])([[ล|ร]])?([่|้|๊|๋|็])?อย)/",$dataIn,$match)) { // กลอย หรอย ปล่อย หอย คอย ง่อย
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ห])([ว|ม|ล|น])?ี([่|้|๊|๋])?)/",$dataIn,$match)) { // หวี หมี่ หี หลี หมี่ หนี หนี้ หวี่
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก|ฉ])วี))/",$dataIn,$match)) { // กวี ฉวี
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ])([ก-ฮ])([ก-ฮ])?(ิ|ี|ึ|ื)?([่|้|๊|๋])?(ย)?(ร|ว|ม|น|ง)(ย์|ร์|ส์)?)/",$dataIn,$match)) { // เกียรติยศ
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])([ก-ฮ])?ั(่|้|๊|๋)?([บ|ม|น|ง|ว]))/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ])([ก-ฮ])([่|้|๊|๋])?ว([ม|น|ง])?)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(โ([ก-ฮ])([ร|ล])?(่|้|๊|๋)?([ะ|ย|ม|น|ว|ง|ด])?)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])([ก-ฮ])?(่|้|๊|๋)?า([ก-ฮ]))/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(เ([ก-ฮ])ียน)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(เ([ก-ฮ])ือน)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(เ([ก-ฮ])็ม)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])([ิ|ี|ึ|ื|่|้|๊|๋|ั|ุ|ู])?[ก-ฮ])/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ])([ก-ฮ])([่|้|๊|๋])า)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ])([ก-ฮ])([ร|ล])?(ิ|ี|ึ|ื)([่|้|๊|๋])?อน)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ])([ก-ฮ])([่|้|๊|๋|็])?[ก|ด|ต|บ|ป|ฟ|พ|ง|ส|ว|ท])/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([เ|แ|ใ|ไ|โ])([ก-ฮ])([่|้|๊|๋])?)/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])(ิ|ี|ึ|ื)([่|้|๊|๋])?[อ|ม|น|ร|ล])/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(([ก-ฮ])([ล|ว|ร])ั?[อ|ม|น|ร|ล|ด|ท|ก|ต])/",$dataIn,$match)) {
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
	...
	if (preg_match("/(มา(?:ก|ย|ตรการ|ตรา|ตุภูมิ|ชิก))/",$dataIn,$match)) { // มา
		$pos_i = strpos($dataIn, $match[0]);
		if ($pos_i < $min_pos_i) {
			$min_pos_i = $pos_i;
			$ret_len = strlen($match[0]);
