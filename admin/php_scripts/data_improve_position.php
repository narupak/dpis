...
//		foreach($list_confirm_id as $ORD_ID){
			$cmd = " select ORD_YEAR, ORD_NO, ORD_TITLE from PER_ORDER_POS where ORD_ID=$ORD_ID ";
			...
			$cmd = " update PER_ORDER_POS set ORD_CONFIRM=1 where ORD_ID=$ORD_ID ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis2->show_error();
			...
			while($data = $db_dpis->get_array()){
				...
				$cmd = " insert into PER_POS_MOVE	(POS_ID, POS_DATE, OT_CODE, PL_CODE, PM_CODE, CL_NAME, PT_CODE, 
								 ...
								 values ($POS_ID_OLD, '$ORD_DATE', '$PREV_OT_CODE', '$PREV_PL_CODE', '$PREV_PM_CODE', '$PREV_CL_NAME', $PREV_PT_CODE_tmp,
				...
				insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > บันทึกประวัติตำแหน่ง [". $POS_ID_OLD ." : ".$ORD_POS_NO_NAME ." : ".trim($ORD_POS_NO)." : ".$PREV_PL_CODE."]");
				...
				insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ปรับปรุงตำแหน่ง [". $POS_ID_OLD ." : ".$ORD_POS_NO_NAME ." : ".trim($ORD_POS_NO)." : ".$PL_CODE."]");
			} // end while
			
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ยืนยันข้อมูล [". $ORD_ID ." : ".trim($ORD_YEAR)." : ".$ORD_NO." : ".$ORD_TITLE."]");
//		} // end foreach
	} // end if
...
//		echo "add select - $cmd (count_duplicate=$count_duplicate)<br>";
		if($count_duplicate <= 0){
			$cmd = " select max(ORD_ID) as max_id from PER_ORDER_POS ";
			$db_dpis->send_cmd($cmd);
			$data = $db_dpis->get_array();
			$data = array_change_key_case($data, CASE_LOWER);
			$ORD_ID = $data[max_id] + 1;

			$cmd = " insert into PER_ORDER_POS (ORD_ID, ORD_YEAR, ORD_NO, ORD_TITLE, ORD_DATE, ORD_CONFIRM, DEPARTMENT_ID, UPDATE_USER, UPDATE_DATE) 
						   values ($ORD_ID, '".trim($ORD_YEAR)."', '".$ORD_NO."', '".$ORD_TITLE."', '".$ORD_DATE."', 0, $DEPARTMENT_ID, $SESS_USERID, '$UPDATE_DATE') ";
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
//			echo "add insert - $cmd<br>";

			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูล [".$ORD_ID." : ".trim($ORD_YEAR)." : ".$ORD_NO." : ".$ORD_TITLE." : ".$ORD_DATE."]");
		}else{
			$data = $db_dpis->get_array();			
			$err_text = "รหัสข้อมูลซ้ำ [$DEPARTMENT_NAME ".$data[ORD_YEAR]." ".$data[ORD_NO]." ".$data[ORD_TITLE]."]";
		} // endif
	}

	if($command == "UPDATE" && trim($ORD_ID) && trim($ORD_YEAR) && trim($ORD_NO)){
		$cmd = " update PER_ORDER_POS set 
							ORD_YEAR='".$ORD_YEAR."', 
							ORD_NO='".$ORD_NO."', 
							ORD_TITLE='".$ORD_TITLE."', 
							ORD_DATE='".$ORD_DATE."', 
...
//		echo "update insert - $cmd<br>";
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูล [".$ORD_ID." : ".trim($ORD_YEAR)." : ".$ORD_NO." : ".$ORD_TITLE." : ".$ORD_DATE."]");
	}
	...
	if($command == "DELETE" && trim($ORD_ID)){
		...
		$cmd = " delete from PER_ORDER_POS_DTL where ORD_ID=$ORD_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
		
		$cmd = " delete from PER_ORDER_POS where ORD_ID=$ORD_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูล [". $ORD_ID ." : ".trim($ORD_YEAR)." : ".$ORD_NO." : ".$ORD_TITLE."]");
	}
	...
	if($UPD || $VIEW){
		...
		while($data = $db_dpis->get_array()){
			if(trim($data[ORD_EFF_DATE]) == ""){
				$ARR_DETAIL_ERR[] = "เลขที่ตำแหน่ง $data[ORD_POS_NO_NAME]$data[ORD_POS_NO] ยังไม่ได้ระบุวันที่มีผลบังคับใช้";
			}elseif(trim($data[ORD_RESULT]) == ""){
				$ARR_DETAIL_ERR[] = "เลขที่ตำแหน่ง $data[ORD_POS_NO_NAME]$data[ORD_POS_NO] ยังไม่ได้ระบุผลการพิจารณา";
			} // end if
		} // end while
	} // end if
