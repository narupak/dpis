...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $AT_CODE => $AT_SEQ_NO){
		if($AT_SEQ_NO=="") { $cmd = " update PER_ATTACHMENT set AT_SEQ_NO='' where AT_CODE='$AT_CODE' "; }
		else { $cmd = " update PER_ATTACHMENT set AT_SEQ_NO=$AT_SEQ_NO where AT_CODE='$AT_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับเอกสารแนบ [$AT_CODE : $AT_NAME]");
	} // end if
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($AT_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', ".$$arr_fields[1].", ".$$arr_fields[2].", ".$$arr_fields[3].", 
							$SESS_USERID, '$UPDATE_DATE', $AT_SEQ_NO, ".$$arr_fields[7].") ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
