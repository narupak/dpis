...
if($db_type == "oci8"){
	...
	// เคสยกเลิกวันลา By Kittiphat  08/02/2561
    ...
    //สร้าง Table 2018/02/03  เคสกรมปศุสัตว์ ผู้ตรวจสอบวันลาไม่สามารถตรวจสอบได้
    ...
    //ไม่มี HRG 
    if(!$cnt_data){
       $cmd = " select ORG_ID from PER_CONTROL "; //	หากรม จากฐานข้อมูลทีเชื่อมต่ออยู่
		$db_dpis->send_cmd($cmd);	
        ...
        $cmdIns = "Insert into user_group (ID,CODE,NAME_TH,NAME_EN,ACCESS_LIST,GROUP_LEVEL,ORG_ID,PV_CODE,DPISDB,DPISDB_HOST,DPISDB_NAME,DPISDB_USER,DPISDB_PWD,GROUP_PER_TYPE,GROUP_ORG_STRUCTURE,GROUP_ACTIVE,GROUP_SEQ_NO,CREATE_DATE,CREATE_BY,UPDATE_DATE,UPDATE_BY,LEVEL_NO_LIST) 
                                           values ($MAXOFID,'HRG','กลุ่มผู้ตรวจสอบการลา','กลุ่มผู้ตรวจสอบการลา',',1,',4,$ORG_ID_TMP,null,null,null,null,null,null,0,2,1,8,sysdate,1,sysdate,1,null)"; 
...
if($data){
    ...
    if($cnt_chk==0){
        ...
        $cmd_insert ="INSERT INTO SYSTEM_CONFIG (CONFIG_ID,CONFIG_NAME,CONFIG_VALUE,CONFIG_REMARK) 
            select max(CONFIG_ID)+1 as CONFIG_ID,'P_EXPIATE' as CONFIG_NAME,'N' as CONFIG_VALUE,'N-ไม่ต้องชดเชยเวลา, Y-ชดเชยเวลา' as CONFIG_REMARK from SYSTEM_CONFIG ";
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (62, 'xlsFmtTitle_color', '8^000000', 'การแสดงสีอักษรใน excel แบบ Title 1') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (63, 'xlsFmtTitle_bgcolor', '9^FFFFFF', 'การแสดงสีพี้นใน excel แบบ Title 1') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (64, 'xlsFmtSubTitle_color', '8^000000', 'การแสดงสีตัวอักษรใน excel แบบ Title 2') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (65, 'xlsFmtSubTitle_bgcolor', '9^FFFFFF', 'การแสดงสีพี้นใน excel แบบ Title 2') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (66, 'xlsFmtTableHeader_color', '8^000000', 'การแสดงสีตัวอักษรหัวตาราง excel') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (67, 'xlsFmtTableHeader_bgcolor', '9^FFFFFF', 'การแสดงสีพี้นหัวตาราง excel') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (68, 'xlsFmtTableDetail_color', '8^000000', 'การแสดงสีตัวอักษรในตาราง excel') ";
$db->send_cmd($cmd);
...
if (!$count_data)
    $cmd = " insert into system_config (config_id, config_name, config_value, config_remark) 
						values (69, 'xlsFmtTableDetail_bgcolor', '9^FFFFFF', 'การแสดงสีพี้นในตาราง excel') ";
$db->send_cmd($cmd);
...
//		$db_dpis->show_error();
    ...
    if ($command == "LOGIN" && $username != "" && ($password != "" || $fpass_pwd == 1)) {
        ...
        //ปรับแก้ใหม่ ให้ทำงานกับ MySQL ได้
        if ($db_type == "oci8"){
             $cmdChk="SELECT COUNT(*) AS CNT FROM USER_TAB_COLS WHERE TABLE_NAME = 'USER_DETAIL' AND UPPER(COLUMN_NAME) = 'USER_FLAG'";
             $db->send_cmd($cmdChk);
             $dataChk = $db->get_array();
        ...
        if ($data[create_by] == 'LDAP') {
            ...
            if (!$ad) {
                ...
                if ($fpass_pwd == 1) { // รับ $_POST  บอกให้ไม่สนใจ password ก็จะไม่ check password
...
//		$db->show_error();
        if ($result && $isUserActive==1) {
            $data = $db->get_array();
            $data = array_change_key_case($data, CASE_LOWER);
            // กรณี เป็น user ที่ ยกเลิกไปแล้ว
...
//echo $group_active.','.$pwd_age.','.$diff.','.$pwd_diff;
            ...
            if ($group_active != 1) {
//				echo "ผู้ใช้ $username ถูกยกเลิกไปแล้ว";
                $ERR = 10;
                // จบ กรณี เป็น user ที่ ยกเลิกไปแล้ว
...
//				echo "password expire...";
                ...
                $SESS_PASSWORD_KEY = $password; //เชคเพื่อให้เปลี่ยนรหัสผ่าน ไม่ให้เป็นวันเดอืนปีเกิด
...
//				echo "2.2. UID=$SESS_USERID, UNAME=$SESS_USERNAME, ULINK=$user_link_id $SESS_FIRSTNAME $SESS_LASTNAME<br>";
                ...
                //กำหนดให้  1=แบบประเมินผลการปฏิบัติราชการ   2=ใบลา   3=สลิปเงินเดือน   4=หนังสือแจ้งผลการเลื่อนเงินเดือน 5=หนังสือรับรอง   (จาก C06 กำหนดค่าโปรแกรม) 
...
//echo ">>>".$SESS_MINISTRY_NAME;
                if ($SESS_MINISTRY_NAME != "กรุงเทพมหานคร") {
                    $MSG_HEADER = 'Patch Update DPIS Release 5.2.1.24';
                    
                    $MSG_DETAIL = '1.เมนู S0203 ตำแหน่งพนักงานราชการ
 -ปรับปรุงช่องทักษะประสบการณ์ ให้กรอกข้อมูลได้มากขึ้นและจำกัดไว้ที่ 2000
2.เมนู P0101 ข้อมูลบุคคล
 -ปรับปรุงช่องหมายเหตุตำแหน่งให้หยิบจากช่องหมายเหตุตำแหน่งในประวัติการดำรงตำแหน่งที่เป็นตำแหน่งล่าสุดมาแสดง
 -ปรับปรุงการเเสดงชื่อสังกัดให้แสดงตามระดับที่มีการตั้งค่าไว้ที่ C06 
3.หน้าแรก ส่วนของการแสดง หนังสือเวียนอิเล็กทรอนิกส์-หนังสือคำสั่งอิเล็กทรอนิกส์
 -ปรับปรุงให้สามารถส่งหนังสือเวียนได้
4.เมนู P0601 การลา
 -ปรับปรุงการส่งออกไฟล์ excel โดยการเพิ่ม วันที่อนุญาต กับ ชื่อผู้อนุญาต ไปในไฟล์
 -ปรับปรุงปุ่มตัวเลือกหน่วยงานเดิมไม่รองรับ IE จะไม่สามารถคลิกได้
 -ปรับปรุงฟังชันก์การคำนวนอายุราชการ
5.เมนู K08
 -ปรับปรุงส่วนสิทธิเรื่องการมองเห็นปุ่มการบันทึกอนุญาตให้เห็นคะแนน จะเห็นได้ในบทบาทของผู้ประเมินเท่านั้น
6.เมนู H19 บัญชีรายชื่อข้าราชการ
 -ปรับปรุงตัวเลือกประเภทบุคลากรให้เห็นแต่ข้าราชการเท่านั้น
7.เมนู R0402 รายชื่อข้าราชการที่เกษียณอายุในสิ้นปีงบประมาณ
 -ปรับปรุงให้สามารถเลือกจำแนกหน่วยงานภายใต้สำนัก / กองได้
8.เมนู P0119 สอบถามข้อมูล
 -ปรับปรุงในจังหวะการออกรายงาน กรณีเลือกประเภทบุคคลากรเป็น ลูกจ้างประจำ หรือ พนักงานราชการ หรือ ลูกจ้างชั่วคราว ให้ปิดส่วนของคอลั่มระดับตำแหน่ง ( ตั้งแต่คอลั่มสายงานระดับ - คอลั่มบริหารสูง )
9.เมนู R0407 รายชื่อข้าราชการ ตำแหน่ง สังกัด
 -ปรับปรุงการเรียงข้อมูลกรณีเลือกเงื่อนไขออกรายงาน เรียงลำดับสำนักกอง> ต่ำกว่าสำนักกอง
10.เมนู R0406 ประวัติข้าราชการ
 -ปรับปรุงการเเสดงชื่อสังกัดให้แสดงตามระดับที่มีการตั้งค่าไว้ที่ C06';
                    $MSG_SOURCE = 'สำนักงาน ก.พ.';
                    $MSG_POST_DATE = '2018-02-01';
                    $MSG_START_DATE = '2018-09-01';
                    $MSG_FINISH_DATE = '2018-09-30';
                    $USER_ID = 1;
                    $MSG_TYPE = 0;
                    $MSG_DOCUMENT = '';
                    $MSG_ORG_NAME = '';
...
//				echo "4. UID=$SESS_USERID, UNAME=$SESS_USERNAME, ULINK=$user_link_id $SESS_FIRSTNAME $SESS_LASTNAME<br>";
                ...
                manage_login(""); // สร้าง รายการ user_last_access สำหรับ เก็บ login
//				setcookie(session_name(), session_id($session_id), time()+28800, "/");
                ...
                session_register("SESS_E_SIGN");  // ย้ายมากจากบรรทัด 716
                ...
                if ($SESS_PER_ID) { //เฉพาะล็อกอินเป็นคน
...
//				echo "PER_ID=$SESS_PER_ID $SESS_FIRSTNAME $SESS_LASTNAME<br>";
                // สร้าง session วันหยุดนักขตฤกษ์
                $holi_year = (string) ((int) substr($today, 0, 4) - 10) . "-0-0";
                $cmd = " select * from PER_HOLIDAY where HOL_DATE > '$holi_year' and HOL_NAME != 'วันเสาร์' and HOL_NAME != 'วันอาทิตย์' ";
...
//						$arr_holi[] = trim($data['hol_date'])."|".trim($data['hol_name']);
                ...
                // จบการสร้าง session วันหยุดนักขตฤกษ์
...
//keng                    header("Location: http://$HTTP_HOST/$session_id/" . $virtual_site . "admin/main_1.html");
                else
                    //echo "Location: http://$HTTP_HOST/$session_id/" . $virtual_site . "admin/main.html";
                    header("Location: /$session_id/" . $virtual_site . "admin/main.html");
//keng                    header("Location: http://$HTTP_HOST/$session_id/" . $virtual_site . "admin/main.html");
