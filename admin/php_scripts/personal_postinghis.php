...
//		echo "$setflagaudit";
		$cmd = " update PER_POSTINGHIS set AUDIT_FLAG = 'N' where POST_ID in (".stripslashes($current_list).") ";
		$db_dpis->send_cmd($cmd);
//		echo "$cmd<br>";
//		$db_dpis->show_error();
		$cmd = " update PER_POSTINGHIS set AUDIT_FLAG = 'Y' where POST_ID in (".stripslashes($setflagaudit).") ";
		$db_dpis->send_cmd($cmd);
//		echo "$cmd<br>";
//		$db_dpis->show_error();
	...
	if($command=="ADD" && $PER_ID){
		...
		if($DPISDB=="oci8")
			$cmd2 = " select POST_STARTDATE,POST_ENDDATE from PER_POSTINGHIS  
		...
		if($count) { ?>  <script> <!--  
			...
			$cmd = " insert into PER_POSTINGHIS (POST_ID, PER_ID, PER_CARDNO, POST_STARTDATE, POST_ENDDATE, POST_ENDTIME, POST_POSITION, 
							POST_ORG_NAME, POST_TEL, POST_DOCNO, POST_DOCDATE, POST_REMARK, POST_REMARK_NAME, UPDATE_USER, UPDATE_DATE)
							values ($POST_ID, $PER_ID, '$PER_CARDNO', '$POST_STARTDATE', '$POST_ENDDATE', '$POST_ENDTIME', '$POST_POSITION', 
							'$POST_ORG_NAME', '$POST_TEL', '$POST_DOCNO', '$POST_DOCDATE', '$POST_REMARK', '$POST_REMARK_NAME', $SESS_USERID, '$UPDATE_DATE') ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
	...
	if($command=="UPDATE" && $PER_ID && $POST_ID){
		...
		if($count) { ?>  <script> <!--  
			...
			$cmd = " UPDATE PER_POSTINGHIS SET
							...
							POST_REMARK_NAME='$POST_REMARK_NAME', 
	...
	if($command=="DELETE" && $PER_ID && $POST_ID){
		$cmd = " select POST_ORG_NAME from PER_POSTINGHIS where POST_ID=$POST_ID ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		$POST_ORG_NAME = $data[POST_ORG_NAME];
		
		$cmd = " delete from PER_POSTINGHIS where POST_ID=$POST_ID ";
		$db_dpis->send_cmd($cmd);
	...
	if(($UPD && $PER_ID && $POST_ID) || ($VIEW && $PER_ID && $POST_ID)){
		$cmd = " select	POST_STARTDATE, POST_ENDDATE, POST_ENDTIME, POST_POSITION, POST_ORG_NAME, 
										POST_TEL, POST_DOCNO, POST_DOCDATE, POST_REMARK, POST_REMARK_NAME, UPDATE_USER, UPDATE_DATE
						from		PER_POSTINGHIS
						where	POST_ID=$POST_ID  ";	
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		...
		$POST_REMARK_NAME = $data[POST_REMARK_NAME];
