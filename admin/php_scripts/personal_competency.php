<?	
	...
	if($command == "UPDATE_SCORE"){
		foreach($_salary_flag as $update_id => $value) {
			$cmd = " update PER_KPI_FORM set 
							SALARY_FLAG = '".$_salary_flag[$update_id]."',
							ORG_ID_ASS = '".$org_id_ass[$update_id]."',
							ORG_ID_SALARY = '".$org_id_salary[$update_id]."',
...
//			$db_dpis->show_error();
//			echo $cmd . "<br>";
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ??????????? [".$search_kf_year." : ".$search_kf_cycle."]");
		}
	}
?>