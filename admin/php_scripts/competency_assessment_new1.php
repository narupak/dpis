<?	
	...
	if($command == "SAVE"){
		...
		if(!$PAGE_ID){
			...
			$cmd = " insert into PER_COMPETENCY_FORM (CF_ID, KF_ID, CF_TYPE, CF_STATUS, UPDATE_USER, UPDATE_DATE,CF_PER_ID)
							 values ($MAX_ID, $KF_ID, $CF_TYPE, 4, $SESS_USERID, '$UPDATE_DATE','$KF_ID') ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			
			$cmd = " select CF_ID from PER_COMPETENCY_FORM where KF_ID=$KF_ID and CF_TYPE = '$CF_TYPE' and 
							UPDATE_USER=$SESS_USERID and UPDATE_DATE='$UPDATE_DATE' and CF_PER_ID = '$KF_ID'";
			$db_dpis->send_cmd($cmd);
			$db_dpis->show_error();
			$data = $db_dpis->get_array();
			$CF_ID = $data[CF_ID];
			...
			$cmd = " insert into PER_COMPETENCY_ANSWER (CF_ID, QS_ID, CA_ANSWER, CA_DESCRIPTION, UPDATE_USER, 
							  UPDATE_DATE,CL_CODE)
							  values	($CF_ID, $QS_ID, $ANSWER_ID, '$DESCRIPTION', $SESS_USERID, '$UPDATE_DATE', '$RAND_CP_CODE') ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			// skip question
			if(!$ANSWER_SCORE) {
	...
	//		echo "$SESS_PASSED_QUESTION<br>";
			...
			// ========================== Prepare Data ===============================		
			$cmd = "select  CL_CODE, count(CL_CODE) as POINT from PER_COMPETENCY_ANSWER where CA_ANSWER > 2 
							and CF_ID=$CF_ID group by CL_CODE order by  CL_CODE";
			$db_dpis->send_cmd($cmd);
			while($data = $db_dpis->get_array()){
				$temp_cl_code = $data[CL_CODE];
				$temp_point = $data[POINT];
				$ARR_SELECTED_ANSWER[] = "$temp_cl_code:$temp_point";
			}
