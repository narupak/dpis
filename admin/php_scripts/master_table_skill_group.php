<?	
	...
	$start_record = ($current_page - 1) * $data_per_page;
        ...
        if($command == "UPDATESKILLLOW43"){
            ...
            $strAlert =  "<font color=green>ปรับปรุงโครงสร้างงานความเชี่ยวชาญเรียบร้อย</font>";
        }
        /*Release 5.2.1.9  End*/
...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับคำด้านความเชี่ยวชาญ [$SG_CODE : $SG_NAME]");
	} // end if
...
//$db_dpis->show_error(); echo "<br>";
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		if($menu_detail == "ด้านความเชี่ยวชาญ >") { $$arr_fields[2] = $PL_CODE1;}
