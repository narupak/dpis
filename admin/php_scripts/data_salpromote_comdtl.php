...
//		echo "$ORG_ID_DTL // $RPT_N // $SALQ_YEAR //   $ORG_ID --- $ORG_ID_ASS ";
		...
		if ($RPT_N) {
			...
			if ($COM_LEVEL_SALP == 1) {
				if ($BKK_FLAG==1) { // 1=ระดับปฏิบัติงาน ชำนาญงาน อาวุโส ปฏิบัติการ ชำนาญการ 
					$where_com_level_salp = " and b.PER_STATUS=1 and SALP_YN=1 and b.LEVEL_NO in ('O1','O2','O3','K1','K2') ";		
				} else { // 1=ระดับปฏิบัติงาน ชำนาญงาน อาวุโส ปฏิบัติการ ชำนาญการ ชำนาญการพิเศษ อำนวยการต้น
					$where_com_level_salp = " and b.PER_STATUS=1 and SALP_YN=1 and b.LEVEL_NO in ('O1','O2','O3','K1','K2','K3','D1') ";		
				}
			} elseif ($COM_LEVEL_SALP == 2) {					// 2=ระดับทักษะพิเศษ เชี่ยวชาญ ทรงคุณวุฒิ อำนวยการสูง บริหารต้น บริหารสูง
				$where_com_level_salp = " and b.PER_STATUS=1 and SALP_YN=1 and b.LEVEL_NO in ('O4','K4','K5','D2','M1','M2') ";
			} elseif ($COM_LEVEL_SALP == 3) { 					
				if ($BKK_FLAG==1) { // 3=เงินเพิ่มพิเศษ ระดับปฏิบัติงาน ชำนาญงาน อาวุโส ปฏิบัติการ ชำนาญการ 
					$where_com_level_salp = " and b.PER_STATUS=1 and SALP_PERCENT>0 and b.LEVEL_NO in ('O1','O2','O3','K1','K2') ";	
				} else { // 3=เงินเพิ่มพิเศษ ระดับปฏิบัติงาน ชำนาญงาน อาวุโส ปฏิบัติการ ชำนาญการ ชำนาญการพิเศษ อำนวยการต้น
					$where_com_level_salp = " and b.PER_STATUS=1 and SALP_PERCENT>0 and b.LEVEL_NO in ('O1','O2','O3','K1','K2','K3','D1') ";	
				}
			} elseif ($COM_LEVEL_SALP == 4) {					// 4=เงินเพิ่มพิเศษ ระดับทักษะพิเศษ เชี่ยวชาญ ทรงคุณวุฒิ อำนวยการสูง บริหารต้น บริหารสูง
				...
				$where_com_level_salp = "";
			} elseif ($COM_LEVEL_SALP == 11) {				// 11=ระดับชำนาญการพิเศษ อำนวยการต้น
				$where_com_level_salp = " and b.PER_STATUS=1 and SALP_YN=1 and b.LEVEL_NO in ('K3','D1') ";		
			} elseif ($COM_LEVEL_SALP == 13) { 				// 13=เงินเพิ่มพิเศษ ระดับชำนาญการพิเศษ อำนวยการต้น 	
		...
		while ($data = $db_dpis->get_array()) {
			...
			if ($BKK_FLAG==1) {
				$cmd = " 	select MOV_CODE from PER_MOVMENT 
									where trim(MOV_NAME)='เลื่อนขั้น $TMP_SALP_LEVEL ขั้น' ";
			...
			if ($OT_CODE == "03") 
				if (!$CMD_ORG5 && !$CMD_ORG4 && $CMD_ORG2=="กรมการปกครอง") 
...
// ============================================================
	...
	if (trim($COM_ID)) {
		...
		$COM_LEVEL_SALP = $data[COM_LEVEL_SALP];
                ...
                //หารอบการประเมินจาก ลงวันที่ของคำสั่ง
                if($ORI_COM_DATE ){
