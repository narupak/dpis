...
//		$db_dpis->show_error();
	...
	if($command=="ADD" && $PER_ID){
		...
		$db_dpis->send_cmd($cmd);
		//$db_dpis->show_error();
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มผลงานดีเด่น [$PER_ID : $EP_ID : $EP_DESC]");
		$ADD_NEXT = 1;
	} // end if
	...
	if($command=="UPDATE" && $PER_ID && $EP_ID){
		...
		$cmd = " UPDATE PER_EXCELLENT_PERFORMANCE SET
					...
					WHERE EP_ID=$EP_ID ";
		$db_dpis->send_cmd($cmd);
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขผลงานดีเด่น [$PER_ID : $EP_ID : $EP_DESC]");
	} // end if
	...
	if($command=="DELETE" && $PER_ID && $EP_ID){
		...
		$db_dpis->send_cmd($cmd);
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบผลงานดีเด่น [$PER_ID : $EP_ID : $EP_DESC]");
	} // end if
