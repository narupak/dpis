...
//	echo ">> ORD_ID=$ORD_ID, ORD_SEQ=$ORD_SEQ, ORD_CONFIRM=$ORD_CONFIRM, UPD_DTL=$UPD_DTL, VIEW_DTL=$VIEW_DTL<br>";
	...
	if($command == "ADD" && trim($ORD_SEQ) && trim($ORD_POS_NO)){
		...
		if($count_duplicate <= 0){
			if(!$CL_NAME)	$CL_NAME = "";
			$cmd = " insert into PER_ORDER_DTL (ORD_ID, ORD_SEQ, ORD_POS_NO_NAME, ORD_POS_NO, ORD_EFF_DATE, ORD_RESULT, POS_ID_OLD,
...
//		$db_dpis->show_error();
	...
	if(($UPD_DTL || $VIEW_DTL) && $ORD_SEQ) {
		$cmd = " select 	ORD_SEQ, ORD_POS_NO_NAME, ORD_POS_NO, ORD_EFF_DATE, ORD_RESULT, POS_ID_OLD, OT_CODE, PL_CODE, 
										...
										ORG_ID_1, ORG_ID_2, ORG_ID_3, ORG_ID_4, ORG_ID_5, LEVEL_NO, ORD_REMARK, UPDATE_USER, UPDATE_DATE
				  		 from 		PER_ORDER_DTL 
						 where 	ORD_ID=$ORD_ID and ORD_SEQ=$ORD_SEQ ";
		$db_dpis->send_cmd($cmd);
...
//============ http://dpis.ocsc.go.th/Service/node/1978	=====================	
			$cmd ="select ORD_CONFIRM from PER_ORDER where ORD_ID=$ORD_ID";
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$CONFIRM = $data2[ORD_CONFIRM];
			$cmd = " select 	OT_CODE, PL_CODE, PM_CODE, CL_NAME,  PT_CODE, SKILL_CODE, PC_CODE, POS_CONDITION,
											POS_SALARY, POS_MGTSALARY, ORG_ID, ORG_ID_1, ORG_ID_2, ORG_ID_3, ORG_ID_4, ORG_ID_5, LEVEL_NO, POS_REMARK
					  		 from 		PER_POSITION 
							 where 	POS_ID=$POS_ID_OLD 	
							 ORDER by UPDATE_DATE desc ";
			if($CONFIRM == 1){//????????????????????? ??????????????? PER_POSITION ????????????????????????????? PER_POS_MOVE
				$cmd = str_replace("PER_POSITION","PER_POS_MOVE",$cmd);
			}
             if($debug==1){echo "<pre>".$cmd."<br>";}
			$db_dpis->send_cmd($cmd);
			$data = $db_dpis->get_array();
			//echo $CONFIRM."<pre>$CONFIRM".$cmd;
