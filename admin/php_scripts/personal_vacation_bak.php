...
//		$db_dpis->show_error();
	...
	if($command == 'COPY_ALL' || $command == 'COPY_NEW') {
		if($search_org_id){
		...
		$END_DATE_2 = $search_vc_year-543 . "-09-30"; //ปรับปรุง
			...
			$PER_STARTDATE = trim($data[PER_STARTDATE]);
                        ...
                        if($debug==0){echo __LINE__.'<br><br>'.$CHECK_STARTDATE." >= ".$PER_STARTDATE."<br>";}
			if ($CHECK_STARTDATE >= $PER_STARTDATE) { /*บรรจุก่อนวันที่ 1 เมษายน*/
                               // echo 'today : '.date('Y-m-d').',CHECK_DATE:'.$CHECK_DATE.'<br>';
                                if($debug==0){echo __LINE__.'<br>CHECK_DATE:'.$CHECK_DATE." , PER_STARTDATE:".$PER_STARTDATE."<br>";}
				$AGE_DIFF = date_difference($CHECK_DATE, $PER_STARTDATE, "full");/* เชค ว่า ฟังชันนี้คำนวณอย่างไร*/
				$arr_temp = explode(" ", $AGE_DIFF);
                                /*เพิ่มเติม*/
                                $AGE_DIFF_NEXT_YEAR = date_difference($CHECK_DATE_NEXT_YEAR, $PER_STARTDATE, "full");
                                $arr_temp_next_year = explode(" ", $AGE_DIFF_NEXT_YEAR);
                                
                                $AGE_DIFF_BEFORE_YEAR = date_difference($CHECK_DATE_BEFORE_YEAR, $PER_STARTDATE, "full");
                                ...
                                if($test==1){
                                    ...
                                    if($debug==0){echo 'Governor_Age_next_year=>'.$Governor_Age_next_year.',Age==>'.$Governor_Age;}
								  ...
								  WHERE AC_FLAG = 1 AND ($arr_temp[2]/12) < AC_GOV_AGE ";
                                            ...
                                            if($debug==0){echo __LINE__.'AC_COLLECT=>'.$AC_COLLECT.'==><pre>'.$cmd;}
                                        }
                                        ...
                                        if($Governor_Age>=1600){ //between 1 to 10 =20 day
                                            $Leave_Cumulative=10;
                                        ...
                                        if($debug==0){echo __LINE__.'<br>sum_absday==>'.$sum_absday.'<br>';}
                                        ...
                                        $db_dpis1->send_cmd($cmd);
                                        if($debug==0){echo __LINE__.'<br><pre>'.$cmd.'<br>';}
                                        $data1 = $db_dpis1->get_array();
                                        ...
                                        if($debug==0){echo __LINE__.':Before_vc_day==>'.$Before_vc_day.','.$Before_abs_day_vacation.'<br>';}
                                        if($debug==0){echo __LINE__.':'.$CHECK_DATE_NEXT_YEAR.' อายุราชการปีที่คลิกประมวลผล ==>'.$Governor_Age.'<br>';}
                                        
                                        if($Before_vc_day>0){
                                            $Leave_Cumulative=$Leave_Cumulative+$Before_vc_day;
                                        }
                                        if($debug==0){echo __LINE__.'<br>Leave_Cumulative==>'.$Leave_Cumulative.'<br>';}
                                    }else{ // N
                                        $Leave_Cumulative=0;
                                    }
                                    ...
                                    if($debug==0){echo __LINE__.'<br>'.$Governor_Age.'>=600 && '.$Leave_Cumulative.'>'.$AC_COLLECT.'<br>';}
...
select nvl(sum(abs_day),0) sum_abs  from (
  ...
  union all
    ...
    $cmd=str_ireplace(":FIRST_DATE","'".$TMP_START_DATE."'",$cmd);
    $cmd=str_ireplace(":END_DATE","'".$TMP_END_DATE."'",$cmd);
