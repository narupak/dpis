...
//		echo "size=".$_FILES["TEXT_FILE"]["size"] ." > 500000<br>";
    if ($_FILES["TEXT_FILE"]["size"] > 2000000) {
...
//			echo "max_id-$cmd ($MAX_ID)<br>";
    ...
    if ($imptype == "xls") {
        ...
        $excel_data = excel_read($config);
			...
			//echo "<pre> con =".$config."excel_data =".$excel_data;
        $tmp_row_num = 1;
        foreach ($excel_data as $row) {
//				if($inx) {
//					echo "163row=".implode("|",$row)."<br>";
            $values = implode("'^'", $row);
//					$fields = explode(',',$values);
//					$fields = str_replace("'", "", $fields);
//					$data = implode(",", $fields);
            //$data = str_replace("'", "", $values);
					//echo "169excel values=".$values."  data=".$data."<br>";
          ...
          if(strlen($num) == 3 && $im_data==0){
             $data = $num."^".$str;  
             if($data){
                insert2temp($data, "^");
             }
           }else if(strlen($num) == 7 && $im_data==1){
             $data = $num."^".$str;  
             if($data){
                insert2temp($data, "^");
             }
           }else{
              echo  "<script> alert('นำเข้าข้อมูลไม่ถูกต้อง'); window.location='read_exel.html?form=read_exel_map.php'; </script>";
           }
            //insert2temp($data, "^");
//				} else {
//					$inx = 1;
                    ...
                    if ($column_map[$arr_fields[$i]]) {
                        ...
                        if ($column_map[$arr_fields[$i]] == "running") {
                            ...
                            if ($search_per_type == "1") { //ข้าราชการ
                                if (strtoupper($a_dat[1]) == "POH_POS_NO" || strtoupper($a_dat[1]) == "POH_POS_NO") { //เลขที่ตำแหน่ง
                                    $a_colname[] = $a_dat[1];
...
//						echo "$i--".$data_fld[$i]."<br>";
               ...
               // echo "<pre>";
               // print_r($a_colname);
                $cmd = "select * from $table WHERE PER_ID='".$a_value[1]."'";
                $cntdel = $db_dpis1->send_cmd($cmd);
                if ($cntdel && $DEL_FLAG) {
                    $cmd = "delete from $table WHERE PER_ID='".$a_value[1]."' AND MOV_CODE=".$a_value[3]." AND POH_DOCDATE=".$a_value[6]." ";
                    $db_dpis1->send_cmd($cmd);
		//    echo "test => ".$cmd;
                }
                /* Release 5.1.0.4 End*/
                ...
                $db_dpis1->send_cmd($cmd);
	//   echo $search_per_type . ",insert cmd=" . $cmd . "<br>";
...
if ($command == "del_chk") { // chk ลบ
		$setflagshow =  implode(",",$list_show_id);
		//echo "$setflagshow";
		$cmd = " delete from IMPORT_TEMP where IMPORT_FILENAME='$form' and ROW_NUM in (".stripslashes($setflagshow).") ";
		$db_dpis->send_cmd($cmd);
               //   echo "tester=>".$cmd;
		//$db_dpis->show_error();
...
//										echo "imptype=$imptype<br>";
                ...
                if ($imptype == "xls"){
                    if($data_type == "s"){
                        //echo "s =".$arr_temp[$data_col_idx - 1];
                    }else{
                    //    echo "s =".$arr_temp[$data_col_idx - 1];
                    }
                }else{
               //     echo "else =".$arr_temp[$data_col_idx - 1];
                }
...
//									echo "data1=".$data1[$colname]."<br>";
		...
		//	echo "colname=$colname , a_dat=$a_dat , a_typ=$a_typ<br>";
                      $cmd = " select SS_NAME from PER_SPECIAL_SKILLGRP where SS_CODE = '$a_dat' ";
                    $count_data = $db_dpis1->send_cmd($cmd);
                    $data1 = $db_dpis1->get_array();
                    $ss_name = $data1[SS_NAME]; 
                  //  echo "kong =".$a_typ = $data_type;
                   
                                    if(strlen($arr_temp[0])==3){
                                           $cmd = " select SS_CODE , SS_NAME from PER_SPECIAL_SKILLGRP where SS_CODE = '$a_dat'";
                                            $count_data = $db_dpis1->send_cmd($cmd);
                                            $data1 = $db_dpis1->get_array();
                                            if($data1[SS_CODE]==null){
                                                 $a_dat = $arr_temp[1];
                                                 $err = 0;
                                            }else if(str_replace("'", "", $a_dat)) {
                                                    $a_dat = "ตรวจพบรหัส หรือ ชื่อซ้ำ $ss_name / $val";
                                                    $err = 1;
                                            }
                                     }else{  
                                        $ref_chk = substr($arr_temp[0],0,3);
                                      //  echo "ตัดเเม่ ตัดลูก=>".$REF_CODE;
                                            $cmd = " select SS_CODE,SS_NAME from PER_SPECIAL_SKILLGRP where SS_CODE = '$ref_chk' ";
                                            $count_data = $db_dpis1->send_cmd($cmd);
                                            if($count_data){
                                                $cmd = " select SS_CODE , SS_NAME from PER_SPECIAL_SKILLGRP where SS_CODE = '$a_dat'";
                                                $count_data = $db_dpis1->send_cmd($cmd);
                                                $data1 = $db_dpis1->get_array();
                                                 if(str_replace("'", "", $a_dat)) {
                                                    $a_dat = "ตรวจพบรหัส หรือ ชื่อซ้ำ $ss_name / $val";
                                                    $err = 1;
                                                }else if($data1[SS_NAME]=="" && $data1[SS_NAME]==null && $data1[SS_CODE]=="" && $data1[SS_CODE]==null){
                                                    $a_dat = $arr_temp[1];
                                                    $err = 0;
                                               }
                                        }else{
                                                $a_dat = "ไม่พบข้อมูลหลัก SS_CODE $ref_chk";
                                                $err = 1;
                                        }
                                    }
