...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $AB_CODE => $AB_SEQ_NO){
		if($AB_SEQ_NO=="") { $cmd = " update PER_ABSENTTYPE set AB_SEQ_NO='' where AB_CODE='$AB_CODE' "; }
		else { $cmd = " update PER_ABSENTTYPE set AB_SEQ_NO=$AB_SEQ_NO where AB_CODE='$AB_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับประเภทการลา [$AB_CODE : $AB_NAME]");
	} // end if
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($AB_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', '".$$arr_fields[1]."', ".$$arr_fields[2].", ".$$arr_fields[3].", ".$$arr_fields[4].", 
							$SESS_USERID, '$UPDATE_DATE',$AB_SEQ_NO, '".$$arr_fields[8]."', '$AB_TYPE') ";
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
...
//		$db_dpis->show_error();
	...
	if($command == "DELETE" && trim($$arr_fields[0])){
		...
		//ถ้ามี รหัสการลาที่จะลบนี้ ในตาราง PER_ABSENTHIS มันจะลบไม่ได้เพราะมันอ้างอิงกันอยู่
		$cmd = " select $arr_fields[0] from PER_ABSENTHIS where $arr_fields[0]='".$$arr_fields[0]."' ";
		$count_absenthis = $db_dpis->send_cmd($cmd);
		if($count_absenthis>0){
			$err_text ="พบประวัติการลา ".trim($$arr_fields[0])." : ".$$arr_fields[1]."  ($count_absenthis รายการ) ไม่สามารถลบประเภทการลานี้ได้";
		}else{	
...
//			$db_dpis->show_error();
	...
	if($UPD){
		$cmd = " select $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4], $arr_fields[8], AB_TYPE, UPDATE_USER, UPDATE_DATE 
