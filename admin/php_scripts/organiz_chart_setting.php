...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()) {
		...
		$arr_content[$datacount][perid] = $PER_ID;
		$arr_content[$datacount][peridref] = $PER_ID_REF;
		$arr_content[$datacount][peridassref] = $PER_ID_ASS_REF;
		$arr_content[$datacount][pername] = $PER_NAME;
		$arr_content[$datacount][orgname] = $ORG_NAME;
		$arr_content[$datacount][plname] = $PL_NAME;
		$datacount++;
	} // end loop while $data
...
//				echo "$onClick";
				...
				if ($count_sub_tree > $PAGE_ROWS && $inArr) {
					...
					for ($ii=$stpage; $ii < $stpage+$PAGE_TABS; $ii++) {
						if ($ii <= $tpages) {
							if ($ii == $this_page) {
								$showpage = $showpage."<b><a href=\"javascript:refresh_opened_org(". $data[ORG_ID] .",".$ii.",".$data[ORG_ID_REF].")\">$ii</a></b> ";
							} else {
								$showpage = $showpage."<a href=\"javascript:refresh_opened_org(". $data[ORG_ID] .",".$ii.",".$data[ORG_ID_REF].")\">$ii</a> ";
							}
						} else {
							break;
						}
					...
					if ($stpage > 1 && $tpages > $PAGE_TABS) {
						...
						if ($ppage < 0) { $ppage=1; }
						$showpage = "<b><a href=\"javascript:refresh_opened_org(". $data[ORG_ID] .",".$ppage.",".$data[ORG_ID_REF].")\">Prev</a></b>&nbsp;&nbsp; ".$showpage;
					}
					if ($stpage + $PAGE_TABS <= $tpages && $tpages > $PAGE_TABS) {
						$npage = $stpage + $PAGE_TABS;
						$showpage = $showpage." &nbsp;&nbsp;<b><a href=\"javascript:refresh_opened_org(". $data[ORG_ID] .",".$npage.",".$data[ORG_ID_REF].")\">Next</a></b>";
					}
					if ($showpage > " ") {
