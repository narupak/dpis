...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $SRT_CODE => $SRT_SEQ_NO){
		if($SRT_SEQ_NO=="") { $cmd = " update PER_SERVICETITLE set SRT_SEQ_NO='' where SRT_CODE='$SRT_CODE' "; }
		else { $cmd = " update PER_SERVICETITLE set SRT_SEQ_NO=$SRT_SEQ_NO where SRT_CODE='$SRT_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับหัวข้อราชการพิเศษ [$SRT_CODE : $SRT_NAME]");
	} // end if
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($SRT_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', '".$$arr_fields[1]."', ".$$arr_fields[2].", 
							$SESS_USERID, '$UPDATE_DATE', '$SV_CODE', $SRT_SEQ_NO, '".$$arr_fields[7]."') ";
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
...
//		$db_dpis->show_error();
	...
	if($UPD){
		$cmd = " select $arr_fields[1], $arr_fields[2], $arr_fields[7], SV_CODE, UPDATE_USER, UPDATE_DATE from $table where $arr_fields[0]='".$$arr_fields[0]."' ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		$$arr_fields[1] = $data[$arr_fields[1]];
