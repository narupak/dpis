<?	
	...
	if($command=="ADD" && $PROJ_ID && $TR_CODE && trim($TR_CLASS)){
		// CLEAR OLD RECORD
		$cmd = "delete from PER_TRAIN_PROJECT_PAYMENT WHERE PROJ_ID=$PROJ_ID and TR_CODE = '$TR_CODE' and TR_CLASS=$TR_CLASS ";
		$db_dpis->send_cmd($cmd);
		
		foreach($SOURCE as 	$PP_ID => $BUDGTE_SOURCE) {
			$PAY_AMOUNT = $AMOUNT[$PP_ID];
			$OTHER_PAYMENT = $OTHER[$PP_ID];
			$cmd = " 	insert into PER_TRAIN_PROJECT_PAYMENT
								(PROJ_ID, TR_CODE, TR_CLASS,PP_ID,BUDGET_SOURCE,OTHER_PAYMENT,PAY_AMOUNT,
			...
			//$db_dpis->show_error();
			//echo $cmd . "<br>";
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มผลสำเร็จของงานที่คาดหวัง [$TR_CODE : $PG_SEQ : $TR_CLASS]");
		}
	} // end if
/*
