...
//          echo "row=".implode("|",$row)."<br>";
         ...
         //echo "<br>excel data=".$data."<br>";
            ...
            if($arr_data[2] != null){
                ...
                for($i=1;$i<=2;$i++){
                    if($i == 1){
                        if(substr($arr_data[11],0,17) == "ความเชี่ยวชาญอื่น"){
                            $Flage_sp = "2"; 
                        } else {
                            $Flage_sp = "1"; 
                        }
                        if($arr_data[13] != ""){
                            $arr_spsub =  explode("/",$arr_data[13]);
                            $var_data = $arr_spsub[1];
                        }else{
                            $var_data = $arr_data[12];
                        }
                    }else if($i == 2){
                        if(substr($arr_data[17],0,17) == "ความเชี่ยวชาญอื่น"){
                            $Flage_sp = "2"; 
                        } else {
                            $Flage_sp = "1"; 
                        }
                        if($arr_data[19] != ""){
                            $arr_spsub =  explode("/",$arr_data[19]);
                            $var_data = $arr_spsub[1];
                        }else{
                            $var_data = $arr_data[18]; 
                        }
                    }    
                    $cmd = " SELECT SS_CODE from PER_SPECIAL_SKILLGRP WHERE trim(SS_NAME) = trim('$var_data')";
                    $count_data = $db_dpis4->send_cmd($cmd);
                    //echo $cmd."(".$count_data.")";
                    if($count_data>0){

                        $data_id = $db_dpis4->get_array();
                        if($i==1){
                            $arr_data[11] = $Flage_sp;
                            $arr_data[12] = $data_id[SS_CODE];
                            $arr_data[14] = substr($arr_data[14],0,2);
                            array_push($arr_data,$i);
                            $data_im = implode("^",$arr_data);    
                        }else if($i==2){
                            $arr_data[11] = $Flage_sp;
                            $arr_data[12] = $data_id[SS_CODE];
                            $arr_data[13] = $arr_data[19];
                            $arr_data[14] = substr($arr_data[20],0,2);
                            $arr_data[15] = $arr_data[21];
                            $arr_data[16] = $arr_data[22];
                            $arr_data[23] = $i;
                            $data_im = implode("^",$arr_data);  
                        }
                        //print_r($arr_data);
...
//						echo "$i--".$data_fld[$i]."<br>";
                ...
                $cmd = " delete from $table where per_id  = $a_value[1] and SS_CODE = '-1'"; 
