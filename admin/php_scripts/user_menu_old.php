		...
		for($i=0; $i<count($arr_auth); $i++){
			...
			if(!$db->send_cmd($cmd)) :
				$cmd = " insert into user_privilege (group_id,page_id,menu_id_lv0,menu_id_lv1,menu_id_lv2,menu_id_lv3,can_add,can_edit,can_del,can_inq,can_print,can_confirm)
								 values ($group_id, $auth_detail[1], $auth_detail[2], $auth_detail[3], $auth_detail[4], $auth_detail[5], '$auth_detail[6]', '$auth_detail[7]', '$auth_detail[8]', '$auth_detail[9]', '$auth_detail[10]', '$auth_detail[11]') ";
				$db->send_cmd($cmd);
			else :
			...
			for($k=0; $k<count($arr_section); $k++){
				...
				if($menu_depth >= 1){			
					for($i=0; $i<count($arr_program_lv0[$id_page]); $i++) :
						...
						$cmd = " 	select 		can_add, can_edit, can_del, can_inq, can_print, can_confirm 
								 			from 		user_privilege
