<?
	...
	if ($command=="CONFIRM") { 
		$cmd = " select Name, TerminalID, StampDateTime, StampStat from tblTransaction a, Employee b 
				 where a.HumanID = b.HumanID and StampDateTime >= format('$TMP_START_DATE','yyyy-mm-dd hh:mm:ss') AND 
				 StampDateTime <= format('$TMP_END_DATE','yyyy-mm-dd hh:mm:ss') 
				 order by Name, StampDateTime ";
		$db_att->send_cmd($cmd);
		$db_att->show_error();
		while ( $data_att = $db_att->get_array() ) {
			$Name = $data_att[Name];
			$TerminalID = trim($data_att[TerminalID]);
			$StampDateTime = trim($data_att[StampDateTime]);
			$StampStat = $data_att[StampStat];
...
//			$db_dpis->show_error();
			...
			$cmd = " select * FROM PER_TIME_ATTENDANCE WHERE PER_ID = $PER_ID AND TIME_STAMP = '$StampDateTime' ";
...
//			$db_dpis->show_error();
			...
			if (!$count_data){
				$cmd = " insert into PER_TIME_ATTENDANCE (PER_ID, TIME_STAMP, TA_CODE, UPDATE_USER, UPDATE_DATE) 
					 values ($PER_ID, '$StampDateTime', '$TerminalID', $SESS_USERID, '$UPDATE_DATE') ";
				$db_dpis->send_cmd($cmd);
//				$db_dpis->show_error();
			}
		} // end while
