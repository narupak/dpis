...
//	echo (implode(", ", $arr_fields));
	...
	if($command == "ADD" && (trim($$arr_fields[0]) && trim($$arr_fields[1]) && trim($$arr_fields[2]))){
		...
		$db_dpis->send_cmd($cmd);
		if($count_duplicate <= 0){			//table = ACCOUNTABILITY_LEVEL_TYPE   
			...
			$cmd = " select ACC_TYPE_NAME from ACCOUNTABILITY_TYPE where ACC_TYPE_ID=".$data[$arr_fields[1]];
			$db_dpis->send_cmd($cmd);
			$data1 = $db_dpis->get_array();
			$err_text = "รหัสข้อมูลซ้ำ [".$data[$arr_fields[0]]." ".$data1[ACC_TYPE_NAME]."]";
		} // endif
	}
/*
	if($command == "UPDATE" && trim($$arr_fields[0])){
		...
		if($count_duplicate <= 0){
			$cmd = " update $table set $arr_fields[0]='".$$arr_fields[0]."', $arr_fields[1]=".$$arr_fields[1]." where $arr_fields[0]='".$$arr_fields[0]."' ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
