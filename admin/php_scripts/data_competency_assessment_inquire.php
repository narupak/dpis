<?	
	$from = "PER_COMPETENCY_ASSESSMENT a, PER_SCHOLARSHIP b";		
	...
	if(trim($SCH_CODE))  {
	$arr_search_condition[] = "(a.SCH_CODE = '$SCH_CODE')";
	}
 ...
 if ($search_sc_startdate) {
	...
	if ($search_sc_enddate) {
  	...
  	if( trim($search_status) )  {
		$from = "PER_COMPETENCY_ASSESSMENT a, PER_SCHOLARSHIP b, PER_SCHOLARINC c";
	...
	$search_condition = (trim($search_condition))?  $search_condition." and "  : "";	
  	...
  	if($DPISDB=="odbc"){ 
		...
		$cmd = " select 	count(SC_ID) as count_data 
						 ...
						 where	$search_condition  a.SCH_CODE=b.SCH_CODE and a.SCH_CODE=b.SCH_CODE(+) and a.PER_ID = d.PER_ID(+) and d.POS_ID=e.POS_ID(+) and e.ORG_ID=f.ORG_ID(+) ";
	}elseif($DPISDB=="mysql"){
		$cmd = " select 	count(SC_ID) as count_data 
						 from 		$from  
						 where	$search_condition  a.SCH_CODE=b.SCH_CODE ";
