...
//		echo "$setflagshow";
		$cmd = " update PER_POSITION set POS_STATUS = 2 where POS_ID in ($current_list) ";
		$db_dpis->send_cmd($cmd);
//		echo "$cmd<br>";
//		$db_dpis->show_error();
		$cmd = " update PER_POSITION set POS_STATUS = 1 where POS_ID in ($setflagshow) ";
		$db_dpis->send_cmd($cmd);
//		echo "$cmd<br>";
//		$db_dpis->show_error();
...
//		$db_dpis->show_error();
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูล [$POS_ID : $POS_NO]");
	} // end if	
	...
	if($command=="ADD" && $POS_NO && $DEPARTMENT_ID){
		...
		if($count_duplicate <= 0 || $DEPARTMENT_NAME=='กรมการปกครอง'){
			...
			//#######################################################
			//echo $cmd;
			$result=$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			
			echo "<p><FONT SIZE='3' COLOR='#0000FF' align='center'>เพิ่มข้อมูลเลขที่ตำแหน่ง $POS_NO  เรียบร้อยแล้ว</FONT></p>";
...
//		die;
	...
	if($POS_ID){
		$cmd = "	select	POS_NO, OT_CODE, PL_CODE, PM_CODE, CL_NAME, PT_CODE, SKILL_CODE, POS_DATE, 
											POS_GET_DATE, POS_SALARY, POS_MGTSALARY, PC_CODE, POS_CONDITION, POS_STATUS, 
											POS_CHANGE_DATE, DEPARTMENT_ID, ORG_ID, ORG_ID_1, ORG_ID_2, ORG_ID_3, ORG_ID_4, 
											ORG_ID_5, POS_DOC_NO, POS_REMARK, UPDATE_USER, UPDATE_DATE, PAY_NO, POS_ORGMGT, LEVEL_NO
							from		PER_POSITION 
							where	POS_ID=$POS_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
