...
//				echo "Upd -> GUIDE_ID=$GUIDE_ID<BR>";
				$cmd = " update PER_DEVELOPE_PLAN set
								PD_GUIDE_ID=$GUIDE_ID, PLAN_FREE_TEXT='$DESCRIPTION', PD_PLAN_START='$STARTDATE', PD_PLAN_END='$ENDDATE', 
...
//				echo "Ins -> GUIDE_ID=$GUIDE_ID<BR>";
				...
				$cmd = " INSERT INTO PER_DEVELOPE_PLAN  
								...
								VALUES ($NEXTID, $KF_ID, $GUIDE_ID, '$DESCRIPTION', '$STARTDATE', '$ENDDATE', $SESS_USERID, '$UPDATE_DATE')";
				$db_dpis2->send_cmd($cmd);
				$db_dpis2->show_error();
				$PLAN_ID = $NEXTID;
			} // end if ($PLA_ID > 0)
...
//			echo "1."; $db_dpis->show_error();
			if ($data = $db_dpis->get_array()) { // มีข้อมูล PER_DEVLOPE_PLAN ป้อนเข้าไปแล้ว
				...
				$DESCRIPTION = $data[PD_GUIDE_DESCRIPTION1];
				$TERM = $data[PD_GUIDE_DESCRIPTION2];
				$STARTDATE = $data[PD_PLAN_START];
				$ENDDATE = $data[PD_PLAN_END];
...
//		$db_dpis2->show_error();
		...
		if ($data2) {
			$fANSWER = 1;
			$planStartDate=$data2[PD_PLAN_START];
			$planEndDate=$data2[PD_PLAN_END];
		} else {
			$fANSWER = 0;
			$planStartDate="";
			$planEndDate="";
		...
		$arr_content[$data_count][description] = $data[PD_GUIDE_DESCRIPTION1];
		$arr_content[$data_count][term] = $data[PD_GUIDE_DESCRIPTION2];
		$arr_content[$data_count][fanswer] = $fANSWER;
		$arr_content[$data_count][startdate] = $planStartDate;
		$arr_content[$data_count][enddate] = $planEndDate;
