<?	
	...
	$cmd = " select ORD_CONFIRM from PER_ORDER where ORD_ID=$ORD_ID ";
	$db_dpis->send_cmd($cmd);
	$data = $db_dpis->get_array();
	...
	if($command == "ADD" && trim($ORD_SEQ) && trim($ORD_POS_NO)){
		$cmd = " select 	ORD_SEQ, ORD_POS_NO
				  		 from 		PER_ORDER_DTL 
						 where 	ORD_ID=$ORD_ID and trim(ORD_POS_NO)='". trim($ORD_POS_NO) ."' ";
		//echo $cmd;
		$count_duplicate = $db_dpis->send_cmd($cmd);
		if($count_duplicate <= 0){
			$cmd = " insert into PER_ORDER_DTL (ORD_ID, ORD_SEQ, ORD_POS_NO, ORD_EFF_DATE, ORD_RESULT, POS_ID_OLD,
							...
							LEVEL_NO, ORD_REMARK, UPDATE_USER, UPDATE_DATE)
							values ($ORD_ID, $ORD_SEQ, '".trim($ORD_POS_NO)."', '".trim($ORD_EFF_DATE)."', $ORD_RESULT, $POS_ID_OLD,
							'$OT_CODE', '$PL_CODE', $PM_CODE, '$CL_NAME', '$PT_CODE', '$SKILL_CODE', $PC_CODE,
							'$ORD_CONDITION', $ORD_SALARY, $ORD_MGTSALARY, $ORG_ID, $ORG_ID_1, $ORG_ID_2, $ORG_ID_3, $ORG_ID_4, $ORG_ID_5,
							'$LEVEL_NO', '$ORD_REMARK', $SESS_USERID, '$UPDATE_DATE') ";
			$db_dpis->send_cmd($cmd);
//$db_dpis->show_error();
	...
	if($command == "DELETE" && trim($ORD_SEQ)){
		$cmd = " select ORD_POS_NO, POS_ID_OLD from PER_ORDER_DTL where ORD_ID=$ORD_ID and ORD_SEQ=$ORD_SEQ ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		...
		$cmd = " delete from PER_ORDER_DTL where ORD_ID=$ORD_ID and ORD_SEQ=$ORD_SEQ ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
	...
	if($UPD || $VIEW){
		$cmd = " select 	ORD_SEQ, ORD_POS_NO, ORD_EFF_DATE, ORD_RESULT,
										POS_ID_OLD, OT_CODE, PL_CODE, PM_CODE, CL_NAME,  PT_CODE, SKILL_CODE, PC_CODE, ORD_CONDITION,
										ORD_SALARY, ORD_MGTSALARY, ORG_ID, ORG_ID_1, ORG_ID_2, ORG_ID_3, ORG_ID_4, ORG_ID_5, LEVEL_NO, ORD_REMARK
				  		 from 		PER_ORDER_DTL 
						 where 	ORD_ID=$ORD_ID and ORD_SEQ=$ORD_SEQ ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
	...
	if( (!$UPD && !$DEL && !$VIEW && !$err_text) ){
		$ORD_SEQ = "";
		$cmd = " select max(ORD_SEQ) as MAX_SEQ from PER_ORDER_DTL where ORD_ID=$ORD_ID ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
