<?	
	...
	if($command=="ADD" && $PROJ_ID && $TR_CODE && trim($TR_CLASS)){
		...
		if($count_duplicate <= 0){			
			...
			$cmd = " 	insert into PER_TRAIN_PROJECT_DTL
								(PROJ_ID, TR_CODE, TR_CLASS,MAX_DAY,TRAIN_PLACE,TARGET_POSITION,
								 LEVEL_NO_START,LEVEL_NO_END,START_DATE,END_DATE,
								 BUDGET,LOCAL_TAX,PER_DEVELOP_FUND,OTHER_BUDGET,
								 BUDGET_USED,LOCAL_TAX_USED,PER_DEVELOP_FUND_USED,OTHER_BUDGET_USED,
...
//			$db_dpis->show_error();
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > à¾ÔèÁ¼ÅÊÓàÃç¨¢Í§§Ò¹·Õè¤Ò´ËÇÑ§ [$TR_CODE : $PG_SEQ : $TR_CLASS]");
		}else{
			$data = $db_dpis->get_array();
			$PG_SEQ = $data[PG_SEQ];
			$TR_CLASS = $data[TR_CLASS];
			
			$err_text = "ÃËÑÊ¢éÍÁÙÅ«éÓ [".$PG_SEQ." - ".$TR_CLASS."]";
		} // end if
	} // end if
	...
	if($command=="UPDATE" && $PROJ_ID && $TR_CODE && trim($TR_CLASS)){
		...
		if($count_duplicate == 1){
			...
			$db_dpis->show_error();
			echo $cmd;	
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > á¡éä¢¼ÅÊÓàÃç¨¢Í§§Ò¹·Õè¤Ò´ËÇÑ§ [$TR_CODE : $PG_SEQ : $PFR_NAME : $TR_CLASS]");
		}else{
			$data = $db_dpis->get_array();
			$PG_SEQ = $data[PG_SEQ];
			$TR_CLASS = $data[TR_CLASS];
	...
	if($command=="DELETE" && $PROJ_ID && $TR_CODE){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > Åº¼ÅÊÓàÃç¨¢Í§§Ò¹·Õè¤Ò´ËÇÑ§ [$TR_CODE : $PG_SEQ : $TR_CLASS]");
	} // end if
