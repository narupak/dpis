...
//	$db_dpis_n = new connect_dpis($dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd);
	...
	// ==========================
 //echo '>>>'.$POEM_ID."<<<<br>";
        /*ปิดไว้ เนื่องจาก เคส http://dpis.ocsc.go.th/Service/node/1162*/
	/*if( trim($PER_ID) ) {
		$cmd = " select 	POEM_ID from PER_PERSONAL 	where PER_ID=$PER_ID  ";
...
//		$db_dpis->show_error();
	...
	if($ORG_NAME){
       ...
       // echo 'CMD_SEQ:'.$CMD_SEQ;
       // echo $ORG_NAME_ASS.'<<<< file';
