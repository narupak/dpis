<?	
	...
	if($command=="ADD" || $command=="UPDATE"){
		...
		if($TR_POSDATE)	$TR_POSDATE =  save_date($TR_POSDATE);
...
*/	
	...
	if($TR_ID){
		...
		$TR_POSDATE = $data[TR_POSDATE];
		$TR_POSDATE = show_date_format($TR_POSDATE, 1);
		
		$PN_CODE = trim($data[PN_CODE]);
		$EN_CODE = trim($data[EN_CODE]);
		$EM_CODE = trim($data[EM_CODE]);
                if(empty($EM_CODE)){$EM_CODE="";} /*Release 5.1.0.3*/
		$INS_CODE = trim($data[INS_CODE]);
                if(empty($INS_CODE)){$INS_CODE="";}/*Release 5.1.0.3*/
...
/*	// POST คือ ค่าที่แก้ไขใหม่ เพื่อไม่ให้มันแสดงค่าเดิมจาก database แล้วไม่เปลี่ยนเป็นค่าใหม่ที่เราแก้ไข
		...
		if($command!='UPDATE' && !$_POST[TR_POSDATE]) $TR_POSDATE = $data[TR_POSDATE];
		else		$TR_POSDATE = save_date($_POST[TR_POSDATE]);
		$TR_POSDATE = show_date_format($TR_POSDATE, 1);
