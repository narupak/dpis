...
//		if($PAGE_ID >= $CONTINUE_PAGE) $CONTINUE = 0;
		...
		$cmd = " select QS_ID, a.CP_CODE, CL_NO, QS_NAME, b.PC_TARGET_LEVEL 
						  ...
						  where	a.CP_CODE = b.CP_CODE and POS_ID=$CF_POS_ID and b.PER_TYPE = 1
...
//			$db_dpis->show_error();
			...
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
			$cmd = " select  LEVEL_NO from PER_PERSONAL where PER_ID=$CF_PER_ID ";
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$U_LEVEL_NO = trim($data2[LEVEL_NO]);
			$data_count=0;
			while($data = $db_dpis->get_array()){
				if ($data[CF_TYPE]==1) {
					$U_KF_ID = $data[KF_ID];
...
//				$db_dpis2->show_error();
				...
				if ($othStartDate == $chkKFStartDate && $othEndDate == $chkKFEndDate) { // if startDate and endDate ตรงกัน
					...
					for($data_count=0; $data_count < count($ARR_POINT_SET); $data_count++) {
						...
						$POINT = $POINT_SET[1];
						$cmd2 = " select PC_TARGET_LEVEL from PER_KPI_COMPETENCE where KF_ID=$U_KF_ID AND CP_CODE='$SUB_CP_CODE' ";
...
//						echo "$KF_ID:$PER_ID:$PER_NAME==>$CP_NAME:$ST_CP_MODEL:$KC_EVALUATE:$PC_TARGET_LEVEL\n";
		...
		//	echo implode("",$arr_cftype)+"|";
			$postype=substr($U_LEVEL_NO,0,1).implode("",$arr_cftype);
...
//					$gap_level = floor(abs($arr_content[$data_count][gap])); // ระดับต่าง gap ทำเป็น + ปัดเศษ 
//					if ($gap_level > 0) {
//						this is my accept gap;
//					}
//				}
				...
				$cmd = " UPDATE PER_KPI_COMPETENCE 
								 ...
								 WHERE KF_ID=$U_KF_ID and trim(CP_CODE)='".$arr_content[$data_count][code]."'  ";
				$db_dpis->send_cmd($cmd);
//				$db_dpis->show_error();
			}
