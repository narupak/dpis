...
if($next_search_pic==1){
			...
			$PER_CARDNO = substr($data[PER_CARDNO],0,4)."XXXXXXXXX";
		...
		if ($BKK_FLAG==1) {
			...
			while($data2 = $db_dpis2->get_array()){
				...
				$PER_SPSALARY .= $EXH_SEQ.". ประเภท : ".$EX_NAME."  จำนวนเงิน :   XXXXX บาท<br>";
			}
			...
			if ($PER_TYPE=="1" || $PER_TYPE==5) {
				...
				while($data2 = $db_dpis2->get_array()){
					...
					$PER_MGTSALARY .= $EX_SEQ.". ประเภท : ".$EX_NAME."  จำนวนเงิน : XXXXX บาท<br>";
				}
				//$PER_TOTALSALARY = number_format($PER_TOTALSALARY,2);
				$PER_TOTALSALARY = "XXXXX";
			}
