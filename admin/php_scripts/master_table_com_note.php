...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $CN_CODE => $CN_SEQ_NO){
			if($CN_SEQ_NO=="") { $cmd = " update PER_COM_NOTE set CN_SEQ_NO='' where CN_CODE='$CN_CODE' "; }
		else {	$cmd = " update PER_COM_NOTE set CN_SEQ_NO=$CN_SEQ_NO where CN_CODE='$CN_CODE' "; }
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			//echo "<hr>".$cmd;
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับหมายเหตุแนบท้ายคำสั่ง [$CN_CODE : $CN_NAME]");
	} // end if
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($CN_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', '".$$arr_fields[1]."', ".$$arr_fields[2].", ".$$arr_fields[3].", 
							$SESS_USERID, '$UPDATE_DATE',$CN_SEQ_NO, '".$$arr_fields[7]."') ";
			$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
