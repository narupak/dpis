<?
	...
	if( $command == "ADD" && trim(!$INV_ID) ){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูลการอบรม [".trim($INV_ID)." : ".$TR_CODE."]");
	}
	...
	if( $command == "UPDATE" && trim($INV_ID) ) {
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูลการอบรม [".trim($INV_ID)." : ".$TR_CODE."]");
	}
	...
	if($command == "DELETE" && trim($INV_ID) ){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูลการอบรม [".trim($INV_ID)." : ".$TR_CODE."]");
	}
