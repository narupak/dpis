...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $EL_CODE => $EL_SEQ_NO){
		if($EL_SEQ_NO=="") { $cmd = " update PER_EDUCLEVEL set EL_SEQ_NO='' where EL_CODE='$EL_CODE' "; }
		else { $cmd = " update PER_EDUCLEVEL set EL_SEQ_NO=$EL_SEQ_NO where EL_CODE='$EL_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับระดับการศึกษา [$EL_CODE : $EL_NAME]");
	} // end if
...
//		$db_dpis->show_error(); echo "<hr>";
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($EL_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', ".$$arr_fields[1].", ".$$arr_fields[2].", $SESS_USERID, '$UPDATE_DATE',$EL_SEQ_NO, ".$$arr_fields[6].", ".$$arr_fields[7].", ".$$arr_fields[8].", ".$$arr_fields[9].") ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			...
			$success_sql = "เพิ่มข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[6]."]เรียบร้อยแล้ว"; //กำหนดในส่วนของ php
...
//		$db_dpis->show_error();
	...
	if($UPD){
		$cmd = " select $arr_fields[1], $arr_fields[2], $arr_fields[6], $arr_fields[7], $arr_fields[8], $arr_fields[9], UPDATE_USER, UPDATE_DATE
