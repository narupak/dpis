...
function get_child_special_skillgrp($db_fnc,$ss_code){
   $cmd ="SELECT SK_SUB.SS_CODE, 
                                                ...
                                                WHERE TRIM(SK_SUB.SS_CODE) ='".trim($ss_code)."'
...
function get_now($db_dpis){
	...
	if ($command == "SETFLAG") {
            ...
            /*Release 5.2.1.7*/    
            for($i=0;$i < count($AUDIT_ID);$i++){
                if($AUDIT_FLAG[$i] != '0'){
                    $cmd = " update PER_SPECIAL_SKILL set AUDIT_FLAG = '$AUDIT_FLAG[$i]' where SPS_ID = $AUDIT_ID[$i] ";
                    $db_dpis->send_cmd($cmd);
                }
		...
		if($count) { ?>  <script> <!--  
			alert("ไม่สามารถเพิ่มข้อมูลได้ เนื่องจากคุณระบุด้านความเชี่ยวชาญพิเศษและเน้นทางซ้ำ !!!");
				-->   </script>	<? 
                        ...
                        $cmd = " delete from PER_SPECIAL_SKILL where per_id  = $PER_ID and SS_CODE = '-1'"; 
                        $db_dpis->send_cmd($cmd);
			...
			$cmd = " insert into PER_SPECIAL_SKILL (SPS_ID, PER_ID, SS_CODE, SPS_EMPHASIZE, SPS_REMARK, PER_CARDNO, UPDATE_USER, UPDATE_DATE,AUDIT_FLAG,SPS_SEQ_NO,LEVELSKILL_CODE,SPS_FLAG)
					values ($SPS_ID, $PER_ID, '$SS_CODE', $val_fld_SPS_EMPHASIZE, $val_fld_SPS_REMARK, '$PER_CARDNO', $SESS_USERID, '$UPDATE_DATE','N',$Save_SPS_SEQ_NO,'$LE_CODE',$SPS_FLAG) ";
			//  echo $cmd;
                                                       $db_dpis->send_cmd($cmd);
	...
	if(($UPD && $PER_ID && $SPS_ID) || ($VIEW && $PER_ID && $SPS_ID)){
		   $cmd = " SELECT 	SPS_ID, pa.SS_CODE, pag.SS_NAME, SPS_EMPHASIZE, SPS_REMARK, pa.UPDATE_USER, 
									pa.UPDATE_DATE ,pa.SPS_SEQ_NO,pa.LEVELSKILL_CODE,pag.REF_CODE,pa.SPS_FLAG
	...
	if( !$UPD && !$VIEW ){
        ...
        /* Release 5.2.1.7*/ 
	if($command=="NO_SP" && $PER_ID ){
        
            $cmd = "SELECT COUNT(SPS_ID) as CNT_SPS_ID from PER_SPECIAL_SKILL where per_id  = $PER_ID and SS_CODE = '-1'"; 
            $db_dpis->send_cmd($cmd); 
            $data_chk = $db_dpis->get_array();  
            if($data_chk[CNT_SPS_ID] == 0){
                $cmd = " select max(SPS_ID) as max_id from PER_SPECIAL_SKILL ";
                            $db_dpis->send_cmd($cmd);
                            $data = $db_dpis->get_array();
                            $data = array_change_key_case($data, CASE_LOWER);
                            ...
                            $cmd = " insert into PER_SPECIAL_SKILL (SPS_ID, PER_ID, SS_CODE, SPS_EMPHASIZE, SPS_REMARK, PER_CARDNO, UPDATE_USER, UPDATE_DATE,AUDIT_FLAG,SPS_SEQ_NO,LEVELSKILL_CODE)
                                            values ($SPS_ID, $PER_ID, '-1', 'NULL', 'NULL', '$PER_CARDNO', $SESS_USERID, '$UPDATE_DATE','N','','NULL') ";
                             //echo $cmd;
                            $db_dpis->send_cmd($cmd);
                            //insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มประวัติความเชี่ยวชาญพิเศษ [$PER_ID : $SPS_ID : $SS_CODE]");
            }            
