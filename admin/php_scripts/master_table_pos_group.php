...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $PG_CODE => $PG_SEQ_NO){
		if($PG_SEQ_NO=="") { $cmd = " update PER_POS_GROUP set PG_SEQ_NO='' where PG_CODE='$PG_CODE' "; }
		else { $cmd = " update PER_POS_GROUP set PG_SEQ_NO=$PG_SEQ_NO where PG_CODE='$PG_CODE' ";  }
			$db_dpis->send_cmd($cmd);
		//	$db_dpis->show_error();
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับหมวดตำแหน่งลูกจ้าง [$PG_CODE : $PG_NAME]");
	} // end if
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		$cmd = " select $arr_fields[0], $arr_fields[1], $arr_fields[6], $arr_fields[2]  from $table where $arr_fields[0]='". trim($$arr_fields[0]) ."' ";
		...
		if($PG_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', ".$$arr_fields[1].", ".$$arr_fields[2].", 
							$SESS_USERID, '$UPDATE_DATE', $PG_SEQ_NO,".$$arr_fields[6].", ".$$arr_fields[7].") ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
			...
			$err_text = "รหัสข้อมูลซ้ำ [".$data[$arr_fields[0]]." ".$data[$arr_fields[1]]." ".$data[$arr_fields[6]]." ".$data[$arr_fields[2]]."]";
		} // endif
	}
	...
	if($command == "UPDATE" && trim($$arr_fields[0])){ 
		...
		//echo $cmd;
		//$db_dpis->show_error();
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[6]." : ".$$arr_fields[2]."]");
	}
	...
	if($command == "DELETE" && trim($$arr_fields[0])){
		...
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[6]." : ".$$arr_fields[2]."]");
	}
