...
//		echo "max_id=$max_id<br>";
		$cmd = "insert into per_develope_guide
						(pd_guide_id, pd_guide_level, pd_guide_competence, pd_guide_description1, pd_guide_description2, update_date, update_user) values 
						($max_id, $guide_level, $guide_cp, '$guide_desc', $guide_term, '$UPDATE_DATE', '$SESS_USERNAME')";
		$db->send_cmd($cmd);
//		$db->show_error();
		$CLEAR = 1;
...
//		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มกลุ่มผู้ใช้งาน [$group_name_th]");
	...
	if ($command == "UPDATE") {
		...
		$cmd = "update per_develope_guide set
						...
						pd_guide_description1 = '$guide_desc',
						pd_guide_description2 = $guide_term,
						update_date = '$UPDATE_DATE',
						update_user = '$SESS_USERNAME'
...
//		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบกลุ่มผู้ใช้งาน [$group_name_th]");
	...
	$count_develop_guide = $db->send_cmd($cmd);
...
//			$db_dpis2->show_error();
			...
			$guide_row[$i]["desc"] = $data[pd_guide_description1];
			$guide_row[$i]["term"] = $data[pd_guide_description2];
			$i++;
		endwhile;
	endif;
...
//		$db->show_error();
		...
		$guide_desc = $data[pd_guide_description1];
		$guide_term = $data[pd_guide_description2];
