...
// หาข้อมูล คำถาม
	...
	echo "08-TotalQ=$totQUESTION, Question List=$listShowCnt:".implode(",",$ARR_EVALUATE_QUESTION)."<br>";
...
//			echo "  SESS_PASSED_QUESTION=$SESS_PASSED_QUESTION";
			...
			// ========================== Prepare Data ===============================		
			$postype=substr($CF_LEVEL_NO,0,1);
...
//			while($data = $db_dpis->get_array()){
				...
				echo "17-RATIO[$postype]=$SEFT_RATIO,$CHIEF_RATIO,$FRIEND_RATIO,$SUB_RATIO<br>";
//			}			

			$cmd = " select count(CP_CODE) as POINT, CP_CODE  
							  from PER_COMPETENCY_ANSWER a, PER_QUESTION_STOCK b
							  where CF_ID=$CF_ID and a.QS_ID = b.QS_ID and CA_ANSWER > 2
							  group by CP_CODE
							  order by CP_CODE ";
			$db_dpis->send_cmd($cmd);
			while($data = $db_dpis->get_array()){
...
//				$db_dpis2->show_error();
				...
				if ($othStartDate == $chkKFStartDate && $othEndDate == $chkKFEndDate) { // if startDate and endDate ตรงกัน
					// % ตามประเภท CF_TYPE
					$perc = ($U_CF_TYPE==1 ? $SEFT_RATIO : ($U_CF_TYPE== 2 ? $CHIFT_RATIO : ($U_CF_TYPE==3 ? $FRIEND_RATIO : $SUB_RATIO)));
