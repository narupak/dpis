	...
	switch($SESS_USERGROUP_LEVEL){								
		...
		//ยืนยันบันทึกปรับลดวันและบันทึกปฏิทิน
  		if($chk_confirm==1){
			...
			$cmd = " insert into PER_HOLIDAY (HOL_DATE,HOL_NAME,UPDATE_USER,UPDATE_DATE) values ('$HOL_DATE_save', '$HOL_NAME_save', $SESS_USERID, '$UPDATE_DATE') ";
			$db_dpis_add->send_cmd($cmd);
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูล [$HOL_DATE_save : $HOL_NAME_save]");
