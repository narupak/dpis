...
//	$db_dpis1 = new connect_dpis($dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd);		
	...
	if( trim($PER_ID) ) {
		...
		// ถ้าเป็นการเพิ่มข้าราชการ/ลูกจ้างประจำ สำหรับบัญชีแนบท้ายคำสั่งเลื่อนขั้นเงินเดือน
		// จะ select เฉพาะเงินเดือนในระดับของข้าราชการ/ลูกจ้างประจำ มาแสดงผลใน listbox เงินเดือน
		if ($sel_salary) {
			$arr = 1;		$selectfield="";
			if ($PER_TYPE == 2) {	
			...
			if ($count_layer_salary)  $cmd_salary_list .= "parent.document.form1.CMD_SALARY.options[0] = new Option('== เลือกอัตราเงินเดือน ==','');";
			while ( $data = $db_dpis->get_array() ) {
	...
	$CMD_LEVEL = $LEVEL_NO_POS;		//ระดับตำแหน่ง 
	$CMD_LEVEL_NAME = $LEVEL_NAME_POS;	//ชื่อระดับตำแหน่ง 
	$CMD_LEVEL1 = $LEVEL_NO_POS;			//ระดับตำแหน่ง 
	$CMD_LEVEL3 = $LEVEL_NAME_POS;		//ชื่อระดับตำแหน่ง 
