...
//echo "<br>tmp_LEVEL_NO=$tmp_LEVEL_NO || count_arr_DC_TYPE=$count_arr_DC_TYPE<br>";	
for ($i=0; $i<$count_arr_DC_TYPE; $i++) {
//echo "<br>2. || type=$dc_type_tmp || code=$dc_code_tmp || code_old=$dc_code_old_tmp ||<br>";	
...
*/
		...
		if ($dcon_time2_tmp <= 0 || $arr_level_startdate[0] >= $dcon_time2_tmp) {
				//echo "2. || ";
				$privilege2 = true;
		}  
		...
		if ($DE_ID && $privilege1==true && $privilege2==true && $privilege3==true && $privilege4==true) {
			if ($dc_type_tmp == 1 || $dc_type_tmp == 2) {
				// ลบ เครื่องราชระดับที่ต่ำกว่า เนื่องจากอาจจะมีโอกาสได้รับมากกว่า 1 เครื่องราชฯ 
				// สำหรับสายสะพาย และต่ำกว่าสายสะพาย รับพร้อมกันไม่ได้ แต่รับพร้อมเหรียญตราได้
				$cmd = " delete from PER_DECORDTL where DE_ID=$DE_ID and PER_ID=$tmp_PER_ID and 
						DC_CODE in (" . implode(", ", ${"arr_dc_code_bytype1"}) .", ". implode(", ", ${"arr_dc_code_bytype2"}) . ")"; 
			} elseif ($dc_type_tmp == 3) {
				$cmd = " delete from PER_DECORDTL where DE_ID=$DE_ID and PER_ID=$tmp_PER_ID and 
						DC_CODE in (" . implode(", ", ${"arr_dc_code_bytype3"}) . ")"; 
...
//echo "<br>:: <font color=red>$cmd</font>";	
			...
			$cmd = " insert into PER_DECORDTL 
				...
				values 
				($DE_ID, '$dc_code_tmp', $tmp_PER_ID, $SESS_USERID, '$UPDATE_DATE', $tmp_PER_CARDNO) ";
//echo "<br>:: <font color=red>$cmd</font><br>";
			$db_dpis1->send_cmd($cmd);
		}
