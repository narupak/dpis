<?
		...
		$cmd = "  	select 	a.PER_ID, a.POS_ID, a.POEM_ID, a.POEMS_ID, a.CMD_POSITION, a.CMD_POS_NO, b.PER_NAME, b.PER_SURNAME, b.DEPARTMENT_ID 
		...
		while ($data = $db_dpis->get_array()) {
			...
			$tmp_pos_no = $tmp_data[1];		//เลขที่ตำแหน่งใหม่
			if (in_array($tmp_pos_no, $arr_pos_no))  { 					
				$num++;
				$error_move_personal.= "<tr><td width='6%'></td><td><font color='#FF0000'>$num.&nbsp;&nbsp;เลขที่ตำแหน่งซ้ำ=$tmp_pos_no&nbsp;::&nbsp;$tmp_per_name</font></td></tr>";
			}  // end if 
			$arr_pos_no[] = trim($tmp_pos_no);

			if ($COM_LEVEL_SALP!=1) {
				if ($tmp_pos_id) {
					$cmd = " select POS_ID from PER_POSITION where POS_NO = $tmp_pos_no and DEPARTMENT_ID = $tmp_department_id ";
					$db_dpis1->send_cmd($cmd);
					$data1 = $db_dpis1->get_array();
					$tmp_pos_id = trim($data1[POS_ID]);
					$search_pos = "POS_ID=$tmp_pos_id and PER_TYPE=1";
				} elseif ($tmp_poem_id) {
					$cmd = " select POEM_ID from PER_POS_EMP where POEM_NO = $tmp_pos_no and DEPARTMENT_ID = $tmp_department_id ";
					$db_dpis1->send_cmd($cmd);
					$data1 = $db_dpis1->get_array();
					$tmp_poem_id = trim($data1[POEM_ID]);
					$search_pos = "POEM_ID=$tmp_poem_id and PER_TYPE=2";
				} elseif ($tmp_poems_id) {
					$cmd = " select POEMS_ID from PER_POS_EMPSER where POEMS_NO = $tmp_pos_no and DEPARTMENT_ID = $tmp_department_id ";
					$db_dpis1->send_cmd($cmd);
					$data1 = $db_dpis1->get_array();
					$tmp_poems_id = trim($data1[POEMS_ID]);
					$search_pos = "POEMS_ID=$tmp_poems_id and PER_TYPE=3";
				}  // end if
