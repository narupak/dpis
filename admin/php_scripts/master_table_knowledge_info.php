...
//	echo (implode(", ", $arr_fields));
	...
	if($command == "ADD" && trim($$arr_fields[1])){
		...
		$cmd = " select max(JOB_DES_ID) as max_id from KNOWLEDGE_INFO ";
		$db_dpis->send_cmd($cmd);
...
//			$db_dpis->show_error();
			...
			for($i=1; $i<=7; $i++){
				$cmd = " insert into KNOWLEDGE_LEVEL ($arr_fields[0], job_des_level, job_des_level_description, UPDATE_USER, UPDATE_DATE) 
								 values (".$$arr_fields[0].", $i, '".${"knowledge_level_desc_".$i}."', $SESS_USERID, '$UPDATE_DATE') ";
				$db_dpis->send_cmd($cmd);
...
//		$db_dpis->show_error();
		...
		for($i=1; $i<=7; $i++){
			$cmd = " update KNOWLEDGE_LEVEL set								
								job_des_level_description = '".${"knowledge_level_desc_".$i}."',
								UPDATE_USER=$SESS_USERID, 
								UPDATE_DATE='$UPDATE_DATE' 
...
//			$db_dpis->show_error();
	...
	if($command == "DELETE" && trim($$arr_fields[0])){
		...
		$cmd = " delete from KNOWLEDGE_LEVEL where $arr_fields[0]=".$$arr_fields[0];
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
...
//		$db_dpis->show_error();
	...
	if($UPD){
		...
		for($i=1; $i<=7; $i++){
			$cmd = " select job_des_level_description from KNOWLEDGE_LEVEL where $arr_fields[0]=".$$arr_fields[0]." and job_des_level=$i ";
			$db_dpis->send_cmd($cmd);			
			$data = $db_dpis->get_array();
			$data = array_change_key_case($data, CASE_LOWER);
			${"knowledge_level_desc_".$i} = $data[job_des_level_description];
		} // end for
	} // end if
	...
	if( (!$UPD && !$DEL && !$err_text) ){
		...
		$$arr_fields[1] = "";
		$$arr_fields[2] = "";
		for($i=1; $i<=7; $i++) ${"knowledge_level_desc_".$i} = "";
