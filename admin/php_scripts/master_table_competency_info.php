...
//	echo (implode(", ", $arr_fields));
	...
	if($command == "ADD" && trim($$arr_fields[1])){
		...
		$$arr_fields[0] = $data[max_id] + 1;	
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[3]."]");
...
//				$db_dpis->show_error(); echo "<hr>";
			...
			$data = $db_dpis->get_array();			
			$err_text = "รหัสข้อมูลซ้ำ [".$data[$arr_fields[0]]." ".$data[$arr_fields[1]]." ".$data[$arr_fields[3]]."]";
		} // endif
	}
...
//		$db_dpis->show_error();
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[3]."]");
...
//		$db_dpis->show_error();
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูล [".trim($$arr_fields[0])." : ".$$arr_fields[1]." : ".$$arr_fields[3]."]");
	}
