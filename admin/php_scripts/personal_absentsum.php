			...
			if ($code[$i]=="01" || $code[$i]=="03") {
				/*$cmd = " select count(ABS_DAY) as abs_count from PER_ABSENTHIS 
								where PER_ID=$PER_ID and AB_CODE='$code[$i]' 
                                                                    and ABS_STARTDATE >= '$START_DATE' and ABS_ENDDATE <= '$END_DATE' ";*/
				//echo $cmd;
                                /*ปรับการนับจำนวนครังใหม่*/
                                $cmd = " select count(ABS_DAY) as abs_count from PER_ABSENTHIS 
                                        where PER_ID=$PER_ID and AB_CODE='$code[$i]' 
                                            and ABS_STARTDATE >= '$START_DATE' and ABS_STARTDATE <= '$END_DATE' ";
                                /**/
                                $db_dpis->send_cmd($cmd);
				$data = $db_dpis->get_array();
				$data = array_change_key_case($data, CASE_LOWER);
...
/*
		...
		if($AS_CYCLE==2){	
			...
			$db_dpis->send_cmd($cmd);
                        if($debug==1){echo __LINE__.':<per>'.$cmd.'<br><br>';}
			$data = $db_dpis->get_array();
			$data = array_change_key_case($data, CASE_LOWER);
			...
			$TMP_AB_CODE_04 += $data[AB_CODE_04]+0; 
                ...
                $cmd = "
                    ...
                    WITH
                            ...
                            from (
                              select to_char(LISTDATE,'YYYY-MM-DD') LISTDATE from (
                                SELECT (TO_DATE((LEAST((select trim(min(ABS_STARTDATE)) from PER_ABSENTHIS where ABS_STARTDATE is not null and PER_ID=$PER_ID),
                                 ...
                                 WHERE OWNER='".strtoupper($dpisdb_user)."' AND (TO_DATE((LEAST((select trim(min(ABS_STARTDATE)) from PER_ABSENTHIS where ABS_STARTDATE is not null and PER_ID=$PER_ID),
                                      (select trim(min(START_DATE)) from PER_ABSENTSUM where START_DATE is not null and PER_ID=$PER_ID))
                        ...
                        WHERE as_year=$AS_YEAR AND AS_CYCLE IN(1,2) 
                        group by AS_YEAR,AS_CYCLE,START_DATE,END_DATE,XSUM1
                ) WHERE as_year=$AS_YEAR AND as_cycle=$AS_CYCLE ";
                //echo '<pre>'.$cmd; //AND as_cycle=$AS_CYCLE
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
                /*++++++++++++++++++++++ นับจำนวนวัน +++++++++++++++++++++++++++*/
                /*ลาป่วย (วัน) :*/
                $AB_CODE_01_1 = trim(round($data[SICK_ABS],2));
                $AB_CODE_01_2 = ($AB_CODE_01_1+$AB_CODE_01)+0;
                /*ลากิจส่วนตัว (วัน) */
                $AB_CODE_03_1 = trim(round($data[KIT_ABS],2)); 
                $AB_CODE_03_2 = ($AB_CODE_03_1+$AB_CODE_03)+0;
                /*มาสาย*/
                $AB_CODE_10_1 = trim(round($data[SAY_ABS],2));
                $AB_CODE_10_2 = ($AB_CODE_10_1+$AB_CODE_10)+0;
                /*ขาดราชการ*/
                $AB_CODE_13_1 = trim(round($data[KHAD_ABS],2));
                $AB_CODE_13_2 = ($AB_CODE_13_1+$AB_CODE_13)+0;
                /*ลาไปศึกษา ฝึกอบรม ดูงาน หรือปฏิบัติการวิจัย*/
                $AB_CODE_07_1 = trim(round($data[LASUKSA_ABS],2));
                $AB_CODE_07_2 = ($AB_CODE_07_1+$AB_CODE_07)+0;
                /*ลาพักผ่อน */
                $AB_CODE_04_1 = trim(round($data[PAKPON_ABS],2)); 
                $AB_CODE_04_2 = ($AB_CODE_04_1+$AB_CODE_04)+0;
                /**/
		...
		$data = $db_dpis->get_array();
		$VC_DAY = $data[VC_DAY]; 
		//$REMAIN_VC_DAY = $VC_DAY - $AB_CODE_04_2 + $AB_CODE_04_3 - $TMP_AB_CODE_04 + $TMP_AB_CODE_04_3;/*เดิม*/
                if($AS_CYCLE==1){ //1
                    $REMAIN_VC_DAY = $XSUM_DIFF1;
                }else{ //2
                    $REMAIN_VC_DAY = $XSUM_DIFF2-$AB_CODE_04_2;
                }
                //ปรับ
