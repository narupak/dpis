<?	
	...
	if($command=="ADD" && $PER_ID){
		...
		-->   </script>	<? }  
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูลประวัติการลาลงตารางสำรอง [$PER_ID : $ABS_ID : $AB_CODE]");
			$F_REFRESH = "1";
		} // end if
...
//		$db_dpis->show_error(); echo "<hr><br>";
	...
	if($command=="COMMAND" && $PER_ID){
		...
		-->   </script>	<? }  
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูลประวัติการลาลงตารางจริง [$PER_ID : $ABS_ID : $AB_CODE]");
			$F_REFRESH = "1";
		} // end if
	} // end if เช็คข้อมูลซ้ำ
