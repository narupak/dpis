...
//	echo (implode(", ", $arr_fields));
	...
	if($command=="REORDER"){
		foreach($ARR_ORDER as $TP_CODE => $TP_SEQ_NO){
			if($TP_SEQ_NO=="") { $cmd = " update $table set TP_SEQ_NO='' where TP_CODE='$TP_CODE' "; }
		else {	$cmd = " update $table set TP_SEQ_NO=$TP_SEQ_NO where TP_CODE='$TP_CODE' "; }
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error(); echo "<hr>";
			//echo "<hr>".$cmd;
		} // end foreach

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > จัดลำดับชื่อตำแหน่งลูกจ้าง [$TP_CODE : $TP_NAME]");
	} // end if
...
//$db_dpis->show_error(); echo "<br>";
	...
	if($command == "ADD" && trim($$arr_fields[0])){
		...
		if($TP_SEQ_NO==''){
			...
			$cmd = " insert into $table (". implode(", ", $arr_fields) .") values ('".trim($$arr_fields[0])."', '".$$arr_fields[1]."', '".$$arr_fields[2]."', $TP_SEQ_NO, ".$$arr_fields[4].", 
							$SESS_USERID, '$UPDATE_DATE', '".$$arr_fields[7]."','') ";
			$db_dpis->send_cmd($cmd);
			//$db_dpis->show_error();
...
//		$db_dpis->show_error();
	...
	if($UPD){
		$cmd = " select $arr_fields[1], $arr_fields[2], $arr_fields[4], $arr_fields[7], TP_SEQ_NO, UPDATE_USER, UPDATE_DATE
