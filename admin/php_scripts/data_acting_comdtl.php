<?
	...
	if( $command == "ADD" && trim(!$COM_ID) && trim($COM_NO) ){
		...
		if(!$search_org_id)  $search_org_id="0";
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูลบัญชีแนบท้ายคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
	...
	if( $command == "UPDATE" && trim($COM_ID) ) {
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูลบัญชีแนบท้ายคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
	}			// 	if( $command == "UPDATE" && trim($COM_ID) )
...
// ============================================================
	...
	if( $command == "COMMAND" && trim($COM_ID) ) {
		...
		include ("php_scripts/data_acting_comdtl_confirm.php");	
		$cmd = " update PER_COMMAND set  
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ยืนยันข้อมูลบัญชีแนบท้ายคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
	}		// 	if( $command == "COMMAND" && trim($COM_ID) ) 	
...
// ============================================================
	...
	if( $command == "SEND" && trim($COM_ID) && $SESS_USERGROUP_LEVEL > 4 ) {
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ส่งข้อมูลบัญชีแนบท้ายคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
...
// ============================================================	
	...
	if($command == "DELETE_COMDTL" && trim($COM_ID) && trim($PER_ID) ){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูลข้าราชการ/ลูกจ้างแนบท้ายบัญชีคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [".trim($COM_ID)." : ".$PER_ID."]");
	...
	if($command == "DELETE_COMMAND" && trim($COM_ID) ){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูลบัญชีและข้าราชการ/ลูกจ้างแนบท้ายบัญชีคำสั่งรักษาราชการแทน/รักษาการในตำแหน่ง [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$PER_ID."]");
		$COM_ID = "";
