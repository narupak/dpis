...
class Spreadsheet_Calc_Reader{
   ...
   var $sheets;
	...
	if ($zip) {
		...
		while($zip_entry != "content.xml") {
 
			$zip_entry = zip_read($zip);
 
		}
 
		$line = zip_entry_read($zip_entry);
	...
	xml_parse_into_struct($xml,$content,$values);
	...
	while(!strcasecmp ($values[$counter++]['tag'], "table:table"));
 
 
 
	do {
 
		if(!strcasecmp ($values[$counter]['tag'], "table:table-row") && !strcasecmp ($values[$counter]['type'], "open")) {
 
			$row++;
 
		}
 
		if(!strcasecmp ($values[$counter]['tag'], "table:table-cell") && !strcasecmp ($values[$counter]['type'], "open")) {
 
			$col++;
 
		}
 
		if(!strcasecmp ($values[$counter]['tag'], "text:p") && !strcasecmp ($values[$counter]['type'], "complete")) {
 
			$this->sheets[0]['cells'][$row][$col] = $values[$counter]['value'];
 
		}
 
		if(!strcasecmp ($values[$counter]['tag'], "table:table-cell") && !strcasecmp ($values[$counter]['type'], "close")) {
 
			// nothing here
 
		}
 
		if(!strcasecmp ($values[$counter]['tag'], "table:table-row") && !strcasecmp ($values[$counter]['type'], "close")) {
	...
	$this->sheets[0]['numRows'] = $row - 1;
 
	$this->sheets[0]['numCols'] = $mcol - 1;
