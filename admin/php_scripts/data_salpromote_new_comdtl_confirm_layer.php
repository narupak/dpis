<?
		...
		while ($data = $db_dpis->get_array()) {
			...
			$cmd = " 	update PER_PERSONAL set MOV_CODE='$tmp_MOV_CODE', PER_SALARY=$tmp_CMD_SALARY, 
							PER_DOCNO='$COM_NO', PER_DOCDATE='$tmp_CMD_DATE', UPDATE_USER=$SESS_USERID, UPDATE_DATE='$UPDATE_DATE'
			...
			$SAH_SEQ_NO = 1;
			$SAH_REMARK = "NULL";
                        if(empty($tmp_CMD_NOTE1)){
                            $tmp_CMD_NOTE1='NULL';
                         }else{
                             $tmp_CMD_NOTE1="'".$tmp_CMD_NOTE1."'";
                         }
                         if(empty($tmp_CMD_NOTE2)){
                            $tmp_CMD_NOTE2='NULL';
                         }else{
                             $tmp_CMD_NOTE2="'".$tmp_CMD_NOTE2."'";
                         }
			...
			$cmd = "	insert into PER_SALARYHIS (SAH_ID, PER_ID, SAH_EFFECTIVEDATE, MOV_CODE, SAH_SALARY, SAH_DOCNO, 
							...
							SAH_SALARY_EXTRA, SAH_SEQ_NO, SAH_REMARK, LEVEL_NO, SAH_POS_NO, SAH_POSITION, 
                                                        SAH_ORG, EX_CODE, SAH_PAY_NO, SAH_SALARY_MIDPOINT, SAH_KF_YEAR, SAH_KF_CYCLE, 
                                                        SAH_TOTAL_SCORE,SAH_LAST_SALARY, SM_CODE, SAH_CMD_SEQ, SAH_ORG_DOPA_CODE, SAH_OLD_SALARY, 
                                                        SAH_POS_NO_NAME,SAH_REMARK1,SAH_REMARK2) 
							...
							$tmp_CMD_SPSALARY, $SAH_SEQ_NO, $SAH_REMARK, '$tmp_LEVEL_NO', '$SAH_POS_NO', '$PL_NAME_WORK', 
                                                        '$ORG_NAME_WORK', '$EX_CODE', '$SAH_POS_NO', $tmp_CMD_MIDPOINT,$search_kf_year, $search_kf_cycle, 
                                                        $tmp_TOTAL_SCORE, 'Y', '$SM_CODE', $tmp_CMD_SEQ_NO, '$SAH_ORG_DOPA_CODE', $tmp_CMD_OLD_SALARY, 
                                                       '$SAH_POS_NO_NAME',$tmp_CMD_NOTE1,$tmp_CMD_NOTE2) ";
			$db_dpis1->send_cmd($cmd);
//			$db_dpis1->show_error();
			//echo '<pre>'.$cmd.'<br>-----------------<br>';
//			echo "<br>";		
		...
		if (!$RPT_N) {
		// update ข้อมูลเงินเดือนจาก PER_LAYER_NEW ไปที่ PER_LAYER
			if ($DPISDB == "odbc") {
				$cmd = " delete from PER_LAYER where LEVEL_NO in (select distinct LEVEL_NO from PER_LAYER_NEW) ";
				$db_dpis1->send_cmd($cmd);
				...
				$cmd = " delete from PER_LAYER where LEVEL_NO in (
							select  	b.LEVEL_NO 
							from 	PER_LAYER a, PER_LAYER_NEW b  
							where 	a.LAYER_TYPE=b.LAYER_TYPE and a.LEVEL_NO=b.LEVEL_NO and a.LAYER_NO=b.LAYER_NO			
						) ";
				$db_dpis1->send_cmd($cmd);
	//$db_dpis1->show_error();			
				$cmd = "  insert into PER_LAYER
						...
						from PER_LAYER_NEW ";
				/*
				$cmd = " 	update	PER_LAYER a   
							set  (LAYER_SALARY, LAYER_SALARY_MIN, LAYER_SALARY_MAX) =  (
							select  	b.LAYER_SALARY, b.LAYER_SALARY_MIN, b.LAYER_SALARY_MAX
							from 	PER_LAYER_NEW b  
							where 	a.LAYER_TYPE=b.LAYER_TYPE and a.LEVEL_NO=b.LEVEL_NO and a.LAYER_NO=b.LAYER_NO ) "; 
				*/						
			}elseif($DPISDB=="mysql"){
				$cmd = " select distinct LEVEL_NO from PER_LAYER_NEW";
				$db_dpis1->send_cmd($cmd);
				while($data = $db_dpis->get_array()){
					$search_condition.=$data[LEVEL_NO].",";
				}
				if(trim($search_condition)){ 
					$search_condition=substr($search_condition,0,strlen($search_condition)-1);
					$cmd = " delete from PER_LAYER where LEVEL_NO in ($search_condition) ";
					$db_dpis1->send_cmd($cmd);
				}
				$cmd = "  insert into PER_LAYER
						(LAYER_TYPE, LEVEL_NO, LAYER_NO, LAYER_SALARY, LAYER_ACTIVE, UPDATE_USER, UPDATE_DATE, LAYER_SALARY_MIN, LAYER_SALARY_MAX)
