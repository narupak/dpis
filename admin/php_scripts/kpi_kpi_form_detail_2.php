	...
	$PAGE_AUTH["attach"] = ($PAGE_AUTH["attach"]=="Y")?"Y":(($data[can_attach])?$data[can_attach]:"N");
       ...
       /* if(empty($hiddenATTACH_FILE)){
            ...
            if($hiddenATTACH_FILE==1){
                $btnwidth=250;
                $Bntlbl='แสดงไฟล์ตามค่า Config แบบทั้งหมด';$ATTACH_FILE=2;
            }else if($hiddenATTACH_FILE==2){
                $btnwidth=350;
                $Bntlbl='แสดงไฟล์ตามค่า Config แบบรายข้อมูล (Transaction)';$ATTACH_FILE=1;
            }
        }*/
        if(empty($hiddenATTACH_FILE)){
            if($ATTACH_FILE==1){
...
//	$db->show_error();
	...
	/*$cmd = " select 	RESULT_COMMENT, COMPETENCE_COMMENT, SALARY_RESULT, SALARY_REMARK1, SALARY_REMARK2, 
        ...
        /*Release 5.1.0.6*/
        $cmd = " select 	RESULT_COMMENT, COMPETENCE_COMMENT, SALARY_RESULT, SALARY_REMARK1, SALARY_REMARK2, 
						PER_ID_REVIEW, PER_ID_REVIEW0, PER_ID_REVIEW1,PER_ID_REVIEW2, AGREE_REVIEW1, DIFF_REVIEW1, AGREE_REVIEW2, DIFF_REVIEW2, 
                                                PERFORMANCE_WEIGHT, COMPETENCE_WEIGHT, OTHER_WEIGHT, KF_STATUS, LEVEL_NO, KF_SCORE_STATUS,OTH_DESC,
	...
	if($SESS_USERGROUP == 1){
		...
		($PER_ID_REVIEW==$SESS_PER_ID || $PER_ID_REVIEW0==$SESS_PER_ID) && !$AGREE_REVIEW1 && !$DIFF_REVIEW1 && 
        ...
        if(($AGREE_REVIEW1 || $DIFF_REVIEW1) || ($AGREE_REVIEW2 || $DIFF_REVIEW2)){
            $CHKPerMissionSection5=FALSE;
        }
        
        /*================================================= คำนวณจาก ส่วนที่ 3 มาใส่ ส่วนที่ 2  ==============================================================*/ 
        
        /*$cmd_sc = " select SCORE_KPI, SCORE_COMPETENCE, SCORE_OTHER, SUM_KPI, SUM_COMPETENCE, SUM_OTHER from PER_KPI_FORM where KF_ID=$KF_ID ";
        if($debug==1){echo '<pre>'.$cmd.'<br>';}
        $db_dpis->send_cmd($cmd_sc);
        $data_sc = $db_dpis->get_array();
        $TEMP_SCORE_KPI = $data_sc[SCORE_KPI];
        $SCORE_COMPETENCE_SC = $data_sc[SCORE_COMPETENCE];
        
        $cmd_wg = " select SUM(KPI_WEIGHT) as TOTAL_KPI_WEIGHT from PER_PERFORMANCE_GOALS where KF_ID=$KF_ID ";
        $db_dpis->send_cmd($cmd_wg);
        $data_wg = $db_dpis->get_array();
        $TOTAL_KPI_WEIGHT_wg = $data_wg[TOTAL_KPI_WEIGHT] + 0;
        
        $cmd_cp = " select CP_CODE from PER_KPI_COMPETENCE where KF_ID=$KF_ID ";
        $COUNT_COMPETENCE_CP = $db_dpis->send_cmd($cmd_cp);
        $FULL_SCORE_CP = 5;
        
        $cmd_lv = " select sum(PC_TARGET_LEVEL) as SUM_TARGET_LEVEL from PER_KPI_COMPETENCE where KF_ID=$KF_ID ";
        $db_dpis->send_cmd($cmd_lv);
        $data_lv = $db_dpis->get_array();
        $SUM_TARGET_LEVEL_LV =$data_lv[SUM_TARGET_LEVEL];
        
        $TMP_SCORE_KPI = ($TEMP_SCORE_KPI / ($TOTAL_KPI_WEIGHT_wg * 5));
        
        if ($COMPETENCY_SCALE==1)
                $TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE_SC / ($COUNT_COMPETENCE_CP * 3));
        elseif ($COMPETENCY_SCALE==2 || $COMPETENCY_SCALE==5 || $COMPETENCY_SCALE==6)
                $TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE_SC / $FULL_SCORE_CP);
        elseif ($COMPETENCY_SCALE==3)
                $TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE_SC / $SUM_TARGET_LEVEL_LV);*/
        
        
        /*============================================== End คำนวณจาก ส่วนที่ 3 มาใส่ ส่วนที่ 2 ==============================================================*/
...
//	echo "USER_AUTH2 :: ".(($USER_AUTH2)?"TRUE":"FALSE")."<br>";
	...
	if($SUBPAGE==1){	
		...
		if($PER_ID_REVIEW==$SESS_PER_ID || $USER_AUTH2){
			//if(!$PG_EVALUATE) $PG_EVALUATE = "NULL"; /*เดิม*/
                    if($PG_EVALUATE=="") $PG_EVALUATE = "NULL"; /*Release 5.2.1.19*/
		...
		if(($UPD && $KF_ID && $PG_ID) || ($VIEW && $KF_ID && $PG_ID)){
			...
			elseif($DPISDB=="oci8") 
				$cmd = "	SELECT 	a.PG_SEQ, a.KPI_ID, a.KPI_NAME, b.KPI_NAME as KPI_ORG_NAME, c.PFR_ID, c.PFR_ID_REF, c.PFR_NAME, a.KPI_TARGET_LEVEL1, 
														a.KPI_TARGET_LEVEL2, a.KPI_TARGET_LEVEL3, a.KPI_TARGET_LEVEL4, a.KPI_TARGET_LEVEL5, a.KPI_TARGET_LEVEL1_DESC, 
														a.KPI_TARGET_LEVEL2_DESC, a.KPI_TARGET_LEVEL3_DESC, a.KPI_TARGET_LEVEL4_DESC, a.KPI_TARGET_LEVEL5_DESC, 
														a.PG_RESULT, NVL(a.PG_EVALUATE,-1) AS PG_EVALUATE, a.PG_REMARK, a.KF_TYPE, a.KF_SCORE_FLAG, a.KPI_OTHER
			...
			$KPI_TARGET_LEVEL5_DESC = $data[KPI_TARGET_LEVEL5_DESC];
                        ...
                        /*Release 5.2.1.19*/
                        $SET_VAL_PG_EVALUATE=$data[PG_EVALUATE];
                        if($data[PG_EVALUATE]==-1){
                            $SET_VAL_PG_EVALUATE="";
                        }
                        
			//$PG_EVALUATE_EDIT = $data[PG_EVALUATE]; /*เดิม*/
                        $PG_EVALUATE_EDIT = $SET_VAL_PG_EVALUATE; /*Release 5.2.1.19*/
			if ($KF_SCORE_STATUS==1 || $KPI_SCORE_CONFIRM!=1 || $SESS_USERGROUP == 1 || $PER_ID_REVIEW==$SESS_PER_ID || 
				...
				//$PG_EVALUATE = $data[PG_EVALUATE]; /*เดิม*/
                                $PG_EVALUATE = $SET_VAL_PG_EVALUATE; /*Release 5.2.1.19*/
			}
...
if($command=="ADD_OTHER"){
		$cmd = " UPDATE PER_KPI_FORM SET
								OTH_DESC = '$OTH_DESC_DTL'
								WHERE KF_ID=$KF_ID ";  
								$db_dpis->send_cmd($cmd);
