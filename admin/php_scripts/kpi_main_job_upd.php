...
//			$db_dpis->show_error();
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > เพิ่มข้อมูล [$PL_CODE : $MJT_CODE]");
		}else{
			$data = $db_dpis->get_array();			
			$err_text = "รหัสข้อมูลซ้ำ [$data[PL_CODE] $data[ORG_ID] $data[MJT_CODE]]";
		} // endif
...
//			$db_dpis->show_error();
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูล [จาก->$PL_CODE : $ORG_ID : $MJT_CODE เป็น->$upd_pl_code : $upd_org_id : $upd_mjt_code]");
		}else{
			$data = $db_dpis->get_array();			
			$err_text = "รหัสข้อมูลซ้ำ [$data[PL_CODE] $data[ORG_ID] $data[MJT_CODE]]";
			$UPD = 1;
...
//		$db_dpis->show_error();
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบข้อมูล [$PL_CODE : $ORG_ID : $MJT_CODE]");
