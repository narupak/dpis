...
//		$TMP_SCORE_KPI = round(round(($SCORE_KPI / ($COUNT_KPI * 5)), 3), 2);
                //เดิม วันที่ 16/02/2018
//		$TMP_SCORE_KPI = round(round(($SCORE_KPI / ($TOTAL_KPI_WEIGHT * 5)), 3), 2);
                $TMP_SCORE_KPI = ($SCORE_KPI / ($TOTAL_KPI_WEIGHT * 5));
		if ($COMPETENCY_SCALE==1)
			$TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE / ($COUNT_COMPETENCE * 3));
		elseif ($COMPETENCY_SCALE==2 || $COMPETENCY_SCALE==5 || $COMPETENCY_SCALE==6)
			$TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE / $FULL_SCORE);
		elseif ($COMPETENCY_SCALE==3)
			$TMP_SCORE_COMPETENCE = ($SCORE_COMPETENCE / $SUM_TARGET_LEVEL);
		...
		//if($CH_KF_SCORE_STATUS ==1)$CH_KF_STATUS=1; //ถ้าติกให้เห็นคะเเนน ให้ติกที่ไม่อนุญาติให้แก้ไขด้วย
