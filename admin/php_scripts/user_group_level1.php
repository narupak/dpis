...
//		$db->show_error();
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > แก้ไขข้อมูล [$group_id : $PROVINCE_CODE => $CH_PROVINCE_CODE ; $MINISTRY_ID => $CH_MINISTRY_ID ; $DEPARTMENT_ID => $CH_DEPARTMENT_ID ; $ORG_ID => $CH_ORG_ID ; $ORG_ID_1 => $CH_ORG_ID_1]");
		...
		if($SESS_MINISTRY_ID != $CH_MINISTRY_ID){
			...
			if($group_level==3 || $group_level==4 || $group_level==5 || $group_level==6){ 
		...
		if($SESS_DEPARTMENT_ID != $CH_DEPARTMENT_ID){
			...
			if($group_level==4 || $group_level==5 || $group_level==6){
		...
		if($SESS_ORG_ID != $CH_ORG_ID){
			...
			if($group_level==5 || $group_level==6){
...
//	echo "$group_org_structure - $group_level :: $PROVINCE_CODE :: $MINISTRY_ID :: $DEPARTMENT_ID :: $ORG_ID :: $ORG_ID_1 :: [$SESS_PER_TYPE]";
