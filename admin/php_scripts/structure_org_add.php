<?	
	...
	if($ORG_ID && !$ORG_ID_REF)	{
		if($DPISDB=="odbc"){
			$cmd = " select top 1 ORG_ID_REF from $ORGTAB where ORG_ID=$ORG_ID ";
		}elseif($DPISDB=="oci8"){
			$cmd = " select ORG_ID_REF from $ORGTAB where ORG_ID=$ORG_ID ";
		}elseif($DPISDB=="mysql"){
			$cmd = " select top 1 ORG_ID_REF from $ORGTAB where ORG_ID=$ORG_ID ";
		} // end if
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
	...
	$cmd = " select		ORG_CODE, ORG_NAME, ORG_SHORT, OL_CODE, OT_CODE, ORG_JOB,
...
//			echo "$k>>$arr_num_page[$k],";
		...
		$cmd = " select ORG_ID , ORG_NAME, ORG_ID_REF from $ORGTAB where ORG_ID_REF = $org_parent and ORG_ID <> $START_ORG_ID 
...
*/
	...
	function delete_org($ORG_ID, $ORG_ID_REF){
		...
		$cmd = " select ORG_ID, ORG_ID_REF from $ORGTAB where ORG_ID_REF=$ORG_ID and ORG_ID<>$START_ORG_ID ";
...
//		$db_dpis->show_error();
		...
		$cmd = " delete from $ORGTAB where ORG_ID=$ORG_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
