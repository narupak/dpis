...
//	$db->show_error();
	...
	function list_per_level_kpi ($name, $val, $PL_NAME,$USER_VIEW='') {
		global $db_list, $db_dpis, $DPISDB, $PER_TYPE, $LEVEL_TITLE,$db_dpis3 ;
                /* ปรับเพิ่ม*/
		...
		if ($PER_TYPE==1) {
           ...
           $cmd ="select PL_CODE from per_line where PL_ACTIVE = 1 and PL_NAME = '$PL_NAME' ";
           $cnt_PL =  $db_dpis3->send_cmd($cmd);
           if($cnt_PL){  
              $data = $db_dpis3->get_array();
              $PL_CODE_QUE =  $data[PL_CODE];
           } 
           $cmd = "	select LEVEL_NO_MIN, LEVEL_NO_MAX FROM PER_LINE	where PL_CODE = '$PL_CODE_QUE' ";
...
//echo $cmd;
		if($count){
			...
			if ($command=="LOOP2" && !$PER_ID_REVIEW) $PER_ID_REVIEW = "";
                        else if ($command=="LOOP2" && $PER_ID_REVIEW) $PER_ID_REVIEW = $PER_ID_REVIEW;
			else if ($command!="LOOP2") $PER_ID_REVIEW = $data[PER_ID_REVIEW];
			if ($command=="LOOP2" && !$PER_ID_REVIEW0) $PER_ID_REVIEW0 = "";
                        else if ($command=="LOOP2" && $PER_ID_REVIEW0) $PER_ID_REVIEW0 = $PER_ID_REVIEW0;
			else if ($command!="LOOP2") $PER_ID_REVIEW0 = $data[PER_ID_REVIEW0];
			if ($command=="LOOP2" && !$PER_ID_REVIEW1) $PER_ID_REVIEW1 = "";
                        else if ($command=="LOOP2" && $PER_ID_REVIEW1) $PER_ID_REVIEW1 = $PER_ID_REVIEW1;
			else if ($command!="LOOP2") $PER_ID_REVIEW1 = $data[PER_ID_REVIEW1];
			if ($command=="LOOP2" && !$PER_ID_REVIEW2) $PER_ID_REVIEW2 = "";
                        else if ($command=="LOOP2" && $PER_ID_REVIEW2) $PER_ID_REVIEW2 = $PER_ID_REVIEW2;
			else if ($command!="LOOP2") $PER_ID_REVIEW2 = $data[PER_ID_REVIEW2];
			...
			if ($command=="LOOP2" && !$LEVEL_NO && $data[LEVEL_NO]) $LEVEL_NO = trim($data[LEVEL_NO]);
                ...
                //============================================== แก้เรื่องรีเฟรชแล้ว ดึงค่าเดิมจาก database มา===========================================================
			/* เดิม 
			...
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME && trim($REVIEW_PN_NAME.$REVIEW_PER_NAME)) 
				...
				$REVIEW_FULL_NAME = $REVIEW_PN_NAME.$REVIEW_PER_NAME;
                        */
                        ...
                        /* ใหม่ แก้เรื่อง ไม่ตำแหน่งทางการบริหาร แต่ระบบนำตำแหน่งทางการบริหารจากแฟ้มการประเมินมาแสดงให้ */
                        /* แก้ไฟล์ ver.21*/
                        if ($command=="LOOP2" && !$REVIEW_PL_NAME) $REVIEW_PL_NAME = "";
                        else if ($command=="LOOP2" && $REVIEW_PL_NAME) $REVIEW_PL_NAME = $REVIEW_PL_NAME;
			else if ($command!="LOOP2") $REVIEW_PL_NAME = trim($data[REVIEW_PL_NAME]);
                        if ($command=="LOOP2" && !$REVIEW_PM_NAME) $REVIEW_PM_NAME = "";
                        else if ($command=="LOOP2" && $REVIEW_PM_NAME) $REVIEW_PM_NAME = $REVIEW_PM_NAME;
			else if ($command!="LOOP2") $REVIEW_PM_NAME = trim($data[REVIEW_PM_NAME]);
			if ($command=="LOOP2" && !$REVIEW_LEVEL_NO) $REVIEW_LEVEL_NO = "";
                        else if ($command=="LOOP2" && $REVIEW_LEVEL_NO) $REVIEW_LEVEL_NO = $REVIEW_LEVEL_NO;
			else if ($command!="LOOP2") $REVIEW_LEVEL_NO = trim($data[REVIEW_LEVEL_NO]);
                        if ($command=="LOOP2" && !$REVIEW_PN_NAME) $REVIEW_PN_NAME = "";
                        else if ($command=="LOOP2" && $REVIEW_PN_NAME) $REVIEW_PN_NAME = $REVIEW_PN_NAME;
			else if ($command!="LOOP2") $REVIEW_PN_NAME = trim($data[REVIEW_PN_NAME]);
                        if ($command=="LOOP2" && !$REVIEW_PER_NAME) $REVIEW_PER_NAME = "";
                        else if ($command=="LOOP2" && $REVIEW_PER_NAME) $REVIEW_PER_NAME = $REVIEW_PER_NAME;
			else if ($command!="LOOP2") $REVIEW_PER_NAME =  trim($data[REVIEW_PER_NAME]);
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME) $REVIEW_FULL_NAME = "";
                        else if ($command=="LOOP2" && $REVIEW_FULL_NAME) $REVIEW_FULL_NAME = $REVIEW_FULL_NAME;
			else if ($command!="LOOP2") $REVIEW_FULL_NAME = $REVIEW_PN_NAME.$REVIEW_PER_NAME;   
                        /* End REVIEW */      
			...
			$REVIEW_PER_NAME0 = trim($data[REVIEW_PER_NAME0]);
				...
				$REVIEW_FULL_NAME0 = $REVIEW_PN_NAME0.$REVIEW_PER_NAME0;
                        */
                        ...
                        /* ใหม่ แก้เรื่อง ไม่ตำแหน่งทางการบริหาร แต่ระบบนำตำแหน่งทางการบริหารจากแฟ้มการประเมินมาแสดงให้ */
                        /* แก้ไฟล์ ver.21*/
                        if ($command=="LOOP2" && !$REVIEW_PL_NAME0) $REVIEW_PL_NAME0 = "";
                        else if ($command=="LOOP2" && $REVIEW_PL_NAME0) $REVIEW_PL_NAME0 = $REVIEW_PL_NAME0;
			else if ($command!="LOOP2") $REVIEW_PL_NAME0 = trim($data[REVIEW_PL_NAME0]);
                        if ($command=="LOOP2" && !$REVIEW_PM_NAME0) $REVIEW_PM_NAME0 = "";
                        else if ($command=="LOOP2" && $REVIEW_PM_NAME0) $REVIEW_PM_NAME0 = $REVIEW_PM_NAME0;
			else if ($command!="LOOP2") $REVIEW_PM_NAME0 = trim($data[REVIEW_PM_NAME0]);
                        if ($command=="LOOP2" && !$REVIEW_LEVEL_NO0) $REVIEW_LEVEL_NO0 = "";
                        else if ($command=="LOOP2" && $REVIEW_LEVEL_NO0) $REVIEW_LEVEL_NO0 = $REVIEW_LEVEL_NO0;
			else if ($command!="LOOP2") $REVIEW_LEVEL_NO0 = trim($data[REVIEW_LEVEL_NO0]);
                        if ($command=="LOOP2" && !$REVIEW_PN_NAME0) $REVIEW_PN_NAME0 = "";
                        else if ($command=="LOOP2" && $REVIEW_PN_NAME0) $REVIEW_PN_NAME0 = $REVIEW_PN_NAME0;
			else if ($command!="LOOP2") $REVIEW_PN_NAME0 = trim($data[REVIEW_PN_NAME0]);
                        if ($command=="LOOP2" && !$REVIEW_PER_NAME0) $REVIEW_PER_NAME0 = "";
                        else if ($command=="LOOP2" && $REVIEW_PER_NAME0) $REVIEW_PER_NAME0 = $REVIEW_PER_NAME0;
			else if ($command!="LOOP2") $REVIEW_PER_NAME0 = trim($data[REVIEW_PER_NAME0]);
                        if ($command=="LOOP2" && !$REVIEW_FULL_NAME0)$REVIEW_FULL_NAME0 = "";
                        else if ($command=="LOOP2" && $REVIEW_FULL_NAME0) $REVIEW_FULL_NAME0 = $REVIEW_FULL_NAME0;
			else if ($command!="LOOP2")$REVIEW_FULL_NAME0 = $REVIEW_PN_NAME0.$REVIEW_PER_NAME0;
                        /* End */
                        //============================================== PER_ID_REVIEW0 ============================================================
			/* เดิม
			...
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME1 && trim($REVIEW_PN_NAME1.$REVIEW_PER_NAME1))
				...
				$REVIEW_FULL_NAME1 = $REVIEW_PN_NAME1.$REVIEW_PER_NAME1;
                        */
                        ...
                        /* ใหม่ แก้เรื่อง ไม่ตำแหน่งทางการบริหาร แต่ระบบนำตำแหน่งทางการบริหารจากแฟ้มการประเมินมาแสดงให้ */
                        /* แก้ไฟล์ ver.21*/
                        if ($command=="LOOP2" && !$REVIEW_PL_NAME1) $REVIEW_PL_NAME1 = "";
                        else if ($command=="LOOP2" && $REVIEW_PL_NAME1) $REVIEW_PL_NAME1 = $REVIEW_PL_NAME1;
			else if ($command!="LOOP2") $REVIEW_PL_NAME1 = trim($data[REVIEW_PL_NAME1]);
                        if ($command=="LOOP2" && !$REVIEW_PM_NAME1) $REVIEW_PM_NAME1 = "";
                        else if ($command=="LOOP2" && $REVIEW_PM_NAME1) $REVIEW_PM_NAME1 = $REVIEW_PM_NAME1;
			else if ($command!="LOOP2") $REVIEW_PM_NAME1 = trim($data[REVIEW_PM_NAME1]);
                        if ($command=="LOOP2" && !$REVIEW_LEVEL_NO1) $REVIEW_LEVEL_NO1 = "";
                        else if ($command=="LOOP2" && $REVIEW_LEVEL_NO1) $REVIEW_LEVEL_NO1 = $REVIEW_LEVEL_NO1;
			else if ($command!="LOOP2") $REVIEW_LEVEL_NO1 = trim($data[REVIEW_LEVEL_NO1]);
                        if ($command=="LOOP2" && !$REVIEW_PN_NAME1) $REVIEW_PN_NAME1 = "";
                        else if ($command=="LOOP2" && $REVIEW_PN_NAME1) $REVIEW_PN_NAME1 = $REVIEW_PN_NAME1;
			else if ($command!="LOOP2") $REVIEW_PN_NAME1 = trim($data[REVIEW_PN_NAME1]);
                        if ($command=="LOOP2" && !$REVIEW_PER_NAME1) $REVIEW_PER_NAME1 = "";
                        else if ($command=="LOOP2" && $REVIEW_PER_NAME1) $REVIEW_PER_NAME1 = $REVIEW_PER_NAME1;
			else if ($command!="LOOP2") $REVIEW_PER_NAME1 = trim($data[REVIEW_PER_NAME1]);
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME1) $REVIEW_FULL_NAME1 = "";
                        else if ($command=="LOOP2" && $REVIEW_FULL_NAME1) $REVIEW_FULL_NAME1 = $REVIEW_FULL_NAME1;
			else if ($command!="LOOP2") $REVIEW_FULL_NAME1 = $REVIEW_PN_NAME1.$REVIEW_PER_NAME1;
                        /* End */
                        //============================================== PER_ID_REVIEW1 ============================================================
			/* เดิม 
			...
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME2 && trim($REVIEW_PN_NAME2.$REVIEW_PER_NAME2))
				...
				$REVIEW_FULL_NAME2 = $REVIEW_PN_NAME2.$REVIEW_PER_NAME2;
                        */
                        ...
                        /* ใหม่ แก้เรื่อง ไม่ตำแหน่งทางการบริหาร แต่ระบบนำตำแหน่งทางการบริหารจากแฟ้มการประเมินมาแสดงให้ */
                        /* แก้ไฟล์ ver.21*/
                        if ($command=="LOOP2" && !$REVIEW_PL_NAME2) $REVIEW_PL_NAME2 = "";
                        else if ($command=="LOOP2" && $REVIEW_PL_NAME2) $REVIEW_PL_NAME2 = $REVIEW_PL_NAME2;
			else if ($command!="LOOP2") $REVIEW_PL_NAME2 = trim($data[REVIEW_PL_NAME2]);
                        if ($command=="LOOP2" && !$REVIEW_PM_NAME2) $REVIEW_PM_NAME2 = "";
                        else if ($command=="LOOP2" && $REVIEW_PM_NAME2) $REVIEW_PM_NAME2 = $REVIEW_PM_NAME2;
			else if ($command!="LOOP2") $REVIEW_PM_NAME2 = trim($data[REVIEW_PM_NAME2]);
                        if ($command=="LOOP2" && !$REVIEW_LEVEL_NO2) $REVIEW_LEVEL_NO2 = "";
                        else if ($command=="LOOP2" && $REVIEW_LEVEL_NO2) $REVIEW_LEVEL_NO2 = $REVIEW_LEVEL_NO2;
			else if ($command!="LOOP2") $REVIEW_LEVEL_NO2 = trim($data[REVIEW_LEVEL_NO2]);
                        if ($command=="LOOP2" && !$REVIEW_PN_NAME2) $REVIEW_PN_NAME2 = "";
                        else if ($command=="LOOP2" && $REVIEW_PN_NAME2) $REVIEW_PN_NAME2 = $REVIEW_PN_NAME2;
			else if ($command!="LOOP2") $REVIEW_PN_NAME2 = trim($data[REVIEW_PN_NAME2]);
                        if ($command=="LOOP2" && !$REVIEW_PER_NAME2) $REVIEW_PER_NAME2 = "";
                        else if ($command=="LOOP2" && $REVIEW_PER_NAME2) $REVIEW_PER_NAME2 = $REVIEW_PER_NAME2;
			else if ($command!="LOOP2") $REVIEW_PER_NAME2 =trim($data[REVIEW_PER_NAME2]);
			if ($command=="LOOP2" && !$REVIEW_FULL_NAME2) $REVIEW_FULL_NAME2 = "";
                        else if ($command=="LOOP2" && $REVIEW_FULL_NAME2) $REVIEW_FULL_NAME2 = $REVIEW_FULL_NAME2;
			else if ($command!="LOOP2") $REVIEW_FULL_NAME2 = $REVIEW_PN_NAME2.$REVIEW_PER_NAME2;
                        /* End */
                        //============================================== PER_ID_REVIEW2 ============================================================
                        
                //==============================================END แก้เรื่องรีเฟรชแล้ว ดึงค่าเดิมจาก database มา===========================================================
...
//				echo "$cmd<br>";
	...
	if($command == "RECALC" && trim($KF_YEAR) && trim($KF_CYCLE) && trim($search_per_type)){
		...
		/*$cmd = " select KF_ID, COMPETENCE_WEIGHT, SCORE_COMPETENCE, SUM_COMPETENCE, SUM_KPI, SUM_OTHER, TOTAL_SCORE
                ...
                $cmd = " select KF_ID, COMPETENCE_WEIGHT, SCORE_COMPETENCE, SUM_COMPETENCE, SUM_KPI, SUM_OTHER, TOTAL_SCORE
						from PER_KPI_FORM where NVL(KF_SCORE_STATUS,0)!=1 and KF_CYCLE = $KF_CYCLE and KF_START_DATE = '$KF_START_DATE' and 
						KF_END_DATE = '$KF_END_DATE' $where ";
		$db_dpis1->send_cmd($cmd);
//		$db_dpis1->show_error();
		while($data1 = $db_dpis1->get_array()){
...
//-------------------------- http://dpis.ocsc.go.th/Service/node/2577 --------------------		
		...
		$count_PER_KPI_F = $db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
			$error = 0;
		if($count_PER_KPI_F == 0){
			$error = 1;
			return false;	
		} 
//-----------------------------------------------------------------------------------------	
...
//		$db_dpis->show_error();
		...
		$KC_ID = $data[max_id] + 1;
                ...
                $db_dpis->send_cmd($cmd);
                /*++++++++++++++++++++++++++++++++++++++++++*/
//=================== http://dpis.ocsc.go.th/Service/node/2170 ===================          
          $cmd = " select	OT_CODE from PER_PERSONAL where	PER_ID = $PER_ID ";		
            $db_dpis2->send_cmd($cmd);	
            $data2 = $db_dpis2->get_array();
            $ot_code = $data2[OT_CODE];        
            
        if ($OT_CODE=="08"){
			$CON_WEIGHT_KPI        = $WEIGHT_KPI_E;
			$CON_WEIGHT_COMPETENCE = $WEIGHT_COMPETENCE_E;
			$CON_WEIGHT_OTHER      = "NULL";
		}elseif ($OT_CODE=="09"){
			$CON_WEIGHT_KPI         = $WEIGHT_KPI_S;
			$CON_WEIGHT_COMPETENCE  = $WEIGHT_COMPETENCE_S;
			$CON_WEIGHT_OTHER       = "NULL";
		}else{
			$CON_WEIGHT_KPI         = $WEIGHT_KPI;
			$CON_WEIGHT_COMPETENCE  = $WEIGHT_COMPETENCE;
			$CON_WEIGHT_OTHER       = $WEIGHT_OTHER;
		}
//================================================================================          
		...
		$cmd = " select 	KF_ID, PER_ID, PER_CARDNO, KF_CYCLE, KF_START_DATE, KF_END_DATE, 
						 ...
						 where KF_CYCLE = $KF_COPY_FROMCYCLE and KF_START_DATE = '$FROM_START_DATE' and 
						KF_END_DATE = '$FROM_END_DATE' and PER_ID in (select PER_ID from PER_PERSONAL where PER_TYPE = $search_per_type) $where ";
		$count_PER_KPI = $db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
	if($count_PER_KPI == 0) return false;
		while($data = $db_dpis->get_array()){
			...
			$TMP_ORG_ID = $data[ORG_ID];
                        /*http://dpis.ocsc.go.th/Service/node/2249 */ //ปรับแก้ไม่ให้คัดลอก "ผลการพิจารณาการเลื่อนเงินเดือน" และใส่ค่าตั้งต้นเป็น "Y"
...
//=================== http://dpis.ocsc.go.th/Service/node/2170 ===================            
            ...
            if($KF_COPY_TOYEAR == $KPI_BUDGET_YEAR){
                $PERFORMANCE_WEIGHT = $CON_WEIGHT_KPI; 
                $COMPETENCE_WEIGHT  = $CON_WEIGHT_COMPETENCE;
                $OTHER_WEIGHT       = $CON_WEIGHT_OTHER;
            }else{
...
//				$db_dpis2->show_error();
			...
			$count_PER_PERFORMANCE = $db_dpis1->send_cmd($cmd);
	//			$db_dpis1->show_error();
		if($count_PER_PERFORMANCE==0)return false;
			while($data1 = $db_dpis1->get_array()){
				$PG_SEQ = $data1[PG_SEQ];
				$KPI_ID = $data1[KPI_ID];
...
//		$db_dpis2->show_error();
			...
			$cmd = " select	a.CP_CODE, PC_TARGET_LEVEL
							 ...
							 where	a.CP_CODE=b.CP_CODE and a.DEPARTMENT_ID=b.DEPARTMENT_ID and a.DEPARTMENT_ID = $DEPARTMENT_ID and 
											POS_ID=$POS_ID and PC_TARGET_LEVEL > 0 and CP_ASSESSMENT = 'Y' and a.PER_TYPE = $search_per_type
							 order by a.CP_CODE ";
			$cnt_COMPETENCE = $db_dpis2->send_cmd($cmd);
                        if($debug==1){echo __LINE__.':'.$cmd.'<br>----------------------<br>';}
//			$db_dpis2->show_error();
			if($cnt_COMPETENCE==0)return false;
...
//				$db_dpis1->show_error();
	...
	if ($command == "SETFLAG") {
                $SCORE_STATUS_DATE = date("Y-m-d H:i:s");
                /*5.2.1.12 begin*/
                $cntChkAll=count($list_kf_id);
                if($SESS_USERGROUP!=1){
                    $disabled_list_kf_id = explode(",", $disabled_list_kf_id);
                    if(!empty($disabled_list_kf_id[0]) && $IS_ACCEPT_CONFIG==1){
                        $list_kf_id=array_merge($list_kf_id,$disabled_list_kf_id);
                        //echo 'if 1:<br>';
                    }
                    if(empty($list_kf_id)){
                        $list_kf_id=$disabled_list_kf_id;
                        //echo 'if 2:<br>';
                    }
                }
                $cnt_chk= count($list_kf_id);
                /*echo $cnt_chk;
                echo 'CNT:'.$cnt_chk.'<br>';
                for($i=0;$i<$cnt_chk;$i++){
                    if(!empty($list_kf_id[$i])){
                        echo '1>>'.$list_kf_id[$i].'<br>';
                    }else{
                        echo '2>>'.$list_kf_id[$i].'<br>';
                    }
                }
                echo '<pre>';
                echo $current_list;*/
                /*5.2.1.12 end $IS_ACCEPT_CONFIG*/
		$setflagshow =  implode(",",$list_kf_id);
		$cmd = " update PER_KPI_FORM set KF_SCORE_STATUS = 0 , SCORE_STATUS_DATE = '$SCORE_STATUS_DATE' where KF_ID in (".stripslashes($current_list).") ";
		$db_dpis->send_cmd($cmd); 
                //echo '<pre>'.$cmd.'<br>';
		//$cmd = " update PER_KPI_FORM set KF_SCORE_STATUS = 1 , SCORE_STATUS_DATE = '$SCORE_STATUS_DATE' where KF_ID in (".stripslashes($setflagshow).") ";
		//$db_dpis->send_cmd($cmd);
                for($i=0;$i<$cnt_chk;$i++){
                    if($cntChkAll>0 || $cnt_chk >0){
                        $cmd = " update PER_KPI_FORM set KF_SCORE_STATUS = 1 , SCORE_STATUS_DATE = '$SCORE_STATUS_DATE' where KF_ID in (".$list_kf_id[$i].") ";
                        $db_dpis->send_cmd($cmd);
                    }
                    //echo '<pre>'.$cmd.'<br>';
                }
