...
//		echo "name=$name, val=$val, PL_CODE=$PL_CODE, PER_ID=$PER_ID, POS_ID=$POS_ID<br>";
...
//		$ES_NAME = "";
	...
	if(trim($LEVEL_NO) && (!$RPT_N || ($COM_PER_TYPE==2 && $PG_CODE_SALARY))) {
		...
		while ( $data_dpis1 = $db_dpis1->get_array() ) {
			...
			$CMD_SALARY = str_replace(",","",$CMD_SALARY);
			//echo $CMD_SALARY." == ".$data_dpis1[$selectfield]."<br>";
			$select = ($CMD_SALARY == $data_dpis1[$selectfield])? "selected" : "";
			$list_layer_temp.= "<option value='$data_dpis1[$selectfield]' $select>$salary_text</option>";
		}		// end while
	}			// end if 
...
//			echo "1..LEVEL_NO=".trim($LEVEL_NO)."<br>";
			if ($form1command != "LEVEL_SELECTED" || !trim($LEVEL_NO)) // ถ้าเป็นการเปลี่ยนค่า level_no ใหม่ ไม่ต้องนำค่าเข้า
...
//			echo "2..LEVEL_NO=".trim($LEVEL_NO)."<br>";
			if(trim($LEVEL_NO)){	
...
//			echo "3..LEVEL_NO=".trim($LEVEL_NO)." , LEVEL_NAME=$LEVEL_NAME<br>";

//			echo "1..POS_POEM_NO=$POS_POEM_NO , POS_POEM_NO_NAME=$POS_POEM_NO_NAME<br>";
			
//		if ($form1command != "LEVEL_SELECTED") { // ถ้าเป็นการเปลี่ยนค่า level_no ใหม่ ไม่ต้องนำค่าเข้า
			//  ===== ถ้าเป็นข้าราชการ SELECT ข้อมูลตำแหน่งจาก table PER_LINE =====  TR_PER_TYPE=1
			if ($COM_PER_TYPE == 1) {			
				//$POS_POEM_ID = trim($data[POS_ID]);
//				echo "sendPOS=$sendPOS<br>";
				if($sendPOS)	 $POS_ID_SETNEW = trim($sendPOS);
				else if ($form1command != "LEVEL_SELECTED") $POS_ID_SETNEW = trim($data[POS_ID]);
				if ($form1command != "LEVEL_SELECTED") $POS_ID = $POS_POEM_ID = $POS_ID_SETNEW;
...
//				echo "2..POS_POEM_NO=$POS_POEM_NO , POS_POEM_NO_NAME=$POS_POEM_NO_NAME<br>";
			}
			...
			if ($COM_PER_TYPE == 2) {
				...
				if($sendPOS)	 $POS_ID_SETNEW = trim($sendPOS);
				else if ($form1command != "LEVEL_SELECTED") $POS_ID_SETNEW = trim($data[POEM_ID]);
				if ($form1command != "LEVEL_SELECTED") $POEM_ID = $POS_POEM_ID = $POS_ID_SETNEW;				
...
//				echo "3..POS_POEM_NO=$POS_POEM_NO , POS_POEM_NO_NAME=$POS_POEM_NO_NAME<br>";
			}
			...
			if ($COM_PER_TYPE == 3) {
				...
				if($sendPOS)	 $POS_ID_SETNEW = trim($sendPOS);
				else if ($form1command != "LEVEL_SELECTED") $POS_ID_SETNEW = trim($data[POEMS_ID]);
				if ($form1command != "LEVEL_SELECTED") $POEMS_ID = $POS_POEM_ID = $POS_ID_SETNEW;
...
//				echo "4..POS_POEM_NO=$POS_POEM_NO , POS_POEM_NO_NAME=$POS_POEM_NO_NAME<br>";
			}			
			...
			if ($COM_PER_TYPE == 4) {
				...
				if($sendPOS)	 $POS_ID_SETNEW = trim($sendPOS);
				else if ($form1command != "LEVEL_SELECTED") $POS_ID_SETNEW = trim($data[POT_ID]);
				if ($form1command != "LEVEL_SELECTED") $POT_ID = $POS_POEM_ID = $POS_ID_SETNEW;
...
//				echo "5..POS_POEM_NO=$POS_POEM_NO , POS_POEM_NO_NAME=$POS_POEM_NO_NAME<br>";
			}
//		} // end if ($form1command != "LEVEL_SELECTED") // ถ้าเป็นการเปลี่ยนค่า level_no ใหม่ ไม่ต้องนำค่าเข้า
			...
			if(trim($LEVEL_NO) && (!$RPT_N || ($COM_PER_TYPE==2 && $PG_CODE_SALARY))) {
				$selectfield="";
				if($COM_PER_TYPE==2){
					$cmd ="select PG_CODE from  PER_LAYEREMP where layere_salary = $data[CMD_SALARY] ";
					//echo $cmd;
					$db_dpis1->send_cmd($cmd);
					$data_upd=$db_dpis1->get_array();
		
					if($_POST[PG_CODE_SALARY]){
						$PG_CODE_SALARY = $_POST[PG_CODE_SALARY];$PG_CODE_UPD = $_POST[PG_CODE_SALARY];	// เพื่อเวลา update เลือกกลุ่มใหม่ให้ select ค่าใหม่ ถ้าไม่มีมันก็ดึงค่าเดิมใน DB ไป set ก็เลือกกลุ่มเดิมไม่เปลี่ยนแปลง
					}	else{
						$PG_CODE_UPD = $data_upd[PG_CODE];
						$PG_CODE_SALARY = $PG_CODE_UPD;
					}
					$selectfield="LAYERE_SALARY";
					$cmd = " select $selectfield FROM PER_LAYEREMP where (PG_CODE='$PG_CODE_UPD' AND LAYERE_ACTIVE=1) order by $selectfield ";
				}else if($COM_PER_TYPE==3){
					$selectfield="LAYER_SALARY";
					$cmd = " select $selectfield FROM PER_LAYER where (PER_TYPE=3 AND LEVEL_NO like '$LEVEL_NO%' AND LAYER_ACTIVE=1) order by $selectfield ";
				}
