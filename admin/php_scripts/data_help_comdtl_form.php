...
//	echo "CMD_SEQ=$CMD_SEQ, VIEW_PER=$VIEW_PER, UPD_PER=$UPD_PER<br>";
	...
	if (!$PL_NAME_WORK) $PL_NAME_WORK = "ช่วยราชการ";
	...
	if($command=="ADD"){
		...
		if ($CMD_ORG8) $CMD_ORG8 = "$CMD_ORG8\|$POS_ORG_ID8";*/
                ...
                if(empty($CMD_ORG1) && empty($POS_ORG_ID1)){$CMD_ORG1 = "NULL";}else{$CMD_ORG1 = "'".$CMD_ORG1."\|".$POS_ORG_ID1."'";}
                if(empty($CMD_ORG2) && empty($POS_ORG_ID2)){$CMD_ORG2 = "NULL";}else{$CMD_ORG2 = "'".$CMD_ORG2."\|".$POS_ORG_ID2."'";}
		if(empty($CMD_ORG3) && empty($POS_ORG_ID3)){$CMD_ORG3 = "NULL";}else{$CMD_ORG3 = "'".$CMD_ORG3."\|".$POS_ORG_ID3."'";}
		if(empty($CMD_ORG4) && empty($POS_ORG_ID4)){$CMD_ORG4 = "NULL";}else{$CMD_ORG4 = "'".$CMD_ORG4."\|".$POS_ORG_ID4."'";}
		if(empty($CMD_ORG5) && empty($POS_ORG_ID5)){$CMD_ORG5 = "NULL";}else{$CMD_ORG5 = "'".$CMD_ORG5."\|".$POS_ORG_ID5."'";}
		if(empty($CMD_ORG6) && empty($POS_ORG_ID6)){$CMD_ORG6 = "NULL";}else{$CMD_ORG6 = "'".$CMD_ORG6."\|".$POS_ORG_ID6."'";}
		if(empty($CMD_ORG7) && empty($POS_ORG_ID7)){$CMD_ORG7 = "NULL";}else{$CMD_ORG7 = "'".$CMD_ORG7."\|".$POS_ORG_ID7."'";}
		if(empty($CMD_ORG8) && empty($POS_ORG_ID8)){$CMD_ORG8 = "NULL";}else{$CMD_ORG8 = "'".$CMD_ORG8."\|".$POS_ORG_ID8."'";}
                /*Release 5.1.0.7 End*/
	...
	if($command=="UPDATE" && $COM_ID){
		...
		if(empty($CMD_ORG8) && empty($POS_ORG_ID8)){$CMD_ORG8 = "NULL";}else{$CMD_ORG8 = "'".$CMD_ORG8."\|".$POS_ORG_ID8."'";}
		 ...
		 where COM_ID=$COM_ID and CMD_SEQ=$CMD_SEQ ";
                 //echo $cmd;
                 //echo "ต่ำกว่าสำนัก/กอง 1 ระดับ: ".$POS_POEM_ORG4." | ID |".$POS_ORG_ID4;
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
...
//	if ($command=="") $VIEW_PER=1;
	if((($PAGE_AUTH["add"]=="Y" && trim($CMD_SEQ_LAST)>0) || $UPD_PER || $VIEW_PER) && trim($COM_ID) && trim($CMD_SEQ)){
		...
		$cmd = "	select	CMD_SEQ, PER_ID, CMD_EDUCATE, CMD_DATE, CMD_POSITION, CMD_LEVEL, 
							...
							CMD_ES_CODE, ES_CODE, UPDATE_USER, UPDATE_DATE 
					from	PER_COMDTL
