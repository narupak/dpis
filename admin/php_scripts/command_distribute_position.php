<?	
	...
	if($command == "ADD" && trim($REQ_YEAR) && trim($REQ_NO)){
		...
		if($count_duplicate <= 0){
			$cmd = " select max(REQ_ID) as max_id from PER_REQ3 ";
			$db_dpis->send_cmd($cmd);
			$data = $db_dpis->get_array();
			$data = array_change_key_case($data, CASE_LOWER);
			$REQ_ID = $data[max_id] + 1;
			
			$cmd = " insert into PER_REQ3 
...
//		$db_dpis->show_error();
	...
	if($command == "DELETE" && trim($REQ_ID)){
		$cmd = " select REQ_YEAR, REQ_NO, REQ_TITLE from PER_REQ3 where REQ_ID=$REQ_ID ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		...
		$cmd = " delete from PER_REQ3_DTL where REQ_ID=$REQ_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
		
		$cmd = " delete from PER_REQ3 where REQ_ID=$REQ_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
