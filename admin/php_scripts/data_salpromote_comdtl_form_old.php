<?	
	...
	if($command=="ADD" || $command=="UPDATE"){
		...
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		if ($PAYMENT_FLAG=="P") $POS_ID = trim($data[PAY_ID]); 
	...
	if($command=="ADD"){
		$cmd = " insert into PER_COMDTL
							(	COM_ID, CMD_SEQ, PER_ID, EN_CODE, CMD_DATE, CMD_POSITION, 
								...
								PL_NAME_WORK, ORG_NAME_WORK,CMD_LEVEL_POS, CMD_SEQ_NO, CMD_MIDPOINT )
						 values
						 	(	$COM_ID, $CMD_SEQ, $PER_ID, $EN_CODE, '$CMD_DATE', '$CMD_POSITION', 
								'$CMD_LEVEL', '$CMD_ORG3', '$CMD_ORG4', '$CMD_ORG5', $CMD_OLD_SALARY, 
								$POS_ID, $POEM_ID, $POEMS_ID, $LEVEL_NO, $CMD_SALARY, $CMD_SPSALARY, 
								...
								$PL_NAME_WORK, $ORG_NAME_WORK,'$CMD_LEVEL1', $CMD_SEQ_NO, $CMD_MIDPOINT) ";
		$db_dpis->send_cmd($cmd);
		//$db_dpis->show_error();
...
//		$MOV_NAME = "";
		...
		if ($CMD_DATE2) {
			$arr_temp = explode("-", substr($CMD_DATE2, 0, 10));
			$CMD_DATE2 =  $arr_temp[2] ."/". $arr_temp[1] ."/". ($arr_temp[0] + 543);
		}
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	*/
	...
	if(($UPD || $VIEW) && trim($COM_ID) && trim($PER_ID)){	//\PER_ID (from submit form)
		$CH_ADD="";	//เคลียร์ค่า เพื่อให้แสดงปุ่มถูกต้อง
		//ค้นหาข้อมูลบุคคล
		...
		if($data_count > 0){		
			...
			if($DPISDB=="mysql"){	$temp_pos = explode("|", trim($data[CMD_POSITION]));	}
			else{	$temp_pos = explode("\|", trim($data[CMD_POSITION]));		}
		...
		if ($PER_TYPE == 1) {			
			...
			if(trim($data[POS_ID])){	
				...
				if ($PAYMENT_FLAG=="P") $POS_ID = trim($data2[PAY_ID]); 
