<?	
	...
	// add project
//	echo "command=$command && PLAN_ID=$PLAN_ID && TP_BUDGET_YEAR=$TP_BUDGET_YEAR && NEW_TP_BUDGET_YEAR=$NEW_TP_BUDGET_YEAR && NEW_PLAN_NAME=$NEW_PLAN_NAME && PROJ_ID=$PROJ_ID && PROJ_NAME=$PROJ_NAME && NEW_PROJ_NAME=$NEW_PROJ_NAME<br>";
	if($command=="ADDPROJ" && $TP_BUDGET_YEAR && trim($NEW_PROJ_NAME)){
		...
		$cmd = " insert into PER_TRAIN_PROJECT (PROJ_ID, PROJ_NAME, TPJ_BUDGET_YEAR, DEPARTMENT_ID,PLAN_ID,TPJ_MANAGE_ORG, TPJ_RESPONSE_ORG, TPJ_APP_PER_ID, PG_ID, TPJ_APP_DATE,TPJ_APP_DOC_NO,	TPJ_INOUT_TRAIN, UPDATE_USER, UPDATE_DATE)
						values	($PROJ_ID, '$NEW_PROJ_NAME', '$TP_BUDGET_YEAR', $DEPARTMENT_ID,$PLAN_ID, '$NEW_PROJ_MANAGE','$NEW_PROJ_REPONSE','$NEW_PROJ_PER_ID','$PG_ID','$NEW_PROJ_DATE','$NEW_PROJ_DOCNO', '$NEW_PROJ_INOUT',$SESS_USERID, '$UPDATE_DATE') ";
		$db_dpis->send_cmd($cmd);
//		echo "add project-$cmd<br>";
//		$db_dpis->show_error();
	...
	// update project
	//	echo $command." && ".$PLAN_ID." && ".$TP_BUDGET_YEAR." && ".trim($PROJ_NAME);
...
//echo "update-$cmd<br>";
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ปรับปรุงโครงการ [$PLAN_ID_REF : $PLAN_ID : $PLAN_NAME : $PROJ_ID : $PROJ_NAME]");
	} // end if

	// add plan
	//	echo $command."&& ".$NEW_TP_BUDGET_YEAR." && ".trim($NEW_PLAN_NAME);
	if($command=="ADD" && $TP_BUDGET_YEAR && trim($NEW_PLAN_NAME)){	//$NEW_TP_BUDGET_YEAR
		if(!$PLAN_ID) $PLAN_ID_REF = "NULL"; else $PLAN_ID_REF = $PLAN_ID;
...
//		$db_dpis->show_error();
		...
		if(!in_array($TP_BUDGET_YEAR, $arr_plan_year)) $arr_plan_year[] = $TP_BUDGET_YEAR;
		sort($arr_plan_year);
		$HAVE_YEAR += 1;				
	} // end if
...
//		$db_dpis->show_error();
	...
	if($command=="DELETEPROJ" && $PLAN_ID && $TP_BUDGET_YEAR && $PROJ_ID){
		delete_proj($PROJ_ID, $PROJ_ID_REF);
						 
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ลบโครงการฝึกอบรมย่อย [$PLAN_ID_REF : $PLAN_ID : $PLAN_NAME : $PROJ_ID : $PROJ_NAME]");
...
//	echo "<pre>"; print_r($arr_plan_year); echo "</pre>";
	...
	if($command!="CANCEL" && (!$TP_BUDGET_YEAR || !in_array($TP_BUDGET_YEAR, $arr_plan_year))) 
		$TP_BUDGET_YEAR = $START_YEAR;
...
//		echo "Data :: <pre>"; print_r($data); echo "</pre>";
	...
	if($PLAN_ID && $TREE_DEPTH == '2'){
		...
		$cmd = " select PER_NAME, PER_SURNAME, PN_NAME
					   from	PER_PERSONAL a, PER_PRENAME b
					  where a.PN_CODE=b.PN_CODE and PER_ID=$TPJ_APP_PER_ID ";
		$db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
		$data = $db_dpis->get_array();	
		$TPJ_APP_PER_NAME = $data[PN_NAME].$data[PER_NAME]." ".$data[PER_SURNAME];
	} // end if
...
//				echo "$cmd<br>";
				...
				$class = "label_normal"; //"table_body";
				if($data[PLAN_ID] == $sel_plan_id) $class = "table_body_2_over";
				...
				echo "<tr $onmouse_event>";
				echo "<td width=\"15\" align=\"center\" valign=\"middle\"><img src=\"images/$icon_name\" width=\"9\" height=\"9\" onClick=\"$onClick\" style=\"cursor:hand;\" vspace=\"5\" $onmouse_event></td>";
				echo "<td class=\"$class\" height=\"22\">&nbsp;<span onClick=\"select_plan(". $data[PLAN_ID] .",". ($data[PLAN_ID_REF] + 0) .",". $tree_depth .");\" style=\"cursor:hand\" $onmouse_event>" . $data[PLAN_NAME] . "</span></td>";
				echo "</tr>";
	...
	function delete_plan($PLAN_ID, $PLAN_ID_REF){	
		...
		$cmd = " select PROJ_ID, PROJ_ID_REF from PER_TRAIN_PROJECT where PLAN_ID=$PLAN_ID ";
...
//		$db_dpis->show_error();
		...
		if($count_child){
			while($data = $db_dpis->get_array()){
				delete_proj($data[PROJ_ID], $data[PROJ_ID_REF]);
			} // end while
		} // end if
...
//		$db_dpis->show_error();
	...
	function delete_proj($PROJ_ID, $PROJ_ID_REF){	
		global $dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd;
		global $LIST_OPENED_PLAN;
