...
//	echo "CMD_SEQ=$CMD_SEQ, VIEW_PER=$VIEW_PER, UPD_PER=$UPD_PER<br>";
	...
	if (!$PL_NAME_WORK) $PL_NAME_WORK = "ตัดโอนตำแหน่งและอัตราเงินเดือน";
	...
	if($command=="ADD"){
		...
		$cmd = " insert into PER_COMDTL (COM_ID, CMD_SEQ, PER_ID, CMD_EDUCATE, CMD_DATE, 
				...
				CMD_LEVEL_POS, CMD_POS_NO_NAME, CMD_POS_NO, CMD_ES_CODE, ES_CODE, PM_CODE)
				...
				0, $PER_CARDNO, $SESS_USERID, '$UPDATE_DATE', $PL_NAME_WORK, 
				$ORG_NAME_WORK, '$CMD_LEVEL1', '$CMD_POS_NO_NAME', '$CMD_POS_NO', '$CMD_ES_CODE', '$ES_CODE', $PM_CODE) ";
		$db_dpis->send_cmd($cmd);
		//echo "$cmd<br>";
		//$db_dpis->show_error();
...
//	if ($command=="") $VIEW_PER=1;
	if((($PAGE_AUTH["add"]=="Y" && trim($CMD_SEQ_LAST)>0) || $UPD_PER || $VIEW_PER) && trim($COM_ID) && trim($CMD_SEQ)){
		...
		$cmd = "	select	CMD_SEQ, PER_ID, CMD_EDUCATE, CMD_DATE, CMD_POSITION, CMD_LEVEL, 
							...
							PL_CODE_ASSIGN, PN_CODE_ASSIGN, EP_CODE_ASSIGN, TP_CODE_ASSIGN,
							CMD_NOTE1, CMD_NOTE2, MOV_CODE, PL_NAME_WORK, 
							ORG_NAME_WORK ,CMD_LEVEL_POS, CMD_POS_NO_NAME, CMD_POS_NO, 
							CMD_ES_CODE, ES_CODE, PM_CODE, UPDATE_USER, UPDATE_DATE 
					from	PER_COMDTL
		...
		if($UPD_PER || $VIEW_PER){		//เฉพาะ เพิ่ม ดู และแก้ไข
            ...
            if($tmp_PL_CODE_ASSIGN){ //ข้าราชการ
                      $cmd = " select PL_NAME from PER_LINE where PL_CODE='$tmp_PL_CODE_ASSIGN' ";		
                      $db_dpis1->send_cmd($cmd);
                      $data_dpis1 = $db_dpis1->get_array();
                      $PL_PN_NAME_ASSIGN = trim($data_dpis1[PL_NAME]);		
            		

            }else if($tmp_PN_CODE_ASSIGN){ //ลูกจ้างประจำ
                      $cmd = " select PN_NAME from PER_POS_NAME where PN_CODE='$tmp_PN_CODE_ASSIGN' ";		
                      $db_dpis1->send_cmd($cmd);
                      $data_dpis1 = $db_dpis1->get_array();
                      $PL_PN_NAME_ASSIGN = trim($data_dpis1[PN_NAME]);
           
            }else if($tmp_EP_CODE_ASSIGN){ //พนักงานราชการ
                      $cmd = " select EP_NAME from PER_EMPSER_POS_NAME where EP_CODE='$tmp_EP_CODE_ASSIGN' ";		
                      $db_dpis1->send_cmd($cmd);
                      $data_dpis1 = $db_dpis1->get_array();
                      $PL_PN_NAME_ASSIGN = trim($data_dpis1[EP_NAME]);
                
            }else if($tmp_TP_CODE_ASSIGN){//ลูกจ้างชั่วคราว       
                      $cmd = " select TP_NAME from PER_TEMP_POS_NAME where TP_CODE='$tmp_TP_CODE_ASSIGN' ";		
                      $db_dpis1->send_cmd($cmd);
                      $data_dpis1 = $db_dpis1->get_array();
                      $PL_PN_NAME_ASSIGN = trim($data_dpis1[TP_NAME]);
            }
