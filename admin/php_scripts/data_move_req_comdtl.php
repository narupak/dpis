<?
	...
	if ($order_by==1) $order_str = "a.CMD_SEQ"; 
		...
		if($DPISDB=="odbc") $order_str = "a.CMD_ORG3, a.CMD_ORG4, a.CMD_POS_NO_NAME, a.CMD_POS_NO";
		elseif($DPISDB=="oci8")  $order_str = "a.CMD_ORG3, a.CMD_ORG4, a.CMD_POS_NO_NAME, a.CMD_POS_NO";
		//elseif($DPISDB=="oci8")  $order_str = "a.CMD_ORG3, a.CMD_ORG4, a.CMD_POSITION";
		elseif($DPISDB=="mysql")  $order_str = "a.CMD_ORG3, a.CMD_ORG4, a.CMD_POSITION";
	...
	if( $command == "ADD" && trim(!$COM_ID) && trim($COM_NO) ){
		...
		if (trim($search_org_id2)=="" || $search_org_id2=="NULL" || is_null($search_org_id2)) {
			$search_org_id2="0";
		}
		...
		$cmd = " insert into PER_COMMAND (COM_ID, COM_NO, COM_NAME, COM_DATE, COM_NOTE, COM_PER_TYPE, 
						...
						'$COM_TYPE', 0, '', $DEPARTMENT_ID, $search_org_id2, $SESS_USERID, '$UPDATE_DATE') ";
		$db_dpis->send_cmd($cmd);
//			$db_dpis->show_error();
		//echo $cmd; die;

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ���������źѭ��Ṻ���¤�������� [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
	...
	if( $command == "UPDATE" && trim($COM_ID) ) {
		...
		$cmd = " update PER_COMMAND set  
						...
						COM_PER_TYPE=$COM_PER_TYPE, COM_TYPE='$COM_TYPE', ORG_ID=$search_org_id2, 
						UPDATE_USER=$SESS_USERID, UPDATE_DATE='$UPDATE_DATE' 
				where COM_ID=$COM_ID  ";
		$db_dpis->send_cmd($cmd);
		//$db_dpis->show_error();

		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ��䢢����źѭ��Ṻ���¤�������� [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
	}			// 	if( $command == "UPDATE" && trim($COM_ID) )

// ============================================================
	// ������ա���׹�ѹ�����Ţͧ�ѭ��Ṻ���¤����
	// 29/03/2555 ��� update �дѺ���˹� ����觵�駷�� �дѺ���˹� �ͧ���˹� ��� �дѺ���˹� �ͧ����� �дѺ���˹� �ͧ���˹觷�����͡��***
	if( $command == "COMMAND" && trim($COM_ID) ) {
		$COM_DATE =  save_date($COM_DATE);
		if ($search_department_name != "�����û���ͧ") include ("php_scripts/data_move_req_command_confirm_check.php");	
		if (!trim($error_move_personal)) {
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > �׹�ѹ�����źѭ��Ṻ���¤�������� [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
...
// ============================================================
	...
	/***	
		if($command == "DELETE" && trim($COM_ID) ){
			...
			insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ź�����źѭ��Ṻ���¤�������� [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$COM_NO." : ".$COM_TYPE."]");
		}
	***/
// ============================================================	
	...
	if($command == "DELETE_COMDTL" && trim($COM_ID) && trim($PER_ID)){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ź�����Ţ���Ҫ���/�١��ҧṺ���ºѭ�դ�������� [".trim($COM_ID)." : ".$PER_ID."]");
	...
	if($command == "DELETE_COMMAND" && trim($COM_ID) ){
		...
		insert_log("$MENU_TITLE_LV0 > $MENU_TITLE_LV1 > ź�����źѭ����Т���Ҫ���/�١��ҧṺ���ºѭ�դ�������� [$DEPARTMENT_ID : ".trim($COM_ID)." : ".$PER_ID."]");
		$COM_ID = "";
...
*/
	...
	if (trim($COM_ID)) {
		...
		$search_org_id2 = trim($data[ORG_ID]);
		...
		$cmd = " select ORG_NAME from PER_ORG where ORG_ID=$search_org_id2 ";
		$db_dpis->send_cmd($cmd);
		$data = $db_dpis->get_array();
		$search_org_name2 = $data[ORG_NAME];
	}
