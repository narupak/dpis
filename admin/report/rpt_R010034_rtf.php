...
//	$arr_rpt_order = array("MINISTRY", "DEPARTMENT", "ORG", "LINE"); 
	...
	include ("rpt_R010034_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
//	echo "default font_id::".$RTF->dfl_FontID."<br>";	
	...
	if ($BKK_FLAG==1)
		$report_title = "$DEPARTMENT_NAME||จำนวนข้าราชการกรุงเทพมหานคร จำแนกตามสายงาน เพศ และระดับตำแหน่ง";
	else
		$report_title = "$DEPARTMENT_NAME||จำนวนข้าราชการพลเรือน จำแนกตามสายงาน เพศ และระดับตำแหน่ง";
...
//				echo "data-".implode(",",$arr_data)."<br>";
			...
			$arr_data = (array) null;
			$arr_data[] = "<**1**>รวมทั้งหมด";	$data_align[0]="C";
			$arr_data[] = "<**1**>รวมทั้งหมด";	$data_align[1]="C";
			$cnt = 0;
			for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
				$LEVEL_GROUP = $ARR_LEVEL_GROUP[$i];
				for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
...
//			echo "รวม-".implode(",",$arr_data)." , align-".implode(",",$data_align)." (cnt=$cnt)<br>";
