<?	
	...
	$report_code = "xls_STRUCTURE_BY_ASSIGN";
...
//	echo "$data_count>>fname=$fname1<br>";
	...
	$head = "โครงสร้างตามมอบหมายงาน";

	//echo "$ORG_ID1:$search_ol_code<br>";
	if (!$ORG_ID1)  {
...
//	$UPDATE_DATE = date("Y-m-d H:i:s");
	...
	if ($count_data) {
		$cmd = " select ORG_CODE, ORG_NAME, OL_CODE, OT_CODE, OS_CODE, CT_CODE, PV_CODE from PER_ORG_ASS where ORG_ID=$ORG_ID1 ";
...
//		$worksheet->write($xlsRow, 0, $data[ORG_NAME], set_format("xlsFmtTitle", "B", "L", "TLBR", 0));
		...
		$worksheet->write($xlsRow, 0, $data[ORG_NAME], set_format("xlsFmtTitle", "B", "L", "TLBR", 0));
		$worksheet->write_string($xlsRow, 1, $data[ORG_CODE], set_format("xlsFmtTitle", "B", "L", "TLBR", 0));
...
//	$worksheet->write($xlsRow, 1, $skey, set_format("xlsFmtTitle", "", "L", "", 0));
	...
	function get_org ($org_parent, $level) {
		...
		$cmd = " select ORG_ID, ORG_CODE, ORG_NAME, OL_CODE, OT_CODE, OS_CODE, CT_CODE, PV_CODE from PER_ORG_ASS where ORG_ID_REF=$org_parent and ORG_ID<>$org_parent order by ORG_SEQ_NO, ORG_CODE ";
