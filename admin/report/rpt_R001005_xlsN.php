...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($i=0; $i<count($arr_title); $i++){
			...
			for($j=1; $j<=count($ARR_LEVEL_NO); $j++) $worksheet->write($xlsRow, $j, "", set_format("xlsFmtTitle", "B", "C", "", 1));
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1){
				...
				for($i=0; $i<count($ARR_LEVEL_NO); $i++){ 
					$tmp_level_no = $ARR_LEVEL_NO[$i];
					$worksheet->write($xlsRow, ($i+1), ($COUNT_LEVEL[$tmp_level_no]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($COUNT_LEVEL[$tmp_level_no])):number_format($COUNT_LEVEL[$tmp_level_no])):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				} // end for
				$worksheet->write($xlsRow, (count($ARR_LEVEL_NO)+1), ($COUNT_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($COUNT_TOTAL)):number_format($COUNT_TOTAL)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				$worksheet->write($xlsRow, (count($ARR_LEVEL_NO)+2), ($PERCENT_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($PERCENT_TOTAL,2)):number_format($PERCENT_TOTAL, 2)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
			}else{
				$xlsRow++;
				$worksheet->write_string($xlsRow, 0, "$NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				for($i=0; $i<count($ARR_LEVEL_NO); $i++){ 
					$tmp_level_no = $ARR_LEVEL_NO[$i];
					$worksheet->write($xlsRow, ($i+1), ($COUNT_LEVEL[$tmp_level_no]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($COUNT_LEVEL[$tmp_level_no])):number_format($COUNT_LEVEL[$tmp_level_no])):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
				} // end for
				...
				$worksheet->write($xlsRow, (count($ARR_LEVEL_NO)+2), ($PERCENT_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($PERCENT_TOTAL,2)):number_format($PERCENT_TOTAL, 2)):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			} // end if
		} // end for
		...
		for($i=0; $i<count($ARR_LEVEL_NO); $i++){ 
			$tmp_level_no = $ARR_LEVEL_NO[$i];
			$worksheet->write($xlsRow, ($i+1), ($LEVEL_TOTAL[$tmp_level_no]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($LEVEL_TOTAL[$tmp_level_no])):number_format($LEVEL_TOTAL[$tmp_level_no])):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
		} // end for
		...
		$worksheet->write($xlsRow, (count($ARR_LEVEL_NO)+2), ($PERCENT_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($PERCENT_TOTAL)):number_format($PERCENT_TOTAL, 2)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
	}else{
		$xlsRow = 0;
