<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 3, "โมเดล", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
                $worksheet->write($xlsRow, 4, "ค่าน้ำหนักตั้งต้น (ร้อยละ)", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 3, $CP_MODEL, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                        $worksheet->write($xlsRow, 4, $DEF_WEIGHT, set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			$worksheet->write($xlsRow, 5, "", set_format("xlsFmtTableDetail", "", "C", "TLRB", 0));
			$worksheet->insert_bitmap($xlsRow, 5, $CP_ACTIVE, 35, 4, 1, 0.8);
		} // end while
	}else{
		$xlsRow = 0;
