		...
		if($position_type == 1){
			...
			if($DPISDB=="odbc"){
				...
				$cmd = " select			count($position_select) as count_position
						   ...
						   where		a.ORG_ID=b.ORG_ID and b.ORG_ID_REF=c.ORG_ID and $position_join and d.PER_STATUS=1 and d.PER_TYPE=$search_per_type
										$search_condition
						   group by		$position_select ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			count($position_select) as count_position
...
//				if(count($arr_exclude)) $search_condition .= " and $position_select not in (". implode(",", $arr_exclude) .")";
			...
			if($DPISDB=="odbc"){
				...
				$cmd = " select			count($position_select) as count_position
								 ...
								 where		a.ORG_ID=b.ORG_ID and b.ORG_ID_REF=c.ORG_ID and $position2
													$search_condition
								 group by	$position_select ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			count($position_select) as count_position
...
//				if(count($arr_exclude)) $search_condition .= " and $position_select not in (". implode(",", $arr_exclude) .")";
			...
			if($DPISDB=="odbc"){
				...
				$cmd = " select			count($position_select) as count_position
								 from			$position_table a, PER_ORG b, PER_ORG c
								 where		a.ORG_ID=b.ORG_ID and b.ORG_ID_REF=c.ORG_ID and $position3
													$search_condition
								 group by	$position_select ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			count($position_select) as count_position
...
//		echo "$head_text1<br>";
		...
		$arr_series_caption = array("","ถือครอง","","ว่างมีเงิน","","ว่างไม่มีเงิน","","",""); // หัวที่ต้องการเฉพาะออก กราฟ map ให้ตรงกับ head ของ pdf
