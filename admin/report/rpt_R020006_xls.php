	...
	for($rpt_order_index=0; $rpt_order_index<count($arr_rpt_order); $rpt_order_index++){
		...
		switch($REPORT_ORDER){
			case "ORG" :
				...
				if($select_org_structure==0) $select_list .= "e.ORG_ID_3";
				else if($select_org_structure==1) $select_list .= "a.ORG_ID";

				if($order_by) $order_by .= ", ";
				if($select_org_structure==0) $order_by .= "e.ORG_ID_3";
...
//	$arr_search_condition[] = "(a.PER_STATUS in (". implode(", ", $search_per_status) ."))";
	$arr_search_condition[] = "(trim(e.MOV_CODE) in ('118', '11810', '11820', '11830'))";
	...
	if($list_type == "PER_ORG_TYPE_1"){
		...
		if(trim($search_org_id)){ 
			if($select_org_structure == 0) $arr_search_condition[] = "(e.ORG_ID_3 = $search_org_id)";
		...
		if(trim($search_org_id_1)){ 
			...
			if($select_org_structure == 1) $arr_search_condition[] = "(a.ORG_ID_1 = $search_org_id_1)";
			$list_type_text .= " - $search_org_name_1";
		} // end if
		if(trim($search_org_id_2)){ 
			if($select_org_structure == 0) $arr_search_condition[] = "(b.ORG_ID_2 = $search_org_id_2)";
			if($select_org_structure == 1) $arr_search_condition[] = "(a.ORG_ID_2 = $search_org_id_2)";
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, 5, "จังหวัดที่", set_format("xlsFmtTableHeader", "B", "C", "LTR", 0));
		...
		$worksheet->write($xlsRow, 5, "ที่ปฏิบัติงาน", set_format("xlsFmtTableHeader", "B", "C", "LBR", 0));
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1){
				...
				$worksheet->write_string($xlsRow, 5, "$PROVINCE_WORK", set_format("xlsFmtTableDetail", "B", "L", "TLRB", 0));
				...
				$worksheet->write_string($xlsRow, 8, "$STARTDATE", set_format("xlsFmtTableDetail", "B", "L", "TLRB", 0));
				$worksheet->write_string($xlsRow, 9, "$EFFECTIVEDATE", set_format("xlsFmtTableDetail", "B", "L", "TLRB", 0));
				...
				$worksheet->write_string($xlsRow, 9, "$EFFECTIVEDATE", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
