...
//	session_cache_limiter("nocache");
	...
	session_start();
		...
		$fname= "rpt_master_table_district.rtf";
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6]
							from		$table
							where		PV_CODE='$PV_CODE' and AP_CODE='$AP_CODE'
											$search_condition
							order by IIF(ISNULL(DT_SEQ_NO), 9999, DT_SEQ_NO),$arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6]
							from		$table
							where		PV_CODE='$PV_CODE' and AP_CODE='$AP_CODE'
											$search_condition
							order by DT_SEQ_NO,$arr_fields[0] ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6]
							from		$table
							where		PV_CODE='$PV_CODE' and AP_CODE='$AP_CODE'
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$arr_data[] = "<*img*".(($data[$arr_fields[6]]==1)?"../images/checkbox_check.jpg":"../images/checkbox_blank.jpg")."*img*>";
