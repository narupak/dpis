<?
	...
	$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
        $pdf->text(20,10,"ฉบับที่ 1 (สำหรับผู้ถูกหักภาษี ณ ที่จ่าย ใช้แนบพร้อมกับแบบแสดงรายการภาษี)");
        $pdf->text(20,16,"ฉบับที่ 2 (สำหรับผู้ถูกหักภาษี ณ ที่จ่าย เก็บไว้เป็นหลักฐาน)");
         $page_start_x = $pdf->x;			
        $page_start_y = $pdf->y;
        
         $pdf->Rect(16, 20, 180, 250 , 'D');//กรอบใหญ่
        $pdf->SetFont($font,'',14);
        $page_start_x = $pdf->x;			
        $page_start_y = $pdf->y;
         ...
         $pdf->SetFont($font,'',14);
         $pdf->text(83,26,"หนังสือรับรองการหักภาษี ณ ที่จ่าย");
         $pdf->text(182,26,"");
         $pdf->text(172,27,"เล่มที่.................");
         $pdf->Ln();
          
         $pdf->SetFont($font,'',12);
         $pdf->text(83,31,"ตามมาตรา 50 ทวิ แห่งประมวลรัษฎากร");
         $pdf->SetFont($font,'',14);
         $pdf->text(182,32,"");
         $pdf->text(172,33,"เลขที่.................");
         $pdf->Ln(15);
        ...
        function getmonth_slip($date){
            ...
            if($mounth == "1"){ $full_mounth_th = "มกราคม"; }
            else if($mounth == "2"){ $full_mounth_th = "กุมภาพันธ์";}
            else if($mounth == "3"){ $full_mounth_th = "มีนาคม"; }
            else if($mounth == "4"){ $full_mounth_th = "เมษายน"; }
            else if($mounth == "5"){ $full_mounth_th = "พฤษภาคม"; }
            else if($mounth == "6"){ $full_mounth_th = "มิถุนายน"; }
            else if($mounth == "7"){ $full_mounth_th = "กรกฎาคม"; }
            else if($mounth == "8"){ $full_mounth_th = "สิงหายน"; }
            else if($mounth == "9"){ $full_mounth_th = "กันยายน"; }
            else if($mounth == "10"){ $full_mounth_th = "ตุลาคม"; }
            else if($mounth == "11"){ $full_mounth_th = "พฤศจิกายน"; }
            else if($mounth == "12"){ $full_mounth_th = "ธันวาคม"; }
   ...
   /*     
	...
	$pdf->Rect(20, 35, 172, 30 , 'D');                                      
        $pdf->SetFont($font,'',14);
        $pdf->text(24,40,"ผู้มีหน้าที่หักภาษี ณ ที่จ่าย :-",0,1,'L');
         
        $pdf->SetFont($font,'',12);
        $pdf->text(114,40,"เลขประจำตัวประชาชน",0,1,'L');
        $pdf->text(24,48,"ชื่อ   ",0,1,'L');
        $pdf->text(28,48,"    -------------------------------------------------------------------------------------");
        $pdf->text(32,52,"( ให้ระบุว่าเป็น บุคคล นิติบุคคล บริษัท สมาคม หรือคณะบุคคล )");
        $pdf->text(116,45,"เลขประจำตัวผู้เสียภาษีอากร",0,1,'L');
        $pdf->text(120,50,"( กรอกเฉพาะกรณีเป็นผู้ไม่มีเลขบัตรประจำตัวประชาชน )",0,1,'L');
        $pdf->text(24,58,"ที่อยู่   ",0,1,'L');
        $pdf->text(28,59,"    -------------------------------------------------------------------------------------------------------------------------------------------------------------------");
        $pdf->text(32,63,"( ให้ระบุ ชื่ออาคาร/หมู่บ้าน ห้องเลขที่ ชั้นที่ เลขที่ ตรอก/ซอย หมู่ที่ ถนน ตำบล/แขวง อำเภอ/เขต จังหวัด )");
         
       
        $pdf->SetFont($font,'',13);
         
        $pdf->text(150,44,$ORGTAX_NO);                                                //ช่องกรอก ข้อมูล 
        $pdf->text(32,45,$ORG_NAME);                                                 //ช่องกรอก ข้อมูล
        $pdf->text(32,57,$ORG_ADDR);                                                //ช่องกรอก ข้อมูล
	$page_start_x = $pdf->x;			
        $page_start_y = $pdf->y;
        ...
        $pdf->SetFont($font,'',14);
        $pdf->text(24,73,"ผู้ถูกหักภาษี ณ ที่จ่าย :-");
         
        $pdf->SetFont($font,'',12);
        $pdf->text(110,73,"เลขประจำตัวประชาชน");
        $pdf->text(24,81,"ชื่อ   ");
        $pdf->text(28,81,"    -----------------------------------------------------------------------");
        $pdf->text(32,84,"( ให้ระบุว่าเป็น บุคคล นิติบุคคล บริษัท สมาคม หรือคณะบุคคล )");
        $pdf->text(115,78,"เลขประจำตัวผู้เสียภาษีอากร");
        $pdf->text(120,83,"( กรอกเฉพาะกรณีเป็นผู้ไม่มีเลขบัตรประจำตัวประชาชน )");
        $pdf->text(24,91,"ที่อยู่   ");
        $pdf->text(28,91,"    -------------------------------------------------------------------------------------------------------------------------------------------------------------------");
        $pdf->text(32,94,"( ให้ระบุ ชื่ออาคาร/หมู่บ้าน ห้องเลขที่ ชั้นที่ เลขที่ ตรอก/ซอย หมู่ที่ ถนน ตำบล/แขวง อำเภอ/เขต จังหวัด )");
        
        $pdf->text(24,102,"ลำดับที่   ");
        $pdf->Rect(37, 97, 18, 7 , 'D');
        $pdf->text(60,102,"ในแบบ   ");
        ...
        $pdf->text(145,72,$PER_CARDNO);                                                      //ช่องกรอก ข้อมูล
        $pdf->text(148,77,$PER_TAXNO);                                                      //ช่องกรอก ข้อมูล
        $pdf->text(35,78,$PER_NAME);                                                       //ช่องกรอก ข้อมูล
        $pdf->text(35,89,$PER_ADDR);                                                      //ช่องกรอก ข้อมูล
        $pdf->text(40,101,$SEQ_NO);                                                      //ช่องกรอก ข้อมูล
	...
	$pdf->Rect(20, 123, 172, 72 , 'D');
        ...
        $pdf->text(50,132,"ประเภทเงินได้พึงประเมินจ่าย");
        $pdf->text(116,130," วัน เดือน ");
        $pdf->text(112,135," หรือปีภาษี ที่จ่าย");
        $pdf->text(140,132," จำนวนเงินที่จ่าย");
        $pdf->text(170,130," ภาษีที่หัก");
        $pdf->text(170,135, "และนำส่งไว้");
        $pdf->text(25,150, "1. เงินเดือน ค่าจ้าง เบี้ยเลี้ยงโบนัส ฯลฯ ตามมาตรา 40(1)"); 
        //$pdf->text(25,155, "2. ค่าธรรมเนียม ค่านายหน้า ฯลฯ ตามมาตรา 40(2)"); 
        $pdf->text(85,181, "รวมเงินที่จ่ายและภาษีที่หักนำส่ง");//-----สรุปเงิน 
        $pdf->text(28,191, "รวมเงินภาษีที่หักนำส่ง (ตัวอักษร)");
	
        $pdf->text(121,150, $TAX_YEAR);                                         //ช่องกรอก ข้อมูล
        $pdf->text(141,150, $INCOME_B);                                          //ช่องกรอก ข้อมูล
        $pdf->text(159,150, $INCOME_S);                                              //ช่องกรอก ข้อมูล
        $pdf->text(171,150, $TAX_INCOME_B);                                      //ช่องกรอก ข้อมูล
        $pdf->text(186,150, $TAX_INCOME_S);                                          //ช่องกรอก ข้อมูล
        
        $pdf->text(141,180, $NET_INCOME_B);                                      //ช่องกรอก ข้อมูล
        $pdf->text(159,180, $NET_INCOME_S);                                                  //ช่องกรอก ข้อมูล
        $pdf->text(171,180, $NETTAX_INCOME_B);                                                   //ช่องกรอก ข้อมูล
        $pdf->text(186,180, $NETTAX_INCOME_S);                                                  //ช่องกรอก ข้อมูล
        
        
        $pdf->setFillColor(230,230,230);                                        
        $pdf->setY(185);                                                            
        $pdf->setX(85);                                                             
        $pdf->Cell(107,8,$NETTAX_CHAR.".",0,1,'',1);                                  //ช่องกรอก ข้อมูล              
	...
	$pdf->Rect(20, 198, 172, 10 , 'D');                            
        $pdf->SetFont($font,'',14);
        $pdf->text(24,204,"เงินสะสมที่จ่ายเข้า");
	...
	$pdf->Rect(20, 211, 172, 10 , 'D');                            
        ...
        $pdf->text(22,235,"คำเตือน  ผู้ที่มีหน้าที่ออกหนังสือรับรองการหักภาษี");
        $pdf->text(35,241,"ณ ที่จ่าย ฝ่าฝืนไม่ปฏิบัติตามมาตรา 50 ทวิ ");
        $pdf->text(35,247,"แห่งประมวลรัษฎากร  ต้องรับโทษทางอาญา");
        $pdf->text(35,253,"ตามมาตรา 35");
        $pdf->SetFont($font,'',14);
        $pdf->text(100,233,"ขอรับรองว่าข้อความและตัวเลขดังกล่าวข้างต้นถูกต้องตรงกับความจริง");
        $pdf->text(136,238,"ทุกประการ");
        if($PIC_SIGN_PER == ""){}
        //else{$pdf->Image($PIC_SIGN_PER, 128, 239, 30, 10,"jpg");}	
        $pdf->text(102,250,"ลงฃื่อ.....................................................................................ผู้จ่ายเงิน");
        
        $pdf->text(122,263,"(วัน เดือน ปี ที่ออกหนังสือรับรอง ฯ)");
        $pdf->Rect(97, 224, 95, 42 , 'D'); 
        
        
       $pdf->text(128,256,$TAX_DATE_A);                                    //ช่องกรอก ข้อมูล
