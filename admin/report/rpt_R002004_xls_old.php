...
//	$arr_rpt_order = array("ORG", "ORG_1", "ORG_2", "LINE", "LEVEL", "SEX", "PROVINCE"); 
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, 1, "ช่วงอายุ", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 1));
		...
		if ($SESS_DEPARTMENT_NAME=="กรมการปกครอง") {
			$worksheet->write($xlsRow, 5, "อายุเฉลี่ย", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		} else {
			...
			$worksheet->write($xlsRow, 9, "อายุเฉลี่ย", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		}

		$xlsRow++;
		$worksheet->write($xlsRow, 0, "", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
		for($i=1; $i<=$heading_column; $i++) $worksheet->write($xlsRow, $i, (($NUMBER_DISPLAY==2)?convert2thaidigit($arr_age_duration[$i][name]):$arr_age_duration[$i][name]), set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
		$worksheet->write($xlsRow, $heading_column+1, "", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
	} // function		
...
//							$db_dpis2->show_error();
               ...
               if(($PL_NAME !="" && $PL_NAME !="-") || ($BKK_FLAG==1 && $PL_NAME !="" && $PL_NAME !="-")){
				...
				}//f(($ORG_NAME !="" && $ORG_NAME !="-") || ($BKK_FLAG==1 && $ORG_NAME !="" && $ORG_NAME !="-"))	
				break;
...
//	echo "GRAND_TOTAL = $GRAND_TOTAL";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if ($list_type_text == "ทั้งส่วนราชการ - กรุงเทพมหานคร" && $REPORT_ORDER == "ORG") {
				...
				if ($parent != $arr_content[$data_count][parent] && strlen($arr_content[$data_count][parent]) > 0) {
					...
					$xlsRow++;
					$worksheet->write_string($xlsRow, 0, $parent, set_format("xlsFmtTitle", "B", "L", "TLRB", 0));
					for($i=1; $i<=$heading_column; $i++) 
					$worksheet->write($xlsRow, $i, "", set_format("xlsFmtTableHeader", "B", "R", "TLRB", 0));
					$worksheet->write($xlsRow, $heading_column+1, "", set_format("xlsFmtTableHeader", "B", "R", "TLRB", 0));
			...
			if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1){
				$xlsRow++;
				$worksheet->write_string($xlsRow, 0, ($f_tab?"     ":"").(($NUMBER_DISPLAY==2)?convert2thaidigit($NAME):$NAME), set_format("xlsFmtTableDetail", "B", "L", "TLRB", 0));
				for($i=1; $i<=$heading_column; $i++) 
				$worksheet->write_string($xlsRow, $i, (${"COUNT_".$i}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_".$i})):number_format(${"COUNT_".$i})):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				$worksheet->write_string($xlsRow, $heading_column+1, ($AVG_AGE?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($AVG_AGE,2)):number_format($AVG_AGE, 2)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
			}else{
				$xlsRow++;
				$worksheet->write_string($xlsRow, 0, ($f_tab?"     ":"").(($NUMBER_DISPLAY==2)?convert2thaidigit($NAME):$NAME), set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				for($i=1; $i<=$heading_column; $i++)
				 $worksheet->write_string($xlsRow, $i, (${"COUNT_".$i}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_".$i})):number_format(${"COUNT_".$i})):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
				$worksheet->write_string($xlsRow, $heading_column+1, ($AVG_AGE?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($AVG_AGE,2)):number_format($AVG_AGE, 2)):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			} // end if
		} // end for
		...
		for($i=1; $i<=$heading_column; $i++){
		 	$TOTAL_GRAND_TOTAL+=${"GRAND_TOTAL_".$i};
		 	$worksheet->write_string($xlsRow, $i, (${"GRAND_TOTAL_".$i}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"GRAND_TOTAL_".$i})):number_format(${"GRAND_TOTAL_".$i})):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));	}
			$worksheet->write_string($xlsRow, $heading_column+1, ($TOTAL_GRAND_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_GRAND_TOTAL,2)):number_format($TOTAL_GRAND_TOTAL, 2)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
	}else{
		$xlsRow = 0;
