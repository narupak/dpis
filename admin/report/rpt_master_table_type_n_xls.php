<?
	...
	$worksheet = &$workbook->addworksheet("ระดับตำแหน่ง(ใหม่)");
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ระดับตำแหน่ง (ใหม่)", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "ประเภทตำแหน่ง (ใหม่)", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $PT_GROUP_NAME, set_format("xlsFmtTableDetail", "", "C", "TLRB", 0));
