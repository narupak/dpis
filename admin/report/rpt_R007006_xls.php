<?
	...
	include ("rpt_R007006_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
	...
	$report_title = "$DEPARTMENT_NAME||รายงานการไม่บันทึกเวลาปฏิบัติราชการของ$PERSON_TYPE[$search_per_type]" . (($search_date_min || $search_date_max)?"||":"") . (($search_date_min)?"ตั้งแต่วันที่ $show_date_min ":"") . (($search_date_max)?"ถึงวันที่ $show_date_max":"");	
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
    	$ws_head_line1 = array("ชื่อ - สกุล","วันที่","รอบ","รายละเอียด","หมายเหตุ");
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){		
		...
		$cmd = " select		b.WC_NAME, START_DATE, END_DATE, ABSENT_FLAG, REMARK
				 from 		PER_WORK_TIME a, PER_WORK_CYCLE b
				 where		PER_ID=$PER_ID and a.WC_CODE=b.WC_CODE and a.ABSENT_FLAG = 9 
			 				$search_condition
				order by START_DATE ";
...
//		$db_dpis2->show_error();		
		while($data2 = $db_dpis2->get_array()){		
			...
			$START_DATE = show_date_format($data2[START_DATE],$DATE_DISPLAY);
...
//		print_header();
		...
		// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนข้อมูล
			...
			$wsdata_align_1 = array("L","C","C","L","L");
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "ORG"){
				$report_title = "$DEPARTMENT_NAME||รายงานการไม่บันทึกเวลาปฏิบัติราชการของ$per_type_name $NAME" . (($search_date_min || $search_date_max)?"||":"") . (($search_date_min)?"ตั้งแต่วันที่ $show_date_min ":"") . (($search_date_max)?"ถึงวันที่ $show_date_max":"");	
                 $report_title = (($NUMBER_DISPLAY==2)?convert2thaidigit($report_title):$report_title);
				$xlsRow = 0;
