...
foreach($PER_EFFECTIVEDATE as $key=>$value){		//[$LEVEL_NOHIS][$PER_ID]
				...
				foreach($PER_EFFECTIVEDATE[$LEVEL_NO] as $key2=>$value2){
					...
					$arr_content[$data_count][position] = $PL_NAME[$PER_ID] . $POSITION_LEVEL[$PER_ID] . (($PT_CODE[$PER_ID] != "11" && $LEVEL_NO[$PER_ID] >= 6)?"$PT_NAME[$PER_ID]":"")."\n$ORG_ID_1_NAME[$PER_ID]  $ORG_NAME[$PER_ID]";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$pdf->y = $start_y;
			$pdf->MultiCell($heading_width[1], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($position):$position), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			$pdf->x = $start_x + $heading_width[0] + $heading_width[1];
			$pdf->y = $start_y;
			$pdf->Cell($heading_width[2], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($posno):$posno), $border, 0, 'C', 0);
			$pdf->MultiCell($heading_width[3], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($educate):$educate), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3];
			$pdf->y = $start_y;
			$pdf->MultiCell($heading_width[4], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($em_name):$em_name), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4];
			$pdf->y = $start_y;
			$pdf->Cell($heading_width[5], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($poh_effectivedate):$poh_effectivedate), $border, 0, 'C', 0);
			$pdf->MultiCell($heading_width[6], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($per_sortlevel):$per_sortlevel), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;
			$pdf->Cell($heading_width[7], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($per_salary):$per_salary), $border, 0, 'R', 0);
			$pdf->MultiCell($heading_width[8], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($decorate):$decorate), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;		
			$pdf->MultiCell($heading_width[9], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($per_startdate):$per_startdate), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;		
			$pdf->MultiCell($heading_width[10], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($per_birthdate):$per_birthdate), $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;		
			$pdf->Cell($heading_width[11], 7, (($NUMBER_DISPLAY==2)?convert2thaidigit($per_retiredate):$per_retiredate), $border, 0, 'C', 0);
			if($REPORT_ORDER == "DETAIL"){
				$pdf->MultiCell($heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4] + $heading_width[5] + $heading_width[6] + $heading_width[7] + $heading_width[8], 7, "", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4] + $heading_width[5] + $heading_width[6] + $heading_width[7] + $heading_width[8];
				$pdf->y = $start_y;		
				$pdf->Cell($heading_width[9], 7, ($per_work?$per_work:"-"), $border, 0, 'C', 0);	
				$pdf->Cell($heading_width[10], 7, ($per_age?$per_age:"-"), $border, 0, 'C', 0);
