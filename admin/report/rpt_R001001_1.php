...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		$pdf->Cell(($heading_width[1] + $heading_width[2]) ,7,"ถือครอง",'LTBR',0,'C',1);
		$pdf->Cell(($heading_width[3] + $heading_width[4]) ,7,"ว่างมีเงิน",'LTBR',0,'C',1);
		$pdf->Cell(($heading_width[5] + $heading_width[6]) ,7,"ว่างไม่มีเงิน",'LTBR',0,'C',1);
		$pdf->Cell(($heading_width[7] + $heading_width[8]) ,7,"รวม",'LTBR',1,'C',1);

		$pdf->Cell($heading_width[0] ,7,"",'LBR',0,'C',1);
		$pdf->Cell($heading_width[1] ,7,"ตำแหน่ง",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[2] ,7,"ร้อยละ",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[3] ,7,"ตำแหน่ง",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[4] ,7,"ร้อยละ",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[5] ,7,"ตำแหน่ง",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[6] ,7,"ร้อยละ",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[7] ,7,"ตำแหน่ง",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[8] ,7,"ร้อยละ",'LTBR',1,'C',1);
	} // function		
	...
	function count_position($position_type, $search_condition, $addition_condition){
		...
		if($position_type == 1){
			...
			}elseif($select_org_structure == 1){	//มอบหมายงาน
				if($DPISDB=="odbc"){
					$cmd = " select			count(a.POS_ID) as count_position
							   from			(
												(
...
//		$count_position = $data[count_position] + 0;
	...
	//แสดงรายชื่อหน่วยงาน
	if($select_org_structure == 0){ //ตามกฎหมาย
		if($DPISDB=="odbc"){
		...
		$cmd = " select		a.ORG_SEQ_NO, a.ORG_ID_REF as MINISTRY_ID, b.ORG_SEQ_NO, b.DEPARTMENT_ID, b.ORG_SEQ_NO, b.ORG_ID
						from			PER_ORG a, PER_ORG_ASS b
						where		a.ORG_ID=b.DEPARTMENT_ID and b.DEPARTMENT_ID = $DEPARTMENT_ID and b.OL_CODE = '03'
						order by		b.ORG_SEQ_NO, b.ORG_ID ";
	} // end if
...
//							$db_dpis2->show_error();
						...
						$arr_content[$data_count][short_name] = str_repeat(" ", ($rpt_order_index * 5)) .$ORG_NAME;
				...
				case "ORG_1" :
					if($ORG_ID_1 != $data[ORG_ID_1]){
						...
						if($ORG_ID_1 == ""){
							...
							$ORG_SHORT_1 = "[ไม่ระบุฝ่าย]";
						}else{
							if($select_org_structure==0) $cmd = " select ORG_NAME,ORG_SHORT from PER_ORG where ORG_ID=$ORG_ID_1 ";
							$db_dpis2->send_cmd($cmd);
//							$db_dpis2->show_error();
							$data2 = $db_dpis2->get_array();
				...
				case "ORG_2" :
					if($ORG_ID_2 != $data[ORG_ID_2]){
						...
						if($ORG_ID_2 == ""){
							...
							$ORG_SHORT_2 = "[ไม่ระบุงาน]";
						}else{
							if($select_org_structure==0) $cmd = " select ORG_NAME,ORG_SHORT from PER_ORG where ORG_ID=$ORG_ID_2 ";
							$db_dpis2->send_cmd($cmd);
//							$db_dpis2->show_error();
							$data2 = $db_dpis2->get_array();
...
//							$db_dpis2->show_error();
	...
	//print("<pre>");	print_r($GRAND_TOTAL);	print("</pre>");
...
//	echo "<pre>"; print_r($GRAND_TOTAL_1); echo "</pre>";
//	echo "<pre>"; print_r($GRAND_TOTAL_2); echo "</pre>";
//	echo "<pre>"; print_r($GRAND_TOTAL_3); echo "</pre>";
//	echo "<pre>"; print_r($GRAND_TOTAL); echo "</pre>";
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "DEPARTMENT"){
				if($data_count > 0){
					...
					$pdf->Cell($heading_width[3], 7, ($GRAND_TOTAL_2[$DEPARTMENT_ID]?number_format($GRAND_TOTAL_2[$DEPARTMENT_ID]):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[4], 7, ($PERCENT_TOTAL_2?number_format($PERCENT_TOTAL_2, 2):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[5], 7, ($GRAND_TOTAL_3[$DEPARTMENT_ID]?number_format($GRAND_TOTAL_3[$DEPARTMENT_ID]):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[6], 7, ($PERCENT_TOTAL_3?number_format($PERCENT_TOTAL_3, 2):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[7], 7, ($GRAND_TOTAL[$DEPARTMENT_ID]?number_format($GRAND_TOTAL[$DEPARTMENT_ID]):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[8], 7, ($PERCENT_TOTAL?number_format($PERCENT_TOTAL, 2):"-"), $border, 1, 'R', 0);
				} // end if

				$ORG_ID_REF = $arr_content[$data_count][id];					//รหัสกรม
				//$REF_NAME = $arr_content[$data_count][ref_name];		//กระทรวง
				$NAME = $arr_content[$data_count][name];							//กรม
				...
				$pdf->Cell($heading_width[7], 7, ($COUNT_TOTAL?number_format($COUNT_TOTAL):"-"), $border, 0, 'R', 0);
				$pdf->Cell($heading_width[8], 7, ($PERCENT_TOTAL?number_format($PERCENT_TOTAL, 2):"-"), $border, 0, 'R', 0);
...
//_________________________
		...
		if(count($GRAND_TOTAL) > 1){
			...
			$pdf->Cell($heading_width[2], 7, ($PERCENT_TOTAL_1?number_format($PERCENT_TOTAL_1, 2):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[3], 7, (array_sum($GRAND_TOTAL_2)?number_format(array_sum($GRAND_TOTAL_2)):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[4], 7, ($PERCENT_TOTAL_2?number_format($PERCENT_TOTAL_2, 2):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[5], 7, (array_sum($GRAND_TOTAL_3)?number_format(array_sum($GRAND_TOTAL_3)):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[6], 7, ($PERCENT_TOTAL_3?number_format($PERCENT_TOTAL_3, 2):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[7], 7, (array_sum($GRAND_TOTAL)?number_format(array_sum($GRAND_TOTAL)):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[8], 7, ($PERCENT_TOTAL?number_format($PERCENT_TOTAL, 2):"-"), $border, 1, 'R', 0);
		} // end if
	}else{
		///$pdf->AddPage();
