...
//	session_cache_limiter("nocache");
	...
	function list_position($search_condition, $addition_condition){
		...
		if($search_per_type == 1 || $search_per_type == 5){
			if($DPISDB=="odbc"){
				$cmd = " select			$line_name, a.CL_NAME, $position_no as POS_NO, $position_no_name as POS_NO_NAME, h.PT_NAME, a.POS_SALARY, a.POS_CHANGE_DATE, f.POSITION_TYPE
						   from	(	
										(
											(
												(
												...
												) left join PER_TYPE d on (a.PT_CODE=h.PT_CODE)
	...
	//			$search_condition = str_replace(" where ", " and ", $search_condition);	
				$cmd = " select			$line_name, a.CL_NAME, $position_no as POS_NO, $position_no_name as POS_NO_NAME, h.PT_NAME, a.POS_SALARY, a.POS_CHANGE_DATE, f.POSITION_TYPE
						   ...
						   where		a.ORG_ID=b.ORG_ID and a.CL_NAME=c.CL_NAME(+) and b.ORG_ID_REF=d.ORG_ID and d.ORG_ID_REF=g.ORG_ID
											...
											$search_condition
						   order by	$line_sort, $line_code, f.LEVEL_SEQ_NO desc, a.LEVEL_NO, $position_no_name, TO_NUMBER($position_no) ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			$line_name, a.CL_NAME, $position_no as POS_NO, $position_no_name as POS_NO_NAME, h.PT_NAME, a.POS_SALARY, a.POS_CHANGE_DATE, f.POSITION_TYPE
						   from	 (	
										(
	...
	//			$search_condition = str_replace(" where ", " and ", $search_condition);	
				$cmd = " select			$line_name, $position_no as POS_NO, $position_no_name as POS_NO_NAME
						   from			$position_table a, PER_ORG b, PER_ORG d, $line_table e, PER_ORG g
						   where		a.ORG_ID=b.ORG_ID and b.ORG_ID_REF=d.ORG_ID and d.ORG_ID_REF=g.ORG_ID	and $line_join 			
											$search_condition
						   order by	$line_sort, $line_code, $position_no_name, TO_NUMBER($position_no) ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			$line_name, $position_no as POS_NO, $position_no_name as POS_NO_NAME
						   from	 (	
										(
...
//			$PL_NAME = trim($data[PL_SHORTNAME])?$data[PL_SHORTNAME]:$data[PL_NAME];
			...
			$arr_content[$data_count][name] = str_repeat(" ", (($rpt_order_index + 1) * 5)) . $position_count .". ". $PL_NAME;
...
/*
		...
		if($data[MINISTRY_ID] && $MINISTRY_ID != $data[MINISTRY_ID]){
			...
			if (in_array("COUNTRY", $arr_rpt_order))  {
				...
				$rpt_order_start_index = 0;
				$addition_condition = generate_condition(0);
			}
