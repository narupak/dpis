<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อสถาบันการศึกษา", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "รหัสกรมบัญชีกลาง", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], INS_CGD_CODE
							from  		$table
							where		CT_CODE='$CT_CODE'
											$search_condition
							order by IIF(ISNULL(INS_SEQ_NO), 9999, INS_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], INS_CGD_CODE
							from  		$table
							where		CT_CODE='$CT_CODE'
											$search_condition
							order by INS_SEQ_NO, $arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], INS_CGD_CODE
							from  		$table
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $INS_CGD_CODE, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
