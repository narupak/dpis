...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$arr_content[$data_count][old_position] = ($OLD_POSITION)? "$OLD_POSITION (ระดับที่บรรจุ ".level_no_format($LEVEL_FILL).")" : "";
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	header("Content-Type: application/x-msexcel; name=\"รายชื่อข้าราชการได้เลื่อนระดับควบปลาย.xls\"");
	header("Content-Disposition: inline; filename=\"รายชื่อข้าราชการได้เลื่อนระดับควบปลาย.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
