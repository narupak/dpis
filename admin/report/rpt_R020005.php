...
//	$arr_rpt_order = array("ORG", "ORG_1", "ORG_2", "LINE"); 
	...
	$arr_search_condition[] = "(trim(e.MOV_CODE) in ('10110', '10120', '10130', '10140', '10510', '10520'))";
	...
	$report_title = "รายชื่อ$PERSON_TYPE[$search_per_type] ที่บรรจุใหม่ในปีงบประมาณ $search_budget_year || ส่วนราชการ $DEPARTMENT_NAME";
	$report_code = "R20005";
	$orientation='L';
...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		$pdf->Cell($heading_width[7] ,7,"วัน/เดือน/ปีเกิด",'LTR',0,'C',1);
		$pdf->Cell($heading_width[8] ,7,"วัน/เดือน/ปี",'LTR',1,'C',1);
		...
		$pdf->Cell($heading_width[5] ,7,"ที่บรรจุ",'LBR',0,'C',1);
		$pdf->Cell($heading_width[6] ,7,"สูงสุด",'LBR',0,'C',1);
		$pdf->Cell($heading_width[7] ,7,"",'LBR',0,'C',1);
		$pdf->Cell($heading_width[8] ,7,"ที่บรรจุ",'LBR',1,'C',1);
	} // function		
	...
	function list_person($search_condition, $addition_condition){
		...
		global $position_table,$position_join,$line_table,$line_join,$line_name;

		if(trim($addition_condition)) $search_condition .= (trim($search_condition)?" and ":" where ") . $addition_condition;
		if($search_per_type==1){
			if($DPISDB=="odbc"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, g.PT_NAME, 
													LEFT(trim(a.PER_BIRTHDATE), 10) as PER_BIRTHDATE, MIN(LEFT(trim(e.POH_EFFECTIVEDATE), 10)) as POH_EFFECTIVEDATE
								 from			(
														(
															(
																(
								...
								$search_condition
								 group by	a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, g.PT_NAME,LEFT(trim(a.PER_BIRTHDATE), 10) ";
			}elseif($DPISDB=="oci8"){	
				$search_condition = str_replace(" where ", " and ", $search_condition);
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, g.PT_NAME,
													SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) as PER_BIRTHDATE, MIN(SUBSTR(trim(e.POH_EFFECTIVEDATE), 1, 10)) as POH_EFFECTIVEDATE													
								 ...
								 where		$position_join(+) and b.ORG_ID=c.ORG_ID(+)
													...
													$search_condition
								 group by	a.PER_ID,d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, g.PT_NAME,SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, g.PT_NAME, 
													LEFT(trim(a.PER_BIRTHDATE), 10), MIN(LEFT(trim(e.POH_EFFECTIVEDATE), 10)) as POH_EFFECTIVEDATE
								 from			(
														(
															(
																(
			...
			if($DPISDB=="odbc"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name as PL_NAME, e.LEVEL_NO, 
								...
								$search_condition
								 group by	a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, LEFT(trim(a.PER_BIRTHDATE), 10) ";
			}elseif($DPISDB=="oci8"){	
				$search_condition = str_replace(" where ", " and ", $search_condition);
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name as PL_NAME, e.LEVEL_NO, 
													SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) as PER_BIRTHDATE, MIN(SUBSTR(trim(e.POH_EFFECTIVEDATE), 1, 10)) as POH_EFFECTIVEDATE
								 ...
								 where		$position_join(+) and b.ORG_ID=c.ORG_ID(+)
													...
													$search_condition
								 group by	a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) ";
			}elseif($DPISDB=="mysql"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name as PL_NAME, e.LEVEL_NO, 
													LEFT(trim(a.PER_BIRTHDATE), 10) as PER_BIRTHDATE, MIN(LEFT(trim(e.POH_EFFECTIVEDATE), 10)) as POH_EFFECTIVEDATE
								 from			(
...
//			echo "<br>$cmd<br>";
			...
			$arr_content[$data_count][posdate] = $POH_EFFECTIVEDATE;	//วันบรรจุ
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$pdf->MultiCell($heading_width[5], 7, "$EDU_USE", $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;
			$pdf->Cell($heading_width[7], 7, "$BIRTHDATE", $border, 0, 'C', 0);
			$pdf->Cell($heading_width[8], 7, "$POSTDATE", $border, 0, 'C', 0);
