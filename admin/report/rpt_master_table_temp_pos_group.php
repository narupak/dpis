...
//	session_cache_limiter("nocache");
	...
	session_start();
		...
		$fname= "rpt_master_table_temp_pos_group.rtf";
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select	$arr_fields[0], $arr_fields[1], $arr_fields[3], TG_SEQ_NO
							from		$table
							$search_condition
							order by IIF(ISNULL(TG_SEQ_NO), 9999, TG_SEQ_NO),$arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select	$arr_fields[0], $arr_fields[1], $arr_fields[3], TG_SEQ_NO 
								from		$table
								$search_condition
								order by TG_SEQ_NO,$arr_fields[0] ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[3], TG_SEQ_NO 
								from		$table
								$search_condition
