...
//หาช่วงวันที่ 15 / 30
	...
	AND NVL(DEPARTMENT_ID,-1)=$DEPARTMENT_ID
    AND NVL(ORG_LOWER1,-1)=$ORG_LOWER1
    AND NVL(ORG_LOWER2,-1)=$ORG_LOWER2
    AND NVL(ORG_LOWER3,-1)=$ORG_LOWER3
		and SET_FLAG=1 
		and CONTROL_ID IS NOT NULL order by ot_date";
    ...
    for($idx=2;$idx<=$cntMaxDay;$idx++){
        $day++;
        $heading_text[$idx] = '<**1**>วันที่ปฏิบัติงาน|'.($dayList[$idx-2]+0);
    }
