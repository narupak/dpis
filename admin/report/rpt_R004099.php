...
//	session_cache_limiter("nocache");
	...
	$heading_width[POSITIONHIS][1] = "50";          
	...
	$heading_width[TRAINING][1] = "95";
	$heading_width[TRAINING][2] = "40";
	$heading_width[TRAINING][3] = "40";
	...
	$heading_width[SERVICEHIS][2] = "30";
	$heading_width[SERVICEHIS][3] = "40";
	$heading_width[SERVICEHIS][4] = "50";
...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		switch($HISTORY_NAME){
			...
			case "SERVICEHIS" :
				...
				$pdf->Cell($heading_width[$HISTORY_NAME][2] ,7,"สถานที่/หน่วยงาน",'LTBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][3] ,7,"เลขที่คำสั่ง",'LTBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][4] ,7,"หมายเหตุ",'LTBR',1,'C',1);
				break;			
	...
	if($DPISDB=="odbc"){
		$cmd = " select			a.PER_ID, a.LEVEL_NO, f.POSITION_LEVEL, b.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.PER_BIRTHDATE, 
...
//				$db_dpis2->show_error();
				...
				$show_persontype = "ลูกจ้างชั่วคราว";
				$POT_ID = $data[POT_ID];
				$POT_NO = $data[EMPTEMP_POS_NO];
...
*/
			...
			$pdf->Cell(95,7,$pdf->Text_print_optimize("ตำแหน่ง  ".(trim($PL_NAME)?($PL_NAME . $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".$POSITION_LEVEL), 100),0,0,"L");
					...
					case "POSITIONHIS" :
							...
							if($DPISDB=="odbc"){
								$cmd = " select			a.POH_ORG2 as ORG_NAME_1, a.POH_ORG3 as ORG_NAME_2, a.POH_EFFECTIVEDATE, d.PL_NAME, 
																	a.LEVEL_NO, g.POSITION_LEVEL, a.POH_POS_NO, a.POH_SALARY, a.PT_CODE, a.PM_CODE, f.PM_NAME, h.MOV_NAME
												   from			(
																		(
																			(
																				PER_POSITIONHIS a
								...
								$cmd = " select			a.POH_ORG2 ORG_NAME_1, a.POH_ORG3 ORG_NAME_2, a.POH_EFFECTIVEDATE, d.PL_NAME, 
																	a.LEVEL_NO, g.POSITION_LEVEL, a.POH_POS_NO, a.POH_SALARY, a.PT_CODE, a.PM_CODE, f.PM_NAME, h.MOV_NAME
												   from			PER_POSITIONHIS a, PER_LINE d, PER_MGT f, PER_LEVEL g, PER_MOVMENT h
							...
							if($count_positionhis){
								...
								while($data2 = $db_dpis2->get_array()){
									...
									if($POH_EFFECTIVEDATE){
										$arr_temp = explode("-", substr($POH_EFFECTIVEDATE, 0, 10));
									...
									if( ("$PM_CODE $PL_NAME $LEVEL_NO $POH_POS_NO") != (trim($data2[PM_CODE])." ".trim($data2[PL_NAME])." ".trim($data2[LEVEL_NO])." ".trim($data2[POH_POS_NO])) ){
										...
										$TMP_PL_NAME = (trim($PL_NAME)?($PL_NAME . $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".$POSITION_LEVEL);
							...
							if($count_salaryhis){
								...
								while($data2 = $db_dpis2->get_array()){
									...
									if($SAH_EFFECTIVEDATE){
										$arr_temp = explode("-", substr($SAH_EFFECTIVEDATE, 0, 10));
										$SAH_EFFECTIVEDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
...
//							$db_dpis2->show_error();
							if($count_exincomehis){
								...
								while($data2 = $db_dpis2->get_array()){
									$exincomehis_count++;
									$EXINH_EFFECTIVEDATE = trim($data2[EXINH_EFFECTIVEDATE]);
									if($EXINH_EFFECTIVEDATE){
										$arr_temp = explode("-", substr($EXINH_EFFECTIVEDATE, 0, 10));
										$EXINH_EFFECTIVEDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
									$EXINH_ENDDATE = trim($data2[EXINH_ENDDATE]);
									if($EXINH_ENDDATE){
										$arr_temp = explode("-", substr($EXINH_ENDDATE, 0, 10));
										$EXINH_ENDDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
									$EXIN_NAME = trim($data2[EXIN_NAME]);
									$EXINH_AMOUNT = $data2[EXINH_AMT];
...
//							$db_dpis2->show_error();
							if($count_absenthis){
								...
								while($data2 = $db_dpis2->get_array()){
									$absenthis_count++;
									$ABS_STARTDATE = trim($data2[ABS_STARTDATE]);
									if($ABS_STARTDATE){
										$arr_temp = explode("-", substr($ABS_STARTDATE, 0, 10));
										$ABS_STARTDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
									$ABS_ENDDATE = trim($data2[ABS_ENDDATE]);
									if($ABS_ENDDATE){
										$arr_temp = explode("-", substr($ABS_ENDDATE, 0, 10));
										$ABS_ENDDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
...
//							$db_dpis2->show_error();
							if($count_punishmenthis){
								...
								while($data2 = $db_dpis2->get_array()){
									...
									$PUN_STARTDATE = trim($data2[PUN_STARTDATE]);
									if($PUN_STARTDATE){
										$arr_temp = explode("-", substr($PUN_STARTDATE, 0, 10));
										$PUN_STARTDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
									$PUN_ENDDATE = trim($data2[PUN_ENDDATE]);
									if($PUN_ENDDATE){
										$arr_temp = explode("-", substr($PUN_ENDDATE, 0, 10));
										$PUN_ENDDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
...
//							$db_dpis2->show_error();
							if($count_rewardhis){
								...
								while($data2 = $db_dpis2->get_array()){
									$rewardhis_count++;
									$REH_DATE = trim($data2[REH_DATE]);
									if($REH_DATE){
										$arr_temp = explode("-", substr($REH_DATE, 0, 10));
										$REH_DATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
							...
							if($count_marryhis){
								...
								while($data2 = $db_dpis2->get_array()){
									...
									if($MAH_MARRY_DATE){
										...
										$MAH_MARRY_DATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
...
//							$db_dpis2->show_error();
							if($count_namehis){
								...
								while($data2 = $db_dpis2->get_array()){
									$namehis_count++;
									$NH_DATE = trim($data2[NH_DATE]);
									if($NH_DATE){
										$arr_temp = explode("-", substr($NH_DATE, 0, 10));
										$NH_DATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
...
//							$db_dpis2->show_error();
							if($count_decoratehis){
								...
								while($data2 = $db_dpis2->get_array()){
									...
									if($DEH_DATE){
										$arr_temp = explode("-", substr($DEH_DATE, 0, 10));
										$DEH_DATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
					...
					case "SERVICEHIS" :
							...
							if($DPISDB=="odbc"){
								$cmd = " select			a.SRH_STARTDATE, b.SV_NAME, a.SRH_DOCNO, a.SRH_NOTE, c.ORG_NAME
												 ...
												 where		a.PER_ID=$PER_ID and
																	a.SV_CODE=b.SV_CODE and a.ORG_ID=c.ORG_ID
...
//							$db_dpis2->show_error();
							if($count_servicehis){
								...
								while($data2 = $db_dpis2->get_array()){
									$servicehis_count++;
									$SRH_STARTDATE = trim($data2[SRH_STARTDATE]);
									if($SRH_STARTDATE){
										$arr_temp = explode("-", substr($SRH_STARTDATE, 0, 10));
										$SRH_STARTDATE = ($arr_temp[2] + 0) ." ". $month_abbr[($arr_temp[1] + 0)][TH] ." ". ($arr_temp[0] + 543);
									} // end if	
									...
									$pdf->MultiCell($heading_width[$HISTORY_NAME][2], 7, "$SRH_ORG", $border, "L");
									if($pdf->y > $max_y) $max_y = $pdf->y;
									$pdf->x = $start_x + $heading_width[$HISTORY_NAME][0] + $heading_width[$HISTORY_NAME][1] + $heading_width[$HISTORY_NAME][2];
									$pdf->y = $start_y;

									$pdf->MultiCell($heading_width[$HISTORY_NAME][3], 7, "$SRH_DOCNO", $border, "L");
									if($pdf->y > $max_y) $max_y = $pdf->y;
									$pdf->x = $start_x + $heading_width[$HISTORY_NAME][0] + $heading_width[$HISTORY_NAME][1] + $heading_width[$HISTORY_NAME][2] + $heading_width[$HISTORY_NAME][3];
									$pdf->y = $start_y;

									$pdf->MultiCell($heading_width[$HISTORY_NAME][4], 7, "$SRH_NOTE", $border, "L");
									if($pdf->y > $max_y) $max_y = $pdf->y;
