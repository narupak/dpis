<?
	...
	$db_dpis3 = new connect_dpis($dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd);	
        ...
        /*end เพิ่ม ตรวจสอบก่อนทำการแทรกคอลัมน์ 11/11/2016 */
...
//	$arr_rpt_order = array("POSNO"); 
	...
	function print_header(){
		...
		$worksheet->set_column(19, 19, 5);
		$worksheet->set_column(20, 20, 5);
		$worksheet->set_column(21, 21, 5);
		...
		$worksheet->set_column(39, 39, 20);
		$worksheet->set_column(40, 40, 20);
		$worksheet->set_column(41, 41, 20);
		$worksheet->set_column(42, 42, 20);
                $worksheet->set_column(43, 43, 50);
                $worksheet->set_column(44, 44, 50);
                $worksheet->set_column(45, 45, 50);
                $worksheet->set_column(46, 46, 50);
                $worksheet->set_column(47, 47, 50);
                $worksheet->set_column(48, 48, 50);
                $worksheet->set_column(49, 49, 50);
                $worksheet->set_column(50, 50, 50);
                $worksheet->set_column(51, 51, 50);
                $worksheet->set_column(52, 52, 50);
                $worksheet->set_column(53, 53, 50);
                $worksheet->set_column(54, 54, 50);
		...
		$worksheet->write($xlsRow, 46, "tempPerSPSkillDes1", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 47, "tempMPerSPSkill2", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 48, "tempSPerSPSkill2", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 49, "tempSkillLevel2", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 50, "tempPerSPSkillDes2", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 51, "tempMPerSPSkill3", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
		$worksheet->write($xlsRow, 52, "tempSPerSPSkill3", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 53, "tempSkillLevel3", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
                $worksheet->write($xlsRow, 54, "tempPerSPSkillDes3", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
	} // function		
...
//		ตัดแปะเข้ามาจาก rpt_gmis_xls_dtl.php
		for($rpt_order_index=0; $rpt_order_index < count($arr_rpt_order); $rpt_order_index++){
			$REPORT_ORDER = $arr_rpt_order[$rpt_order_index];
			switch($REPORT_ORDER){
				case "POSNO" :
...
*/							
							...
							if($PER_ID){
								...
								/*if(!$count_educate){
									if($DPISDB=="odbc"){
										// === หาการระดับการศึกษาในตำแหน่งปัจจุบัน สาขา โรงเรียน 
										$cmd = " 	select 	c.EL_CODE, c.EL_NAME, b.EN_NAME, d.EM_NAME, a.INS_CODE, a.ST_CODE, EDU_INSTITUTE
												from 	((
															PER_EDUCATE a
															left join PER_EDUCNAME b on (a.EN_CODE=b.EN_CODE)
								...
								/*if(!$count_educate){
									if($DPISDB=="odbc"){
										// === หาการระดับการศึกษาที่ใช้บรรจุ สาขา โรงเรียน 
										$cmd = " 	select 	c.EL_CODE, c.EL_NAME, b.EN_NAME, d.EM_NAME, a.INS_CODE, a.ST_CODE, EDU_INSTITUTE
												from 	((
															PER_EDUCATE a
															left join PER_EDUCNAME b on (a.EN_CODE=b.EN_CODE)
									...
									$count_educate = $db_dpis2->send_cmd($cmd);
                                                                ...
                                                                /*วุฒิในตำแหน่งปัจจุบัน [http://dpis.ocsc.go.th/Service/node/1136]*/
                                                            /*$cmd = " select c.EL_CODE, c.EL_NAME, b.EN_NAME, d.EM_NAME, a.INS_CODE, a.ST_CODE, EDU_INSTITUTE  
                                                                    from PER_EDUCATE a, PER_EDUCNAME b, PER_EDUCLEVEL c, PER_EDUCMAJOR d  
                                                                    where a.PER_ID=$PER_ID and a.EDU_TYPE like '%2%' 
                                                            ...
                                                            /*ปรับใหม่ เพิ่มเงือนไขใหญ่คุม http://dpis.ocsc.go.th/Service/node/1153*/
                                                                ...
                                                                if($PER_ID){
                                                                        ...
                                                                        $cmd = "  select * from (
                                                                                        select rownum rnum, q1.* from ( 
                                                                                                select 	 	b.PER_ID, c.SS_NAME, SPS_EMPHASIZE, b.SS_CODE, b.LEVELSKILL_CODE,NVL(b.AUDIT_FLAG,'N') as AUDIT_FLAG
                                                                                                from 		PER_SPECIAL_SKILL b 
                                                                                                                left join PER_SPECIAL_SKILLGRP c on(b.SS_CODE = c.SS_CODE)
                                                                                                where 		PER_ID=$PER_ID   
                                                                                                ORDER BY        NVL(b.AUDIT_FLAG,'N')  DESC, b.SPS_SEQ_NO  asc  
                                                                                         )  q1
                                                                        ...
                                                                        $REF_CODE = get_chind_special_skill($db_dpis2,$SS_CODE);//trim($data2[SS_NAME]);
                                                                        ...
                                                                        //echo $LEVELSKILL_NAME;
                                                                        //echo "ความเชี่ยวชาญพิเศษ 1 =>".$SS_NAME."<br>";
                                                                        //echo "เน้นทาง 1 =>".$SPS_EMPHASIZE."<br>";
                                                                
                                                                        // ความเชี่ยวชาญพิเศษ และ เน้นทาง col 2  07/11/2016
                                                                        $cmd = "  select * from (
                                                                                        select rownum rnum2, q2.* from ( 
                                                                                                select 	 	b.PER_ID, c.SS_NAME as SS_NAME2, SPS_EMPHASIZE as SPS_EMPHASIZE2, b.SS_CODE as SS_CODE2, b.LEVELSKILL_CODE as LEVELSKILL_CODE2, NVL(b.AUDIT_FLAG,'N') as AUDIT_FLAG2
                                                                                                from 		PER_SPECIAL_SKILL b 
                                                                                                                left join PER_SPECIAL_SKILLGRP c on(b.SS_CODE = c.SS_CODE)
                                                                                                where 		PER_ID=$PER_ID  
                                                                                                ORDER BY        NVL(b.AUDIT_FLAG,'N')  DESC, b.SPS_SEQ_NO  asc   
                                                                                         )  q2
                                                                                ) where rnum2 between 2 and 2 ";
                                                                                                
                                                                        $db_dpis2->send_cmd($cmd);
                                                                        $data2 = $db_dpis2->get_array();
                                                                        $SS_CODE2 = trim($data2[SS_CODE2]);
                                                                        $REF_CODE2 = get_chind_special_skill($db_dpis2,$SS_CODE2);//trim($data2[SS_NAME2]);
                                                                        $SPS_EMPHASIZE2 = trim($data2[SPS_EMPHASIZE2]);
                                                                        $AUDIT_FLAG2 = trim($data2[AUDIT_FLAG2]);
                                                                        $LEVELSKILL_CODE2 = trim($data2[LEVELSKILL_CODE2]);
                                                                        //echo    "LV2CODE =".$LEVELSKILL_CODE2;
                                                                        ...
                                                                        //echo    "LV2NAME =".$LEVELSKILL_NAME2;
                                                                        
                                                                
                                                                        // ความเชี่ยวชาญพิเศษ และ เน้นทาง col 3 07/11/2016
                                                                        $cmd = " select * from (
                                                                                        select rownum rnum3, q3.* from ( 
                                                                                                select 	 	b.PER_ID, c.SS_NAME as SS_NAME3, SPS_EMPHASIZE as SPS_EMPHASIZE3, b.SS_CODE as SS_CODE3, b.LEVELSKILL_CODE as LEVELSKILL_CODE3, NVL(b.AUDIT_FLAG,'N') as AUDIT_FLAG3
                                                                                                from 		PER_SPECIAL_SKILL b 
                                                                                                                left join PER_SPECIAL_SKILLGRP c on(b.SS_CODE = c.SS_CODE)
                                                                                                where 		PER_ID=$PER_ID  
                                                                                                ORDER BY        NVL(b.AUDIT_FLAG,'N')  DESC, b.SPS_SEQ_NO  asc   
                                                                                         )  q3
                                                                                ) where rnum3 between 3 and 3 ";
                                                                                                
                                                                        $db_dpis2->send_cmd($cmd);
                                                                        $data2 = $db_dpis2->get_array();
                                                                        $SS_CODE3 = trim($data2[SS_CODE3]);
                                                                        $REF_CODE3 = get_chind_special_skill($db_dpis2,$SS_CODE3);//trim($data2[SS_NAME3]);
                                                                        $SPS_EMPHASIZE3 = trim($data2[SPS_EMPHASIZE3]);
                                                                        $AUDIT_FLAG3 = trim($data2[AUDIT_FLAG3]);
                                                                       
                                                                        $LEVELSKILL_CODE3 = trim($data2[LEVELSKILL_CODE3]);
                                                                            
                                                                        $cmd = "   select LEVELSKILL_NAME AS LEVELSKILL_NAME3
                                                                                   from 		PER_LEVELSKILL
                                                                                   where LEVELSKILL_CODE = $LEVELSKILL_CODE3";
                                                                                                
                                                                        $db_dpis2->send_cmd($cmd);
                                                                        $data2 = $db_dpis2->get_array();
                                                                        $LEVELSKILL_NAME3 = $data2[LEVELSKILL_NAME3];
		...
		//					$arr_content[$data_count][per_mgtsalary] = (($PER_MGTSALARY)?number_format($PER_MGTSALARY,0,'',','):"");
							...
							$arr_content[$data_count][percent_salary2] = $PERCENT_SALARY2; 
                                                        ...
                                                        if($REF_CODE == NULL){
                                                            ...
                                                            if($AUDIT_FLAG3 == "Y"){
                                                                $arr_content[$data_count][SS_CODE3] = $SS_CODE3;
                                                                $arr_content[$data_count][REF_CODE3] = $REF_CODE3;
                                                                $arr_content[$data_count][LEVELSKILL_NAME3] = $LEVELSKILL_NAME3;
                                                                $arr_content[$data_count][SPS_EMPHASIZE3] = $SPS_EMPHASIZE3;
                                                            }else if($AUDIT_FLAG3 == "N"){
                                                                $arr_content[$data_count][SS_CODE3] = $SS_CODE3;
                                                                $arr_content[$data_count][REF_CODE3] = $REF_CODE3;
                                                                $arr_content[$data_count][LEVELSKILL_NAME3] = $LEVELSKILL_NAME3;
                                                                $arr_content[$data_count][SPS_EMPHASIZE3] = "*".$SPS_EMPHASIZE3;
                                                            ...
                                                            if($AUDIT_FLAG3 == "Y"){
                                                                $arr_content[$data_count][SS_CODE3] = $REF_CODE3;
                                                                $arr_content[$data_count][REF_CODE3] = $SS_CODE3;
                                                                $arr_content[$data_count][LEVELSKILL_NAME3] = $LEVELSKILL_NAME3;
                                                                $arr_content[$data_count][SPS_EMPHASIZE3] = $SPS_EMPHASIZE3;
                                                            }else if($AUDIT_FLAG3 == "N"){
                                                                $arr_content[$data_count][SS_CODE3] = $REF_CODE3;
...
//		จบการ ตัดแปะเข้ามาจาก rpt_gmis_xls_dtl.php
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$PERCENT_SALARY2 = $arr_content[$data_count][percent_salary2];
                        ...
                        if($PER_NAME && $PER_ID){
                                ...
                                $SS_CODE3 = $arr_content[$data_count][SS_CODE3];
                                $REF_CODE3 = $arr_content[$data_count][REF_CODE3];
                                $LEVELSKILL_NAME3 = $arr_content[$data_count][LEVELSKILL_NAME3];
                                $SPS_EMPHASIZE3 = $arr_content[$data_count][SPS_EMPHASIZE3];
                        }else{
                            ...
                            $SS_CODE3 = $arr_content[SS_CODE3];
                            $REF_CODE3 = $arr_content[REF_CODE3];
                            $LEVELSKILL_NAME3 = $arr_content[LEVELSKILL_NAME3];
                            $SPS_EMPHASIZE3 = $arr_content[SPS_EMPHASIZE3];
                        }
...
//				require_once "../../Excel/my.defined_format.inc.php";	// use set_format(set of parameter) funtion , help is in file
				...
				if($company_name){
					...
					$worksheet->write($xlsRow, 42, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
                                        ...
                                        $worksheet->write($xlsRow, 53, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
                                        $worksheet->write($xlsRow, 54, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
			...
			if(!$f_org_ref_skip){ // ถ้ารายการนี้มี $REPORT_ORDER == "ORG_REF" จะไม่พิมพ์รายละเอียดข้างล่าง
				...
				$worksheet->write_string($xlsRow, 42, "$PERCENT_SALARY2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                ...
                                $worksheet->write_string($xlsRow, 47, "$SS_CODE2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 48, "$REF_CODE2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 49, "$LEVELSKILL_NAME2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 50, "$SPS_EMPHASIZE2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 51, "$SS_CODE3", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 52, "$REF_CODE3", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 53, "$LEVELSKILL_NAME3", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                                $worksheet->write_string($xlsRow, 54, "$SPS_EMPHASIZE3", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				...
				if(!$repeat_POS_NO){
					$cmd = " insert into GMIS_GPIS (TEMPMINISTRY, TEMPORGANIZE, TEMPDIVISIONNAME, TEMPORGANIZETYPE, TEMPPOSITIONNO, TEMPMANAGEPOSITION, 
									...
									TEMPDECORATION, TEMPUNION, TEMPRESULT1, TEMPPERCENTSALARY1, TEMPRESULT2, TEMPPERCENTSALARY2,TEMPMPERSPSKILL1,TEMPSPERSPSKILL1,TEMPSKILLLEVEL1,TEMPPERSPSKILLDES1,
                                                                        TEMPMPERSPSKILL2,TEMPSPERSPSKILL2,TEMPSKILLLEVEL2,TEMPPERSPSKILLDES2,TEMPMPERSPSKILL3,TEMPSPERSPSKILL3,TEMPSKILLLEVEL3,TEMPPERSPSKILLDES3)
									...
									'$RESULT2', '$PERCENT_SALARY2','$SS_CODE','$REF_CODE','$LEVELSKILL_NAME','$SPS_EMPHASIZE','$SS_CODE2','$REF_CODE2','$LEVELSKILL_NAME2','$SPS_EMPHASIZE2','$SS_CODE3','$REF_CODE3','$LEVELSKILL_NAME3','$SPS_EMPHASIZE3') ";
					$db_dpis->send_cmd($cmd);
                                       //echo "cmd ="."<pre>".$cmd;
				}
				//$db_dpis->show_error();
