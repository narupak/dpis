<?
	...
	if($order_by==1) $order_str = "LAST_LOGIN $SortType[$order_by], FULLNAME $SortType[$order_by]";
  	elseif($order_by==2) $order_str = "FULLNAME $SortType[$order_by], LAST_LOGIN $SortType[$order_by]";
...
//	echo "search_date_from=$search_date_from , search_date_to=$search_date_to<br>";
	...
	if(trim($search_date_from)) {
		...
		if($DPISDB=="odbc") 
			$arr_search_condition[] = "(LEFT(trim(LAST_LOGIN), 10) >= '$temp_start') ";
		elseif($DPISDB=="oci8") 
			$arr_search_condition[] = "(SUBSTR(trim(LAST_LOGIN), 1, 10) >= '$temp_start') ";
		elseif($DPISDB=="mysql")
			$arr_search_condition[] = "(SUBSTRING(trim(LAST_LOGIN), 1, 10) >= '$temp_start') ";
	}
	if(trim($search_date_to)){
		$temp_end = (substr($search_date_to, 6, 4) - 543) ."-". substr($search_date_to, 3, 2) ."-". substr($search_date_to, 0, 2);
		if($DPISDB=="odbc") 
			$arr_search_condition[] = "(LEFT(trim(LAST_LOGIN), 10) >= '$temp_end') ";
		elseif($DPISDB=="oci8") 
			$arr_search_condition[] = "(SUBSTR(trim(LAST_LOGIN), 1, 10) <= '$temp_end') ";
		elseif($DPISDB=="mysql")
			$arr_search_condition[] = "(SUBSTRING(trim(LAST_LOGIN), 1, 10) >= '$temp_end') ";
	}
	...
	if($search_log_detail) {  $arr_search_condition[] = "UPPER(FROM_IP) LIKE UPPER ('%$search_log_detail%') or LOWER(FROM_IP) LIKE LOWER ('%$search_log_detail%')";}

	$search_condition = "";           
	if ($arr_search_condition)		$search_condition = implode(" and ", $arr_search_condition);
		$cmd ="select 	LOG_ID, USER_ID, USERNAME, FULLNAME, FROM_IP, LAST_LOGIN
									  from 		USER_LAST_ACCESS ".($search_condition ? "where ".$search_condition : "")." ";
	...
	if($DPISDB=="odbc"){
			...
			$cmd = "select * from (
							   select rownum rnum, q1.* from ( 
									  select 	LOG_ID, USER_ID, USERNAME, FULLNAME, FROM_IP, LAST_LOGIN
									  from 		USER_LAST_ACCESS ".($search_condition ? "where ".$search_condition : "")."
									  order by 	$order_str  
							   )  q1
						) ";	
...
//												$search_condition 
	...
	//		echo "$head_text1<br>";
		...
		$TMP_FROM_IP = trim($data[FROM_IP]);
		include("../php_scripts/user_log_inquire_decode.php");
		$TMP_LAST_LOGIN = show_date_format($data[LAST_LOGIN], 1);


	//new format************************************************************			
            	$arr_data = (array) null;
				$arr_data[] ="$data_num";
				$arr_data[] ="$TMP_LAST_LOGIN";
				$arr_data[] ="$TMP_FULLNAME";
				$arr_data[] ="$TMP_FROM_IP";
