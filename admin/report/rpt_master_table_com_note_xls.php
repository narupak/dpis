<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อหมายเหตุ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "หมายเหตุ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by IIF(ISNULL(CN_SEQ_NO), 9999, CN_SEQ_NO),$arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by CN_SEQ_NO,$arr_fields[0] ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			if ($data[$arr_fields[2]] == 1) 	 $$arr_fields[2] = "ทั่วไป";
			elseif ($data[$arr_fields[2]] == 2) 	 $$arr_fields[2] = "วิชาชีพเฉพาะ";
			elseif ($data[$arr_fields[2]] == 3) 	 $$arr_fields[2] = "บริหาร";
