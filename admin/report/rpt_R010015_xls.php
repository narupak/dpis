...
//	$list_type_text = $ALL_REPORT_TITLE;
	...
	if(trim($search_per_level)){ 
		for($i=0; $i < count($search_per_level); $i++){
			$search_per_level[$i] = stripslashes($search_per_level[$i]);
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
		$ws_head_line1 = array("ส่วนราชการ     ชื่อ - สกุล","ตำแหน่ง","ตำแหน่งประเภท","วันเข้าสู่ระดับปัจจุบัน","วันดำรงตำแหน่งปัจจุบัน","วัน เดือน ปีเกิด","วันเกษียณอาย (1 ต.ค.)","อบรม");
...
//				echo "$i-map[".$arr_column_map[$i]."], width=".$ws_width[$arr_column_map[$i]]."<br />";
	...
	if($DPISDB=="odbc"){
		...
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, b.PT_CODE, g.PT_NAME, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								SUBSTR(trim(a.PER_STARTDATE), 1, 10) as PER_STARTDATE, a.PER_SALARY, b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $select_list,
				   ...
				   group by		a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, b.PT_CODE, g.PT_NAME, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $group_by, m.LEVEL_NAME, m.POSITION_TYPE, m.POSITION_LEVEL, m.LEVEL_SEQ_NO
				   order by	$order_by, m.LEVEL_SEQ_NO desc, a.LEVEL_NO desc, h.PM_SEQ_NO,  b.PT_CODE desc, NLSSORT(a.PER_NAME,'NLS_SORT=THAI_DICTIONARY'), NLSSORT(a.PER_SURNAME,'NLS_SORT=THAI_DICTIONARY'), MIN(SUBSTR(trim(j.POH_EFFECTIVEDATE), 1, 10)), a.PER_SALARY desc, 
