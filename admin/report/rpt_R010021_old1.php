...
//	$arr_rpt_order = array("MINISTRY", "DEPARTMENT", "ORG", "LINE"); 
	...
	$report_title = "$DEPARTMENT_NAME||จำนวน$PERSON_TYPE[$search_per_type]ที่จะเกษียณอายุ ประจำปีงบประมาณ พ.ศ. $search_budget_year||จำแนกตามตำแหน่งประเภท และเพศ";
	$report_code = "R1021";
	$orientation='L';
...
//	session_cache_limiter("nocache");
	...
	include ("rpt_R010021_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
/*
	$heading_width[0] = "10";
	$heading_width[1] = "53";
	$heading_width[2] = "8";
...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		$pdf->Cell(($heading_width[2] * 2) ,7,"",'LTR',1,'C',1);
		...
		$pdf->Cell(($heading_width[2] * 2) ,7,"รวม",'LR',1,'C',1);
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				...
				$pdf->Cell(($heading_width[2] * 2) ,7,$ARR_LEVEL_NAME[$LEVEL_NO][0],'LTR',0,'C',1);
			} // loop for
		} // loop for
		$pdf->Cell(($heading_width[2] * 2) ,7,"",'LR',1,'C',1);
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
				$pdf->Cell(($heading_width[2] * 2) ,7,$ARR_LEVEL_NAME[$LEVEL_NO][1],'LBR',0,'C',1);
			} // loop for
		} // loop for
		$pdf->Cell(($heading_width[2] * 2) ,7,"",'LBR',1,'C',1);
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
				$pdf->Cell($heading_width[2] ,7,"ชาย",'LTBR',0,'C',1);
				$pdf->Cell($heading_width[2] ,7,"หญิง",'LTBR',0,'C',1);
			} // loop for
		} // loop for
		$pdf->Cell($heading_width[2] ,7,"ชาย",'LTBR',0,'C',1);
		$pdf->Cell($heading_width[2] ,7,"หญิง",'LTBR',1,'C',1);
...
//							$db_dpis2->show_error();
							...
							if (!$ORG_NAME) $DEPARTMENT_NAME = $data2[ORG_NAME];
						} // end if
...
/*
			...
			$pdf->MultiCell($heading_width[0], 7, "$DISPLAY_ORDER", $border, "L");
			...
			for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
				...
				for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
					$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
					$pdf->Cell($heading_width[2], 7, (${"COUNT_M_".$LEVEL_NO}?number_format(${"COUNT_M_".$LEVEL_NO}):"-"), $border, 0, 'R', 0);
					$pdf->Cell($heading_width[2], 7, (${"COUNT_F_".$LEVEL_NO}?number_format(${"COUNT_F_".$LEVEL_NO}):"-"), $border, 0, 'R', 0);
				} // loop for
			} // loop for
			$pdf->Cell($heading_width[2], 7, ($TOTAL_M?number_format($TOTAL_M):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[2], 7, ($TOTAL_F?number_format($TOTAL_F):"-"), $border, 0, 'R', 0);
...
*/
		...
		$result = $pdf->add_data_tab($arr_data, 7, "TRHLB", $data_align, "cordia", "14", "b", "000000", "");		//TRHBL
		if (!$result) echo "****** error ****** add data to table at record count = $data_count <br>";
/*
		$border = "LTBR";
		$pdf->SetFont($fontb,'',14);
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
				$pdf->Cell($heading_width[2], 7, ($LEVEL_GRAND_TOTAL["M"][$LEVEL_NO]?number_format($LEVEL_GRAND_TOTAL["M"][$LEVEL_NO]):"-"), $border, 0, 'R', 0);
				$pdf->Cell($heading_width[2], 7, ($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]?number_format($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]):"-"), $border, 0, 'R', 0);
			} // loop for
		} // loop for
		$pdf->Cell(($heading_width[2] * 2), 7, ($GRAND_TOTAL?number_format($GRAND_TOTAL):"-"), $border, 0, 'C', 0);
*/
	}else{
