<?
	...
	function print_header($xlsRow){
		...
		$worksheet->set_column($row+4, $row+4, 20);
		$worksheet->set_column($row+5, $row+5, 20);
		$worksheet->set_column($row+6, $row+6, 20);
		$worksheet->set_column($row+7, $row+7, 20);
		$worksheet->set_column($row+8, $row+8, 20);
		$worksheet->set_column($row+9, $row+9, 20);
		$worksheet->set_column($row+10, $row+10, 20);
		$worksheet->set_column($row+11, $row+11, 15);
		$worksheet->set_column($row+12, $row+12, 15);
		$worksheet->set_column($row+13, $row+13, 20);
		$worksheet->set_column($row+14, $row+14, 50);
		$worksheet->set_column($row+15, $row+15, 15);
		$worksheet->set_column($row+16, $row+16, 10);
		$worksheet->set_column($row+17, $row+17, 20);
		$worksheet->set_column($row+18, $row+18, 15);
		...
		$worksheet->write($xlsRow, $row+4, "", set_format("xlsFmtTableHeader", "B", "C", "T", 0));
		$worksheet->write($xlsRow, $row+5, "", set_format("xlsFmtTableHeader", "B", "C", "TR", 0));
		$worksheet->write($xlsRow, $row+6, "", set_format("xlsFmtTableHeader", "B", "C", "TL", 0));
		$worksheet->write($xlsRow, $row+7, "ตามมอบหมายงาน", set_format("xlsFmtTableHeader", "B", "C", "T", 0));
		$worksheet->write($xlsRow, $row+8, "", set_format("xlsFmtTableHeader", "B", "C", "T", 0));
		$worksheet->write($xlsRow, $row+9, "", set_format("xlsFmtTableHeader", "B", "C", "TR", 0));
		$worksheet->write($xlsRow, $row+10, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+11, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+12, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+13, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+14, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+15, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+16, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+17, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+18, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		...
		$worksheet->write($xlsRow, $row+4, "$ORG_TITLE1", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+5, "$ORG_TITLE2", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+6, "$DEPARTMENT_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+7, "$ORG_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+8, "$ORG_TITLE1", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+9, "$ORG_TITLE2", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, $row+10, "ผู้ครองตำแหน่ง", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+11, "วันที่ครองตำแหน่ง", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+12, "$LEVEL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+13, "รายละเอียดเงื่อนไขตำแหน่ง", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+14, "หมายเหตุ", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+15, "อัตราเงินเดือนถือจ่าย", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+16, "$ACTIVE_TITLE", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+17, "ผู้ครองถือจ่าย", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
		$worksheet->write($xlsRow, $row+18, "วันที่ครองถือจ่าย", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
	} // end if
...
//			$db_dpis1->show_error();
			while($data1 = $db_dpis1->get_array()){
				...
				if($PER_ID[$cnt] && $PER_STATUS != 2){
					$cmd = " select 	b.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, a.PER_STATUS, DEPARTMENT_ID_ASS, ORG_ID, ORG_ID_1, ORG_ID_2
					...
					$cmd = "select ORG_NAME from PER_ORG where ORG_ID='$DEPARTMENT_ID_ASS'";
					$db_dpis2->send_cmd($cmd);
					$data_dpis2 = $db_dpis2->get_array();
					$DEPARTMENT_NAME_ASS = $data_dpis2[ORG_NAME];
		
					$cmd = "select ORG_NAME from PER_ORG where ORG_ID='$ORG_ID_ASS'";
					$db_dpis2->send_cmd($cmd);
					$data_dpis2 = $db_dpis2->get_array();
					$ORG_NAME_ASS = $data_dpis2[ORG_NAME];
		
					$cmd = "select ORG_NAME from PER_ORG where ORG_ID='$ORG_ID_1_ASS'";
					$db_dpis2->send_cmd($cmd);
					$data_dpis2 = $db_dpis2->get_array();
					$ORG_NAME_1_ASS = $data_dpis2[ORG_NAME];
		
					$cmd = "select ORG_NAME from PER_ORG where ORG_ID='$ORG_ID_2_ASS'";
					$db_dpis2->send_cmd($cmd);
					$data_dpis2 = $db_dpis2->get_array();
					...
					if ($POH_EFFECTIVEDATE) {	
						$POH_DATE[$cnt] = show_date_format(substr($POH_EFFECTIVEDATE, 0, 10), 1);
					}
				}else{
					$POS_SITUATION = 1;
				} // end if
			...
			$worksheet->write($xlsRow, $row+4, "$ORG_NAME_1", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+5, "$ORG_NAME_2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+6, "$DEPARTMENT_NAME_ASS", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+7, "$ORG_NAME_ASS", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+8, "$ORG_NAME_1_ASS", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+9, "$ORG_NAME_2_ASS", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+10, "$FULLNAME[1]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+11, (($NUMBER_DISPLAY==2)?convert2thaidigit("$POH_DATE[1]"):"$POH_DATE[1]"), set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+12, "$LEVEL_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+13, "$POS_CONDITION", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+14, "$POS_REMARK", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+15, (($NUMBER_DISPLAY==2)?convert2thaidigit("$POS_SALARY"):"$POS_SALARY"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			$worksheet->write($xlsRow, $row+16, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->insert_bitmap($xlsRow, $row+16, "$POS_STATUS", 35, 4, 1, 0.8);
			$worksheet->write($xlsRow, $row+17, "$PAYNAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, $row+18, (($NUMBER_DISPLAY==2)?convert2thaidigit("$PAY_DATE"):"$PAY_DATE"), set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			...
			if ($FULLNAME[2]) {
				...
				$worksheet->write($xlsRow, $row+4, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+5, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+6, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+7, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+8, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+9, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+10, "$FULLNAME[2]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+11, "$POH_DATE[2]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+12, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+13, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+14, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+15, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+16, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+17, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+18, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			}
			...
			if ($FULLNAME[3]) {
				...
				$worksheet->write($xlsRow, $row+8, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+9, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+10, "$FULLNAME[3]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+11, "$POH_DATE[3]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+12, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+13, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+14, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+15, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			...
			if ($FULLNAME[4]) {
				...
				$worksheet->write($xlsRow, $row+8, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+9, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+10, "$FULLNAME[4]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+11, "$POH_DATE[4]", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+12, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+13, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+14, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, $row+15, "", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
