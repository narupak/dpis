...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$RESIGNDATE = show_date_format($data[PER_POSDATE],1);
		if (!$RESIGNDATE) $RESIGNDATE = show_date_format($data[PER_RETIREDATE],1);
...
//				require_once "../../Excel/my.defined_format.inc.php";	// use set_format(set of parameter) funtion , help is in file
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$worksheet->write_string($xlsRow, 42, "$PERCENTSALARY2", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
                        ...
                        if(empty($POS_NO)){$POS_NO="0";}
			$cmd = " insert into GMIS_GPIS_FLOW_OUT (tempMinistry, tempOrganize, tempDivisionName, tempOrganizeType, tempPositionNo, tempManagePosition, 
			...
			$cmd1 = " SELECT tempPositionNo FROM GMIS_GPIS_FLOW_OUT WHERE tempPositionNo = '$POS_NO' "; 
			$count_data = $db_dpis2->send_cmd($cmd1);
			if (!$count_data) {
                            echo "[FLOW_OUT]ไม่พบข้อมูลเลขที่ตำแหน่งของ ".$PER_NAME." ".$PER_SURNAME."[".$PER_CARDNO."]<br>";
				//echo "$cmd<br>==================<br>";
				//$db_dpis->show_error();
				//echo "<br>end ". ++$i  ."=======================<br>";
			}
...
<?
	...
	if (count($arr_file) > 0) {
		...
		for($i_file = 0; $i_file < count($arr_file); $i_file++) {
			if (strpos($arr_file[$i_file],"rpt_gmis_xls2")!==false && $grp!="2") {
				$grp="2";
				echo "<tr><td width='16%'>2. data ข้าราชการ Flow In</td>";
				$num=1;
			} else if (strpos($arr_file[$i_file],"rpt_gmis_xls3")!==false && $grp != "3") {
				$grp="3";
