...
//	echo "$head_text1<br>";
	...
	if($count_data){
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "ORG"){
				$report_title = "$DEPARTMENT_NAME||รายงานการมาปฏิบัติราชการของ$PERSON_TYPE[$search_per_type] $NAME||ประจำเดือน $show_month พ.ศ. ".(($NUMBER_DISPLAY==2)?convert2thaidigit($show_year):$show_year)." ||รวมวันมาปฏิบัติราชการ".(($NUMBER_DISPLAY==2)?convert2thaidigit($WORK_DAY):$WORK_DAY)." วันทำการ";
				$RTF->set_report_title($report_title);
