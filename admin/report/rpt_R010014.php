<?
	...
	///$arr_search_condition[] = "(a.LEVEL_NO in ('K4','K5'))";	
...
//	$list_type_text = $ALL_REPORT_TITLE;
	...
	$report_title = "รายชื่อข้าราชการ".str_replace("-", "", $list_type_text);
	$report_code = "H14";
	include ("rpt_R010014_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
//		$sum_w = array_sum($heading_width);
	...
	if($DPISDB=="odbc"){
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								LEFT(trim(a.PER_RETIREDATE), 10) as PER_RETIREDATEa.PER_SALARY, b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $select_list, 
								m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL
				   from		
				   				(
									(
										(
				   ...
				   order by		$order_by, m.LEVEL_SEQ_NO desc, h.PM_SEQ_NO, a.PER_NAME, a.PER_SURNAME, MIN(LEFT(trim(j.POH_EFFECTIVEDATE), 10)), a.PER_SALARY desc, 
		...
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								SUBSTR(trim(a.PER_RETIREDATE), 1, 10) as PER_RETIREDATE,	a.PER_SALARY, b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $select_list, 
								m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL											
				   ...
				   group by		a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $group_by, m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL, m.LEVEL_SEQ_NO
				   order by $order_by, m.LEVEL_SEQ_NO desc, h.PM_SEQ_NO, NLSSORT(a.PER_NAME,'NLS_SORT=THAI_DICTIONARY'), NLSSORT(a.PER_SURNAME,'NLS_SORT=THAI_DICTIONARY'), MIN(SUBSTR(trim(j.POH_EFFECTIVEDATE), 1, 10)), a.PER_SALARY desc, 
				   				SUBSTR(trim(a.PER_STARTDATE), 1, 10), SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) ";
	}elseif($DPISDB=="mysql"){
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								LEFT(trim(a.PER_BIRTHDATE), 10) as PER_BIRTHDATE, LEFT(trim(a.PER_STARTDATE), 10) as PER_STARTDATE, 
								LEFT(trim(a.PER_RETIREDATE), 10) as PER_RETIREDATE, a.PER_SALARY, b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $select_list, 
								m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL
				   from		
				   				(
									(
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	//		echo "$head_text1<br>";
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$POSITION_LEVEL = $arr_content[$data_count][level];
