<?
	...
	include ("rpt_R007005_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
	...
	$report_title = "$DEPARTMENT_NAME||รายงานการมาปฏิบัติราชการของ$PERSON_TYPE[$search_per_type]" . (($search_date)?"||":"") . (($search_date)?"ประจำวันที่ $show_date ":"");
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
    	$ws_head_line1 = array("ชื่อ - สกุล","สถานที่ปฏิบัติราชการ","เวลามา","เวลากลับ","รอบ","รายละเอียด","หมายเหตุ");
		...
		$ws_width = array(30,45,12,12,15,25,30);
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){		
		...
		if(trim($search_date)){
			if($DPISDB=="odbc") $arr_search_condition[] = "(LEFT(START_DATE, 10) = '$search_date')";
			elseif($DPISDB=="oci8") $arr_search_condition[] = "(SUBSTR(START_DATE, 1, 10) = '$search_date')";
			elseif($DPISDB=="mysql") $arr_search_condition[] = "(LEFT(START_DATE, 10) = '$search_date')";
		} // end if
...
//		$db_dpis2->show_error();		
		...
		//$START_DATE = substr($data2[START_DATE], 11, 8);
		$START_DATE = $data2[START_DATE];
		if($START_DATE){$START_DATE = show_date_format(substr($START_DATE, 0, 10),$DATE_DISPLAY); } // end if
		
		$END_DATE = $data2[END_DATE];
		if($END_DATE){$END_DATE = show_date_format(substr($END_DATE, 0, 10),$DATE_DISPLAY); } // end if
...
//		print_header();
		...
		// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนข้อมูล
			...
			$wsdata_align_1 = array("L","L","C","C","C","L","L");
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "ORG"){
				$report_title = "$DEPARTMENT_NAME||รายงานการมาปฏิบัติราชการของ$per_type_name $NAME" . (($search_date)?"||":"") . (($search_date)?"ประจำวันที่ $show_date ":"");
