<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อด้านสาขาเชี่ยวชาญ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "ด้านความเชี่ยวชาญ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if(trim($search_code)) $arr_search_condition[] = "(a.$arr_fields[0] like '$search_code%')";
  	...
  	if(trim($search_group_code)) $arr_search_condition[] = "(a.$arr_fields[2] = '$search_group_code')";
	$search_condition = "";
	if(count($arr_search_condition)) $search_condition = " and " . implode(" and ", $arr_search_condition);

	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], SG_NAME
							from  		$table a, PER_SKILL_GROUP b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by  IIF(ISNULL(SKILL_SEQ_NO), 9999, SKILL_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], SG_NAME
							from  		$table a, PER_SKILL_GROUP b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by  SKILL_SEQ_NO, $arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], SG_NAME
							from  		$table a, PER_SKILL_GROUP b
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $sg_name, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
