<?
//	fix head for rpt_R0040031

	if ($search_level_no=="M2") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "30";
		$heading_width[3] = "12";
		$heading_width[4] = "10";
		$heading_width[5] = "18";
		$heading_width[6] = "18";
		$heading_width[7] = "18";
		$heading_width[8] = "15";
		$heading_width[9] = "18";
		$heading_width[10] = "18";
		$heading_width[11] = "12";
		$heading_width[12] = "12";
		$heading_width[13] = "40";
		$heading_width[14] = "30";
		$heading_width[15] = "30";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "ประเภท|ตำแหน่ง";
		$heading_text[4] = "ระดับ|";
		$heading_text[5] = "ดำรงตำแหน่ง|ปัจจุบันเมื่อ";
		$heading_text[6] = "วันที่ครอง|ระดับ 10";
		$heading_text[7] = "วันที่ครอง|ระดับ 9";
		$heading_text[8] = "เงินเดือน|ปัจจุบัน";
		$heading_text[9] = "วันเข้า|รับราชการ";
		$heading_text[10] = "วันเดือนปีเกิด|";
		$heading_text[11] = "อายุ|";
		$heading_text[12] = "เกษียณ|อายุ";
		$heading_text[13] = "วุฒิ|";
		$heading_text[14] = "การอบรม|";
		$heading_text[15] = "หมายเหตุ|";
	} elseif ($search_level_no=="M1") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "25";
		$heading_width[3] = "12";
		$heading_width[4] = "10";
		$heading_width[5] = "18";
		$heading_width[6] = "18";
		$heading_width[7] = "18";
		$heading_width[8] = "15";
		$heading_width[9] = "18";
		$heading_width[10] = "18";
		$heading_width[11] = "12";
		$heading_width[12] = "12";
		$heading_width[13] = "40";
		$heading_width[14] = "30";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "ประเภท|ตำแหน่ง";
		$heading_text[4] = "ระดับ|";
		$heading_text[5] = "วันที่เลื่อน|ระดับ 11";
		$heading_text[6] = "วันที่เลื่อน|ระดับ 10";
		$heading_text[7] = "วันที่เลื่อน|ระดับ 10";
		$heading_text[8] = "เงินเดือน|ปัจจุบัน";
		$heading_text[9] = "วันเข้า|รับราชการ";
		$heading_text[10] = "วันเดือนปีเกิด|";
		$heading_text[11] = "อายุ|";
		$heading_text[12] = "เกษียณ|อายุ";
		$heading_text[13] = "วุฒิ|";
		$heading_text[14] = "การอบรม|";
	} elseif ($search_level_no=="D2" || $search_level_no=="K4") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "30";
		$heading_width[3] = "30";
		$heading_width[4] = "18";
		$heading_width[5] = "18";
		$heading_width[6] = "15";
		$heading_width[7] = "18";
		$heading_width[8] = "18";
		$heading_width[9] = "15";
		$heading_width[10] = "15";
		$heading_width[11] = "40";
		$heading_width[12] = "30";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "สังกัด|";
		$heading_text[4] = "วันที่เลื่อน|ระดับ 9";
		$heading_text[5] = "วันที่เลื่อน|ระดับ 8";
		$heading_text[6] = "เงินเดือน|ปัจจุบัน";
		$heading_text[7] = "วันเข้า|รับราชการ";
		$heading_text[8] = "วันเดือนปีเกิด|";
		$heading_text[9] = "อายุ|";
		$heading_text[10] = "เกษียณ|อายุ";
		$heading_text[11] = "วุฒิ|";
		$heading_text[12] = "การอบรม|";
	} elseif ($search_level_no=="D1") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "30";
		$heading_width[3] = "30";
		$heading_width[4] = "18";
		$heading_width[5] = "18";
		$heading_width[6] = "15";
		$heading_width[7] = "18";
		$heading_width[8] = "18";
		$heading_width[9] = "12";
		$heading_width[10] = "12";
		$heading_width[11] = "40";
		$heading_width[12] = "30";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "สังกัด|";
		$heading_text[4] = "วันที่ครอง|ตำแหน่ง บก.";
		$heading_text[5] = "วันที่เลื่อน|ระดับ 8";
		$heading_text[6] = "เงินเดือน|ปัจจุบัน";
		$heading_text[7] = "วันเข้า|รับราชการ";
		$heading_text[8] = "วันเดือนปีเกิด|";
		$heading_text[9] = "อายุ|";
		$heading_text[10] = "เกษียณ|อายุ";
		$heading_text[11] = "วุฒิ|";
		$heading_text[12] = "การอบรม|";
	} elseif ($search_level_no=="K3") {
		$heading_width[0] = "8";
		$heading_width[1] = "45";
		$heading_width[2] = "40";
		$heading_width[3] = "20";
		$heading_width[4] = "18";
		$heading_width[5] = "15";
		$heading_width[6] = "18";
		$heading_width[7] = "18";
		$heading_width[8] = "12";
		$heading_width[9] = "12";
		$heading_width[10] = "50";
		$heading_width[11] = "30";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "สังกัด|";
		$heading_text[4] = "วันที่เลื่อน|ระดับ 8";
		$heading_text[5] = "เงินเดือน|ปัจจุบัน";
		$heading_text[6] = "วันเข้า|รับราชการ";
		$heading_text[7] = "วันเดือนปีเกิด|";
		$heading_text[8] = "อายุ|";
		$heading_text[9] = "เกษียณ|อายุ";
		$heading_text[10] = "วุฒิ|";
		$heading_text[11] = "การอบรม|";
	} elseif ($search_level_no=="K2") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "30";
		$heading_width[3] = "20";
		$heading_width[4] = "20";
		$heading_width[5] = "18";
		$heading_width[6] = "18";
		$heading_width[7] = "18";
		$heading_width[8] = "15";
		$heading_width[9] = "18";
		$heading_width[10] = "18";
		$heading_width[11] = "12";
		$heading_width[12] = "12";
		$heading_width[13] = "40";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "สังกัด|";
		$heading_text[4] = "ปฏิบัติงาน|";
		$heading_text[5] = "วันที่เลื่อน|ระดับ 7";
		$heading_text[6] = "วันที่เลื่อน|ระดับ 6";
		$heading_text[7] = "วันที่เลื่อน|ชำนาญการ";
		$heading_text[8] = "เงินเดือน|ปัจจุบัน";
		$heading_text[9] = "วันเข้า|รับราชการ";
		$heading_text[10] = "วันเดือนปีเกิด|";
		$heading_text[11] = "อายุ|";
		$heading_text[12] = "เกษียณ|อายุ";
		$heading_text[13] = "วุฒิ|";
	} elseif ($search_level_no=="O2") {
		$heading_width[0] = "8";
		$heading_width[1] = "40";
		$heading_width[2] = "30";
		$heading_width[3] = "25";
		$heading_width[4] = "30";
		$heading_width[5] = "12";
		$heading_width[6] = "18";
		$heading_width[7] = "15";
		$heading_width[8] = "18";
		$heading_width[9] = "18";
		$heading_width[10] = "12";
		$heading_width[11] = "12";
		$heading_width[12] = "50";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "สังกัด|";
		$heading_text[4] = "ปฏิบัติงาน|";
		$heading_text[5] = "ระดับ|เดิม";
		$heading_text[6] = "วันเดือนปี|ที่เลื่อน";
		$heading_text[7] = "เงินเดือน|ปัจจุบัน";
		$heading_text[8] = "วันเข้า|รับราชการ";
		$heading_text[9] = "วันเดือนปีเกิด|";
		$heading_text[10] = "อายุ|";
		$heading_text[11] = "เกษียณ|อายุ";
		$heading_text[12] = "วุฒิ|";
	} else {
		$heading_width[0] = "8";
		$heading_width[1] = "45";
		$heading_width[2] = "40";
		$heading_width[3] = "10";
		$heading_width[4] = "10";
		$heading_width[5] = "25";
		$heading_width[6] = "25";
		$heading_width[7] = "20";
		$heading_width[8] = "25";
		$heading_width[9] = "25";
		$heading_width[10] = "15";
		$heading_width[11] = "15";
		$heading_width[12] = "50";
		$heading_width[13] = "40";
		
		$heading_text[0] = "ลำดับ|ที่";
		$heading_text[1] = "ชื่อ - สกุล|";
		$heading_text[2] = "ตำแหน่ง|";
		$heading_text[3] = "ประเภท|ตำแหน่ง";
		$heading_text[4] = "ระดับ|";
		$heading_text[5] = "วันที่เลื่อน|ระดับ 11";
		$heading_text[6] = "วันที่เลื่อน|ระดับ 10";
		$heading_text[7] = "เงินเดือน|ปัจจุบัน";
		$heading_text[8] = "วันเข้า|รับราชการ";
		$heading_text[9] = "วันเดือนปีเกิด|";
		$heading_text[10] = "อายุ|";
		$heading_text[11] = "เกษียณ|อายุ";
		$heading_text[12] = "วุฒิ|";
		$heading_text[13] = "การอบรม|";
	}

	// function ประเภท aggregate มี  SUM, AVG, PERC ตามแนวบรรทัด (ROW)
	// 									SUM-columnindex1-columnindex2-....  Ex SUM-1-3-4-7 หมายถึง ผลรวมของ column ที่ 1,3,4 และ 7
	// 									AVG-columnindex1-columnindex2-....  Ex AVG-1-3-4-7 หมายถึง ค่าเฉลี่ยของ column ที่ 1,3,4 และ 7
	// 									PERCn-columnindex1-columnindex2-....  Ex PERCn-1-3-4-7 n คือ column ที่ เป็นเป้าหมายเปรียบเทียบร้อยละ (ค่า 1 ค่า ใน column 1 3 4 7) 
	//																												ถ้าไม่ใส่ค่า n ก็คือ ค่าสรุป (100%) PERC3-1-3-4-7 ก็คือ ค่าใน column ที่ 3 เป็นร้อยละเท่าใดของผลรวม column 1,3,4,7
	//	function ประเภท รูปแบบ (format) ซึ่งจะต้องตามหลัง function ประเภท aggregate เสมอ (ถ้ามี)
	//									TNUM-n คือ เปลี่ยนเป็นตัวเลขของไทย n คือ จำนวนทศนิยม ถ้ามีค่าเป็น ศูนย์ จะแสดงเป็น -
	//									TNUM0-n คือ เปลี่ยนเป็นตัวเลขของไทย n คือ จำนวนทศนิยม ถ้ามีค่าเป็น ศูนย์ จะแสดงเป็น 0
	//									ENUM-n คือ แสดงเป็นเลขอาราบิค n คือ จำนวนทศนิยม ถ้ามีค่าเป็น ศูนย์ จะแสดงเป็น -
	//									ENUM0-n คือ แสดงเป็นเลขอาราบิค n คือ จำนวนทศนิยม ถ้ามีค่าเป็น ศูนย์ จะแสดงเป็น 0
	//
	if ($search_level_no=="M2") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[12] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[13] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[14] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[15] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","C","C","C","C","C","C","C","C","L","L","L");
	} elseif ($search_level_no=="M1") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[12] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[13] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[14] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","C","C","C","C","C","C","C","C","C","C","L","L");
	} elseif ($search_level_no=="D2" || $search_level_no=="K4") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[12] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","L","C","C","C","C","C","C","C","L","L");
	} elseif ($search_level_no=="D1") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[12] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","L","C","C","C","C","C","C","L","L","L");
	} elseif ($search_level_no=="K3") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","L","C","C","C","C","C","C","L","L");
	} elseif ($search_level_no=="K2") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[12] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[13] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","L","L","C","C","C","C","C","C","L","L","L");
	} elseif ($search_level_no=="O2") {
		$column_function[0] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[1] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[5] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[6] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[7] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[8] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[9] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
		$column_function[10] = (($NUMBER_DISPLAY==2)?"TNUM-x":"ENUM-x");
		$column_function[11] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	
		$heading_align = array("C","C","C","C","C","C","C","C","C","C","C","C");
	
		$data_align = array("C","L","L","L","L","C","C","C","C","C","C","L");
	}

	$total_head_width = 0;
	for($iii=0; $iii < count($heading_width); $iii++) 	$total_head_width += (int)$heading_width[$iii];
	
	if (!$COLUMN_FORMAT) {	// ต้องกำหนดเป็น element ให้อยู่ใน form1 ด้วย  <input type="hidden" name="COLUMN_FORMAT" value="<?=$COLUMN_FORMAT
		$arr_column_map = (array) null;
		$arr_column_sel = (array) null;
		for($i=0; $i < count($heading_text); $i++) {
			$arr_column_map[] = $i;		// link index ของ head 
			$arr_column_sel[] = 1;			// 1=แสดง	0=ไม่แสดง   ถ้าไม่มีการปรับแต่งรายงาน ก็เป็นแสดงหมด
		}
		$arr_column_width = $heading_width;	// ความกว้าง
		$arr_column_align = $data_align;		// align
		$COLUMN_FORMAT = implode(",",$arr_column_map)."|".implode(",",$arr_column_sel)."|".implode(",",$arr_column_width)."|".implode(",",$arr_column_align);
	} else {
		$arrbuff = explode("|",$COLUMN_FORMAT);
		$arr_column_map = explode(",",$arrbuff[0]);		// index ของ head เริ่มต้น
		$arr_column_sel = explode(",",$arrbuff[1]);	// 1=แสดง	0=ไม่แสดง
		$arr_column_width = explode(",",$arrbuff[2]);	// ความกว้าง
		$heading_width = $arr_column_width;	// ความกว้าง
		$arr_column_align = explode(",",$arrbuff[3]);		// align
	}
	
	$total_show_width = 0;
	for($iii=0; $iii < count($arr_column_width); $iii++) 	if ($arr_column_sel[$iii]==1) $total_show_width += (int)$arr_column_width[$iii];

?>