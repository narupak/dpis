<?
	...
	$report_title = "ตารางแสดงรายการผลการประเมินสมรรถนะของ$PERSON_TYPE[$PER_TYPE] $DEPARTMENT_NAME";
	$company_name = "ประจำรอบการประเมินครั้งที่ $KF_CYCLE พ.ศ. $KF_YEAR  $ORG_TITLE $ORG_NAME";
	$company_name1 = "ชื่อผู้ประเมิน $PER_NAME  ชื่อตำแหน่งงาน $PL_NAME";
	$report_code = "R_Person_Competency_MeToWho";
	...
	$data_count = 0;
	$cmd = " SELECT * FROM PER_COMPETENCY_FORM WHERE KF_ID = $KF_ID AND CF_STATUS=1 ";
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$cmd1 = " select 	PN_CODE, PER_NAME, PER_SURNAME, LEVEL_NO, PER_SALARY, PER_TYPE, POS_ID, POEM_ID, POEMS_ID
						 from PER_PERSONAL where	PER_ID=$CF_PER_ID ";
...
//			$db_dpis3->show_error(); 
			...
			$arr_content[$data_count][perid] = $CF_PER_ID;
...
//			echo "$U_PER_ID != ".$arr_content[$data_count][perid]."<br>";
			if ($U_PER_ID != $arr_content[$data_count][perid]) {
				...
				$xlsRow++;
				$worksheet->write($xlsRow, 0, "ผู้รับการประเมิน : $U_PER_ID - $U_PER_NAME", set_format("xlsFmtSubTitle", "B", "L", "", 1));
				$worksheet->write($xlsRow, 1, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
				$worksheet->write($xlsRow, 2, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
				$worksheet->write($xlsRow, 3, "", set_format("xlsFmtSubTitle", "B", "L", "", 1));
				$seq_no=0;
