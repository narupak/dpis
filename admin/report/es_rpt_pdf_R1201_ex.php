<?php include("../../php_scripts/connect_database.php");
    ...
    for($idx=0;$idx<count($arrayDateWorkDay);$idx++){
        ...
        //                     ";
        $cmd="select a.PER_ID, ORG.ORG_NAME AS KONG, wc.WC_CODE from per_personal a $POS_NO_FROM
        $conTPER_ORG   
        LEFT JOIN per_work_cyclehis wc on(a.PER_ID = wc.PER_ID)     
        WHERE 1=1 and per_status in(1,0)  $search_condition $search_condition_n  and ROWNUM <50 order by ORG_ID";
        //echo  "<pre>".$cmd;
        ...
        if($cnt>0){
            ...
            while ($data_x = $db_dpis2->get_array_array()) {
                $cmd = file_get_contents('../GetWorkTimeByPerID.sql');	
                $cmd=str_ireplace(":PER_ID",$data_x[PER_ID],$cmd);
                ...
                $cnt_se = $db_dpis_cnt->num_rows();
                $db_dpis->send_cmd_fast($cmd);
                $data = $db_dpis->get_array_array();
               //if($data_x[WC_CODE]){ 
                       echo $CurWC_CODE."!=".$data_x[WC_CODE].$data_x[KONG]."<br>";
                if($CurWC_CODE!=$data_x[WC_CODE].$data_x[KONG]){ // รอบ
                        if($CurWC_CODE!=""){
                            $pdf->close_tab(""); 
                            $y = $pdf->y;	
                            $h = 6;
                        ...
                        $pdf->report_title = "บัญชีลงเวลาปฏิบัติราชการของ".$PERSON_TYPE[$search_per_type]."||$show_date $varORGNAME_1||".$data_x[KONG]."||$DEPARTMENT_NAME";
                        $pdf->AddPage();
                        $pdf->SetTextColor(0, 0, 0);
                        $page_start_x = $pdf->x;
                        $page_start_y = $pdf->y;
                        ...
                        $CurWC_CODE=$data_x[WC_CODE].$data_x[KONG];
                        $cntbWorkFlag_0=0;
                        $cntbWorkFlag_1=0;
                        $cntbWorkFlag_2=0;
                        $cntbWorkFlag_3=0;
                ...
                $arr_data = (array) null;
                $arr_data[] = $no;
                $arr_data[] = getNamePersonal($db_dpis,$data_x[PER_ID]);
                $arr_data[] = $dbEntTime;
                $arr_data[] = $dbExitTime;
                $arr_data[] = $Comment;
