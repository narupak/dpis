...
//	$arr_rpt_order = array("POSNO"); 
	...
	// ===== count all data =====
	if($DPISDB=="odbc"){
		$cmd = " select	 count(*) as count_data	
				 		 from	(
						 				(
											(
												PER_POS_EMPSER a
		...
		$cmd = " select	 count(*) as count_data	
						 from	PER_POS_EMPSER a, PER_ORG b, PER_EMPSER_POS_NAME c, PER_PROVINCE f, 	PER_ORG_TYPE g
	...
	$data_per_file = 1000;/*5000*/
	
	$total_files = ceil( $count_data / $data_per_file );
	$limit_data = "";	
	if ($command) {
		$current_file = (int) $command;
		
		// ===== select data =====
		if($DPISDB=="odbc"){
			$select_top = ($current_file==$total_files)?($count_data - ($data_per_file * ($current_file - 1))):$data_per_file;
			$cmd = " select		top ". ($data_per_file * $current_file) ."
			...
			$rec_start = (($current_file-1) * $data_per_file) + 1;
			$rec_end = ($current_file > 1)? ($current_file * $data_per_file) : $data_per_file;
			$search_condition = str_replace(" where ", " and ", $search_condition);	
			$cmd = "select * from (
			...
			$limit_data = " limit " . (($current_file - 1) * $data_per_file) . ", " . $data_per_file;
			$cmd = " select		
								b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 
								f.PV_NAME, g.OT_NAME, b.ORG_CODE, a.EP_CODE, b.PV_CODE, 
								b.OT_CODE, a.ORG_ID, a.ORG_ID_1, a.ORG_ID_2, PPT_CODE, PEF_CODE, PPS_CODE, POEMS_SKILL, POEMS_SOUTH 
	...
	//						echo("rpt_order_index = ".$rpt_order_index."/arr_rpt_order = ".(count($arr_rpt_order)-1)."<BR>");
							...
							if($rpt_order_index == (count($arr_rpt_order) - 1)){	
								...
								$ORG_NAME_2 = trim($data[ORG_NAME]);	
                                                                $EMP_LEVEL_NO = trim($data[EMP_LEVEL_NO]);	
								...
								$where = "POEMS_ID=$POEMS_ID and PER_TYPE=3 and PER_STATUS=1";
								...
								//$cmd = " select LEVEL_NAME from PER_LEVEL where LEVEL_NO='$LEVEL_NO' ";/*เดิม*/
                                                                $cmd = " select LEVEL_NAME from PER_LEVEL where LEVEL_NO='$EMP_LEVEL_NO' ";/* Release 5.2.1.5 */
								$db_dpis2->send_cmd($cmd);
								$data2 = $db_dpis2->get_array();
...
<body>
<form action="rpt_geis_sub_xls.php" method="post" enctype="multipart/form-data" name="form1">
	...
	<input type="hidden" name="RPTORD_LIST" value="<?=$RPTORD_LIST?>">
...
<?
		if ($count_data) {
			...
			for($ii = 0; $ii < $total_files; $ii++) {
				$show_end_data=($ii+1) * $data_per_file;
				if ($show_end_data > $count_data) $show_end_data=$count_data;
				
?>
   				<input type="button" name="xlsfile_<?=($ii+1)?>" class="button" value="<?=(($ii * $data_per_file)+1)." ถึง $show_end_data"?>" onclick="form1.command.value=<?=($ii+1)?>; form1.submit()"/>&nbsp;
<?
			}
		} else {
			echo "&nbsp;***************** ไม่พบข้อมูล ......";
...
<?
	...
	if (count($arr_file) > 0) {
		...
		for($i_file = 0; $i_file < count($arr_file); $i_file++) {
			if (strpos($arr_file[$i_file],"rpt_geis_xls")!==false && $grp!="1") {
				$grp="1";
				echo "<tr><td width='25%'>ข้อมูลระบบสารสนเทศพนักงานราชการ </td>";
				$num=1;
			} else {
