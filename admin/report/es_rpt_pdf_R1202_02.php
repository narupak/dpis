...
*/	
    ...
    $report_title = $DEPARTMENT_NAME."||รายงานการมาปฏิบัติราชการของ".$PERSON_TYPE[$varPerType]."||ประจำเดือน ".$month_full[($search_month + 0)][TH]." พ.ศ. ". $search_year_Show;
    ...
    include ("es_rpt_pdf_R1202_02_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
    ...
    $pdf->Open();
    $pdf->SetFont($font,'',$CH_PRINT_SIZE-5);
    ...
    $cmdMain ="WITH
                ...
                (
                  ...
                  where p.PER_STATUS in (". implode(", ", $search_per_status) .") and p.per_type=1 and p.per_type in (select per_type from List_Per_Type)
                    and :LAW_OR_ASS=1 and pp.ORG_ID in (select org_id from GetList_ORG_ID)
                  union
                  select per_id from PER_PERSONAL p
                  LEFT JOIN PER_POS_EMP pp on (pp.POEM_ID=p.POEM_ID)
                  where p.PER_STATUS in (". implode(", ", $search_per_status) .") and p.per_type=2 and p.per_type in (select per_type from List_Per_Type)
                    and :LAW_OR_ASS=1 and pp.ORG_ID in (select org_id from GetList_ORG_ID)
                  union
                  select per_id from PER_PERSONAL p
                  LEFT JOIN PER_POS_EMPSER pp on (pp.POEMS_ID=p.POEMS_ID)
                  where p.PER_STATUS in (". implode(", ", $search_per_status) .") and p.per_type=3 and p.per_type in (select per_type from List_Per_Type)
                    and :LAW_OR_ASS=1 and pp.ORG_ID in (select org_id from GetList_ORG_ID)
                  union
                  select per_id from PER_PERSONAL p
                  LEFT JOIN PER_POS_TEMP pp on (pp.POT_ID=p.POT_ID)
                  where p.PER_STATUS in (". implode(", ", $search_per_status) .") and p.per_type=4 and p.per_type in (select per_type from List_Per_Type)
                    and :LAW_OR_ASS=1 and pp.ORG_ID in (select org_id from GetList_ORG_ID)
                  /* By Assignment */
                  union
                  select per_id from PER_PERSONAL p
                  where p.PER_STATUS in (". implode(", ", $search_per_status) .") and p.per_type in (select per_type from List_Per_Type)
   ...
   //die("<pre>$cmdMain");
	...
	if ($data) {
		do  {
			...
			if($CurKONG!=$data[KONG]){ // รอบ
				...
				$pdf->SetFont($font,'',$CH_PRINT_SIZE-5);
				$report_titleshow = $DEPARTMENT_NAME."||รายงานการมาปฏิบัติราชการของ".$PERSON_TYPE[$varPerType]."||ประจำเดือน ".$month_full[($search_month + 0)][TH]." พ.ศ. ". $search_year_Show;
				$report_titleshow = (($NUMBER_DISPLAY==2)?convert2thaidigit($report_titleshow):$report_titleshow);
				$pdf->report_title = $report_titleshow;
				$pdf->company_name = "รูปแบบการออกรายงาน : ". ($select_org_structure==1 || $PER_AUDIT_FLAG==1 ?"โครงสร้างตามมอบหมายงาน - ":"โครงสร้างตามกฎหมาย - ") .$data[KONG];
				$pdf->AddPage();
				$pdf->SetTextColor(0, 0, 0);
	...
	//		$CurRowY = $pdf->GetY();
			...
			$arr_data[] = $sum1==0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit(round($sum1,2)):round($sum1,2));
			$arr_data[] = $sum3==0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit(round($sum3,2)):round($sum3,2));
			...
			$arr_data[] = $sum4==0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit(round($sum4,2)):round($sum4,2));
			$arr_data[] = '';
			for($idx=0;$idx<7;$idx++){
				$data_align[]="C";
			}
			...
			$pdf->SetFillColor(218,218,218);
			//print_r($RectHoliday); die();
			
			for ($i=1; $i < sizeof($RectHoliday); $i++) {
				$x = $CurRowX + 35.5 + ($RectHoliday[$i] * 6.5);
				$pdf->Rect($x,$CurRowY,6.5,$pdf->GetY() - $CurRowY+7,'FD');   
	 
			}
			
			$pdf->add_data_tab($arr_data, 7, "TRHBL", $data_align, "",$CH_PRINT_SIZE-5, "", "000000", "");
