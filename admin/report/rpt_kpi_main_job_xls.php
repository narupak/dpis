<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 2, "หน้าที่ความรับผิดชอบหลัก", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		a.PL_CODE, a.MJT_CODE, LC_ACTIVE, PL_NAME, MJT_NAME 
					from		PER_MAIN_JOB a, PER_LINE b, PER_MAIN_JOB_TYPE c 
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $MJT_NAME, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
