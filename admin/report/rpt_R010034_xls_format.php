...
//	fix head for rpt_R010036
	...
	$heading_text[1] = "||ส่วนราชการ/สายงาน|";
	...
	for($i=0; $i<$cnt_level_grp; $i++){
		...
		for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
			...
			$heading_text[$cnt_level_det*3+2] = "<**1**>ตำแหน่งประเภท|<**".($i+1)."**>".$ARR_LEVEL_GROUP_NAME[$LEVEL_GROUP]."|<**".($cnt_level_det+1)."**>".$ARR_LEVEL_NAME[$LEVEL_NO][1]."|ชาย";
			$heading_text[$cnt_level_det*3+3] = "<**1**>ตำแหน่งประเภท|<**".($i+1)."**>".$ARR_LEVEL_GROUP_NAME[$LEVEL_GROUP]."|<**".($cnt_level_det+1)."**>".$ARR_LEVEL_NAME[$LEVEL_NO][1]."|หญิง";
			$heading_text[$cnt_level_det*3+4] = "<**1**>ตำแหน่งประเภท|<**".($i+1)."**>".$ARR_LEVEL_GROUP_NAME[$LEVEL_GROUP]."|<**".($cnt_level_det+1)."**>".$ARR_LEVEL_NAME[$LEVEL_NO][1]."|รวม";
			$heading_width[$cnt_level_det*3+2] = "6";
			$heading_width[$cnt_level_det*3+3] = "6";
			$heading_width[$cnt_level_det*3+4] = "6";
			$col_sum1 .= "-".($cnt_level_det*3+2);
			$col_sum2 .= "-".($cnt_level_det*3+3);
			$col_sum3 .= "-".($cnt_level_det*3+4);
			$column_function[$cnt_level_det*3+2] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
			$column_function[$cnt_level_det*3+3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
			$column_function[$cnt_level_det*3+4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
			$heading_align[$cnt_level_det*3+2] = "C";
			$heading_align[$cnt_level_det*3+3] = "C";
			$heading_align[$cnt_level_det*3+4] = "C";
			$data_align[$cnt_level_det*3+2] = "R";
			$data_align[$cnt_level_det*3+3] = "R";
			$data_align[$cnt_level_det*3+4] = "R";
//			echo "idx:".($cnt_level_det*3+2)."->".$heading_text[$cnt_level_det*3+2]."<br>";
//			echo "idx:".($cnt_level_det*3+3)."->".$heading_text[$cnt_level_det*3+3]."<br>";
//			echo "idx:".($cnt_level_det*3+4)."->".$heading_text[$cnt_level_det*3+4]."<br>";
			$cnt_level_det++;
		}
	} // loop for
	$heading_text[$cnt_level_det*3+2] = "<**2**>|<**".($cnt_level_grp+1)."^**>รวม|<**".($cnt_level_det+1)."^**>|ชาย";
	$heading_text[$cnt_level_det*3+3] = "<**2**>|<**".($cnt_level_grp+1)."^**>รวม|<**".($cnt_level_det+1)."^**>|หญิง";
	$heading_text[$cnt_level_det*3+4] = "<**2**>|<**".($cnt_level_grp+1)."^**>รวม|<**".($cnt_level_det+1)."^**>|รวม";
//	echo "idx:".($cnt_level_det*3+2)."->".$heading_text[$cnt_level_det*3+2]."<br>";
//	echo "idx:".($cnt_level_det*3+3)."->".$heading_text[$cnt_level_det*3+3]."<br>";
//	echo "idx:".($cnt_level_det*3+4)."->".$heading_text[$cnt_level_det*3+4]."<br>";
	$heading_width[$cnt_level_det*3+2] = "6";
...
//	$column_function[$cnt_level_det*3+3] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
//	$column_function[$cnt_level_det*3+4] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");	
	$column_function[$cnt_level_det*3+2] = "SUM".$col_sum1."|".(($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	$column_function[$cnt_level_det*3+3] = "SUM".$col_sum2."|".(($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
	$column_function[$cnt_level_det*3+4] = "SUM".$col_sum3."|".(($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
