<?
	...
	$worksheet = &$workbook->addworksheet("รายงานข้อมูลอำเภอ");
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							where		PV_CODE='$PV_CODE'
											$search_condition
							order by IIF(ISNULL(AP_SEQ_NO), 9999, AP_SEQ_NO),$arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							where		PV_CODE='$PV_CODE'
											$search_condition
							order by AP_SEQ_NO,$arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
