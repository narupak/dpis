...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		switch($HISTORY_NAME){
			case "POSITIONHIS" :
				...
				$pdf->Cell($heading_width[$HISTORY_NAME][3] ,7,"วันที่",'LTR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][4] ,7,"ตำแหน่ง",'LTR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][5] ,7,"เลขที่",'LTR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][6] ,7,"อัตรา",'LTR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][7] ,7,"ประเภท",'LTR',1,'C',1);

				$pdf->Cell($heading_width[$HISTORY_NAME][0] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][1] ,7,"ตามกฎหมาย",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][2] ,7,"ตามมอบหมายงาน",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][3] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][4] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][5] ,7,"ตำแหน่ง",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][6] ,7,"เงินเดือน",'LBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][7] ,7,"การเคลื่อนไหว",'LBR',1,'C',1);
				break;			
			case "SALARYHIS" :
				$pdf->Cell($heading_width[$HISTORY_NAME][0] ,7,"วันที่",'LTBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][1] ,7,"เงินเดือน",'LTBR',0,'C',1);
			...
			case "NAMEHIS" :
				...
				$pdf->Cell($heading_width[$HISTORY_NAME][1] ,7,"คำนำหน้า - ชื่อ - สกุล (เดิม)",'LTBR',0,'C',1);
				$pdf->Cell($heading_width[$HISTORY_NAME][2] ,7,"คำนำหน้า - ชื่อ - สกุล (ใหม่)",'LTBR',1,'C',1);
				break;			
			case "DECORATEHIS" :
...
//				$db_dpis2->show_error();
                ...
                switch ($SESS_ORG_SETLEVEL){//ระดับสำนักกองจาก C06 
                    case 2 : 
                         $ORG_ID_3 = "";
                         $ORG_ID_4 = "";
                         $ORG_ID_5 = "";
...
//echo "img_file=$img_file // $PIC_SERVER_ID<br>";
			...
			$PER_SALARY = $data[PER_SALARY];
                        ...
                        //$PER_CERT_OCC = trim($data[PER_CERT_OCC]);/*เดิม*/
...
//echo $cmd;
                        ...
                        /*$cmd = "WITH TMPSALARY AS (SELECT PMH_AMT FROM PER_POS_MGTSALARYHIS PEH, PER_EXTRATYPE PET
                                WHERE TRIM(PEH.EX_CODE)=TRIM(PET.EX_CODE) AND PET.EX_NAME LIKE '%เงินประจำตำแหน่ง%'
...
//                        $db_dpis4->send_cmd($cmd);/*วันที่มีผลที่ดำรงตำแหน่งล่าสุด*/
...
//                        }
			...
			$pdf->Cell(100,7,"วันเกษียณอายุราชการ  ".($PER_RETIREDATE?$PER_RETIREDATE:"-"),0,1,"L");
                        ...
                        if($PER_REMARK != ''){
                            //$pdf->Cell(200,7,"หมายเหตุ  ".$PER_REMARK,0,0,"L");
                            $pdf->MultiCell(200, 7, "หมายเหตุ  ".$PER_REMARK, $border, "L");
...
//							$db_dpis2->show_error();
	...
	//				$db_dpis2->show_error();
					...
					case "LICENSEHIS" :		
							...
							$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
							if($pdf->y != $page_start_y) $pdf->Cell(200,7,"",0,1,"L");
							$pdf->Cell(200,7,(($NUMBER_DISPLAY==2)?convert2thaidigit(($history_index + 1)):($history_index + 1))."  ประวัติใบอนุญาตประกอบวิชาชีพ ",0,1,"L");
							print_header($HISTORY_NAME);
