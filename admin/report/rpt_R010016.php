...
//		$sum_w = array_sum($heading_width);
	...
	function list_person($search_condition, $addition_condition){
		global $DPISDB, $db_dpis2,$db_dpis3,$db_dpis4;
...
//				$db_dpis2->show_error();
	...
	//หา per_id ที่ตรงตามเงื่อนไขการดำรงตำแหน่งที่ครบตามปีที่เลือก เพื่อค้นหาต่อไป		
		...
		while($data2 = $db_dpis2->get_array()) {
			$count++;
			if ($count < 1000) $arr_per_id[] = $data2[PER_ID];
			elseif ($count < 2000) $arr_per_id2[] = $data2[PER_ID];
			elseif ($count < 3000) $arr_per_id3[] = $data2[PER_ID];
			elseif ($count < 4000) $arr_per_id4[] = $data2[PER_ID];
			elseif ($count < 5000) $arr_per_id5[] = $data2[PER_ID];
			else $arr_per_id6[] = $data2[PER_ID];
		}
		$addition_condition_perid = "";
		if (count($arr_per_id6)) $arr_search_perid[] = "((a.PER_ID in (". implode(",", $arr_per_id) .")) or (a.PER_ID in (". implode(",", $arr_per_id2) .")) or 
		...
		elseif (count($arr_per_id5)) $arr_search_perid[] = "((a.PER_ID in (". implode(",", $arr_per_id) .")) or (a.PER_ID in (". implode(",", $arr_per_id2) .")) or 
																															(a.PER_ID in (". implode(",", $arr_per_id3) .")) or (a.PER_ID in (". implode(",", $arr_per_id4) .")) or 
																															(a.PER_ID in (". implode(",", $arr_per_id5) .")))";
		elseif (count($arr_per_id4)) $arr_search_perid[] = "((a.PER_ID in (". implode(",", $arr_per_id) .")) or (a.PER_ID in (". implode(",", $arr_per_id2) .")) or 
																															(a.PER_ID in (". implode(",", $arr_per_id3) .")) or (a.PER_ID in (". implode(",", $arr_per_id4) .")))";
		elseif (count($arr_per_id3)) $arr_search_perid[] = "((a.PER_ID in (". implode(",", $arr_per_id) .")) or (a.PER_ID in (". implode(",", $arr_per_id2) .")) or 
																															(a.PER_ID in (". implode(",", $arr_per_id3) .")))";
		elseif (count($arr_per_id2)) $arr_search_perid[] = "((a.PER_ID in (". implode(",", $arr_per_id) .")) or (a.PER_ID in (". implode(",", $arr_per_id2) .")))";
		else $arr_search_perid[] = "(a.PER_ID in (". implode(",", $arr_per_id) ."))";
