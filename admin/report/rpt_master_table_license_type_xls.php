...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], LT_SEQ_NO
							from		$table
							$search_condition
						order by IIF(ISNULL(LT_SEQ_NO), 9999, LT_SEQ_NO),$arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], LT_SEQ_NO 
								from		$table
								$search_condition
						order by LT_SEQ_NO,$arr_fields[0] ";
		}elseif($DPISDB=="mysql"){
			$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], LT_SEQ_NO 
								from		$table
								$search_condition
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$TMP_SEQ_NO = $data[LT_SEQ_NO];
			$$arr_fields[1] = $data[$arr_fields[1]];
			$$arr_fields[2] = $data[$arr_fields[2]];
			$$arr_fields[3] = $data[$arr_fields[3]];
			
			$$arr_fields[4] = ($data[$arr_fields[3]]==1)?"../images/checkbox_check.bmp":"../images/checkbox_blank.bmp";
