...
//	session_cache_limiter("nocache");
	...
	if ($FLAG_RTF) {
		...
		$fname= "rpt_master_table_accountabillity_level_type.rtf";
	...
	if ($FLAG_RTF) {
		$heading_width[0] = "32.5";
		$heading_width[1] = "32.5";
		$heading_width[2] = "35";
	}else{
		$heading_width[0] = "65";
		$heading_width[1] = "65";
	...
	$heading_text[0] = "$PL_TITLE";
	$heading_text[1] = "$LEVEL_TITLE";
	$heading_text[2] = "ประเภทหน้าที่ความรับผิดชอบ";
...
//		echo "$head_text1<br>";
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$arr_data[] = trim($data1[PL_NAME]);

			$LEVEL_NO = $data[LEVEL_NO];
			$cmd = " select LEVEL_NAME from PER_LEVEL where LEVEL_NO='$LEVEL_NO' ";
			$db_dpis1->send_cmd($cmd);
			$data1 = $db_dpis1->get_array();
			$arr_data[] = trim($data1[LEVEL_NAME]);

			$arr_data[] = $data[ACC_TYPE_NAME];
