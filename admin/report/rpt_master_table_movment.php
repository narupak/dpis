...
//	session_cache_limiter("nocache");
	...
	if ($FLAG_RTF) {
		...
		$fname= "rpt_master_table_movment.rtf";
	...
	$heading_text[1] = "ชื่อประเภทการเคลื่อนไหว";
	$heading_text[2] = "ประเภท";
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_type)) $arr_search_condition[] = "($arr_fields[2] = $search_type)";
        
        /*เพิ่มเติม 03/03/2017 */
        if($search_type_ref == "ALL" || $search_type_ref == ""){}
        else{ $arr_search_condition[] = "($arr_fields[7] = $search_type_ref)";}
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by IIF(ISNULL(MOV_SEQ_NO), 9999, MOV_SEQ_NO), $arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by MOV_SEQ_NO, $arr_fields[0] ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
...
//		echo "$head_text1<br>";
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			if ($data[$arr_fields[2]] == 1) 	 $$arr_fields[2] = "ความเคลื่อนไหวตำแหน่ง";
			elseif ($data[$arr_fields[2]] == 2) 	 $$arr_fields[2] = "ความเคลื่อนไหวเงินเดือน";
