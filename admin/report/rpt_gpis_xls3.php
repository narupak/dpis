<?
	$report_code = "data ข้าราชการ Flow Out";
	
	$fname= "../../Excel/tmp/rpt_gpis_xls3";
	$fname1 = $fname.".xls";
//	$fnum = 0; $fnumtext="";
...
//	require_once "../../Excel/my.defined_format.inc.set_format.php";	// use set_format(set of parameter) funtion , help is in file
	...
	$cmd = " select 	PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE, LEVEL_NO, 
					...
					ORG_ID, MOV_CODE, POS_ID, PER_POSDATE, PER_RETIREDATE
			from		PER_PERSONAL  
			where	PER_TYPE=1 and PER_STATUS=2 and ((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
...
//				require_once "../../Excel/my.defined_format.inc.php";	// use set_format(set of parameter) funtion , help is in file
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			elseif (substr($MOV_CODE,0,1)=="2") $EFFECTIVEDATE = $SAH_EFFECTIVEDATE;
				...
				$cmd = " insert into GPIS1_FLOW_OUT (TEMPMINISTRY, ORGANZIE, DIVISIONNAME, TEMPPOSITIONNO, TEMPLINE, TEMPPOSITIONTYPE, TEMPLEVEL, 
								...
								'$EM_NAME', '$INS_NAME', '$CT_NAME_EDU', '$ST_NAME', '$MOV_NAME', '$EFFECTIVEDATE', '$CL_NAME', '$POSDATE') ";
...
<?
	...
	if (count($arr_file) > 0) {
		...
		for($i_file = 0; $i_file < count($arr_file); $i_file++) {
			if (strpos($arr_file[$i_file],"rpt_gpis_xls2")!==false && $grp!="2") {
				$grp="2";
				echo "<tr><td width='16%'>2. data ข้าราชการ Flow In</td>";
				$num=1;
			} else if (strpos($arr_file[$i_file],"rpt_gpis_xls3")!==false && $grp != "3") {
				$grp="3";
				echo "<tr><td width='18%'>3. data ข้าราชการ Flow Out</td>";
				$num=1;
			} else if ($grp == "") {
				echo "<tr><td width='16%'>1. data ข้าราชการ</td>";
				$grp="1";
				$num=1;
			} else {
	...
	echo "<tr><td width='3%'>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>";
	echo "<tr><td width='3%'>&nbsp;</td><td>จบรายงาน สำหรับข้าราชการ</td></tr>";
?>
	</table></div>
    </td></tr>
</table>
