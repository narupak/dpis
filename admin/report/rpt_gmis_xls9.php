<?
	$report_code = "data พนักงานราชการ Flow Out";
	...
	$cmd = " select 	PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE, LEVEL_NO, 
					...
					ORG_ID, MOV_CODE, POEMS_ID, PER_POSDATE, PER_RETIREDATE
			from		PER_PERSONAL  
			where	PER_TYPE=3 and PER_STATUS=2 and ((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
		...
		if($DPISDB=="odbc"){
			$cmd = " select		
				 ...
				 from		(
							...
							where a.POEMS_ID = $POEMS_ID ";
		}elseif($DPISDB=="oci8"){
			$cmd = " select		
							b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 
							g.OT_NAME, b.ORG_CODE, a.EP_CODE, b.PV_CODE, f.PV_NAME, 
							b.OT_CODE, a.ORG_ID, a.ORG_ID_1, a.ORG_ID_2 
				from			PER_POS_EMPSER a, PER_ORG b, PER_EMPSER_POS_NAME c, PER_PROVINCE f, PER_ORG_TYPE g
				where		a.POEMS_ID = $POEMS_ID and a.ORG_ID=b.ORG_ID and a.EP_CODE=c.EP_CODE and b.PV_CODE=f.PV_CODE(+) and 
							b.OT_CODE=g.OT_CODE(+) ";
		}elseif($DPISDB=="mysql"){
			$cmd = " select		
							b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 
				 ...
				 from		(
									(
											(
												PER_POS_EMPSER a
												inner join PER_ORG b on (a.ORG_ID=b.ORG_ID)
											) inner join PER_POS_EMPSER c on (a.EP_CODE=c.EP_CODE)
								) left join PER_PROVINCE f on (b.PV_CODE=f.PV_CODE)
							) left join PER_ORG_TYPE g on (b.OT_CODE=g.OT_CODE)
							where a.POEMS_ID = $POEMS_ID ";
		} // end if
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	header("Content-Type: application/x-msexcel; name=\"gmis_peis.xls\"");
	header("Content-Disposition: inline; filename=\"gmis_peis.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
