<?
	...
	//ผู้ดำรงตำแหน่งระดับสูง=>ตำแหน่งประเภทอำนวยการ (ระดับสูง)	/วิชาการ (ระดับเชี่ยวชาญขึ้นไป) /ทั่วไป (ระดับทักษะพิเศษ)
...
//	$list_type_text = $ALL_REPORT_TITLE;
	...
	include ("rpt_R010018_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
///	$report_title = "รายงานผู้ดำรงตำแหน่งประเภทบริหาร อำนวยการระดับสูง วิชาการระดับเชี่ยวชาญขึ้นไปและทั่วไประดับทักษะพิเศษ";
	if ($BKK_FLAG==1)
		$report_title = "รายงานข้าราชการกรุงเทพมหานครสามัญระดับสูง";
	else
		$report_title = "รายงานข้าราชการพลเรือนสามัญระดับสูง";
	...
	if($DPISDB=="odbc"){
		$cmd = " select	a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								LEFT(trim(a.PER_RETIREDATE), 10) as PER_RETIREDATE, a.PER_SALARY, b.ORG_ID, c.ORG_NAME, $select_list,
				   ...
				   group by	a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,	
									LEFT(trim(a.PER_BIRTHDATE), 10), LEFT(trim(a.PER_STARTDATE), 10), LEFT(trim(a.PER_RETIREDATE), 10), a.PER_SALARY, b.ORG_ID, c.ORG_NAME, 
		...
		$cmd = " select	 a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
									...
									SUBSTR(trim(a.PER_RETIREDATE), 1, 10) as PER_RETIREDATE,	a.PER_SALARY,b.ORG_ID, c.ORG_NAME, $select_list,
				   ...
				   group by	a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
									...
									b.ORG_ID, c.ORG_NAME, $group_by, m.LEVEL_SEQ_NO
				   order by	$order_by, m.LEVEL_SEQ_NO desc, a.LEVEL_NO desc, h.PM_SEQ_NO, 
									a.PER_NAME, a.PER_SURNAME, MIN(SUBSTR(trim(j.POH_EFFECTIVEDATE), 1, 10)), 
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){		
		...
		if ($PM_NAME=="ผู้ตรวจราชการกระทรวง") $PM_NAME = "";
		if ($PM_NAME=="ผู้ว่าราชการจังหวัด" || $PM_NAME=="รองผู้ว่าราชการจังหวัด") {
			$PM_NAME .= $data[ORG_NAME];
		}
...
***/
		...
		//หาวันเลื่อนระดับ
		$cmd="select MAX(POH_EFFECTIVEDATE) as POH_EFFECTIVEDATE2 from PER_POSITIONHIS where PER_ID=$PER_ID and LEVEL_NO < 'A' ";
		$db_dpis2->send_cmd($cmd);
		//$db_dpis2->show_error();
		$data2 = $db_dpis2->get_array();
		$POH_EFFECTIVEDATE = show_date_format($data2[POH_EFFECTIVEDATE2],$DATE_DISPLAY);

		//หาวันดำรงตำแหน่งปัจจุบัน
		$cmd="select MIN(POH_EFFECTIVEDATE) as PER_STARTDATE2 from PER_POSITIONHIS where PER_ID=$PER_ID and LEVEL_NO > 'A' ";
		$db_dpis2->send_cmd($cmd);
		//$db_dpis2->show_error();
		$data2 = $db_dpis2->get_array();
		$PER_STARTDATE = show_date_format($data2[PER_STARTDATE2],$DATE_DISPLAY);
		...
		$arr_content[$data_count][effectivedate] = 		$PER_STARTDATE;		//วันดำรงตำแหน่งปัจจุบัน
		$arr_content[$data_count][retiredate] = 			$PER_RETIREDATE;	//วันเกษียณอายุ    
		
		//เครื่องราชอิสริยาภรณ์ ชั้น/วันที่ได้รับ  สูงสุดอันเดียว
		...
		$arr_content[$data_count][remark] = 	$REMARK;		//หมายเหตุ
