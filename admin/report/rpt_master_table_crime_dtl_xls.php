<?
	...
	function print_header($xlsRow){
		...
		$worksheet->set_column(1, 2, 55);
		$worksheet->set_column(3, 3, 10);
		
		$worksheet->write($xlsRow, 0, "รหัส", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 1, "ชื่อกรณีความผิด", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "ฐานความผิด", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_crime_code)) $arr_search_condition[] = "(a.$arr_fields[2] = '$search_crime_code')";
	$search_condition = "";
	if(count($arr_search_condition)) $search_condition = " and " . implode(" and ", $arr_search_condition);

	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], CR_NAME
							from  		$table a, PER_CRIME b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by IIF(ISNULL(CRD_SEQ_NO), 9999, CRD_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], CR_NAME
							from  		$table a, PER_CRIME b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by CRD_SEQ_NO, $arr_fields[0]
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], CR_NAME
							from  		$table a, PER_CRIME b
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $cr_name, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
