<?php include("../../php_scripts/connect_database.php");
    ...
    //$show_date = "ตั้งแต่วันที่ 1 เดือน ตุลาคม ". $search_yearBgn ." ถึงวันที่ 30 กันยายน ".$search_yearEnd;
    
    $report_title = ' ';
    $report_code = "R1208";
    include ("es_rpt_R1204_02_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
  ...
  //  var_dump($data);
    ...
    $no++;  
        ...
        if($CurPerId!=$data[PER_ID]){
			...
			$pdf->SetFont($font,'',$CH_PRINT_SIZE);
            $pdf->Cell(25, 7, "รายงานสรุปการมาปฏิบัติงานของ".getNamePersonal($db_dpis,trim($data[PER_ID]))." ตำแหน่ง ".getPositionName($db_dpis,$data[PER_ID]), 0, 50, 'L', 0);
			$pdf->SetFont($font,'',$CH_PRINT_SIZE);
			$pdf->Cell(25, 7, "หน่วยงาน ".$data[ORG_NAME]." ".$DEPARTMENT_NAME, 0, 50, 'L', 0);
