<?
	...
	include ("rpt_R0070031_format.php");	// ใช้เฉพาะในส่วนการแปลง COLUMN_FORMAT เท่านั้น
...
//	include ("../report/rpt_condition3.php");
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
		...
		for($i=0; $i<count($ws_width); $i++){ 
			$ws_colmerge_line[$i] = 1;
			$ws_border_line[$i] = "T";
			$ws_fontfmt_line[$i] = "B";
			$ws_headalign_line[$i] = "C";
		} // end for		
	...
	function print_header(){
		...
		global $ws_head_line, $ws_colmerge_line, $ws_colmerge_line;
		global $ws_border_line, $ws_fontfmt_line;
...
//				echo "$i-map[".$arr_column_map[$i]."], width=".$ws_width[$arr_column_map[$i]]."<br />";
		...
		for($i=0; $i < count($ws_width); $i++) {
			if ($arr_column_sel[$arr_column_map[$i]]==1) {	// พิมพ์เฉพาะที่เลือกให้แสดง
				$buff = explode("|",doo_merge_cell($ws_head_line[$arr_column_map[$i]], $ws_border_line[$arr_column_map[$i]], $ws_colmerge_line[$arr_column_map[$i]], $pgrp, ($colseq == $colshow_cnt-1)));
				$ndata = $buff[0]; $border = $buff[1]; $merge = $buff[2]; $pgrp = $buff[3];
				$worksheet->write($xlsRow, $colseq, $ndata, set_format("xlsFmtTableHeader", $ws_fontfmt_line[$arr_column_map[$i]], $ws_headalign_line[$arr_column_map[$i]], $border, $merge));
				$colseq++;
			}
		}
...
//	$msort_result = array_multisort($DATE, SORT_ASC, $SEQ, SORT_NUMERIC, SORT_ASC, $DATE_HIS);
	...
	if($count_data){
		...
		for($i=0; $i<count($ws_width); $i++){ 
			$wsdata_fontfmt_1[$i]  = "B";
			$wsdata_fontfmt_2[$i]  = "";
			$wsdata_align_1[$i] = "L";
			$wsdata_border_1[$i] = "TRBL";
			$wsdata_colmerge_1[$i] = 0;
			$wsdata_colmerge_1[$i] = 0;
		} // end for
					...
					for($i=0; $i < count($arr_column_map); $i++) {
						if ($arr_column_sel[$arr_column_map[$i]]==1) {
							...
							if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1)
								$worksheet->write_string($xlsRow, $colseq, $ndata, set_format("xlsFmtTableDetail", $wsdata_fontfmt_1[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $border, $merge));
							else
								$worksheet->write_string($xlsRow, $colseq, $ndata, set_format("xlsFmtTableDetail", $wsdata_fontfmt_2[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $border, $merge));
							$colseq++;
						}
					}
...
//			$a = "";
		...
		$xlsRow = 0;
		$worksheet->write($xlsRow, 0, "***** ไม่มีข้อมูล *****", set_format("xlsFmtTableDetail", "B", "C", "", 1));
		$worksheet->write($xlsRow, 1, "", set_format("xlsFmtTableDetail", "B", "C", "", 1));
		$worksheet->write($xlsRow, 2, "", set_format("xlsFmtTableDetail", "B", "C", "", 1));
		$worksheet->write($xlsRow, 3, "", set_format("xlsFmtTableDetail", "B", "C", "", 1));
	} // end if
