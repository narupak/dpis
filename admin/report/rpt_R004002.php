...
//		$sum_w = array_sum($heading_width);
	...
	function list_person($search_condition, $addition_condition){
		global $DPISDB, $db_dpis2, $db_dpis3,$db_dpis4, $position_table, $position_join, $line_table, $line_join, $f_name, $f_position;
...
//			$WORK_DURATION = date_difference($data[PER_RETIREDATE], $data[PER_STARTDATE], "ymd");
			/*เดิม*/
					   ...
					   //$WORK_DURATION = date_difference((($search_budget_year - 543)."-10-01"), $data[PER_STARTDATE], "full");
                        $WORK_DURATION = date_difference((($search_budget_year - 543)."-09-30"), $data[PER_STARTDATE], "full");//ให้นับวันที่เกษียณอายุ ถึงวันที่ 30 กันยายน
...
//					$total_day = floor((($total_day / $days_per_month) - floor($total_day / $days_per_month)) * $days_per_month);
			...
			//$arr_content[$data_count][position] = $PL_NAME . $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"") . "<br>" . $ORG_NAME_1;/*เดิม*/
	...
	if (!$result) echo "****** error ****** on open table for $table<br>";
 	...
 	if($count_data){
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$arr_data = (array) null;
                        //echo $REPORT_ORDER."<br>"; /*$arr_content[$data_count][type] = "MINISTRY"*/
