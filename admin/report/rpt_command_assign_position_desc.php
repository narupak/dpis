...
//	session_cache_limiter("nocache");
	...
	function print_header(){
		...
		$pdf->Cell(($heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4] + $heading_width[5]) ,7,"ส่วนราชการและตำแหน่งที่ขอ",'LTRB',0,'C',1);
		$pdf->Cell(($heading_width[6] + $heading_width[7] + $heading_width[8] + $heading_width[9] + $heading_width[10]) ,7,"ส่วนราชการและตำแหน่งที่นำมาเกลี่ย",'LTRB',0,'C',1);
		...
		$pdf->Cell($heading_width[5] ,7,"ขั้น",'LTR',0,'C',1);
		...
		$pdf->Cell($heading_width[10] ,7,"ขั้น",'LTR',0,'C',1);
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($CONTENT_TYPE == "ORG"){
				...
				$pdf->MultiCell(($heading_width[2] + $heading_width[3]), 7, "$REQ_ORG_NAME", $border, "L");
				...
				$pdf->MultiCell(($heading_width[7] + $heading_width[8]), 7, "$ORG_NAME", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
...
//				if(($pdf->h - $max_y - 10) < 22){ 
				...
				$pdf->Cell($heading_width[1], 7, "$REQ_POS_NO", $border, 0, 'R', 0);
				$pdf->MultiCell($heading_width[2], 7, "$REQ_PM_NAME", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2];
				$pdf->y = $start_y;
				$pdf->MultiCell($heading_width[3], 7, "$REQ_PL_NAME", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3];
				$pdf->y = $start_y;
				$pdf->MultiCell($heading_width[4], 7, "$REQ_CL_NAME", $border, "C");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4];
				$pdf->y = $start_y;
				$pdf->Cell($heading_width[5], 7, ($REQ_SALARY?number_format($REQ_SALARY):""), $border, 0, 'R', 0);
				$pdf->Cell($heading_width[6], 7, "$POS_NO", $border, 0, 'R', 0);
				$pdf->MultiCell($heading_width[7], 7, "$PM_NAME", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				...
				$pdf->y = $start_y;
				$pdf->MultiCell($heading_width[8], 7, "$PL_NAME", $border, "L");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				$pdf->x = $start_x + $heading_width[0] + $heading_width[1] + $heading_width[2] + $heading_width[3] + $heading_width[4] + $heading_width[5] + $heading_width[6] + $heading_width[7] + $heading_width[8];
				$pdf->y = $start_y;
				$pdf->MultiCell($heading_width[9], 7, "$CL_NAME", $border, "C");
				if($pdf->y > $max_y) $max_y = $pdf->y;
				...
				$pdf->y = $start_y;
				$pdf->Cell($heading_width[10], 7, ($POS_SALARY?number_format($POS_SALARY):""), $border, 0, 'R', 0);
