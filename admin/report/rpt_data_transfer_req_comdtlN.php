...
//		$sum_w = array_sum($heading_width);
		...
		$fname= "rpt_data_tranfer_req_comdtlN.rtf";
...
//		$db_dpis->show_error();
		...
		if($PER_TYPE==1){
			...
			$NEW_PM_NAME = trim($data2[PM_NAME]);
                        ...
                        /* Release 5.1.0.4 ปิดไว้ เพราะไม่ต้องการให้แสดง ระดับ */
...
//						 where 	PER_ID=$PER_ID and trim(LEVEL_NO)='$LEVEL_NO' ";
		...
		if ($COMMAND_PRT==1 && $BKK_FLAG!=1) {
			if ($PER_BIRTHDATE) $arr_content[$data_count][name] .= "*Enter*(".trim($PER_BIRTHDATE).")";
			//if ($cardID) $arr_content[$data_count][name] .= "*Enter*".trim($cardID);
		}
		$arr_content[$data_count][educate] = $EN_SHORTNAME;
		if ($COMMAND_PRT==1) {
			if ($EM_NAME) $arr_content[$data_count][educate] .= "*Enter*".trim($EM_NAME);
			if ($INS_NAME) $arr_content[$data_count][educate] .= "*Enter*".trim($INS_NAME);
		...
		if ($print_order_by==1 || $COMMAND_PRT==1) {
			if ($DEPARTMENT_NAME=="กรมการปกครอง") {
				...
				if ($CMD_ORG4) $arr_content[$data_count][cmd_position] .= "*Enter*".trim($CMD_ORG4);
				if ($CMD_ORG3) $arr_content[$data_count][cmd_position] .= "*Enter*".trim($CMD_ORG3);
				if ($CMD_ORG2 && $MFA_FLAG==1) $arr_content[$data_count][cmd_position] .= "*Enter*".trim($CMD_ORG2);
			}
		} 
		...
		if ($COMMAND_PRT==1) {
			...
			if ($DEPARTMENT_NAME && $MFA_FLAG==1) $arr_content[$data_count][new_position] .= "*Enter*".trim($DEPARTMENT_NAME);
		} 
...
//		$arr_content[$data_count][cmd_note] = $CMD_NOTE1;
		...
		if ($print_order_by==1 && $COMMAND_PRT!=1) {
			if($BKK_FLAG!=1){
				$arr_content[$data_count][type] = "CONTENT";
				if ($EM_NAME)
					$arr_content[$data_count][educate] = $EM_NAME."*Enter*".$INS_NAME;
				else
...
/*		if($CMD_NOTE2){
                        //ปิดไว้เพราะมีช่องตารางเกินมา 1 ช่อง(ตารางจะเพิ่มตาม array)
			$arr_content[$data_count][type] = "CONTENT";
                        $arr_content[$data_count][cmd_note2] = $CMD_NOTE2;
	...
	//		echo "$head_text1<br>";
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($COM_TYPE=="0101" || $COM_TYPE=="5011"){
				...
				$data_align = array("C", "L", "L", "L", "C", "L", "L", "C", "C", "C", "C", "C", "L");

//				echo "1..$ORDER:$NAME<br>";
				if ($FLAG_RTF)
					$result = $RTF->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "14", "", $fmtTableDetail_col_idx, $fmtTableDetail_bgcol_idx);
				else
					$result = $pdf->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "14", "", "$fmtTableDetail_col_idx", "");		//TRHBL
				if (!$result) echo "****** error ****** add data to table at record count = $data_count <br>";
				if (!$FLAG_RTF) $pdf->add_data_tab("", 0, "RHBL", $data_align, "", "12", "", "000000", "");		// เส้นปิดบรรทัด			
...
//					echo "4.1..$ORDER:$NAME<br>";
				...
				if(in_array($COM_TYPE, array("0105","0106", "5016", "5017"))){
					...
					$data_align = array("C", "L", "L", "L", "C", "C", "R", "C", "C", "L", "C", "C", "C", "R", "C", "L");
					...
					$data_align = array("C", "L", "L", "L", "C", "C", "R", "C", "L", "C", "C", "C", "R", "C", "L");
					...
					$data_align = array("C", "L", "L", "L", "L", "L", "L", "L", "L", "C", "C", "L", "C", "C", "C", "R", "C", "L");
				}else{		//โอน
					$arr_data[] = "$NEW_POSITION";
					$arr_data[] = "$NEW_POSITION_TYPE";
					$arr_data[] = "$NEW_POSITION_LEVEL";
					...
					$data_align = array("C", "L", "L", "L", "C", "C", "C", "L", "C", "C", "C", "R", "C", "L");
				}	 //end else

//				echo "5..$ORDER:$NAME<br>";
				if ($FLAG_RTF)
					$result = $RTF->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "14", "", $fmtTableDetail_col_idx, $fmtTableDetail_bgcol_idx);
				else
