...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		$pdf->Cell($heading_width[5] ,7,"ที่ให้โอน",'LBR',0,'C',1);
		$pdf->Cell($heading_width[6] ,7,"สูงสุด",'LBR',0,'C',1);
		$pdf->Cell($heading_width[7] ,7,"เกิด",'LBR',0,'C',1);
		$pdf->Cell($heading_width[8] ,7,"ที่ให้โอน",'LBR',0,'C',1);
		$pdf->Cell($heading_width[9] ,7,"ที่รับโอน",'LBR',0,'C',1);
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$pdf->y = $start_y;
			$pdf->Cell($heading_width[7], 7, "$BIRTHDATE", $border, 0, 'L', 0);
			$pdf->Cell($heading_width[8], 7, "$EFFECTIVEDATE", $border, 0, 'L', 0);
			$pdf->MultiCell($heading_width[9], 7, "$RCV_ORG", $border, "L");
			if($pdf->y > $max_y) $max_y = $pdf->y;
