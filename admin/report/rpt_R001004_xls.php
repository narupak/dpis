<?
	...
	include ("rpt_R001004_format.php");	// ใช้เฉพาะในส่วนการแปลง COLUMN_FORMAT เท่านั้น
	...
	$report_title = "จำนวนตำแหน่ง$PERSON_TYPE[$search_per_type] จำแนกตามประเภทตำแหน่ง";
	$report_code = "R0104";
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
		$ws_head_line1 = array("$heading_name","<**1**>ประเภทตำแหน่ง","<**1**>ประเภทตำแหน่ง","<**1**>ประเภทตำแหน่ง","<**1**>ประเภทตำแหน่ง","รวม");
		$ws_head_line2 = array("","ทั่วไป","วิชาการ","อำนวยการ","บริหาร","");
...
//echo "<br> $count_position = $cmd<hr>";
	...
	while($data = $db_dpis->get_array()){
		...
		$arr_DEPARTMENT_ID[] = $DEPARTMENT_ID;			//สำหรับเก็บทุกกรม กรณีกระทรวง
					...
					if ($CTRL_TYPE < 3) {
						if($MINISTRY_ID != trim($data[MINISTRY_ID])){
							...
							if ($f_all) {   //เพิ่มใหม่	
								...
								//if($data_count > 1){
								$arr_content[$data_count][count_1] = count_position(1, $search_condition, $addition_condition); //ทั่วไป
								$arr_content[$data_count][count_2] = count_position(2, $search_condition, $addition_condition); //วิชาการ
								$arr_content[$data_count][count_3] = count_position(3, $search_condition, $addition_condition); //อำนวยการ
								$arr_content[$data_count][count_4] = count_position(4, $search_condition, $addition_condition); //บริหาร
...
//							$db_dpis2->show_error();
				...
				case "CO_LEVEL" :
					if($CL_NAME != trim($data[CL_NAME])){
						...
						$arr_content[$data_count][name] = str_repeat(" ", (($rpt_order_index - $first_order) * 5)) . (trim($CL_NAME)?"ระดับ $CL_NAME":"[ไม่ระบุช่วงระดับตำแหน่ง]");
						$arr_content[$data_count][short_name] = "";
						$arr_content[$data_count][count_1] = count_position(1, $search_condition, $addition_condition);
						$arr_content[$data_count][count_2] = count_position(2, $search_condition, $addition_condition);
						$arr_content[$data_count][count_3] = count_position(3, $search_condition, $addition_condition);
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$GRAND_TOTAL = $GRAND_TOTAL_1 + $GRAND_TOTAL_2 + $GRAND_TOTAL_3 + $GRAND_TOTAL_4; //รวม
				...
				for($i=0; $i < count($arr_column_map); $i++) {
					if ($arr_column_sel[$arr_column_map[$i]]==1) {
						...
						if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1)
							$worksheet->write_string($xlsRow, $colseq, $ndata, set_format("xlsFmtTitle", $wsdata_fontfmt_1[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $wsdata_border_1[$arr_column_map[$i]], $wsdata_colmerge_1[$arr_column_map[$i]]));	// แสดงตัวหนา
						else
							$worksheet->write_string($xlsRow, $colseq, $ndata, set_format("xlsFmtTitle", $wsdata_fontfmt_2[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $wsdata_border_1[$arr_column_map[$i]], $wsdata_colmerge_1[$arr_column_map[$i]]));	// แสดงตัวปกติ
						$colseq++;
					}
				}
		} // end for
