<?
	...
	include ("rpt_data_move_req_comdtlN_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
//		$sum_w = array_sum($heading_width);
		...
		$fname= "rpt_data_move_comdtlN.rtf";
...
//		$arr_content[$data_count][educate] = $EN_NAME ."*Enter*". ($EM_NAME?"$EM_NAME":"");
		...
		$arr_content[$data_count][cmd_level_name] = $CMD_LEVEL_NAME;
                if($chk_salary == 1){//เลือกเเสดงเงินเดือน
		...
		$arr_content[$data_count][new_pos_no] = $NEW_POS_NO;
                if($chk_salary == 1){//เลือกเเสดงเงิน
				...
				if($CONTENT_TYPE=="ORG" || $CONTENT_TYPE=="ORG_1" || $CONTENT_TYPE=="ORG_2" ){
					...
					if($COM_TYPE=="0202" || $COM_TYPE=="5022"){
						...
						if ($FLAG_RTF)
							...
							$result = $pdf->add_data_tab($arr_data, 7, "RBL", $data_align, "", "14", "", "000000", "");
			...
			if($CONTENT_TYPE=="CONTENT"){
				...
				if($COM_TYPE=="0202" || $COM_TYPE=="5022"){
					...
					$result = $RTF->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "14", "", $fmtTableDetail_col_idx, $fmtTableDetail_bgcol_idx);
						...
						$result = $pdf->add_data_tab($arr_data, 7, "TRBL", $data_align, "", "14", "", "000000", "");
						if (!$result) echo "****** error ****** add data to table at record count = $data_count <br>";
					} // end if
				}else{
					$arr_data[] = $ORDER;
