...
//	session_cache_limiter("nocache");
	...
	//	echo "[$today] $print_today $print_time<br>";
		...
		$fname= "rpt_structure_lawchart.rtf";
...
//				echo "img_file=$img_file (".file_exists($img_file).")<br>";
...
//				$RTF->add_text($RTF->bold(1) . (($NUMBER_DISPLAY==2)?convert2thaidigit($oo):$oo) . $RTF->bold(0) , "center");
				...
				$pdf->SetDrawColor(hexdec("FF"),hexdec("00"),hexdec("00")); //สีดำ
...
//				echo "af...x=".$pdf->x.", y=".$pdf->y." , max_y=$max_y<br>";
				...
				$pdf->Rect($line_start_x, $line_start_y, $line_w, $line_h);
//				echo "$i--pdf->Rect($line_start_x, $line_start_y, $line_w, $line_h)<br>";
				if ($arr_content[ldept][$i]==2) { // ตั้งแต่ $i==2 เป็นต้นไป จะขัด
					$point_x1 = $line_start_x + (75 / 2);  $point_y1 = $line_start_y;
					$pdf->SetDrawColor(hexdec("FF"),hexdec("00"),hexdec("00")); //สีแดง
//					echo "L..$i--dept=".$arr_content[ldept][$i]."  x=$point_x, y=$point_y  x1=$point_x1, y1=$point_y1<br>";
...
//					$dept1 = 2;
					...
					$pdf->SetDrawColor(hexdec("00"),hexdec("00"),hexdec("FF")); //สีแดง
...
//					echo "af...x=".$pdf->x.", y=".$pdf->y." , max_y=$max_y<br>";
					...
					if ($arr_content[rdept][$i]==2) {
						$point_x1 = $line_start_x + (75 / 2);  $point_y1 = $line_start_y;
						$pdf->SetDrawColor(hexdec("FF"),hexdec("00"),hexdec("00")); //สีแดง
//						echo "R..$i--dept=".$arr_content[rdept][$i]."  x=$point_x, y=$point_y  x1=$point_x1, y1=$point_y1<br>";
						$pdf->Line($point_x, $point_y, $point_x, $point_y1 - 5);
						$pdf->Line($point_x, $point_y1 - 5, $point_x1, $point_y1 - 5);
						$pdf->Line($point_x1, $point_y1 - 5, $point_x1, $point_y1);
...
//		$db_dpis->show_error();
		if($count_data){
			...
			while($data = $db_dpis->get_array()){
				...
				//----------------------------------------------------------
//				echo "PER_CARDNO=$PER_CARDNO , TMP_PIC_NAME=$TMP_PIC_NAME , file_exist(".file_exists($TMP_PIC_NAME).")<br>";
				$img_file = "";
