...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$pdf->y = $start_y;
			for($i=1; $i<=3; $i++) $pdf->Cell($heading_width[2], 7, (${"COUNT_".$i}?number_format(${"COUNT_".$i}):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[3], 7, ($COUNT_ALL?number_format($COUNT_ALL):"-"), $border, 0, 'R', 0);
			$pdf->Cell($heading_width[4], 7, ($COUNT_QUOTA?number_format($COUNT_QUOTA, 2):"-"), $border, 0, 'R', 0);
			...
			for($i=0; $i<=7; $i++){
				if($i <= 1){
					...
					$line_start_y = $start_y;		$line_start_x += $heading_width[$i - 2];
					$line_end_y = $max_y;		$line_end_x += $heading_width[$i - 2];
				} // end if
				$pdf->Line($line_start_x, $line_start_y, $line_end_x, $line_end_y);
			} // end for
			//====================================================
		...
		for($i=1; $i<=3; $i++) $pdf->Cell($heading_width[2], 7, ($GRAND_LEVEL[$i]?number_format($GRAND_LEVEL[$i]):"-"), $border, 0, 'R', 0);
		$pdf->Cell($heading_width[3], 7, ($GRAND_ALL?number_format($GRAND_ALL):"-"), $border, 0, 'R', 0);
		$pdf->Cell($heading_width[4], 7, ($GRAND_QUOTA?number_format($GRAND_QUOTA, 2):"-"), $border, 0, 'R', 0);
		$pdf->Cell($heading_width[5], 7, "", $border, 1, 'L', 0);
	}else{
