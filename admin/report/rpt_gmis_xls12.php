<?
	$report_code = "data ลูกจ้างชั่วคราว Flow Out";

	// ===== select data =====
	$cmd = " select 	PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE, LEVEL_NO, 
					PER_SALARY, PER_MGTSALARY, PER_OFFNO, PER_CARDNO, TP_CODE, 
					PER_NAME, PER_SURNAME, PER_ENG_NAME, PER_ENG_SURNAME, 
					ORG_ID, MOV_CODE, POT_ID, PER_POSDATE, PER_RETIREDATE
			from		PER_PERSONAL  
			where	PER_TYPE=4 and PER_STATUS=2 and ((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$POSDATE = show_date_format($data[PER_POSDATE],1);
		...
		if($DPISDB=="odbc"){
			...
			$cmd = " select		
				...
				where		a.POT_ID = $POT_ID and a.ORG_ID=b.ORG_ID and a.TP_CODE=c.TP_CODE and b.PV_CODE=f.PV_CODE(+) and 
							b.OT_CODE=g.OT_CODE(+) ";
		}elseif($DPISDB=="mysql"){
			$cmd = " select		
							b.ORG_ID_REF, $select_list, a.POT_ID, b.ORG_NAME, c.TP_NAME, 
		...
		$cmd = " select PN_NAME from PER_PRENAME where TP_CODE='$TP_CODE' ";
		$db_dpis2->send_cmd($cmd);
		$data2 = $db_dpis2->get_array();
		...
		if($PER_ID){
			...
			$arr_content[$data_count][pn_code] = $TP_CODE;
			...
			elseif ($PER_TYPE == 3) $arr_content[$data_count][pos_no] = $POT_NO;
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$TP_CODE = $arr_content[$data_count][pn_code];
	...
	header("Content-Type: application/x-msexcel; name=\"gmis.xls\"");
	header("Content-Disposition: inline; filename=\"gmis.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
