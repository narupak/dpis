...
//	$arr_rpt_order = array("MINISTRY", "DEPARTMENT", "ORG", "LINE"); 
	...
	include ("rpt_R010020_format.php");	// ใช้เฉพาะในส่วนการแปลง COLUMN_FORMAT เท่านั้น
...
//			echo "heading_text [$i]=".$heading_text[$i]."<br>";
	...
	function set_cell_width(){
		...
		$worksheet->set_column(1, ($TOTAL_LEVEL), 8);
		$worksheet->set_column(($TOTAL_LEVEL+1), ($TOTAL_LEVEL+1), 8);
	} // function
...
//					echo "1--$i--".$arr_column_map[$i]."--".$wsdata_align_1[$arr_column_map[$i]]." >> ";
					$colseq++;
				}
			}
//			echo "<br>";
...
//				echo "2--$i--".$arr_column_map[$i]."--".$wsdata_align_1[$arr_column_map[$i]]." >> ";
				$colseq++;
			}
		}
//		echo "<br>";
		...
		for($j=1; $j<=14; $j++) $worksheet->write($xlsRow, $j, "", set_format("xlsFmtTitle", "B", "", "", "1"));
	} // end if
