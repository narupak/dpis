<?php
    ...
    if($chkmonth==15){
        if($NUMBER_DISPLAY==2){
       		 $objPHPExcel = $objReader->load("../../Excel/eslip/templates/R1204_template15ColTH.xls");
		}else{
			$objPHPExcel = $objReader->load("../../Excel/eslip/templates/R1204_template15Col.xls");
		}
    }else{
        if($NUMBER_DISPLAY==2){
       		 $objPHPExcel = $objReader->load("../../Excel/eslip/templates/R1204_template31ColTH.xls");
		}else{
    ...
    $objPHPExcel->getActiveSheet()->getStyle('A1:AL7')->getFont()->setName(getToFont($CH_PRINT_FONT));
    $objPHPExcel->getActiveSheet()->getStyle('A1:AL7')->getFont()->setSize($CH_PRINT_SIZE-4);
    ...
    $Val_in = iconv('TIS-620','UTF-8','     เวลาเข้า');
    $Val_out = iconv('TIS-620','UTF-8','     เวลาออก');
		...
		do  {
			...
			if($CurKONG!=$data[KONG]){ // รอบ
				...
				if($CurKONG!=""){
					...
					$clonedSheet->setCellValue('L'.($no+7+2), $HolidayStr2);  
					$clonedSheet->setCellValue('V'.($no+7+2), $HolidayStr3);  
					$clonedSheet->setCellValue('AF'.($no+7+2), $HolidayStr4); 
					...
					$clonedSheet->setCellValue('L'.($no+7+4), $HolidayStr6);  
					$clonedSheet->setCellValue('V'.($no+7+4), $HolidayStr7);  
					$clonedSheet->setCellValue('AF'.($no+7+4), $HolidayStr8); 
					
					$clonedSheet->setCellValue('C'.($no+7+6), $HolidayStr9);
					$clonedSheet->setCellValue('L'.($no+7+6), $HolidayStr10);  
					$clonedSheet->setCellValue('V'.($no+7+6), $HolidayStr11);  
					$clonedSheet->setCellValue('AF'.($no+7+6), $HolidayStr12); 
					
					$clonedSheet->setCellValue('C'.($no+7+8), $HolidayStr13);
					$clonedSheet->setCellValue('L'.($no+7+8), $HolidayStr14);  
					$clonedSheet->setCellValue('V'.($no+7+8), $HolidayStr15);  
					$clonedSheet->setCellValue('AF'.($no+7+8), $HolidayStr16); 		
				
					$clonedSheet->setCellValue('C'.($no+7+10), $HolidayStr17);  
					$clonedSheet->setCellValue('L'.($no+7+10), $HolidayStr18);  
					
					if($IS_OPEN_TIMEATT_ES=="OPEN"){ // กรณีมีระบบลงเวลา
						$clonedSheet->setCellValue('V'.($no+7+10), $HolidayStr19);  
					}
				...
				$clonedSheet
						...
						->setCellValue('A2', iconv('TIS-620','UTF-8','รายงานการมาปฏิบัติราชการของ'.$PERSON_TYPE[$search_per_type]))
			...
			$arr_data[] = (($NUMBER_DISPLAY==2)?convert2thaidigit($no1):$no1);
			...
			for($idx=0;$idx<count($ArrworkDateIN);$idx++){
				...
				$timeIN[]=$NUMBER_DISPLAY==2?convert2thaidigit($ArrInShow):$ArrInShow;
				$timeOut[]= $NUMBER_DISPLAY==2?convert2thaidigit($ArrOutShow):$ArrOutShow;
			}
			
			 $clonedSheet->insertNewRowBefore($no+7,1);
			$clonedSheet->setCellValue('A'.($no+6), $no1)
			...
			if(count($ArrworkDateIN)>15){
				$clonedSheet->setCellValue('AH'.($no+6), $sum0<=0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit( round($sum0,2) ): round($sum0,2) ))
											->setCellValue('AI'.($no+6), ($sum1+$sum3+$sum4+$sum5)==0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit(round($sum1+$sum3+$sum4+$sum5,2)):round($sum1+$sum3+$sum4+$sum5,2)))
											...
											->setCellValue('AL'.($no+6), '');
				$clonedSheet
						->getStyle('A'.($no+6).':AL'.($no+6))
						->getFill()
						->getStartColor()
						->setRGB('FFFFFF');
			}else{
				$clonedSheet->setCellValue('R'.($no+6), $sum0<=0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit( round($sum0,2) ): round($sum0,2) ))
											->setCellValue('S'.($no+6), ($sum1+$sum3+$sum4+$sum5)==0?'':(($NUMBER_DISPLAY==2)?convert2thaidigit(round($sum1+$sum3+$sum4+$sum5,2)):round($sum1+$sum3+$sum4+$sum5,2)))
			...
			$no++;
			$clonedSheet->insertNewRowBefore($no+7,1);
			$clonedSheet->setCellValue('A'.($no+6), '')->setCellValue('B'.($no+6), $Val_in);
			for($idx=0;$idx<count($ArrworkDateIN);$idx++){
				/*if($ArrHol[$idx]==1){ //วันหยุด
					$clonedSheet->setCellValue($ArrCol[$idx].($no+6),'n'.$timeIN[$idx])->getStyle($ArrCol[$idx].($no+6))->applyFromArray($styleArrayHoliday);
				}else{*/
					$clonedSheet->setCellValue($ArrCol[$idx].($no+6),$timeIN[$idx]);
				//}
			}
			
			$no++;
			$clonedSheet->insertNewRowBefore($no+7,1);
			$clonedSheet->setCellValue('A'.($no+6), '')->setCellValue('B'.($no+6), $Val_out);
			for($idx=0;$idx<count($ArrworkDateIN);$idx++){
				/*if($ArrHol[$idx]==1){ //วันหยุด
					$clonedSheet->setCellValue($ArrCol[$idx].($no+6), 'n'.$timeOut[$idx])->getStyle($ArrCol[$idx].($no+6))->applyFromArray($styleArrayHoliday);
				}else{*/
					$clonedSheet->setCellValue($ArrCol[$idx].($no+6), $timeOut[$idx]);
				//}
    ...
    $clonedSheet->getStyle('A'.($no+7+2).':AL7'.($no+7+2))->getFont()->setSize($CH_PRINT_SIZE-4);
    ...
    header('Content-Disposition: attachment;filename="R1204.xls"');
