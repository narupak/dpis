<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 2, "จำนวนวันลา", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 3, "นับวันหยุด", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_day_min)) $arr_search_condition[] = "($arr_fields[2] >= $search_day_min)";
  	if(trim($search_day_max)) $arr_search_condition[] = "($arr_fields[2] <= $search_day_max)";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4] 
							from		$table
							$search_condition
							order by IIF(ISNULL(AB_SEQ_NO), 9999, AB_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4] 
							from		$table
							$search_condition
							order by AB_SEQ_NO, $arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4] 
							from		$table
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->insert_bitmap($xlsRow, 3, $$arr_fields[3], 30, 4, 1, 0.6);
