<?
	$report_code = "data ลูกจ้างประจำ Flow Out";
	...
	$cmd = " select 	PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE, LEVEL_NO, 
					...
					ORG_ID, MOV_CODE, POEM_ID, PER_POSDATE, PER_RETIREDATE
			from		PER_PERSONAL  
			where	PER_TYPE=2 and PER_STATUS=2 and ((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
							(PER_RETIREDATE >= '$budget_year_from' and PER_RETIREDATE <= '$budget_year_to' and PER_POSDATE is NULL)) ";
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$POSDATE = show_date_format($data[PER_STARTDATE],1);
		if (!$POSDATE) $POSDATE = show_date_format($data[PER_RETIREDATE],1);
		...
		if($DPISDB=="odbc"){
			$cmd = " select		
							b.ORG_ID_REF, $select_list, a.POEM_ID, a.PG_CODE_SALARY, b.ORG_NAME, c.PN_NAME, 
			...
			$cmd = " select		
				...
				where		a.POEM_ID = $POEM_ID and a.ORG_ID=b.ORG_ID and a.PN_CODE=c.PN_CODE and b.PV_CODE=f.PV_CODE(+) and 
							b.OT_CODE=g.OT_CODE(+) ";
		}elseif($DPISDB=="mysql"){
			$cmd = " select		
							b.ORG_ID_REF, $select_list, a.POEM_ID, a.PG_CODE_SALARY, b.ORG_NAME, c.PN_NAME, 
		...
		$LEVEL_SHORTNAME = $data2[LEVEL_SHORTNAME];
		...
		if($PER_ID){
			...
			$arr_content[$data_count][posdate] = $POSDATE;
			...
			$arr_content[$data_count][level_name] = $LEVEL_SHORTNAME;
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			elseif (substr($MOV_CODE,0,1)=="2") $EFFECTIVEDATE = $SAH_EFFECTIVEDATE;
				...
				$worksheet->write_string($xlsRow, 6, "$LEVEL_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				...
				$worksheet->write_string($xlsRow, 30, "$POSDATE", set_format("xlsFmtTableDetail", "", "C", "TLRB", 0));
				$worksheet->write_string($xlsRow, 31, "$PG_CODE_SALARY", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));

				$cmd = " insert into GMIS_PEIS_FLOW_OUT (TEMPMINISTRY, ORGANZIE, DIVISIONNAME, TEMPPROVINCE, TEMPORGANIZETYPE, TEMPPRENAME, TEMPFIRSTNAME, 
	...
	header("Content-Type: application/x-msexcel; name=\"gmis_geis.xls\"");
	header("Content-Disposition: inline; filename=\"gmis_geis.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
