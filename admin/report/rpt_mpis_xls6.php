...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			elseif (substr($MOV_CODE,0,1)=="2") $EFFECTIVEDATE = $SAH_EFFECTIVEDATE;
				...
				$cmd = " insert into GMIS2_FLOW_OUT (tempMinistry, Organzie, DivisionName, tempProvince, tempOrganizeType, tempPrename, tempFirstName, 
	...
	header("Content-Type: application/x-msexcel; name=\"mpis.xls\"");
	header("Content-Disposition: inline; filename=\"mpis.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
