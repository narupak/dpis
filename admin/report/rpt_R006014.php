	...
	if($DPISDB=="odbc"){
		...
		$cmd = " select			distinct
						 ...
						 where		$position_join(+) and b.ORG_ID=c.ORG_ID(+) and a.PN_CODE=d.PN_CODE(+) and $line_join(+) and 
											...
											$search_condition
						 order by		g.MOV_CODE, $order_by, $position_no_name, to_number(replace($position_no,'-','')), a.PER_NAME ";
...
//		print_header();
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "PROMOTELEVEL"){
				...
				if ($FLAG_RTF) {
					...
					$pdf->AddPage();
					$result = $pdf->add_text_line($NAME, 7, "", "L", "", "14", "b", 0, 0);
					if (!$result) echo "****** error ****** add text line to table at record count = $data_count <br>";
		...
		if ($FLAG_RTF)
			...
			$result = $pdf->add_text_line("********** ไม่มีข้อมูล **********", 7, "", "C", "", "16", "b", 0, 0);
		if (!$result) echo "****** error ****** add text line to table at record count = $data_count <br>";
	} // end if
	if ($FLAG_RTF) {
		$RTF->close_tab(); 
