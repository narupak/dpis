...
//				require_once "../../Excel/my.defined_format.inc.php";	// use set_format(set of parameter) funtion , help is in file
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			elseif (substr($MOV_CODE,0,1)=="2") $EFFECTIVEDATE = $SAH_EFFECTIVEDATE;
				...
				$worksheet->write_string($xlsRow, 8, "$LEVEL_NO", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				...
				$cmd = " insert into GMIS1_FLOW_OUT (tempMinistry, Organzie, DivisionName, tempPositionNo, tempLine, tempPositionType, tempLevel, 
...
<?
	...
	if (count($arr_file) > 0) {
		...
		for($i_file = 0; $i_file < count($arr_file); $i_file++) {
			if (strpos($arr_file[$i_file],"rpt_mpis_xls2")!==false && $grp!="2") {
				$grp="2";
				echo "<tr><td width='16%'>2. data ข้าราชการ Flow In</td>";
				$num=1;
			} else if (strpos($arr_file[$i_file],"rpt_mpis_xls3")!==false && $grp != "3") {
				$grp="3";
