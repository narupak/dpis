<?
	...
	$xlsRow = 0;
	$worksheet->write($xlsRow, 0, "ข้อมูลมาตรฐานกำหนดตำแหน่ง แบบบรรยายลักษณะงาน - $PL_NAME - $LEVEL_NAME", set_format("xlsFmtTitle", "B", "C", "TLRB", 1));
	...
	foreach($ARR_ACCOUNT_TYPE as $ACC_TYPE_ID){
		...
		$cmd = "	select		ACC_DESCRIPTION
							from		ACCOUNTABILITY_INFO_SECONDARY
							where		POS_ID=$POS_ID and ACC_TYPE_ID=$ACC_TYPE_ID
							order by ACC_ID ";
		$db_dpis->send_cmd($cmd);
		$ACC_COUNT = 0;
		while($data = $db_dpis->get_array()){
			...
			$xlsRow++;
			$worksheet->write_string($xlsRow, 0, $ACC_DESCRIPTION, set_format("xlsFmtTableDetail", "", "L", "", 0));
	...
	foreach($ARR_JOB_TYPE as $JOB_TYPE){
		if($JOB_TYPE == "k"){
			...
			$xlsRow++;
			$worksheet->write($xlsRow, 0, "ความรู้ที่จำเป็นในงาน", set_format("xlsFmtSubTitle", "B", "C", "", 1, "black"));
			...
			if($DPISDB=="odbc"){
				$cmd = "	select		a.POS_JOB_DES_SEC_ID, b.JOB_DES_NAME, c.JOB_DES_LEVEL_DESCRIPTION
									from		POS_JOB_DES_SECONDARY a, KNOWLEDGE_INFO b, KNOWLEDGE_LEVEL c
			...
			if($DPISDB=="odbc"){
				$cmd = "	select		a.POS_JOB_DES_SEC_ID, b.JOB_DES_NAME, c.JOB_DES_LEVEL_DESCRIPTION
									from		POS_JOB_DES_SECONDARY a, SKILL_INFO b, SKILL_LEVEL c
									where		a.POS_ID=$POS_ID and a.POS_DES_ID=$POS_DES_ID and a.JOB_TYPE='$JOB_TYPE' and a.JOB_DES_ID=b.JOB_DES_ID 
													and a.JOB_DES_ID=c.JOB_DES_ID and a.JOB_DES_LEVEL=c.JOB_DES_LEVEL
									order by a.POS_JOB_DES_SEC_ID ";
			}elseif($DPISDB=="mssql"){
			...
			$xlsRow++;
			$worksheet->write($xlsRow, 0, "ประสบการณ์ที่จำเป็นในงาน", set_format("xlsFmtSubTitle", "B", "C", "", 1, "black"));
			$worksheet->write($xlsRow, 1, "", set_format("xlsFmtSubTitle", "B", "C", "", 1));
			...
			if($DPISDB=="odbc"){
				$cmd = "	select		a.POS_JOB_DES_SEC_ID, b.JOB_DES_NAME, a.JOB_DES_LEVEL
		...
		if($count_info){
			...
			while($data = $db_dpis->get_array()){
				...
				if($JOB_TYPE == "k")			$JOB_DES_LEVEL_DESCRIPTION = trim($data[JOB_DES_LEVEL_DESCRIPTION]);
				elseif($JOB_TYPE == "s")	$JOB_DES_LEVEL_DESCRIPTION = trim($data[JOB_DES_LEVEL_DESCRIPTION]);
				elseif($JOB_TYPE == "e")	$JOB_DES_LEVEL_DESCRIPTION = trim($data[JOB_DES_LEVEL]);

				$xlsRow++;
				$worksheet->write_string($xlsRow, 0, $JOB_DES_NAME, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				$worksheet->write($xlsRow, 1, $JOB_DES_LEVEL_DESCRIPTION, set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			} // loop while
		}else{
		} // end if
	} // loop foreach
	...
	$JOB_TYPE = "c";
	$xlsRow++;
	$worksheet->write($xlsRow, 0, "สมรรถนะที่จำเป็นในงาน", set_format("xlsFmtSubTitle", "B", "C", "", 1, "black"));
	$worksheet->write($xlsRow, 1, "", set_format("xlsFmtSubTitle", "B", "C", "", 1));
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		a.POS_JOB_DES_SEC_ID, b.JOB_DES_NAME, c.JOB_DES_LEVEL_DESCRIPTION, 
											b.COMPETENCY_TYPE
							from		POS_JOB_DES_SECONDARY a, COMPETENCY_INFO b, COMPETENCY_LEVEL c
							where		a.POS_ID=$POS_ID and a.POS_DES_ID=$POS_DES_ID and a.JOB_TYPE='$JOB_TYPE' and a.JOB_DES_ID=b.JOB_DES_ID 
											and a.JOB_DES_ID=c.JOB_DES_ID and a.JOB_DES_LEVEL=c.JOB_DES_LEVEL
							order by b.COMPETENCY_TYPE desc, a.POS_JOB_DES_SEC_ID ";
	}elseif($DPISDB=="mssql"){
		$cmd = "	select		a.POS_JOB_DES_SEC_ID, b.JOB_DES_NAME, c.JOB_DES_LEVEL_DESCRIPTION,
											b.COMPETENCY_TYPE
							from		POS_JOB_DES_SECONDARY a, COMPETENCY_INFO b, COMPETENCY_LEVEL c
