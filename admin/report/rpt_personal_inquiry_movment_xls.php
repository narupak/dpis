<?
	...
	function print_header($xlsRow){
		...
		global $PER_LEVEL_TITLE, $EL_TITLE, $EN_TITLE, $INS_TITLE, $POS_NO_TITLE, $PM_TITLE, $PL_TITLE, $CL_TITLE, $LEVEL_TITLE, $ORG_TITLE, $ORG_TITLE1, $ORG_TITLE2;
		...
		$worksheet->write($xlsRow, 10, "$PER_LEVEL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));		
		$worksheet->write($xlsRow, 11, "เงินเดือน", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 12, "$EL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 13, "$EN_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 14, "สาขาวิชา/วิชาเอก", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 15, "$INS_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 16, "หมายเหตุวุฒิ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 17, "$POS_NO_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 18, "$PM_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 19, "$PL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 20, "$CL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));		
		$worksheet->write($xlsRow, 21, "$LEVEL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));		
...
//	if(trim($search_pos_ot_code)) $arr_search_condition[] = "(b.OT_CODE='$search_pos_ot_code')";
	...
	if(count($arr_search_positionhis_condition)){
		...
		$arr_search_positionhis_condition[] = "(POH_LAST_POSITION='Y')"; 
...
//		$db_dpis->show_error();
	...
	if(count($arr_search_salaryhis_condition)){
		...
		$arr_search_positionhis_condition[] = "(SAH_LAST_SALARY='Y')"; 
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$BIRTHDATE = show_date_format($data[PER_BIRTHDATE], 1);
			$STARTDATE = show_date_format($data[PER_STARTDATE], 1);
			$OCCUPYDATE = show_date_format($data[PER_OCCUPYDATE], 1);
			$MEMBERDATE = show_date_format($data[PER_MEMBERDATE], 1);
			$EFFECTIVEDATE = show_date_format($data[PER_EFFECTIVEDATE], 1);
...
//				echo $cmd."<hr>";
	...
	//				echo $cmd."<hr>";
			...
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$POS_UP_DATE = show_date_format($data2[POH_EFFECTIVEDATE], 1);
			...
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$POH_DATE = show_date_format($data2[POH_EFFECTIVEDATE], 1);
			if ($PER_TYPE=="1") {
				if ($PAY_ID) {			
					$cmd = " select 	POS_NO 
...
//				$db_dpis2->show_error();
			...
			for ( $i=0; $i<count($level); $i++ ) { 
				...
				$db_dpis2->send_cmd($cmd);
				$data2 = $db_dpis2->get_array();
				$LEVEL_DATE[$level[$i]] = show_date_format($data2[POH_EFFECTIVEDATE], 1);
			} // end for
			...
			$worksheet->write_string($xlsRow, 2, "$CARDNO", set_format("xlsFmtTableDetail", "", "C", "TLRB", 0));
