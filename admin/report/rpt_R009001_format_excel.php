...
//	fix head for rpt_R009001
	...
	$heading_text[] = "ผลรวม";//"ระดับผลการประเมิน";
        if ($have_am) {
             $heading_text[] = "ระดับผลการประเมิน(หลัก)"; // เพิ่มใหม่ Modify By Pitak 26.10.2558
        }
        if ($have_al) {
            $heading_text[] = "ระดับผลการประเมิน(ย่อย)"; 
        }    
        $heading_text[] = "อนุญาตให้เห็นคะแนน"; 
        if($IS_ACCEPT_CONFIG){
            $heading_text[] = "รับทราบในการประเมินคะแนน"; 
        }
	...
	$column_function[] = (($NUMBER_DISPLAY==2)?"TNUM-2":"ENUM-2");
        if ($have_am) {
            $column_function[] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM"); // เพิ่มใหม่ Modify By Pitak 26.10.2558
        }
        if ($have_al) {
            $column_function[] = (($NUMBER_DISPLAY==2)?"TNUM":"ENUM");
        }    
	...
	$data_align = array("R","L","C","L","C","R","R","R","R","R","R","R","R");
