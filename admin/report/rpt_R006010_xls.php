<?
	...
	include ("rpt_R006010_format.php");	// ใช้เฉพาะในส่วนการแปลง COLUMN_FORMAT เท่านั้น
...
//	$search_per_type = 1;
	...
	if ($search_promote_level=="0") {
		$arr_search_condition[] = "(MOV_SUB_TYPE in ('49') or SM_CODE = '$SM_CODE_00' or (SAH_PERCENT_UP = 0 and SAH_SALARY_EXTRA = 0))";
		$search_promote_name = "ไม่ได้เลื่อนเงินเดือน";
	} elseif ($search_promote_level=="0.5") {
		$arr_search_condition[] = "(MOV_SUB_TYPE in ('45') or SM_CODE = '$SM_CODE_05' or (SAH_PERCENT_UP > 0 and SAH_PERCENT_UP <= 1))";
		$search_promote_name = "เลื่อน 0.5 ขั้น หรือไม่เกิน 1 %";
	} elseif ($search_promote_level=="1") {
		$arr_search_condition[] = "(MOV_SUB_TYPE in ('46') or SM_CODE = '$SM_CODE_10' or (SAH_PERCENT_UP > 1 and SAH_PERCENT_UP <= 2))";
		$search_promote_name = "เลื่อน 1.0 ขั้น หรือไม่เกิน 2 %";
	} elseif ($search_promote_level=="1.5") {
		$arr_search_condition[] = "(MOV_SUB_TYPE in ('47') or SM_CODE = '$SM_CODE_15' or (SAH_PERCENT_UP > 2 and SAH_PERCENT_UP <= 3))";
		$search_promote_name = "เลื่อน 1.5 ขั้น หรือไม่เกิน 3 %";
	} elseif ($search_promote_level=="2") {
		$arr_search_condition[] = "(MOV_SUB_TYPE in ('48') or SM_CODE = '$SM_CODE_20' or (SAH_PERCENT_UP > 3 and SAH_PERCENT_UP <= 4))";
		$search_promote_name = "เลื่อน 2.0 ขั้น หรือไม่เกิน 4 %";
	} elseif ($search_promote_level=="3") {
		$arr_search_condition[] = "(SAH_PERCENT_UP > 4 and SAH_PERCENT_UP <= 5)";
		$search_promote_name = "เลื่อน ไม่เกิน 5 %";
	} elseif ($search_promote_level=="4") {
		$arr_search_condition[] = "(SAH_PERCENT_UP > 5 and SAH_PERCENT_UP <= 6)";
		$search_promote_name = "เลื่อน ไม่เกิน 6 %";
	}
	...
	$report_title = "บัญชีรายชื่อ$PERSON_TYPE[$search_per_type]ที่ ". $search_promote_name ." ปีงบประมาณ $search_budget_year";
...
*/
	...
	// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนหัวรายงาน
		$ws_head_line1 = array("สังกัด/ชื่อ-สกุล","ตำแหน่ง/ระดับ","เลขที่ตำแหน่ง","เงินเดือน (บาท)");
...
//				echo "$i-map[".$arr_column_map[$i]."], width=".$ws_width[$arr_column_map[$i]]."<br />";
	...
	if($DPISDB=="odbc"){
		...
		$cmd = " select			$select_list, a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $position_no_name as POS_NO_NAME, to_number(replace($position_no,'-','')) as POS_NO, 
											a.PER_SALARY, $line_code as PL_CODE, $line_name as PL_NAME, a.LEVEL_NO, h.MOV_SUB_TYPE, g.SAH_SALARY $select_type_code $select_mgt_code
						 from			PER_PERSONAL a, $position_table b, PER_ORG c, PER_PRENAME d, $line_table e, PER_SALARYHIS g, PER_ORG f, PER_MOVMENT h
...
//	echo "$cmd<br>";
	...
	while($data = $db_dpis->get_array()){
		if($MOV_SUB_TYPE != trim($data[MOV_SUB_TYPE])){
			...
			$arr_content[$data_count][name] = number_format(($MOV_SUB_TYPE + 0), 1);
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "PROMOTELEVEL"){
				$worksheet = &$workbook->addworksheet($search_promote_name);
				$worksheet->set_margin_right(0.50);
				$worksheet->set_margin_bottom(1.10);
