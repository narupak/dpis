<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 2, "กลุ่มบัญชีค่าจ้าง", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if(trim($search_code)) $arr_search_condition[] = "($arr_fields[0] like '$search_code%')";
  	if(trim($search_name)) $arr_search_condition[] = "(($arr_fields[1] like '%$search_name%') or ($arr_fields[6] like '%$search_name%'))";
	$search_condition = "";
	if(count($arr_search_condition)) $search_condition = " where " . implode(" and ", $arr_search_condition);

	if($DPISDB=="odbc"){			
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[6], $arr_fields[2], PG_SEQ_NO 
							from		$table
										$search_condition
						 order by IIF(ISNULL(PG_SEQ_NO), 9999, PG_SEQ_NO),PG_CODE
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[6], $arr_fields[2], PG_SEQ_NO 
							from		$table
										$search_condition
						 order by PG_SEQ_NO,PG_CODE
					   ";
	} elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[6], $arr_fields[2], PG_SEQ_NO 
							from		$table
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$TMP_SEQ_NO = $data[PG_SEQ_NO];
			$$arr_fields[1] = $data[$arr_fields[1]];
			$$arr_fields[6] = $data[$arr_fields[6]];
			$$arr_fields[2] = $data[$arr_fields[2]];
			
			$$arr_fields[3] = ($data[$arr_fields[2]]==1)?"../images/checkbox_check.bmp":"../images/checkbox_blank.bmp";
			...
			$worksheet->write($xlsRow, 2, $$arr_fields[6], set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
