<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 2, "กลุ่มงาน", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 3, "สิทธิ์การได้รับเครื่องราชฯ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_level)) $arr_search_condition[] = "(a.$arr_fields[2] = '$search_level')";
  	if(trim($search_type) != "") $arr_search_condition[] = "(a.$arr_fields[3] = $search_type)";
	$search_condition = "";
	if(count($arr_search_condition)) $search_condition = " and " . implode(" and ", $arr_search_condition);

	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], $arr_fields[4], LEVEL_NAME 
							from		$table a, PER_LEVEL b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by IIF(ISNULL(EP_SEQ_NO), 9999, EP_SEQ_NO), $arr_fields[0] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], $arr_fields[4], LEVEL_NAME
							from		$table a, PER_LEVEL b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by EP_SEQ_NO, $arr_fields[0] ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], $arr_fields[4], LEVEL_NAME 
							from		$table a, PER_LEVEL b
							where		a.$arr_fields[2] = b.$arr_fields[2]
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->write($xlsRow, 2, $level_name, set_format("xlsFmtTableDetail", "", "C", "TLRB", 0));
