<?
	$report_code = "data ลูกจ้างประจำ Flow In";
	...
	$cmd = " select 	PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE, LEVEL_NO, 
					...
					ORG_ID, MOV_CODE, POEM_ID, PER_OCCUPYDATE 
			from		PER_PERSONAL  
			where	PER_TYPE=2 and PER_OCCUPYDATE >= '$budget_year_from' and PER_OCCUPYDATE <= '$budget_year_to' ";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			elseif (substr($MOV_CODE,0,1)=="2") $EFFECTIVEDATE = $SAH_EFFECTIVEDATE;
				...
				$cmd = " insert into GMIS_PEIS_FLOW_IN (TEMPMINISTRY, ORGANZIE, DIVISIONNAME, TEMPPROVINCE, TEMPORGANIZETYPE, TEMPPRENAME, TEMPFIRSTNAME, 
