<?
	...
	$report_title = "รายชื่อข้าราชการระดับ 9 - 11";
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){		
		...
		$arr_content[$data_count][position] = (trim($PM_NAME)?"$PM_NAME (":"") . (trim($PL_NAME)?($PL_NAME ." ". level_no_format($LEVEL_NO) . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):"ระดับ ".level_no_format($LEVEL_NO)) . (trim($PM_NAME)?")":"");
