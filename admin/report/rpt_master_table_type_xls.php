<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อประเภทตำแหน่ง", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "กลุ่ม", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by IIF(ISNULL(PT_SEQ_NO), 9999, PT_SEQ_NO),$arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
