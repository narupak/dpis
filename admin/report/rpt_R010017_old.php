...
//		echo "default font_id::".$RTF->dfl_FontID."<br>";	
	...
	$heading_width[EDUCATE][1] = "40";
	$heading_width[EDUCATE][2] = "40";
	$heading_width[EDUCATE][3] = "40";
	$heading_width[EDUCATE][4] = "40";
...
//			$RTF->new_page();	
				...
				$RTF->color("0");	// 0='BLACK'
				$RTF->cell($RTF->bold(1)."ส่วนราชการ".$RTF->bold(0), $heading_width[$HISTORY_NAME][0], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."ชื่อ-สกุล".$RTF->bold(0), $heading_width[$HISTORY_NAME][1], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."ตำแหน่ง".$RTF->bold(0), $heading_width[$HISTORY_NAME][2], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."ระดับวุฒิ การศึกษา".$RTF->bold(0), $heading_width[$HISTORY_NAME][3], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."ชื่อวุฒิ การศึกษา".$RTF->bold(0), $heading_width[$HISTORY_NAME][4], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."สาขาวิชาเอก".$RTF->bold(0), $heading_width[$HISTORY_NAME][5], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."สถาบัน การศึกษา".$RTF->bold(0), $heading_width[$HISTORY_NAME][6], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."ประเทศ".$RTF->bold(0), $heading_width[$HISTORY_NAME][7], "center", "25", "TBLR");	
				$RTF->cell($RTF->bold(1)."วันเกษียณ (1 ต.ค.)".$RTF->bold(0), $heading_width[$HISTORY_NAME][8], "center", "25", "TBLR");	
				$RTF->close_line();
/*						$RTF->open_line();
						$RTF->set_font($font, 14);
						$RTF->color("0");	// 0='BLACK'
						$RTF->cell($RTF->bold(1)."ด้านความเชี่ยวชาญพิเศษ".$RTF->bold(0), $heading_width[$HISTORY_NAME][0], "center", "25", "TBLR");	
						$RTF->cell($RTF->bold(1)."เน้นทาง".$RTF->bold(0), $heading_width[$HISTORY_NAME][1], "center", "25", "TBLR");	
						$RTF->close_line(); */
		} // function
	}else{
		function print_header($HISTORY_NAME){
	...
	//		$pdf->Cell(100,2,"",0,1,'C');
				...
				$pdf->Cell($heading_width[3] ,7,"ระดับวุฒิ",'LTR',0,'C',1);
				$pdf->Cell($heading_width[4] ,7,"ชื่อวุฒิ",'LTR',0,'C',1);
				$pdf->Cell($heading_width[5] ,7,"สาขาวิชาเอก",'LTR',0,'C',1);
				$pdf->Cell($heading_width[6] ,7,"สถาบัน",'LTR',0,'C',1);
				$pdf->Cell($heading_width[7] ,7,"ประเทศ",'LTR',0,'C',1);
				$pdf->Cell($heading_width[8] ,7,"วันเกษียณ",'LTR',1,'C',1);
				
				//แถว 2 ---------------------
				$pdf->Cell($heading_width[0] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[1] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[2] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[3] ,7,"การศึกษา",'LBR',0,'C',1);
				$pdf->Cell($heading_width[4] ,7,"การศึกษา",'LBR',0,'C',1);
				$pdf->Cell($heading_width[5] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[6] ,7,"การศึกษา",'LBR',0,'C',1);
				$pdf->Cell($heading_width[7] ,7,"",'LBR',0,'C',1);
				$pdf->Cell($heading_width[8] ,7,"(1 ต.ค.)",'LBR',1,'C',1);
			}//end if
...
//			$RTF->new_page();
			...
			$RTF->set_font($font, 14);
			$RTF->color("0");	// 0='BLACK'
			$RTF->cell("", 100, "left", "25", "");	
			$RTF->close_line();
		} else {
			$pdf->AutoPageBreak = false;
			//#########################################
			//แสดงข้อมูลด้านประวัติต่างๆ
		...
		//							$db_dpis2->show_error();
							...
							$pdf->Cell(200,7,($history_index + 1)."  $MINISTRY_NAME ",0,1,"L");
...
//								$RTF->new_page();
								...
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								if ($NUMBER_DISPLAY==2) $MINISTRY_NAME= convert2thaidigit($MINISTRY_NAME);
								$RTF->cell($RTF->bold(1).$MINISTRY_NAME.$RTF->bold(0), 100, "left", "25", "");	
								$RTF->close_line();
							} else {
								if(($pdf->h - $pdf->y - 10) < 30) $pdf->AddPage();
								$pdf->SetFont($font,'b',14);
								...
								$pdf->Cell(200,7,"$MINISTRY_NAME",0,1,"L");
							}
						} // end if
					break;
		...
		//							$db_dpis2->show_error();
							...
							$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
							if($pdf->y != $page_start_y) $pdf->Cell(200,7,"",0,1,"L");
							$pdf->Cell(200,7,($history_index + 1)."  $DEPARTMENT_NAME ",0,1,"L");
							print_header($HISTORY_NAME);*/
							if ($FLAG_RTF) {
//								$RTF->new_page();
								$RTF->open_line();
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								if ($NUMBER_DISPLAY==2) $DEPARTMENT_NAME= convert2thaidigit($DEPARTMENT_NAME);
								$RTF->cell($RTF->bold(1).$DEPARTMENT_NAME.$RTF->bold(0), 100, "left", "25", "");	
								$RTF->close_line();
							} else {
								if(($pdf->h - $pdf->y - 10) < 30) $pdf->AddPage();
								$pdf->SetFont($font,'b',14);
								...
								$pdf->Cell(200,7,"$DEPARTMENT_NAME",0,1,"L");
							}
						} // end if
				break;
	...
	//							$db_dpis2->show_error();
							...
							$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
							if($pdf->y != $page_start_y) $pdf->Cell(200,7,"",0,1,"L");
							$pdf->Cell(200,7,($history_index + 1)."  $ORG_NAME ",0,1,"L");
							print_header($HISTORY_NAME);*/
							if ($FLAG_RTF) {
//								$RTF->new_page();
								$RTF->open_line();
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								if ($NUMBER_DISPLAY==2) $ORG_NAME= convert2thaidigit($ORG_NAME);
								$RTF->cell($RTF->bold(1).$ORG_NAME.$RTF->bold(0), 100, "left", "25", "");	
								$RTF->close_line();
							} else {
								if(($pdf->h - $pdf->y - 10) < 30) $pdf->AddPage();
								$pdf->SetFont($font,'b',14);
...
//								$RTF->new_page();
								...
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								if ($NUMBER_DISPLAY==2) $buff = convert2thaidigit($data_count." ".$FULLNAME); else $buff = $data_count." ".$FULLNAME;
								$RTF->cell($RTF->bold(1).$buff.$RTF->bold(0), 35, "left", "25", "");	
								$buff= "ปีเกษียณอายุ  ".($PER_RETIREDATE?(($NUMBER_DISPLAY==2)?convert2thaidigit($PER_RETIREDATE):$PER_RETIREDATE):"-");
								$RTF->cell($RTF->bold(1).$buff.$RTF->bold(0), 35, "center", "25", "");	
								if ($NUMBER_DISPLAY==2) 
									$buff = convert2thaidigit("ตำแหน่ง  ".(trim($PL_NAME)?($PL_NAME ." (". $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".$LEVEL_NAME).")"); 
								else 
									$buff = "ตำแหน่ง  ".(trim($PL_NAME)?($PL_NAME ." (". $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".$LEVEL_NAME).")"; 
								$RTF->cell($RTF->bold(1).$buff.$RTF->bold(0), 30, "right", "25", "");	
								$RTF->close_line();
...
//									$pdf->Image($img_file, $image_x, $image_y, $image_w, $image_h);
								...
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								$RTF->cell($RTF->bold(1)."  ประวัติการศึกษา ".$RTF->bold(0), 100, "center", "25", "");	
								$RTF->close_line();
								print_header($HISTORY_NAME);
							} else {
								if(($pdf->h - $pdf->y - 10) < 30) $pdf->AddPage();
								...
								$pdf->Cell(85,7,"$data_count. $FULLNAME",0,0,"L");
								$pdf->Cell(85,7,"ปีเกษียณอายุ  ".($PER_RETIREDATE?(($NUMBER_DISPLAY==2)?convert2thaidigit($PER_RETIREDATE):$PER_RETIREDATE):"-"),0,1,"L");
								$pdf->Cell(90,7,$pdf->Text_print_optimize("ตำแหน่ง  ".(trim($PL_NAME)?($PL_NAME ." (". $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".$LEVEL_NAME).")", 100),0,0,"L");
								//*****************************************************************************************************
								...
								$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
								if($pdf->y != $page_start_y) $pdf->Cell(200,7,"",0,1,"L");
								$pdf->Cell(200,7,"  ประวัติการศึกษา ",0,1,"C"); //width=200
								if($have_pic && $img_file){
									$image_x = ($pdf->x + 170);		$image_y = ($pdf->y - 33);		$image_w = 25;			$image_h = 35;
									$pdf->Image($img_file, $image_x, $image_y, $image_w, $image_h);
								} // end if
...
//										$RTF->new_page();
										...
										$RTF->set_font($font, 14);
										$RTF->color("0");	// 0='BLACK'
										if ($NUMBER_DISPLAY==2) $EL_NAME = convert2thaidigit($EL_NAME);
										$RTF->cell($EL_NAME, $heading_width[$HISTORY_NAME][0], "left", "8", $border);
										if ($NUMBER_DISPLAY==2) $EN_NAME = convert2thaidigit($EN_NAME);
										$RTF->cell($EN_NAME, $heading_width[$HISTORY_NAME][1], "left", "8", $border);
										if ($NUMBER_DISPLAY==2) $EM_NAME = convert2thaidigit($EM_NAME);
										$RTF->cell($EM_NAME, $heading_width[$HISTORY_NAME][2], "left", "8", $border);
										if ($NUMBER_DISPLAY==2) $INS_NAME = convert2thaidigit($INS_NAME);
										$RTF->cell($INS_NAME, $heading_width[$HISTORY_NAME][2], "left", "8", $border);
										if ($NUMBER_DISPLAY==2) $CT_NAME = convert2thaidigit($CT_NAME);
										$RTF->cell($CT_NAME, $heading_width[$HISTORY_NAME][2], "left", "8", $border);
										$RTF->close_line();
										print_header($HISTORY_NAME);
									} else {
										$pdf->SetFont($font,'',14);
										...
										$pdf->MultiCell($heading_width[$HISTORY_NAME][0], 7, "$EL_NAME", $border, "L");
										if($pdf->y > $max_y) $max_y = $pdf->y;
										$pdf->x = $start_x + $heading_width[$HISTORY_NAME][0];
										$pdf->y = $start_y;
	
										$pdf->MultiCell($heading_width[$HISTORY_NAME][1], 7, "$EN_NAME", $border, "L");
										if($pdf->y > $max_y) $max_y = $pdf->y;
										$pdf->x = $start_x + $heading_width[$HISTORY_NAME][0] + $heading_width[$HISTORY_NAME][1];
										$pdf->y = $start_y;
	
										$pdf->MultiCell($heading_width[$HISTORY_NAME][2], 7, "$EM_NAME", $border, "L");
										if($pdf->y > $max_y) $max_y = $pdf->y;
										$pdf->x = $start_x + $heading_width[$HISTORY_NAME][0] + $heading_width[$HISTORY_NAME][1] + $heading_width[$HISTORY_NAME][2];
										$pdf->y = $start_y;
	
										$pdf->MultiCell($heading_width[$HISTORY_NAME][3], 7, "$INS_NAME", $border, "L");
										if($pdf->y > $max_y) $max_y = $pdf->y;
										...
										$pdf->MultiCell($heading_width[$HISTORY_NAME][4], 7, "$CT_NAME", $border, "L");
										if($pdf->y > $max_y) $max_y = $pdf->y;
...
//								$RTF->new_page();
								...
								$RTF->set_font($font, 14);
								$RTF->color("0");	// 0='BLACK'
								$RTF->cell($RTF->bold(1)."  ความเชี่ยวชาญพิเศษ ".$RTF->bold(0), 100, "center", "25", "");
//								if ($NUMBER_DISPLAY==2) $SS_NAME= convert2thaidigit($SS_NAME);
//								$RTF->cell($RTF->bold(1).$SS_NAME.$RTF->bold(0), $heading_width[$HISTORY_NAME][0], "left", "25", "TBL");	
//								if ($NUMBER_DISPLAY==2) $SS_NAME= convert2thaidigit($SS_NAME);
//								$RTF->cell($RTF->bold(1).$SPS_EMPHASIZE.$RTF->bold(0), $heading_width[$HISTORY_NAME][1], "left", "25", "TBL");	
//								$RTF->cell($RTF->bold(1).$ORG_NAME.$RTF->bold(0), 83, "left", "25", "TBL");	
								$RTF->close_line();
								print_header($HISTORY_NAME);
							} else {
								if(($pdf->h - $pdf->y - 10) < 30) $pdf->AddPage();
								...
								$pdf->SetTextColor(hexdec("00"),hexdec("00"),hexdec("00"));
								if($pdf->y != $page_start_y) $pdf->Cell(200,7,"",0,1,"L");
								$pdf->Cell(200,7,"  ความเชี่ยวชาญพิเศษ ",0,1,"C");
								print_header($HISTORY_NAME);
							}
...
//										$RTF->new_page();
										...
										$RTF->color("0");	// 0='BLACK'
										if ($NUMBER_DISPLAY==2) $SS_NAME= convert2thaidigit($SS_NAME);
										$RTF->cell($SS_NAME, $heading_width[$HISTORY_NAME][0], "left", "25", "TBL");	
										if ($NUMBER_DISPLAY==2) $SS_NAME= convert2thaidigit($SS_NAME);
										$RTF->cell($SPS_EMPHASIZE, $heading_width[$HISTORY_NAME][1], "left", "25", "TBL");	
										$RTF->close_line();
										print_header($HISTORY_NAME);
									} else {
										$pdf->SetFont($font,'',14);
...
//			$RTF->new_page();
			...
			$RTF->set_font($font, 16);
			$RTF->color("0");	// 0='BLACK'
			$RTF->cell($RTF->bold(1)."********** ไม่มีข้อมูล **********".$RTF->bold(0), 200, "center", "25", "");	
			$RTF->close_line();
		} else {
