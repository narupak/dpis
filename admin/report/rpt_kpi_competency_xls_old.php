<?
	...
	if($PER_TYPE==1){
		...
		$PL_NAME = trim($PL_NAME)?($PL_NAME ." ". level_no_format($LEVEL_NAME) . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):" ".level_no_format($LEVEL_NAME);
	}elseif($PER_TYPE==2){
	...
	$report_title = "ตารางแสดงผลการประเมินสมรรถนะของข้าราชการ $DEPARTMENT_NAME";
	$company_name = "ประจำรอบการประเมินครั้งที่ $KF_CYCLE พ.ศ. $KF_YEAR  สำนัก/กอง $ORG_NAME";
...
//		$db_dpis2->show_error();
		...
		if ($othStartDate == $chkKFStartDate && $othEndDate == $chkKFEndDate) { // if startDate and endDate ตรงกัน
			...
			$cmd1 = " SELECT *  FROM PER_KPI_COMPETENCE  WHERE KF_ID=$KF_ID "; // KF_ID ของผู้ถูกประเมิน เจ้าของรายงานนี้
			$db_dpis2->send_cmd($cmd1);
//			$db_dpis2->show_error();
			// % ตามประเภท CF_TYPE
