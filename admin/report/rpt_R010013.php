...
//	$list_type_text = $ALL_REPORT_TITLE;
	...
	$report_code = "H13";
	include ("rpt_R010013_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
	...
	if ($search_pos_status==1) {
		if($DPISDB=="odbc"){
			...
			$cmd = " select			b.PER_ID, i.PN_NAME, b.PER_NAME, b.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
						...
						order by		$order_by, m.LEVEL_SEQ_NO desc, a.LEVEL_NO desc, h.PM_SEQ_NO, NLSSORT(b.PER_NAME,'NLS_SORT=THAI_DICTIONARY'), NLSSORT(b.PER_SURNAME,'NLS_SORT=THAI_DICTIONARY'), MIN(SUBSTR(trim(j.POH_EFFECTIVEDATE), 1, 10)), a.PER_SALARY desc, 
	...
	//		echo "$head_text1<br>";
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "MINISTRY"){
				...
				if ($have_pic && $img_file)
				$data_align = array("C","L", "L", "L", "C", "C", "C", "C", "C");
				else
				$data_align = array("L", "L", "L", "C", "C", "C", "C", "C");
