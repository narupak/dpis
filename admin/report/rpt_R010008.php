...
//			$RTF->close_section(); 
	...
	if($set_header=="SEX"){
	    $arr_content_map[0] = "name";
		$arr_content_map[1] = "count_1";
		$arr_content_map[2] = "count_2";
		$arr_content_map[3] = "count_3";
		$arr_content_map[4] = "count_4";
		$arr_content_map[5] = "count_m";
		$arr_content_map[6] = "count_5";
		$arr_content_map[7] = "count_6";
		$arr_content_map[8] = "count_7";
		$arr_content_map[9] = "count_8";
		$arr_content_map[10] = "count_f";
		$arr_content_map[11] = "count_t";
		
	    $arr_series_caption[0] = "ส่วนราชการ";
		$arr_series_caption[1] = "ชาย|ต่ำกว่า|ปริญญาตรี";
		$arr_series_caption[2] = "ชาย|ปริญญาตรี";
		$arr_series_caption[3] = "ชาย|ปริญญาโท";
		$arr_series_caption[4] = "ชาย|ปริญญาเอก";
		$arr_series_caption[5] = "ชาย|รวม";
		$arr_series_caption[6] = "หญิง|ต่ำกว่า|ปริญญาตรี";
		$arr_series_caption[7] = "หญิง|ปริญญาตรี";
		$arr_series_caption[8] = "หญิง|ปริญญาโท";
		$arr_series_caption[9] = "หญิ|ปริญญาเอก";
		$arr_series_caption[10] = "หญิง|รวม";
		$arr_series_caption[11] = "จำนวน|รวม";
	}else{
		$arr_content_map[0] = "name";
		$arr_content_map[1] = "count_1";
		$arr_content_map[2] = "percent_1";
		$arr_content_map[3] = "count_2";
		$arr_content_map[4] = "percent_2";
		$arr_content_map[5] = "count_3";
		$arr_content_map[6] = "percent_3";
		$arr_content_map[7] = "count_4";
		$arr_content_map[8] = "percent_4";
		$arr_content_map[9] = "count_t";
		$arr_content_map[10] = "percent_t";

		$arr_series_caption[0] = "ส่วนราชการ";
		$arr_series_caption[1] = "ต่ำกว่าปริญญาตรี|จำนวน";
		$arr_series_caption[2] = "ต่ำกว่าปริญญาตรี|ร้อยละ";
		$arr_series_caption[3] = "ปริญญาตรี|จำนวน";
		$arr_series_caption[4] = "ปริญญาตรี|ร้อยละ";
		$arr_series_caption[5] = "ปริญญาโท|จำนวน";
		$arr_series_caption[6] = "ปริญญาโท|ร้อยละ";
		$arr_series_caption[7] = "ปริญญาเอก|จำนวน";
		$arr_series_caption[8] = "ปริญญาเอก|ร้อยละ";
		$arr_series_caption[9] = "รวม|จำนวน";
		$arr_series_caption[10] = "รวม|ร้อยละ";
	}
...
//		echo "name:".$arr_content[$i][name]."<br>";
		$cntseq=0; $ss=0;
/*		$grand_total = 0;
		if ($set_header!="SEX") {
			for($j=0; $j<count($arr_content_map); $j++){ 
				if ($arr_column_sel[$arr_column_map[$j]]==1 && strpos($arr_content_map[$arr_column_map[$j]],"count_")!==false) {
					$grand_total += $arr_content[$i][$arr_content_map[$arr_column_map[$j]]];
//					echo "grand_total=$grand_total<br>";
		...
		for($j=0; $j<count($arr_content_map); $j++){ 
			if ($arr_column_sel[$arr_column_map[$j]]==1 && strpos($arr_content_map[$arr_column_map[$j]],"count_")!==false) {
//				echo "mapname:".$arr_content_map[$arr_column_map[$j]]."<br>";
				$n = (int)substr($arr_content_map[$arr_column_map[$j]],6,1);
				if ($n > 0) {
...
/*				if ($set_header=="SEX") {
					$n = (int)substr($arr_content_map[$arr_column_map[$j]],6,1);
					if ($n > 0) {
						$ss += $arr_content[$i][$arr_content_map[$arr_column_map[$j]]];
						if ($n == 4) {
							$arr_series_caption_data[$cntseq][] = $ss;
							$tt = $ss;
							$ss = 0;
							if ($f_first) $arr_series_caption_list[] = $arr_series_caption[$arr_column_map[$j+1]];
						} else if ($n == 8) {
							$arr_series_caption_data[$cntseq][] = $ss;
							if ($f_first) $arr_series_caption_list[] = $arr_series_caption[$arr_column_map[$j+1]];
							$tt += $ss;
							$arr_series_caption_data[$cntseq][] = $tt;
							if ($f_first) $arr_series_caption_list[] = $arr_series_caption[$arr_column_map[$j+2]];
						}
					}
				} else {
					$n = (int)substr($arr_content_map[$arr_column_map[$j]],6,1);
					if ($n > 0) {
						$arr_series_caption_data[$cntseq][] = ($arr_content[$i][$arr_content_map[$arr_column_map[$j]]] / $grand_total) * 100;
//						echo "percent_$n=".(($arr_content[$i][$arr_content_map[$arr_column_map[$j]]] / $grand_total) * 100)."<br>";
						if ($n == 4) {
							$arr_series_caption_data[$cntseq][] = ($grand_total / $grand_total) * 100;
//							echo "percent_total=".(($grand_total / $grand_total) * 100)."<br>";
							if ($f_first) $arr_series_caption_list[] = $arr_series_caption[$arr_column_map[$j+1]];
						}
					}
				}*/
