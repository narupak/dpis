...
//	$UPDATE_DATE = date("Y-m-d H:i:s");
	...
	$arr_data = (array) null;
	//$data_count = 0;
	$count_data = get_kpi_v2($KPI_YEAR, 0);//get_kpi($KPI_YEAR, 0);		//$count_data = get_kpi($KPI_ID, 0);
       
	if ($count_data) {
		$cmd = " select KPI_ID,KPI_NAME from PER_KPI where KPI_YEAR='$KPI_YEAR' ";		//$cmd = " select KPI_NAME from PER_KPI where KPI_ID=$KPI_ID ";
...
//		$db_dpis->show_error();
		...
		for($data_count=0; $data_count < count($arr_data); $data_count++) {
			...
			//$worksheet->write($xlsRow, 0, $pre_fix.$arr_data[$data_count][kpi_parent].":".$arr_data[$data_count][kpi_id]."-".$arr_data[$data_count][kpi_name], set_format("xlsFmtTitle", "B", "L", "TLBR", 0));
                        $worksheet->write($xlsRow, 0, $arr_data[$data_count][kpi_parent].":".$arr_data[$data_count][kpi_id]."-".$arr_data[$data_count][kpi_name], set_format("xlsFmtTitle", "B", "L", "TLBR", 0));
		} // end for
	} else {
		$xlsRow++;
	...
	require_once("excel_tailpart_subrtn.php");
        /*
         * select lpad(' ',2*(level-1)) || to_char(kpi_name) s 
  from PER_KPI 
  where KPI_YEAR='2559'
  start with kpi_id_ref is null
  connect by prior kpi_id = kpi_id_ref;
         */
        function get_kpi_v2($year_parent, $level) {
            global $dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd;
            global $arr_data, $data_count;
            $count_all = 0;
            $db_dpis = new connect_dpis($dpisdb_host, $dpisdb_name, $dpisdb_user, $dpisdb_pwd);

            $cmd = " select KPI_ID, KPI_NAME,KPI_YEAR 
                    from PER_KPI 
                    where KPI_YEAR='$year_parent'
                    start with kpi_id_ref is null
                    connect by prior kpi_id = kpi_id_ref";
...
//		$db_dpis->show_error();
            ...
            if($count_data){
                    while($data = $db_dpis->get_array()){
                            ...
                            //$arr_data[$data_count][level] = $level+1;
                            $data_count++;
                            //$level++;
                    } // end while
            } // end if
	...
	function get_kpi ($year_parent, $level) {		//function get_kpi ($kpi_parent, $level) {
		...
		//$cmd = " select KPI_ID, KPI_NAME from PER_KPI where KPI_ID_REF=$kpi_parent ";
		$cmd = " select KPI_ID, KPI_NAME,KPI_YEAR from PER_KPI where KPI_YEAR='$year_parent' ";
		$count_data = $db_dpis->send_cmd($cmd);
//		$db_dpis->show_error();
		if($count_data){
			while($data = $db_dpis->get_array()){
