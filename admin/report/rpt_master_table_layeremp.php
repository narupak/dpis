...
//	session_cache_limiter("nocache");
	...
	session_start();
		...
		$fname= "rpt_master_table_layeremp.rtf";
	...
	$heading_text[0] = "กลุ่มบัญชีค่าจ้าง";
	$heading_text[1] = "ขั้น";
	$heading_text[2] = "อัตราค่าจ้างรายเดือน";
	$heading_text[3] = "อัตราค่าจ้างรายวัน";
	$heading_text[4] = "อัตราค่าจ้างรายชั่วโมง";
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		a.$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4], $arr_fields[5], PG_NAME_SALARY
							from  		$table a, PER_POS_GROUP b  
							where		a.$arr_fields[0] = b.$arr_fields[0]
											$search_condition
							order by PG_NAME_SALARY, $arr_fields[1] ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		a.$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], $arr_fields[4], $arr_fields[5], PG_NAME_SALARY
							from  		$table a, PER_POS_GROUP b
							where		a.$arr_fields[0] = b.$arr_fields[0]
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$arr_data = (array) null;
			$arr_data[] = $data[PG_NAME_SALARY];
			$arr_data[] = $data[$arr_fields[1]];
			$arr_data[] = number_format($data[$arr_fields[2]], 2, ".", ",");
			$arr_data[] = number_format($data[$arr_fields[3]], 2, ".", ",");
			$arr_data[] = number_format($data[$arr_fields[4]], 2, ".", ",");
			$arr_data[] = "<*img*".(($data[$arr_fields[5]]==1)?"../images/checkbox_check.jpg":"../images/checkbox_blank.jpg")."*img*>";

			$data_align = array("C", "C", "R", "R", "R", "C");
			if ($FLAG_RTF)
