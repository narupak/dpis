<?
	...
	$worksheet = &$workbook->addworksheet("หลักสูตรการฝึกอบรม-ดูงาน");
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อหลักสูตรการฝึกอบรม/ดูงาน", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_type)) $arr_search_condition[] = "($arr_fields[1] = $search_type)";
	$search_condition = "";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by IIF(ISNULL(TR_SEQ_NO), 9999, TR_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
							$search_condition
							order by TR_SEQ_NO, $arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3]
							from		$table
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			if ($data[$arr_fields[1]] == 1) 	 $$arr_fields[1] = "อบรม";
			elseif ($data[$arr_fields[1]] == 2) 	 $$arr_fields[1] = "ดูงาน";
			elseif ($data[$arr_fields[1]] == 3) 	 $$arr_fields[1] = "สัมมนา";
