...
//	$arr_search_condition[] = "(a.PER_STATUS in (". implode(", ", $search_per_status) ."))";
	$arr_search_condition[] = "(trim(e.MOV_CODE) in ('105','10510','10520'))";	//ประเภทการเคลื่อนไหวรับโอน
	...
	$report_title = "แบบสำรวจ$PERSON_TYPE[$search_per_type] ที่รับโอนในปีงบประมาณ $search_budget_year || ส่วนราชการ $DEPARTMENT_NAME";
	$report_code = "R20008";
	$orientation='L';
...
//		$pdf->Cell(100,2,"",0,1,'C');
		...
		$pdf->Cell($heading_width[5] ,7,"วุฒิการศึกษา",'LTR',0,'C',1);
		...
		$pdf->Cell($heading_width[9] ,7,"ส่วนราชการ",'LTR',0,'C',1);
		$pdf->Cell($heading_width[10] ,7,"ลักษณะการโอน",'LTR',1,'C',1);
		...
		$pdf->Cell($heading_width[5] ,7,"ที่รับโอน",'LBR',0,'C',1);
		$pdf->Cell($heading_width[6] ,7,"สูงสุด",'LBR',0,'C',1);
		$pdf->Cell($heading_width[7] ,7,"เกิด",'LBR',0,'C',1);
		$pdf->Cell($heading_width[8] ,7,"ที่รับโอน",'LBR',0,'C',1);
		$pdf->Cell($heading_width[9] ,7,"เดิมก่อนโอน",'LBR',0,'C',1);
		$pdf->Cell($heading_width[10] ,7,"(ปกติ/สอบ)",'LBR',1,'C',1);
	} // function		

	//หาส่วนราชการเดิมก่อนโอน
	function find_oldorg($POH_ENDDATE,$PER_ID){
		global $DPISDB, $db_dpis3;

		//echo "<br>$PER_ID : $POH_ENDDATE<br>";
		//ระบุวันก่อนหน้าที่ได้รับโอน หาจากวันสิ้นสุดการดำรงตำแหน่ง
		if($DPISDB=="odbc"){
			$BEFORE_MOVDATE = "LEFT(POH_EFFECTIVEDATE,10)	< '$POH_ENDDATE'";
			$cmd = " SELECT 		POH_ID, LEFT(POH_EFFECTIVEDATE,10), LEFT(POH_ENDDATE,10), POH_ORG2, POH_ORG3
							FROM				PER_POSITIONHIS 
							WHERE			PER_ID=$PER_ID and $BEFORE_MOVDATE
							ORDER BY	 	LEFT(POH_EFFECTIVEDATE,10) desc, LEFT(POH_ENDDATE,10) desc ";
		}elseif($DPISDB=="oci8"){	
			$BEFORE_MOVDATE = "SUBSTR(POH_EFFECTIVEDATE,1,10)	< '$POH_ENDDATE'";
			$cmd = "select 		POH_ID, SUBSTR(POH_EFFECTIVEDATE,1,10),SUBSTR(POH_ENDDATE,1,10), POH_ORG2, POH_ORG3
							from 		PER_POSITIONHIS
							where 	PER_ID=$PER_ID and $BEFORE_MOVDATE
...
//			echo "<br>$cmd<br>";
	...
	function list_person($search_condition, $addition_condition){
		...
		if($search_per_type==1){
			if($DPISDB=="odbc"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, 
													LEFT(trim(a.PER_BIRTHDATE), 10) as PER_BIRTHDATE, LEFT(trim(a.PER_STARTDATE), 10) as PER_STARTDATE,
													MAX(LEFT(trim(e.POH_EFFECTIVEDATE), 10)) as POH_EFFECTIVEDATE, MAX(LEFT(trim(e.POH_ENDDATE), 10)) as POH_ENDDATE, e.PT_CODE, e.MOV_CODE
								 from		(	
								 					(
														(
															(
				...
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, 
													...
													MAX(SUBSTR(trim(e.POH_EFFECTIVEDATE), 1, 10)) as POH_EFFECTIVEDATE, MAX(SUBSTR(trim(e.POH_ENDDATE), 1, 10)) as POH_ENDDATE, e.PT_CODE, e.MOV_CODE
								 from			PER_PERSONAL a, $position_table b, PER_ORG c, PER_PRENAME d, PER_POSITIONHIS e, $line_table f, PER_TYPE g
								 where		$position_join(+) and b.ORG_ID=c.ORG_ID(+)
								 					and a.PN_CODE=d.PN_CODE(+) and a.PER_ID=e.PER_ID(+) and $line_join(+) and e.PT_CODE=g.PT_CODE(+)
													$search_condition
								 group by	a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name, e.LEVEL_NO, e.PT_CODE, 
			...
			if($DPISDB=="odbc"){
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name as PL_NAME, e.LEVEL_NO, 
													LEFT(trim(a.PER_BIRTHDATE), 10) as PER_BIRTHDATE, LEFT(trim(a.PER_STARTDATE), 10) as PER_STARTDATE,
													MAX(LEFT(trim(e.POH_EFFECTIVEDATE), 10)) as POH_EFFECTIVEDATE, MAX(LEFT(trim(e.POH_ENDDATE), 10)) as POH_ENDDATE, e.MOV_CODE
								 from			(
														(
															(
																(
				...
				$cmd = " select			a.PER_ID, d.PN_NAME, a.PER_NAME, a.PER_SURNAME, $line_name as PL_NAME, e.LEVEL_NO, 
													SUBSTR(trim(a.PER_BIRTHDATE), 1, 10) as PER_BIRTHDATE, SUBSTR(trim(a.PER_STARTDATE), 1, 10) as PER_STARTDATE,
													MAX(SUBSTR(trim(e.POH_EFFECTIVEDATE), 1, 10)) as POH_EFFECTIVEDATE, MAX(SUBSTR(trim(e.POH_ENDDATE), 1, 10)) as POH_ENDDATE, e.MOV_CODE
...
//			$db_dpis3->show_error();
			...
			//หาวุฒิที่รับโอน (ในตำแหน่งปัจจุบัน)
			...
			$OLD_ORG = find_oldorg($POH_ENDDATE,$PER_ID);
			...
			$arr_content[$data_count][edu_move] = $EDU_MOVE;	//วุฒิที่รับโอน	
			$arr_content[$data_count][edu_max] = $EDU_MAX;	//วุฒิสูงสุด
			$arr_content[$data_count][birthdate]	= $PER_BIRTHDATE;
			$arr_content[$data_count][posdate] = $POH_EFFECTIVEDATE;	//วันที่รับโอน
...
//							$db_dpis2->show_error();
						...
						$arr_content[$data_count][name] = str_repeat(" ", ($rpt_order_index * 8)) . $ORG_NAME_1;
...
//							$db_dpis2->show_error();
						...
						if($ORG_ID_2){
							...
							$arr_content[$data_count][name] = str_repeat(" ", ($rpt_order_index * 8)) . $ORG_NAME_2;
						} // end if
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$EDU_MOVE =  $arr_content[$data_count][edu_move];
			...
			$pdf->Cell($heading_width[8], 7, "$EFFECTIVEDATE", $border, 0, 'R', 0);
			$pdf->MultiCell($heading_width[9], 7, "$OLD_ORG", $border, "R");
			if($pdf->y > $max_y) $max_y = $pdf->y;
			...
			$pdf->y = $start_y;
			$pdf->MultiCell($heading_width[10], 7, "$MOV_NAME", $border, "R");
			if($pdf->y > $max_y) $max_y = $pdf->y;
