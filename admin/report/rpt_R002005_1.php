...
//			$list_type_text .= " - $MINISTRY_NAME";
	...
	$report_code = "R0205";
	include ("rpt_R002005_1format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
//		$sum_w = array_sum($heading_width);
	...
	$arr_age_duration[1][name] = "<5";
	$arr_age_duration[2][name] = "5";	
	$arr_age_duration[3][name] = "6";	
	$arr_age_duration[4][name] = "7";	
	$arr_age_duration[5][name] = "8";	
	$arr_age_duration[6][name] = "9";	
	$arr_age_duration[7][name] = "10";	
	$arr_age_duration[8][name] = "11";	
	$arr_age_duration[9][name] = "12";
	$arr_age_duration[10][name] = "13";
	$arr_age_duration[11][name] = "14";
	$arr_age_duration[12][name] = "15";	
	$arr_age_duration[13][name] = ">15";
...
//		else $age_condition = "a.PER_STARTDATE is not null and trim(a.PER_STARTDATE) <> ''";
		...
		//echo "<pre>".$cmd." || -- ||".$age_condition."<br><hr><br>";
...
//			echo "<pre>"; print_r($arr_age); echo "</pre>";
	...
	function generate_age_condition($age_index){
		...
		if($arr_age_duration[$age_index][name]=="<5"){
//			คิดตามอายุจริง (จากวันเข้ารับราชการ ถึง วันปัจจุบัน)
...
//			คิดเฉพาะปีเข้ารับราชการ กับปีปัจจุบัน 0-4
			...
			//echo "age <5 :: $age_condition<br>";
		}elseif($arr_age_duration[$age_index][name]==">15"){
//			คิดตามอายุจริง (จากวันเข้ารับราชการ ถึง วันปัจจุบัน)
...
//			คิดเฉพาะปีเข้ารับราชการ กับปีปัจจุบัน
			$startyear_max = substr($search_effective_date,0,4) - 15;
...
//			คิดเฉพาะปีเข้ารับราชการ กับปีปัจจุบัน
	...
	for($i=1; $i<=13; $i++) ${"GRAND_TOTAL_".$i} = 0;
							...
							if($MINISTRY_ID != trim($data[MINISTRY_ID])){
								...
								if($MINISTRY_ID != "" && $MINISTRY_ID!=0 && $MINISTRY_ID!=-1){
									...
									if ($f_all) {
										...
										$GRAND_TOTAL11 = count_person(11, $search_condition, "$addition_condition");
										$GRAND_TOTAL12 = count_person(12, $search_condition, "$addition_condition");
										$GRAND_TOTAL13 = count_person(13, $search_condition, "$addition_condition");
										...
										$arr_content[$data_count][count_11] = $GRAND_TOTAL11;
										$arr_content[$data_count][count_12] = $GRAND_TOTAL12;
										$arr_content[$data_count][count_13] = $GRAND_TOTAL13;
										$arr_content[$data_count][sum_age] = $GRAND_TOTAL;
										
										if($rpt_order_index==$first_order){
											for($i=1; $i<=13; $i++) { 
												${"GRAND_TOTAL_".$i} += $arr_content[$data_count]["count_".$i]; 
		//										echo "GRAND_TOTAL_".$i."=".${"GRAND_TOTAL_".$i}."<br>"; 
											}
										} // end if
						...
						if ($CTRL_TYPE < 5) {
							if($DEPARTMENT_ID != trim($data[DEPARTMENT_ID])){
								...
								if($DEPARTMENT_ID != "" && $DEPARTMENT_ID != 0 && $DEPARTMENT_ID != -1){
									...
									if($rpt_order_index==$first_order){
										for($i=1; $i<=13; $i++) ${"GRAND_TOTAL_".$i} += $arr_content[$data_count]["count_".$i];
									} // end if
	...
	//							$db_dpis2->show_error();
								...
								if(($ORG_NAME !="" && $ORG_NAME !="-") || ($BKK_FLAG==1 && $ORG_NAME !="" && $ORG_NAME !="-")){
									...
									$arr_content[$data_count][count_9] = count_person(9, $search_condition, $addition_condition);
									$arr_content[$data_count][count_10] = count_person(10, $search_condition, $addition_condition);
									$arr_content[$data_count][count_11] = count_person(11, $search_condition, $addition_condition);
									$arr_content[$data_count][count_12] = count_person(12, $search_condition, $addition_condition);
									$arr_content[$data_count][count_13] = count_person(13, $search_condition, $addition_condition);
	...
	//		echo "$head_text1<br>";
		...
		if($count_data){
			...
			for($data_count=0; $data_count<count($arr_content); $data_count++){
				...
				$arr_data = (array) null;
				$arr_data[] = $NAME;
				for($i=1; $i<=13; $i++) $arr_data[] = ${"COUNT_".$i};
...
//				echo "<br>";
		...
		if(strtolower($graph_type)=="pie"){
			$series_list = $GRAND_TOTAL_1.";".$GRAND_TOTAL_2.";".$GRAND_TOTAL_3.";".$GRAND_TOTAL_4.";".$GRAND_TOTAL_5.";".$GRAND_TOTAL_6.";".$GRAND_TOTAL_7.";".$GRAND_TOTAL_8.";".$GRAND_TOTAL_9.";".$GRAND_TOTAL_10.";".$GRAND_TOTAL_11.";".$GRAND_TOTAL_12.";".$GRAND_TOTAL_13;
		}else{
