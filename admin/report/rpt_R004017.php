...
//	$arr_rpt_order = array("ORG"); 
	...
	$report_code = "R0417";
	include ("rpt_R004017_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงา?
...
//	initialize_parameter(0);
	while($data = $db_dpis->get_array()){
        ...
        if(trim($ORG_NAME_1)){$ORG_NAME_1 ="/".$ORG_NAME_1;}
...
//				$db_dpis3->show_error();
		...
		// === BEG_C_DATE วันเ?าสู่ระดับก่อนปัจจุบั?
...
//				$db_dpis3->show_error();
			$data3 = $db_dpis3->get_array();
			if($POSITIONHIS) $POSITIONHIS .= "*Enter*";
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	//				echo "$head_text1<br>";
					...
					$pdf->report_title = "$DEPARTMENT_NAME||บัญชีรายชื่อ$PERSON_TYPE[$search_per_type] ". $LEVEL_NAME ." สายงาน $PL_NAME||ในสังกัด$ORG_NAME เรียงตามอาวุโส";
	...
	//				$pdf->AliasNbPages();
		...
		if ($FLAG_RTF)
			$result = $RTF->add_text_line("********** ไม่มีข้อมู?**********", 7, "", "C", "", "14", "b", 0, 0);
		else
			$result = $pdf->add_text_line("********** ไม่มีข้อมู?**********", 7, "", "C", "", "14", "b", 0, 0);
