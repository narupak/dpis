...
//	echo "cmd=$cmd<br>";
	...
	if ($COM_LEVEL_SALP==3 || $COM_LEVEL_SALP==8) {
		if ($BKK_FLAG==1)
		...
		/*if ($BKK_FLAG==1)
			$report_title = "บัญชีรายละเอียดการปรับอัตราเงินเดือน$PERSON_TYPE[$COM_PER_TYPE]||แนบท้ายคำสั่ง $COM_NO ลงวันที่ $COM_DATE";                     
                ...
                $report_title = "บัญชีรายละเอียดการให้ข้าราชการได้รับเงินเดือนสูงกว่าขั้นสูงของตำแหน่งที่ได้รับแต่งตั้ง||แนบท้ายคำสั่ง $DEPARTMENT_NAME ที่ $COM_NO ลงวันที่ $COM_DATE";/*Release 5.2.1.3 */
	} else {
		if ($BKK_FLAG==1) {
...
//				$NEW_ORG_NAME = $arr_content[$data_count][new_org_name];
				...
				$data_align = array("C", "L", "L", "L", "C", "R", "R", "R", "L");
			     if ($FLAG_RTF)
			     $result = $RTF->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "14", "", $fmtTableDetail_col_idx, $fmtTableDetail_bgcol_idx);
				 else
				$result = $pdf->add_data_tab($arr_data, 7, "RHL", $data_align, "", "14", "", "000000", "");
