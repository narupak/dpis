<?
	...
	$report_title = trim($report_title) . " ประจำปี ". ($yr_perholiday + 543);
	$report_code = "";
...
//	session_cache_limiter("nocache");
	...
	session_start();
		...
		$fname= "rpt_master_table_holiday.rtf";
	...
	$heading_text[0] = "วันที่";
	$heading_text[1] = "วันหยุด";
...
$yr_perholiday_min= $yr_perholiday-1;
	...
	/*  if($DPISDB=="odbc")  $cmd = "SELECT DISTINCT LEFT(hol_date, 4) FROM PER_HOLIDAY ORDER BY 1";
	$db_dpis->send_cmd($cmd);
	$db_dpis->show_error();
	while ($data = $db_dpis->get_array()) {  echo  $data["$arr_fields[0]"]; echo "<hr><br>"; }  */
...
//	echo $cmd;
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$arr_data = (array) null;
			$arr_data[] = show_date_format($data[$arr_fields[0]],$DATE_DISPLAY);
			$arr_data[] = $data[$arr_fields[1]];

			$data_align = array("C", "L");
			  if ($FLAG_RTF)
