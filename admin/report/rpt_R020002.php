...
//	session_cache_limiter("nocache");
	...
	function print_header(){
		...
		if($search_per_type == 1){
			$pdf->Cell(($heading_width[5] + $heading_width[6]) ,7,"เงินเดือน ณ 1 เม.ย. 51",'LTR',0,'C',1);
			$pdf->Cell($heading_width[7] ,7,"ค่าตอบแทน",'LTR',0,'C',1);
			$pdf->Cell($heading_width[8] ,7,"ให้ได้รับ",'LTR',0,'C',1);
			$pdf->Cell($heading_width[9] ,7,"หมายเหตุ",'LTR',1,'C',1);
		}else{
			$pdf->Cell($heading_width[5] ,7,"อัตราค่าจ้าง",'LTR',0,'C',1);
		...
		if($search_per_type == 1){
			$pdf->Cell($heading_width[5] ,7,"อันดับ",'LTBR',0,'C',1);
			$pdf->Cell($heading_width[6] ,7,"ขั้น",'LTBR',0,'C',1);
			$pdf->Cell($heading_width[7] ,7,"พิเศษชั่วคราว",'LBR',0,'C',1);
			$pdf->Cell($heading_width[8] ,7,"",'LBR',0,'C',1);
			$pdf->Cell($heading_width[9] ,7,"",'LBR',1,'C',1);
		}else{
		 	$pdf->Cell($heading_width[5] ,7,"ณ 1 เม.ย. 51",'LBR',0,'C',1);
...
//				$NEW_ORG_NAME = $arr_content[$data_count][new_org_name];
				...
				$border = "";
				if($CONTENT_TYPE == "ORG") $pdf->SetFont('angsa','',14);
				...
				if($search_per_type==1){
					...
					$pdf->Cell($heading_width[7], 7, "$CMD_EXTRA", $border, 0, 'R', 0);
					$pdf->Cell($heading_width[8], 7, "$CMD_SUMRCV", $border, 0, 'R', 0);
