...
//	$RPTORD_LIST = "COUNTRY|$RPTORD_LIST";		//กำหนดค่าเริ่มต้น ต้องแสดงประเทศด้วยทุกครั้ง เริ่ม index ที่ตำแหน่งแรกคือ 0
	...
	$arr_search_condition[] = "(trim(f.EL_TYPE) in ('1', '2', '3', '4', '9'))";
...
//		$sum_w = array_sum($heading_width);
	...
	function count_person($education_level, $search_condition, $addition_condition){
		...
		elseif($education_level == 9) $search_condition .= (trim($search_condition)?" and ":" where ") . "(trim(f.EL_TYPE) in ('9'))";
...
//	echo "$cmd <br>";	exit;
	...
	while($data = $db_dpis->get_array()){
		...
		for($rpt_order_index=0; $rpt_order_index < count($arr_rpt_order); $rpt_order_index++){
			$REPORT_ORDER = $arr_rpt_order[$rpt_order_index];
                        //echo $REPORT_ORDER.'=>idx:'.$rpt_order_index.'<br>';
			switch($REPORT_ORDER){
...
//						echo "CT_CODE=$CT_CODE, CT_NAME=$CT_NAME<br>";
						...
						//} // end if
                                                for($i=1; $i<=4; $i++) ${"GRAND_TOTAL_".$i} += $arr_content[$data_count]["count_".$i];/*Release 5.2.1.14 */        
	...
	//		echo "$head_text1<br>";
		...
		if($count_data){
			...
			for($data_count=0; $data_count<count($arr_content); $data_count++){
				...
				$COUNT_9 = $arr_content[$data_count][count_9];
				...
				$COUNT_TOTAL = $COUNT_9 + $COUNT_1 + $COUNT_2 + $COUNT_3 + $COUNT_4;
				
				$PERCENT_9 = $PERCENT_1 = $PERCENT_2 = $PERCENT_3 = $PERCENT_4 = $PERCENT_TOTAL = 0;
				if($COUNT_TOTAL){ 
					$PERCENT_9 = ($COUNT_9 / $COUNT_TOTAL) * 100;
			...
			if($GRAND_TOTAL){ 
				$PERCENT_TOTAL_9 = ($GRAND_TOTAL_9 / $GRAND_TOTAL) * 100;
...
//			$RTF->close_section(); 
		...
		$arr_content_map = array("name","count_1","percent_1","count_2","percent_2","count_3","percent_3","count_4","percent_4","",""); 
		// ชื่อ column ใน content ที่ map ให้ตรงกับ head pdf
		$arr_series_caption = array("","ต่ำกว่าป.ตรี","","ป.ตรี","","ป.โท","","ป.เอก","","",""); // หัวที่ต้องการเฉพาะออก กราฟ map ให้ตรงกับ head ของ pdf
