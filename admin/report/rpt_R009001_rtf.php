...
//	$arr_search_condition[] = "(b.PER_TYPE in (". implode(", ", $search_per_type) .") and (b.POS_ID >= 0 or e.POEM_ID >= 0))";
	...
	include ("rpt_R009001_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
...
//	echo "default font_id::".$RTF->dfl_FontID."<br>";	
	...
	if($DPISDB=="odbc"){
		$cmd = " select			a.PER_ID, f.PN_NAME, b.PER_NAME, b.PER_SURNAME, b.LEVEL_NO, c.ORG_ID, d.ORG_NAME, $line_name,
											a.SUM_KPI, a.SUM_COMPETENCE, (a.SUM_KPI + a.SUM_COMPETENCE) as SUM_TOTAL
						 from			(
												(
													(
														(
						 ...
						 $search_condition
						 order by		c.ORG_ID, (a.SUM_KPI + a.SUM_COMPETENCE) desc, b.LEVEL_NO ";
	}elseif($DPISDB=="oci8"){
...
//		$db_dpis->show_error();
		...
		$arr_content[$data_count][perline] = trim($PL_NAME)?($PL_NAME . $POSITION_LEVEL . (($PT_NAME != "ทั่วไป" && $LEVEL_NO >= 6)?"$PT_NAME":"")):"".$POSITION_LEVEL;
		$arr_content[$data_count][sum_kpi] = $SUM_KPI;
		$arr_content[$data_count][sum_competence] = $SUM_COMPETENCE;
		$arr_content[$data_count][sum_total] = $SUM_TOTAL;
...
//	echo "$head_text1<br>";
	...
	if($count_data){
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$SUM_TOTAL = $arr_content[$data_count][sum_total];
