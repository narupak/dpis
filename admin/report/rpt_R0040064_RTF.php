...
//			$db_dpis2->show_error();
			...
			if($img_file){
	    		...
	    		$RTF->open_line();
//				$RTF->paragraph();
				$RTF->cellImage("$img_file", $img_dim, "40", "left", "8");
...
//				$RTF->add_text($RTF->bold(1) . ($ORG_NAME_2?$ORG_NAME_2." ":"").($ORG_NAME_1?$ORG_NAME_1." ":"").($ORG_NAME?$ORG_NAME:"")." ".$DEPARTMENT_NAME . $RTF->bold(0) , "center");
    		...
    		$RTF->open_line();			
			$RTF->cell("วัน เดือน ปี เกิด:", "20", "left", "0");
			$RTF->cell(trim((($NUMBER_DISPLAY==2) ? convert2thaidigit($PER_BIRTHDATE):$PER_BIRTHDATE)), "30", "left", "0");
			$RTF->cell("ภูมิลำเนา:", "20", "left", "0");
			$RTF->cell($PV_NAME, "30", "left", "0");
    		$RTF->close_line();
			
    		$RTF->open_line();			
			$RTF->cell("บรรจุเข้ารับราชการ:", "20", "left", "0");
			$RTF->cell(trim((($NUMBER_DISPLAY==2) ? convert2thaidigit($PER_STARTDATE):$PER_STARTDATE)), "30", "left", "0");
			$RTF->cell("เกษียณอายุราชการ:", "20", "left", "0");
			$RTF->cell(trim((($NUMBER_DISPLAY==2) ? convert2thaidigit($PER_RETIREDATE):$PER_RETIREDATE)), "30", "left", "0");
    		$RTF->close_line();

    		$RTF->open_line();			
			$RTF->cell("คู่สมรส:", "20", "left", "0");
			$RTF->cell($SHOW_SPOUSE, "30", "left", "0");
			$RTF->cell("ระดับ: ".$LEVEL_NAME, "20", "left", "0");
			$RTF->cell(" เงินเดือน: ".trim((($NUMBER_DISPLAY==2) ? convert2thaidigit(number_format($PER_SALARY)):number_format($PER_SALARY)))." บาท", "30", "left", "0");
    		$RTF->close_line();
			// จบตาราง
...
//			$db_dpis2->show_error();
			...
			$RTF->new_line();
			$RTF->add_text("ประวัติการดำรงตำแหน่ง:", "left");
			...
			$cmd = " select	POH_EFFECTIVEDATE, POH_PL_NAME, POH_ORG, MOV_CODE from PER_POSITIONHIS 
...
//                    die($cmd);
					...
					$arr_buff[] = trim(($NUMBER_DISPLAY==2) ? convert2thaidigit($postext):$postext);
				} // end while
				$RTF->new_line();
				$RTF->add_list($arr_buff, "left", $font, 16);
			} else {
