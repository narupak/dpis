...
//	$arr_search_condition[] = "(a.PER_TYPE in (". implode(", ", $search_per_type) .") and (b.POS_ID >= 0 or e.POEM_ID >= 0))";
	...
	$report_title = "$DEPARTMENT_NAME||รายงานข้อมูล$PERSON_TYPE[$search_per_type]ที่เคยเลื่อนขั้นเงินเดือน ". number_format($search_promote_level, 1) ." ขั้น จำแนกตาม$ORG_TITLE ในปีงบประมาณ $search_budget_year";
	$report_code = "R0611";
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			for($i=1; $i<=11; $i++) $worksheet->write($xlsRow, $i, (${"COUNT_".$i}?number_format(${"COUNT_".$i}):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			$worksheet->write($xlsRow, 12, ($COUNT_TOTAL?number_format($COUNT_TOTAL):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			$worksheet->write($xlsRow, 13, ($COUNT_ALL?number_format($COUNT_ALL):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			$worksheet->write($xlsRow, 14, ($PERCENT_TOTAL?number_format($PERCENT_TOTAL, 2):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
		} // end for				
		...
		for($i=1; $i<=11; $i++) $worksheet->write($xlsRow, $i, ($GRAND_LEVEL[$i]?number_format($GRAND_LEVEL[$i]):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
		$worksheet->write($xlsRow, 12, ($GRAND_TOTAL?number_format($GRAND_TOTAL):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
		$worksheet->write($xlsRow, 13, ($GRAND_ALL?number_format($GRAND_ALL):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
		$worksheet->write($xlsRow, 14, ($PERCENT_TOTAL?number_format($PERCENT_TOTAL, 2):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
	}else{
		$xlsRow = 0;
