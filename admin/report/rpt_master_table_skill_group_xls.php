<?
	...
	function print_header($xlsRow){
		...
		$worksheet->write($xlsRow, 1, "ชื่อด้านความเชี่ยวชาญ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "$PL_TITLE", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], PL_NAME
							from  		$table a, PER_LINE b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by IIF(ISNULL( SG_SEQ_NO), 9999,  SG_SEQ_NO),$arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], PL_NAME
							from  		$table a, PER_LINE b
							where		a.$arr_fields[2] = b.$arr_fields[2]
											$search_condition
							order by SG_SEQ_NO,$arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], a.$arr_fields[2], $arr_fields[3], PL_NAME
							from  		$table a, PER_LINE b
