<?
	...
	include ("rpt_R010031_format.php");	// ใช้เฉพาะในส่วนการแปลง COLUMN_FORMAT เท่านั้น
...
//	$arr_rpt_order = array("MINISTRY", "DEPARTMENT", "ORG", "LINE"); 
	...
	$report_title = "$DEPARTMENT_NAME||จำนวน$PERSON_TYPE[$search_per_type] ประจำปีงบประมาณ พ.ศ. $search_budget_year||จำแนกตามตำแหน่งในการบริหารงาน และระดับตำแหน่ง";
...
//			echo "heading_text [$i]=".$heading_text[$i]."<br>";
	...
	$ws_border_line4[] = "LRB";  $ws_border_line4[] = "LRB";
	...
	for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
		...
		for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
			...
			$ws_border_line3[] = "L";  $ws_border_line3[] = "L";
			$ws_border_line4[] = "TLRB";  $ws_border_line4[] = "TLRB";
			$ws_fontfmt_line1[] = "B";  $ws_fontfmt_line1[] = "B";
	...
	$ws_border_line3[] = "L";  $ws_border_line3[] = "R";
	$ws_border_line4[] = "TLRB";  $ws_border_line4[] = "TLRB";
	$ws_fontfmt_line1[] = "B";  $ws_fontfmt_line1[] = "B";
	$ws_width[] = "5";  $ws_width[] = "5";
	$ws_headalign_line1[] = "C";
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		// กำหนดค่าตัวแปรเพื่อพิมพ์ ส่วนข้อมูล
			...
			$wsdata_align_1[] = "L";  $wsdata_align_1[] = "L";
			$wsdata_fontfmt_1[] = "B";  $wsdata_fontfmt_1[] = "B";
			$wsdata_border_1[] = "TLBR";  $wsdata_border_1[] = "TLBR";
			$wsdata_colmerge_1[] = 0;  $wsdata_colmerge_1[] = 0;
			$wsdata_colmerge_2[] = 0;  $wsdata_colmerge_2[] = 0;
			$wsdata_fontfmt_2[] = "";  $wsdata_fontfmt_2[] = "";
			for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
				$LEVEL_GROUP = $ARR_LEVEL_GROUP[$i];
				for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
					$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
					$wsdata_align_1[] = "R";  $wsdata_align_1[] = "R";
					$wsdata_fontfmt_1[] = "B";  $wsdata_fontfmt_1[] = "B";
					$wsdata_border_1[] = "TLBR";  $wsdata_border_1[] = "TLBR";
					$wsdata_colmerge_1[] = 0;  $wsdata_colmerge_1[] = 0;
					$wsdata_colmerge_2[] = 0;  $wsdata_colmerge_2[] = 0;
			...
			$wsdata_border_1[] = "TLBR";  $wsdata_border_1[] = "TLBR";
			$wsdata_colmerge_1[] = 0;  $wsdata_colmerge_1[] = 0;
			$wsdata_colmerge_2[] = 1;  $wsdata_colmerge_2[] = 1;
			$wsdata_fontfmt_2[] = "";  $wsdata_fontfmt_2[] = "";
		...
		$arr_aggreg_data = explode("|",do_aggregate($arr_data, $column_function, $arr_column_sel, $arr_column_map));
//		echo "total aggreg=".implode("|",$arr_aggreg_data)."<br>";
		...
		for($i=0; $i < count($arr_column_map); $i++) {
			if ($arr_column_sel[$arr_column_map[$i]]==1) {
				...
				$buff = explode("|",doo_merge_cell($ndata, $wsdata_border_1[$arr_column_map[$i]], $wsdata_colmerge_2[$arr_column_map[$i]], $pgrp, ($i == $colshow_cnt-1)));
				$ndata = $buff[0]; $border = $buff[1]; $merge = $buff[2]; $pgrp = $buff[3];
				if (trim($ndata))
					$worksheet->write($xlsRow, $colseq, $ndata, set_format("xlsFmtTableDetail", $wsdata_fontfmt_1[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $border, $merge));
				else
					$worksheet->write($xlsRow, $colseq, "", set_format("xlsFmtTableDetail", $wsdata_fontfmt_1[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $border, $merge));
//				if ($arr_aggreg_data[$i]) $ndata = $arr_aggreg_data[$i];	// arr_aggreg_data ผ่านการ map มาเรียบร้อยแล้ว
//				else $ndata = $arr_data[$arr_column_map[$i]];
//				$worksheet->write_string($xlsRow, $colseq, $ndata, set_format("xlsFmtTableDetail", $wsdata_fontfmt_1[$arr_column_map[$i]], $wsdata_align_1[$arr_column_map[$i]], $wsdata_border_1[$arr_column_map[$i]], $wsdata_colmerge_2[$arr_column_map[$i]]));
				$colseq++;
			}
		}
	}else{
