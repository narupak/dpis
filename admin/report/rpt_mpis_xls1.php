...
//	$arr_rpt_order = array("POSNO"); 
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, 1, "tempOrganzie", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
...
//		ตัดแปะเข้ามาจาก rpt_mpis_xls_dtl.php
		for($rpt_order_index=0; $rpt_order_index < count($arr_rpt_order); $rpt_order_index++){
			$REPORT_ORDER = $arr_rpt_order[$rpt_order_index];
			switch($REPORT_ORDER){
				case "POSNO" :
						...
						if($rpt_order_index == (count($arr_rpt_order) - 1)){	
							...
							if($PER_ID){
								...
								if ($POH_EFFECTIVEDATE) {	
									$PROMOTEDATE = show_date_format(substr($POH_EFFECTIVEDATE, 0, 10),$DATE_DISPLAY);
								}
...
//		จบการ ตัดแปะเข้ามาจาก rpt_mpis_xls_dtl.php
...
//				require_once "../../Excel/my.defined_format.inc.php";	// use set_format(set of parameter) funtion , help is in file
			...
			if(!$f_org_ref_skip){ // ถ้ารายการนี้มี $REPORT_ORDER == "ORG_REF" จะไม่พิมพ์รายละเอียดข้างล่าง
				...
				$cmd = " insert into GMIS1 (tempMinistry, Organzie, DivisionName, tempPositionNo, tempLine, tempPositionType, tempLevel, 
								...
								'$EM_NAME', '$INS_NAME', '$CT_NAME_EDU', '$ST_NAME', '$MOV_NAME', '$EFFECTIVEDATE', '$CL_NAME', '') ";
				$db_dpis->send_cmd($cmd);
				//$db_dpis->show_error();
			} // end if
