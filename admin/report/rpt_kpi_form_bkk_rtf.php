...
//	$SHOW_SCORE_COMPETENCE = round(round(($SCORE_COMPETENCE / ($COUNT_COMPETENCE * 3)), 3), 2);
	...
	function print_header($header_select){
		global $RTF, $heading_width1, $heading_width2, $heading_width3, $heading_width4, $heading_width5, $heading_width6;
		global $font;
		...
		if($header_select == 2){
			...
			$RTF->cell("ช่วงเวลาที่ต้องการ", $heading_width3[2], "center", "16", "TRL");
			$RTF->cell("วิธีการวัดผลใน", $heading_width3[3], "center", "16", "TRL");	// 9='BLUE'
			$RTF->close_line();
			$RTF->open_line();			
			$RTF->cell("", $heading_width3[0], "center", "16", "RBL");	// 9='BLUE'
			$RTF->cell("", $heading_width3[1], "center", "16", "RBL");
			$RTF->cell("การพัฒนา", $heading_width3[2], "center", "16", "RBL");
			$RTF->cell("การพัฒนา", $heading_width3[3], "center", "16", "RBL");	// 9='BLUE'
			$RTF->close_line();
			...
			$RTF->cell("ระบุเหตุการณ์/พฤติกรรม", $heading_width4[6], "center", "16", "TRL");
			$RTF->close_line();
			$RTF->open_line();			
			$RTF->set_font($font, 14);
			...
			$RTF->cell("(ที่ผู้รับการประเมินแสดงออก)", $heading_width4[6], "center", "16", "RBL");
			$RTF->close_line();
		}elseif($header_select == 5){	//แบบสรุปการประเมินผลสัมฤทธิ์ของงาน (PAGE 6)
			$RTF->open_line();			
			$RTF->set_font($font, 14);
			$RTF->cell("ตัวชี้วัด", $heading_width5[0], "center", "16", "TRL");
			$RTF->cell("น้ำหนัก", $heading_width5[1], "center", "16", "TRL");
			$RTF->cell("เป้าหมาย", $heading_width5[2], "center", "16", "TRL");
			$RTF->cell("ผลงาน", $heading_width5[3], "center", "16", "TRL");
			$RTF->cell("คะแนนที่ได้", ($heading_width5[4]*5), "center", "16", "TRL");
			$RTF->cell("ผลการประเมิน", $heading_width5[5], "center", "16", "TRL");
			$RTF->cell("เหตุผลที่ทำให้งานบรรลุ/", $heading_width5[6], "center", "16", "TRL");
			$RTF->close_line();
			$RTF->open_line();
			$RTF->cell("ผลสัมฤทธิ์ของงาน", $heading_width5[0], "center", "16", "RBL");
			$RTF->cell("", $heading_width5[1], "center", "16", "RBL");
			$RTF->cell("", $heading_width5[2], "center", "16", "RBL");
			$RTF->cell("ที่ทำได้", $heading_width5[3], "center", "16", "RBL");
			for($i=1; $i<=5; $i++){ 
				$RTF->cell($RTF->bold(1).convert2thaidigit($i).$RTF->bold(0), $heading_width5[4], "center", "16", "TRBL");
			} // end for
			$RTF->cell(convert2thaidigit("[(น้ำหนักxคะแนนที่ได้) / 5]"), $heading_width5[5], "center", "16", "RBL");
			$RTF->cell("ไม่บรรลุเป้าหมาย", $heading_width5[6], "center", "16", "RBL");
			$RTF->close_line();
		}	// end if
	} // function		
	...
	$RTF->cell($RTF->color("0").$RTF->bold(1)."แบบประเมินผลการปฏิบัติราชการของข้าราชการกรุงเทพมหานครสามัญ".$RTF->bold(0), "220", "center", "0");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0")."ระยะการประเมิน", "40", "left", "0", "TL");
	if($KF_CYCLE==1) $RTF->cellImage("../images/checkbox_check.jpg", 100, 1, "left", 0, "T");
	else $RTF->cellImage("../images/checkbox_blank.jpg", 100, 1, "left", 0, "T");
	$RTF->cell(convert2thaidigit("ระยะที่ ๑ ตั้งแต่ ๑ ตุลาคม ".($KF_YEAR - 1)." ถึง ๓๑ มีนาคม ".$KF_YEAR), "179", "left", "0", "TR");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell("", "40", "left", "0", "BL");
	if($KF_CYCLE==2) $RTF->cellImage("../images/checkbox_check.jpg", 100, 1, "left", 0, "B");
	else $RTF->cellImage("../images/checkbox_blank.jpg", 100, 1, "left", 0, "B");
	$RTF->cell($RTF->color("0").convert2thaidigit("ระยะที่ ๒ ตั้งแต่ ๑ เมษายน ".$KF_YEAR." ถึง ๓๐ กันยายน ".$KF_YEAR), "179", "left", "0", "RB");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)." ผู้รับการประเมิน".$RTF->bold(0), "220", "left", "16", "TRBL");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0")."ชื่อ - นามสกุล  $PER_NAME", "160", "left", "0", "TRBL");
	$RTF->cell($RTF->color("0").convert2thaidigit("ตำแหน่งเลขที่  $POS_NO"), "60", "left", "0", "TRBL");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0")."ตำแหน่ง    (ชื่อตำแหน่งในสายงาน)  ".convert2thaidigit("$PL_NAME"), "150", "left", "0", "TRBL");
	$RTF->cell($RTF->color("0")."ประเภทตำแหน่ง  $POSITION_TYPE  ", "70", "left", "0", "TRBL");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0")."ระดับตำแหน่ง  $POSITION_LEVEL  ", "70", "left", "0", "TRBL");
	$RTF->cell($RTF->color("0")."สังกัด  ".convert2thaidigit("$ORG_NAME  "), "150", "left", "0", "TRBL");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)." ผู้ประเมิน (ผู้บังคับบัญชาชั้นต้น)".$RTF->bold(0), "220", "left", "16", "TRBL");
	$RTF->close_line();

	$RTF->open_line();			
	$RTF->set_font($font,16);
	if ($REVIEW_PER_NAME0 > " ") {
		$RTF->cell($RTF->color("0")."ชื่อ - นามสกุล  $REVIEW_PER_NAME0", "120", "left", "0", "TRBL");
		$RTF->cell($RTF->color("0")."ตำแหน่ง  $REVIEW_PL_NAME0", "100", "left", "0", "TRBL");
	} else {
		$RTF->cell($RTF->color("0")."ชื่อ - นามสกุล  $REVIEW_PER_NAME", "120", "left", "0", "TRBL");
		$RTF->cell($RTF->color("0")."ตำแหน่ง  $REVIEW_PL_NAME", "100", "left", "0", "TRBL");
	}
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)." ผู้ประเมิน (ผู้บังคับบัญชาเหนือขึ้นไป)".$RTF->bold(0), "220", "left", "16", "TRBL");
	$RTF->close_line();
	...
	if ($REVIEW_PER_NAME0 > " ") {
		...
		$RTF->cell($RTF->color("0")."ชื่อ - นามสกุล  $REVIEW_PER_NAME1", "120", "left", "0", "TRBL");
		$RTF->cell($RTF->color("0")."ตำแหน่ง  $REVIEW_PL_NAME1", "100", "left", "0", "TRBL");
	}
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,14);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ส่วนที่ ๑ ผลสัมฤทธิ์ของงาน (ร้อยละ ๗๐)".$RTF->bold(0), "220", "left", 0);
	$RTF->close_line();
	...
	foreach($ARR_KPI as $PG_ID => $KPI_NAME){
		...
		$RTF->open_line();
		$RTF->set_font($font,14);
		if (($KPI_NAME=="ตัวชี้วัดตามแผนปฏิบัติราชการ" || $KPI_NAME=="ตัวชี้วัดตามหน้าที่ความรับผิดชอบหลัก" || $KPI_NAME=="ตัวชี้วัดตามงานที่ได้รับมอบหมายพิเศษ") && !$ARR_KPI_WEIGHT[$PG_ID]) {
			$RTF->cell($RTF->bold(1).$KPI_NAME.$RTF->bold(0), ($heading_width5[0]+$heading_width5[1]+$heading_width5[2]+$heading_width5[3]+($heading_width5[4]*5)+$heading_width5[5]+$heading_width5[6]), "left", "0", "TRBL");
		} else {
			$RTF->cell(convert2thaidigit($data_count.$KPI_NAME), $heading_width5[0], "left", "0", "TRBL");
			$RTF->cell(convert2thaidigit($ARR_KPI_WEIGHT[$PG_ID]), $heading_width5[1], "center", "0", "TRBL");
			$RTF->cell(convert2thaidigit($ARR_KPI_TARGET5_DESC[$PG_ID]), $heading_width5[2], "center", "0", "TRBL");
			$RTF->cell(convert2thaidigit($ARR_KPI_RESULT[$PG_ID]), $heading_width5[3], "center", "0", "TRBL");
			for($i=1; $i<=5; $i++){ 
				if($i == $ARR_KPI_EVALUATE[$PG_ID]){
					$RTF->cell(convert2thaidigit($ARR_KPI_EVALUATE[$PG_ID]), $heading_width5[4], "center", "0", "TRBL");
				} else {
					$RTF->cell("", $heading_width5[4], "center", "0", "TRBL");
				}	
			} // end for
			$RTF->cell($RTF->bold(1).convert2thaidigit(($ARR_KPI_WEIGHT[$PG_ID]*$ARR_KPI_EVALUATE[$PG_ID])/5).$RTF->bold(0), $heading_width5[5], "center", "0", "TRBL");
			$RTF->cell(convert2thaidigit($ARR_KPI_REMARK[$PG_ID]), $heading_width5[6], "center", "0", "TRBL");
...
//	echo "$data_count - ".count($ARR_KPI);
	...
	$RTF->open_line();
	$RTF->set_font($font,14);
	$RTF->cell("น้ำหนักรวม ", $heading_width5[0], "center", "0", "TRBL");
	$RTF->cell(convert2thaidigit($TOTAL_KPI_WEIGHT), $heading_width5[1], "center", "0", "TRBL");
	$RTF->cell("รวม ", ($heading_width5[2]+$heading_width5[3]+($heading_width5[4]*5)), "right", "0", "TRBL");
	$RTF->cell($RTF->bold(1).convert2thaidigit($TOTAL_KPI_RESULT).$RTF->bold(0), $heading_width5[5], "center", "0", "TRBL");
	$RTF->cell("", $heading_width5[6], "center", "0", "TRBL");
	$RTF->close_line();
	$RTF->ln();

	$RTF->open_line();
	$RTF->set_font($font,14);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ส่วนที่ ๒ พฤติกรรมการปฏิบัติราชการ (สมรรถนะหลัก) (ร้อยละ ๓๐)".$RTF->bold(0), "220", "left", 0);
	$RTF->close_line();
	...
	foreach($ARR_COMPETENCE as $KC_ID => $CP_NAME){
		...
		$RTF->cell(convert2thaidigit($ARR_COMPETENCE_EVALUATE[$KC_ID]), $heading_width4[3], "center", "0", "TRBL");
		$RTF->cell(convert2thaidigit($PC_SCORE[$KC_ID]), $heading_width4[4], "center", "0", "TRBL");
		$RTF->cell(convert2thaidigit($PC_SCORE[$KC_ID]), $heading_width4[5], "center", "0", "TRBL");
	...
	$RTF->cell("รวม", 10, "right", "0", "TRBL");
	$RTF->cell(convert2thaidigit(array_sum($ARR_COMPETENCE_EVALUATE)), $heading_width4[5]-10, "center", "0", "TRBL");
	$RTF->cell(convert2thaidigit($TOTAL_PC_SCORE), $heading_width4[6], "center", "0", "TRBL");
	$RTF->close_line();
	$RTF->ln();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ส่วนที่ ๓ แผนพัฒนาการปฏิบัติราชการรายบุคคล".$RTF->bold(0), "220", "left", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."สรุปผลการประเมิน".$RTF->bold(0), "220", "left", "0");
	$RTF->close_line();
	...
	$RTF->open_line();			
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ระดับผลการประเมิน".$RTF->bold(0), "60", "left", "0");
	$RTF->close_line();
	...
	while($data = $db_dpis->get_array()){
		...
		$AM_POINT_MAX = $data[AM_POINT_MAX];
			...
			$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "center", 0);
			$RTF->cell("$AM_NAME", "80", "left", "0");
			$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ส่วนที่ ๔ การรับทราบผลการประเมิน".$RTF->bold(0), "220", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(" ผู้รับการประเมิน :", "220", "left", "16");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	if ($PER_NAME)
		$RTF->cellImage("../images/checkbox_check.jpg", 100, 2, "center", 0);
	else
		$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "center", 0);
	$RTF->cell("ได้รับทราบผลการประเมินและแผนพัฒนาการปฏิบัติราชการรายบุคคลแล้ว", "200", "left", "0");
	$RTF->close_line();
	...
	if($SESS_E_SIGN[1]==1){	// ใช้รูปแบบของลายเซ็นอิเล็คทรอนิกส์	
		...
		if($PIC_SIGN){
			...
			$RTF->set_font($font,16);
			$RTF->cell("", "100", "left", "0");
			$RTF->cell(("ลงชื่อ ". str_repeat(".", 100)), "100", "right", "0");
			$RTF->close_line();
		}
	}else{
		$RTF->open_line();
	...
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("( $PER_NAME )"), "100", "center", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("ตำแหน่ง ".convert2thaidigit("$PL_NAME$POSITION_LEVEL")), "100", "center", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("วันที่ ". str_repeat(".", 13) ." เดือน ". str_repeat(".", 30) ." พ.ศ. ". str_repeat(".", 20)), "100", "center", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(" ผู้ประเมิน :", "220", "left", "16");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "center", 0);
	$RTF->cell("ได้แจ้งผลการประเมินและผู้รับการประเมินได้ลงนามรับทราบแล้ว", "200", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "center", 0);
	$RTF->cell("ได้แจ้งผลการประเมิน เมื่อวันที่ ". str_repeat(".", 50), "200", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "2", "left", "0");
	$RTF->cell("แต่ผู้รับการประเมินไม่ลงนามรับทราบ", "198", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "2", "left", "0");
	$RTF->cell("โดยมี ". str_repeat(".", 75)."เป็นพยาน", "198", "left", "0");
	$RTF->close_line();
	...
	$RTF->set_font($font,16);
	$RTF->cell("", "2", "left", "0");
	$RTF->cell(("ลงชื่อ ". str_repeat(".", 80)."พยาน"), "198", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "2", "left", "0");
	$RTF->cell(("ตำแหน่ง". str_repeat(".", 77)), "198", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "2", "left", "0");
	$RTF->cell(("วันที่ ". str_repeat(".", 13) ." เดือน ". str_repeat(".", 30) ." พ.ศ. ". str_repeat(".", 20)), "198", "left", "0");
	$RTF->close_line();
	...
	if($SESS_E_SIGN[1]==1){	// ใช้รูปแบบของลายเซ็นอิเล็คทรอนิกส์	
		...
		$RTF->set_font($font,16);
		$RTF->cell("", "100", "left", "0");
		$RTF->cell(("ลงชื่อ ". str_repeat(".", 100)), "100", "left", "0");
		$RTF->close_line();
		}
	}else{
		$RTF->open_line();
	...
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("( $REVIEW_PER_NAME )"), "100", "center", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("ตำแหน่ง $REVIEW_PL_NAME "), "100", "center", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell($RTF->color("0").$RTF->bold(1)."ส่วนที่ ๕ ความเห็นของผู้บังคับบัญชาเหนือขึ้นไป".$RTF->bold(0), "220", "left", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(" ผู้บังคับบัญชาเหนือขึ้นไป :", "220", "left", "16");
	$RTF->close_line();
	
	$RTF->open_line();
	if(trim($AGREE_REVIEW1)) 
		$RTF->cellImage("../images/checkbox_check.jpg", 100, 2, "left", 0);
	else 
		$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "left", 0);
	$RTF->set_font($font,16);
	$RTF->cell("เห็นด้วยกับผลการประเมิน", "198", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(convert2thaidigit($AGREE_REVIEW1), "220", "left", "0");
	$RTF->close_line();
	...
	if(trim($DIFF_REVIEW1)) 
		...
		$RTF->cellImage("../images/checkbox_blank.jpg", 100, 2, "left", 0);
	$RTF->set_font($font,16);
	$RTF->cell("มีความเห็นต่าง ดังนี้", "198", "left", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(convert2thaidigit($DIFF_REVIEW1), "190", "left", "0");
	$RTF->close_line();
	...
	if($SESS_E_SIGN[1]==1){	// ใช้รูปแบบของลายเซ็นอิเล็คทรอนิกส์	
		...
		if($PIC_SIGN){
			...
			$RTF->set_font($font,16);
			$RTF->cell("", "100", "left", "0");
			$RTF->cell(("ลงชื่อ ". str_repeat(".", 100)), "100", "center", "0");
			$RTF->close_line();
		}
	}else{
		$RTF->open_line();
	...
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("( $REVIEW_PER_NAME1 )"), "100", "center", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "left", "0");
	$RTF->cell(("ตำแหน่ง $REVIEW_PL_NAME1"), "100", "center", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(" ผู้บังคับบัญชาเหนือขึ้นไปอีกชั้นหนึ่ง (ถ้ามี) :", "220", "left", "16");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(convert2thaidigit($AGREE_REVIEW2), "200", "center", "0");
	$RTF->close_line();
	...
	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell(convert2thaidigit($DIFF_REVIEW2), "198", "left", "0");
	$RTF->close_line();
	...
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "center", "0");
	$RTF->cell(("( $REVIEW_PER_NAME2 )"), "100", "center", "0");
	$RTF->close_line();

	$RTF->open_line();
	$RTF->set_font($font,16);
	$RTF->cell("", "100", "center", "0");
	$RTF->cell(("ตำแหน่ง $REVIEW_PL_NAME2"), "100", "center", "0");
	$RTF->close_line();
