...
//		$sum_w = array_sum($heading_width);
	...
	function count_person($search_condition, $addition_condition){
		global $DPISDB, $db_dpis2,$db_dpis3,$position_table, $position_join;
...
//	$db_dpis2->show_error();
	...
	if($DPISDB=="odbc"){
		$cmd = " select			a.PER_ID, a.PN_CODE, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, g.LEVEL_NAME, g.POSITION_LEVEL,
...
//	echo "<br>$cmd<br>";
	...
	//	echo "$head_text1<br>";
		...
		/*$result = $pdf->open_tab($head_text1, $head_width1, 7, "TRHBL", $head_align1, "", "14", "b", "0066CC", "EEEEFF", $COLUMN_FORMAT, $col_function, $nofirsthead);*/
	}
...
//					$pdf->Cell(200,7,((($NUMBER_DISPLAY==2)?convert2thaidigit($group_count):$group_count). ' ' . $EN_NAME . $CT_NAME),0,1,"L");
					if ($FLAG_RTF) {
						...
						if($group_count > 1)  $result = $pdf->add_text_line("", 7, "", "L", "", "14", "b", 0, 0);
						$result = $pdf->add_text_line(((($NUMBER_DISPLAY==2)?convert2thaidigit($group_count):$group_count). ' ' . $EN_NAME . $CT_NAME), 7, "", "L", "", "14", "b", 0, 0);
		 				if (!$result) echo "****** error ****** add text line to table at record count = $data_count <br>";
					}
			...
			if ($FLAG_RTF)
				...
				if($data_row == 0) 	$pdf->print_tab_header();
