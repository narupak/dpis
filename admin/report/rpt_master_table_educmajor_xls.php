...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc" || $DPISDB=="oci8"){
  	...
  	if(trim($search_name)) $arr_search_condition[] = "($arr_fields[1] like '%$search_name%')";
        if($edu_ca == 1) $arr_search_condition[] = " EM_CODE in (select  DISTINCT EM_CODE from PER_EDUCATE)";
       
	$search_condition = "";
	if(count($arr_search_condition)) $search_condition = " where " . implode(" and ", $arr_search_condition);

	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6], EM_SEQ_NO 
								from		$table
								 $search_condition
							order by IIF(ISNULL(EM_SEQ_NO), 9999, EM_SEQ_NO), $arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6], EM_SEQ_NO 
								from		$table
								 $search_condition
							order by EM_SEQ_NO, $arr_fields[0] 
					   ";
	}elseif($DPISDB=="mysql"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[6], EM_SEQ_NO 
								from		$table
