...
//	fix head for rpt_R010021
	...
	for($i=0; $i<$cnt_level_grp; $i++){
		...
		for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
			...
			$heading_text[$cnt_level_det*2+2] = "<**1**>ตำแหน่งประเภท|<**".($i+1)."**>".$ARR_LEVEL_GROUP_NAME[$LEVEL_GROUP]."|<**".($cnt_level_det+1)."**>".$ARR_LEVEL_NAME[$LEVEL_NO][1]."|ชาย";
			$heading_text[$cnt_level_det*2+3] = "<**1**>ตำแหน่งประเภท|<**".($i+1)."**>".$ARR_LEVEL_GROUP_NAME[$LEVEL_GROUP]."|<**".($cnt_level_det+1)."**>".$ARR_LEVEL_NAME[$LEVEL_NO][1]."|หญิง";
			if ($ISCS_FLAG==1) {
				$heading_width[$cnt_level_det*2+2] = "12";
				$heading_width[$cnt_level_det*2+3] = "12";
			} else {
...
//			echo "idx:".($cnt_level_det*2+3)."->".$heading_text[$cnt_level_det*2+3]."<br>";
	...
	$heading_text[$cnt_level_det*2+2] = "<**2**>|<**".($cnt_level_grp+1)."^**>รวม|<**".($cnt_level_det+1)."^**>|ชาย";
	$heading_text[$cnt_level_det*2+3] = "<**2**>|<**".($cnt_level_grp+1)."^**>รวม|<**".($cnt_level_det+1)."^**>|หญิง";
//	echo "idx:".($cnt_level_det*2+2)."->".$heading_text[$cnt_level_det*2+2]."<br>";
//	echo "idx:".($cnt_level_det*2+3)."->".$heading_text[$cnt_level_det*2+3]."<br>";
	if ($ISCS_FLAG==1) {
		$heading_width[$cnt_level_det*2+2] = "12";
