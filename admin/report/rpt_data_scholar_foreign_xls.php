<?
	...
	$company_name = "";
	$report_title = "รายงานสรุปผลข้าราชการขอลาศึกษาต่อ ณ ต่างประเทศ ตั้งแต่วันที่ ";
	$report_code = "";
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, 1, "ศึกษาระดับ", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 2, "สาขาวิชา", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 3, "จำนวน (คน)", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 4, "ชื่อผู้เข้าอบรม", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		...
		$worksheet->write($xlsRow, 8, "จำนวนวัน", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 9, "วันที่อบรม", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
		$worksheet->write($xlsRow, 10, "ประเทศ", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 1));
		$worksheet->write($xlsRow, 11, "หน่วยงานที่ให้ทุน", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 1));
	...
	while($data = $db_dpis->get_array()){
		...
		if ($TMP_PER_ID) {
			$cmd = "select POS_ID, POEM_ID, POEMS_ID,POT_ID, a.LEVEL_NO, b.LEVEL_NAME,b.POSITION_LEVEL, PN_CODE, PER_NAME, PER_SURNAME 
							from PER_PERSONAL a, PER_LEVEL b 
							where PER_ID=$TMP_PER_ID and a.LEVEL_NO=b.LEVEL_NO";
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
...
//		}
		...
		$arr_content[$data_count][position_level] = $LEVEL_NAME;
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			$worksheet->write($xlsRow, 1, "$EL_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 2, "$EM_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 3, "$xx", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 4, "$PER_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 5, "$POSITION", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 6, "$POSITION_LEVEL", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			...
			$worksheet->write($xlsRow, 10, "$CT_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
			$worksheet->write($xlsRow, 11, "$SCH_NAME", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
		} // end for				
	}else{
		$xlsRow = 0;
	...
	header("Content-Type: application/x-msexcel; name=\"รายงานสรุปผลข้าราชการขอลาศึกษาต่อ ณ ต่างประเทศ.xls\"");
	header("Content-Disposition: inline; filename=\"รายงานสรุปผลข้าราชการขอลาศึกษาต่อ ณ ต่างประเทศ.xls\"");
	$fh=fopen($fname, "rb");
	fpassthru($fh);
	fclose($fh);
	unlink($fname);
