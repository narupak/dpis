...
//	$report_title = "บัญชีรายละเอียดการแต่งตั้งข้าราชการพลเรือนสามัญให้ดำรงตำแหน่งใหม่ตามพระราชบัญญัติระเบียบข้าราชการพลเรือน พ.ศ.2551 $DEPARTMENT_NAME ที่ $COM_NO ลงวันที่ $COM_DATE";
	...
	if($DPISDB=="odbc"){
		...
		$cmd = "  select		b.PN_CODE, b.PER_NAME, b.PER_SURNAME, b.PER_TYPE, a.PL_CODE, a.PL_CODE_ASSIGN, b.PER_MGTSALARY,
				...
				where		a.COM_ID=$COM_ID and a.PER_ID=b.PER_ID and b.POS_ID=c.POS_ID(+) and c.ORG_ID=d.ORG_ID(+) and c.ORG_ID_1=e.ORG_ID(+) and c.ORG_ID_2=f.ORG_ID(+) and b.PER_STATUS = 1
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($CONTENT_TYPE=="ORG" || $CONTENT_TYPE=="ORG_1" || $CONTENT_TYPE=="ORG_2"){
				...
				if($CMD_NOTE2){
					...
					$worksheet->write_string($xlsRow, 17, "$POS_CONDITION", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				} // end if
			} // end if
		} // end for				
