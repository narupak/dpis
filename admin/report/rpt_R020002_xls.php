<?
	...
	$arr_search_condition[] = "(peh.EXIN_CODE in('01','02'))";	//เงินช่วยค่าครองชีพ,เงินตอบแทนพิเศษรายเดือน
	//ค้นหา ณ วันที
	...
	$company_name = "";
	$report_title = "บัญชีรายละเอียดเงินเพิ่มการครองชีพชั่วคราว$PERSON_TYPE[$search_per_type] ||แนบท้ายคำสั่ง $DEPARTMENT_NAME ที่ $COM_NO ลงวันที่ $COM_DATE";
	$report_code = "R20002";
	...
	function print_header(){
		...
		if($search_per_type == 1){
			$worksheet->write($xlsRow, 5, "เงินเดือน ณ 1 เม.ย. 51", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 1));
			$worksheet->write($xlsRow, 6, "", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 1));
			$worksheet->write($xlsRow, 7, "ค่าตอบแทน", set_format("xlsFmtTableHeader", "B", "C", "TLR", 1));
			...
			$worksheet->write($xlsRow, 5, "อัตราค่าจ้าง", set_format("xlsFmtTableHeader", "B", "C", "TLR", 1));
		...
		if($search_per_type == 1){
			...
			$worksheet->write($xlsRow, 7, "พิเศษชั่วคราว", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
			...
			$worksheet->write($xlsRow, 5, "ณ 1 เม.ย. 51", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
	...
	if($DPISDB=="odbc"){
		$cmd = "select 		EXINH_ID, EXINH_EFFECTIVEDATE, pet.EXIN_NAME, EXINH_AMT, EXINH_ENDDATE,
						...
						from 		PER_PERSONAL b,$position_table c,PER_ORG d, PER_ORG e,PER_EXTRA_INCOMEHIS peh, PER_EXTRA_INCOME_TYPE pet
						where 	b.PER_ID=peh.PER_ID and $position_join and c.ORG_ID=d.ORG_ID and c.ORG_ID_1=e.ORG_ID and	peh.EXIN_CODE=pet.EXIN_CODE
										$search_condition
						order by 	$select_position, d.ORG_SEQ_NO, e.ORG_SEQ_NO, EXINH_EFFECTIVEDATE desc 
						";
	}elseif($DPISDB=="oci8"){
...
//	$db_dpis->show_error();
	...
	while($data = $db_dpis->get_array()){
		...
		$EXINH_EFFECTIVEDATE = substr($data[EXINH_EFFECTIVEDATE], 0, 10);
		$EXINH_EFFECTIVEDATE = show_date_format($EXINH_EFFECTIVEDATE,$DATE_DISPLAY);
		$EXIN_NAME = trim($data[EXIN_NAME]);
		$EXINH_AMT = trim($data[EXINH_AMT]);
		$EXINH_ENDDATE = substr($data[EXINH_ENDDATE], 0, 10);
		$EXINH_ENDDATE = show_date_format($EXINH_ENDDATE,$DATE_DISPLAY);

		//นำเงินเพิ่มค่าครองชีพ (เช่น เงินช่วยเหลือค่าครองชีพ, เงินตอบแทนพิเศษรายเดือน) มารวมกับเงินประจำตำแหน่ง (ถ้ามี)
		$MGTS_NAME = "";
		if($PER_MGTSALARY > 0){
			$EXINH_AMT = ($EXINH_AMT+$PER_MGTSALARY);
			$MGTS_NAME = ",เงินประจำตำแหน่ง";
		}
		//ให้ได้รับ = รวมเงินเดือนปัจจุบัน กับ เงินที่ได้รับ	
		$SUM_RCV = ($PER_SALARY + $EXINH_AMT);
		...
		***/
		...
		$arr_content[$data_count][cmd_extra] = 	$EXINH_AMT?number_format($EXINH_AMT):"-";
		$arr_content[$data_count][cmd_sumrcv] = 	$SUM_RCV?number_format($SUM_RCV):"-";
		$arr_content[$data_count][cmd_note1] = $EXIN_NAME.$MGTS_NAME;
...
//				$NEW_ORG_NAME = $arr_content[$data_count][new_org_name];
				...
				if($CMD_NOTE2){
					...
					if($search_per_type==1){
						...
						$worksheet->write($xlsRow, 7, "$CMD_EXTRA", set_format("xlsFmtTableDetail", "", "R", "LTRB", 0));
						$worksheet->write($xlsRow, 8, "$CMD_SUMRCV", set_format("xlsFmtTableDetail", "", "R", "LTRB", 0));
