...
//	$arr_rpt_order = array("POSNO"); 
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, 23, "RETURN_DATE", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
		$worksheet->write($xlsRow, 24, "RESIGN_DATE", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
		$worksheet->write($xlsRow, 25, "REMARK", set_format("xlsFmtTableHeader", "B", "C", "TLRB", 0));
	} // function		
...
if ($BKK_FLAG==1) $DC_CODE_COND = "17";
	...
	for ( $i=0; $i<count($level); $i++ ) { 
		...
		$search_condition1 = str_replace( " where ", " and ", $search_condition);
		$cmd = " select a.PER_ID from PER_PERSONAL a, PER_DECORATEHIS b 
						where a.PER_ID=b.PER_ID and trim(DC_CODE)='$DC_CODE_COND' and a.LEVEL_NO='$search_level_no' $search_condition1 
						order by a.PER_ID ";
		...
		$addition_condition = str_replace(" where ", " and ", $addition_condition);
	// ===== select data =====
	if($DPISDB=="odbc"){
		$cmd = " select		a.PER_ID, PER_GENDER, PER_BIRTHDATE, PER_STARTDATE,PER_OCCUPYDATE,PER_POSDATE, a.LEVEL_NO,
...
//				echo "newsheet..$report_code".($count_org_ref?"_$count_org_ref":"").$sheet_no_text."<br>";
		...
		$POS_DATE_ENG = substr(trim($data[PER_POSDATE]), 0, 10);
		$POS_DATE = show_date_format($POS_DATE_ENG,1);
		...
		$OCCUPYDATE = show_date_format($PER_OCCUPYDATE,1);
		
		
		if($STARTDATE==$OCCUPYDATE)
			{
			$OCCUPYDATE ="";
			}
...
*/
		...
		$worksheet->write_string($xlsRow, 23, "$POS_DATE", set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
		$worksheet->write_string($xlsRow, 24, "$OCCUPYDATE", set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
		$worksheet->write_string($xlsRow, 25, "$REMARK", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
