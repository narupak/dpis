...
//	fix head for rpt_R002001
	...
	while($data = $db_dpis->get_array()){ 
		...
		if(trim($data[LEVEL_SHORTNAME])){	$LNAME=str_replace("ระดับตำแหน่ง","",$data[LEVEL_SHORTNAME]); }
	...
	if (count($ARR_LEVEL_SHORTNAME)==0) { echo "***** ไม่มีข้อมูล ระดับ สำหรับประเภทบุคคลากรนี้ *****<br>"; exit; }
//	echo "per_type=$search_per_type   count level : ".count($ARR_LEVEL_NO)."<br>";
	...
	$heading_text[count($ARR_LEVEL_SHORTNAME)+1] = "<**2**>รวม|คน";
	$heading_text[count($ARR_LEVEL_SHORTNAME)+2] = "<**2**>รวม|ร้อยละ";
