...
//	$arr_search_condition[] = "(a.PER_TYPE in (". implode(", ", $search_per_type) .") and (b.POS_ID >= 0 or e.POEM_ID >= 0))";
	...
	function count_person($level_no, $search_condition, $addition_condition){
		...
		global $search_per_type, $search_tr_code, $search_year_min, $search_year_max, $select_org_structure;
		...
		if($level_no){
			...
			if($DPISDB=="odbc"){ 
				...
				if($search_tr_code) $search_condition .= (trim($search_condition)?" and ":" where ") . "(trim(d.TR_CODE) = '$search_tr_code')";
