		...
		switch($REPORT_ORDER){
			case "POSNO" :
				...
				if($DPISDB=="oci8") {
					...
					$order_by .= "to_number(replace(a.POEM_NO,'-',''))";
				}elseif($DPISDB=="mysql"){
					$select_list .= "a.POEM_NO";				
					$order_by .= "a.POEM_NO";
				}
	...
	if($count_data){
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if($REPORT_ORDER == "ORG_REF"){
				...
				// error because GMIS_PEIS no table
				$cmd1 = " SELECT TEMPPOSITIONNO FROM GMIS_PEIS WHERE TEMPPOSITIONNO = '$POS_NO' ";
				$db_dpis->send_cmd($cmd1);
				$data1 = $db_dpis->get_array();
				$repeat_POS_NO = $data1[TEMPPOSITIONNO]; //error because TEMPLEVEL set 10 over 
				
				if(!$repeat_POS_NO){
					$cmd = " insert into GMIS_PEIS (TEMPMINISTRY, ORGANZIE, DIVISIONNAME, TEMPPROVINCE, TEMPORGANIZETYPE, TEMPPRENAME, TEMPFIRSTNAME, 
