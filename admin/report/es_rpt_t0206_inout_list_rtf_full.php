<?php
   ...
   //$RTF = new RTF("a4", 1150, 500, 600, 200);
    ...
    function show_checkbox_cause($result,$result_type,$FONTSPACE,$forgate1,$forgate2,$forgate3,$SCAN_FLAG){
        global $RTF,$font,$FONTSIZE;
        
        $RTF->open_line();
        $RTF->set_font($font,$FONTSIZE);
        $RTF->color("0");	// 0=BLACK
        $result_title1="ไม่ได้บันทึกเวลาเริ่มปฏิบัติราชการ";
		if($forgate1==1 && $SCAN_FLAG==1){
			$forgate1_show="checkbox_check";
		}else{
			$forgate1_show="checkbox_blank";
		}
        $RTF->cellImage("../images/".$forgate1_show.".jpg", 100, 2.7, "center",0);
        $RTF->cell(convert2rtfascii($result_title1),"32", "left", "0");  
        
        //str_repeat(" ", 100)
        $result_title2="ไม่ได้บันทึกเวลาเลิกปฏิบัติราชการ";
        if($forgate2==1 && $SCAN_FLAG==1){
			$forgate2_show="checkbox_check";
		}else{
			$forgate2_show="checkbox_blank";
		}
        $RTF->cellImage("../images/".$forgate2_show.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title2),"62.6", "left", "0");  
        
        /*$result_title3="ขออนุญาตออกก่อนเวลาปฏิบัติราชการ";
        if($forgate3==1){
			$forgate3_show="checkbox_check";
		}else{
			$forgate3_show="checkbox_blank";
		}
        $RTF->cellImage("../images/".$forgate3_show.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title3),"34", "left", "0");  */
        
        $RTF->close_line();
    }
    
    function show_checkbox_flag($forgate1,$END_HH_1,$END_II_1,$forgate2,$END_HH_2,$END_II_2,$forgate3,$END_HH_3,$END_II_3,$SCAN_FLAG){
        global $RTF,$font,$FONTSIZE;
        ...
        $RTF->cell(" ", 10, "left", "0");
		if($SCAN_FLAG==1 && $forgate1==1){//if($forgate1==1){
			$forgate1_show="checkbox_check";
        ...
        $result_title1="ลืมบันทึกเวลาเริ่มปฏิบัติงาน โดยมาถึงเวลา ".$END1." น.";
        $RTF->cellImage("../images/".$forgate1_show.".jpg", 100,2.7, "center",0);
        $RTF->cell(convert2rtfascii($result_title1),"87.3", "left", "0");
        $RTF->close_line();
        ...
        $RTF->color("0");// 0=BLACK
        $RTF->cell(" ", 10, "left", "0");
		if($SCAN_FLAG==1 && $forgate2==1){//if($forgate2==1){
			$forgate2_show="checkbox_check";
        ...
        $result_title2="ลืมบันทึกเวลาเลิกปฏิบัติงาน โดยออกเวลา ".$END2." น.";
        $RTF->cellImage("../images/".$forgate2_show.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title2),"87.3", "left", "0");
        $RTF->close_line();
		...
		if($forgate3==1){
			$forgate3_show="checkbox_check";
			$END3= $END_HH_3.":".$END_II_3;
		}else{
			$forgate3_show="checkbox_blank";
			$END3= "..........";
		}
        $result_title3="ขออนุญาตออกก่อนเวลาปฏิบัติราชการ โดยออกเวลา ".$END3." น.";
        $RTF->cellImage("../images/".$forgate3_show.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title3),"87.3", "left", "0");
        $RTF->close_line();*/
        
        
    }
    
    function show_checkbox_Allow($ABS_REVIEW1_FLAG){
       global $RTF,$font,$FONTSIZE;
        ...
        $RTF->cellImage("../images/".$ABS_REVIEW1.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title1),"97.3", "left", "0");
        $RTF->close_line();

        $RTF->open_line();
        $RTF->color("0");// 0=BLACK
        $result_title2="ไม่รับรองการมาปฏิบัติราชการและเห็นสมควรให้เป็นการลา";
        $RTF->cellImage("../images/".$ABS_REVIEW2.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title2),"97.3", "left", "0");
        $RTF->close_line();

    }
    function show_checkbox_Command($ABS_APPROVE_FLAG){
       global $RTF,$font,$FONTSIZE;
        ...
        $RTF->cellImage("../images/".$ABS_APPROVE1.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title1),"97.3", "left", "0");
        $RTF->close_line();

        $RTF->open_line();
        $RTF->color("0");// 0=BLACK
        $result_title2="ไม่อนุมัติ แต่ให้เป็นการลาตามความเห็นของผู้บังคับบัญชาชั้นต้น";
        $RTF->cellImage("../images/".$ABS_APPROVE2.".jpg", 100, 2.7, "center", 0);
        $RTF->cell(convert2rtfascii($result_title2),"97.3", "left", "0");
        $RTF->close_line();
    ...
    $RTF->color("0");	// 0=BLACK
    $RTF->cell($RTF->bold(1).$RTF->underline(1)."แบบฟอร์มรับรองการไม่บันทึกเวลาเข้า - ออก ปฏิบัติราชการ".$RTF->underline(0).$RTF->bold(0).$RTF->color("0"), "100", "center", "0");
    //$RTF->cell($RTF->bold(1)."แบบฟอร์มรับรองการไม่บันทึกเวลาเข้า - ออก ปฏิบัติราชการ".$RTF->bold(0).$RTF->color("0"), "100", "center", "0");
    $RTF->close_line();
	...
	$cmd = "	select 		a.PER_ID, a.POS_ID, c.POS_NO, d.POEM_NO as EMP_POS_NO, e.POEMS_NO as EMPS_POS_NO, 
						  ...
						  from 		PER_PERSONAL a, PER_PRENAME b, PER_POSITION c, PER_POS_EMP d, 
						  				PER_POS_EMPSER e,PER_LEVEL f,PER_POS_TEMP j
    ...
    $RTF->open_line();			
    $RTF->set_font($font,$FONTSIZE);
    $RTF->cell($RTF->color("0").$RTF->bold(0)."เขียนที่  ".convert2rtfascii($DEPARTMENT_NAME).$RTF->bold(0), "100", "right", "0");
    $RTF->close_line();
    
    $SUBMITTED_DATE="วันที่ ".$SUBMITTED_DATE;
    $RTF->open_line();			
    $RTF->set_font($font,$FONTSIZE);
    $RTF->cell($RTF->color("0").$RTF->bold(0).convert2rtfascii($SUBMITTED_DATE).$RTF->bold(0), "100", "right", "0");
    $RTF->close_line();
    ...
    $RTF->set_font($font,$FONTSIZE);
    $RTF->color("0");	// 0=BLACK
    $RTF->cell(str_repeat(" ", 15)."ข้าพเจ้า  ".convert2rtfascii($PER_NAME)."    ตำแหน่ง  ".convert2rtfascii($DATA_PL_NAME)."".convert2rtfascii($DATA_LEVEL_NAME)."    สังกัด  ".convert2rtfascii($SHOW_ORG_NAME), "100", "left", "0");  
    $RTF->close_line();
    
    show_checkbox_cause('','','',$forgate1,$forgate2,$forgate3,$SCAN_FLAG);
    
    $RTF->open_line();			
    $RTF->set_font($font,$FONTSIZE);
    $RTF->color("0");	// 0=BLACK
    $RTF->cell(convert2rtfascii("ในวันที่ ".$REQUEST_DATE." เนื่องจาก"), "100", "left", "0");  
    $RTF->close_line();
    
    show_checkbox_flag($forgate1,$END_HH_1,$END_II_1,$forgate2,$END_HH_2,$END_II_2,$forgate3,$END_HH_3,$END_II_3,$SCAN_FLAG);
    
    $Title0="จึงเรียนมาเพื่อโปรดพิจารณาให้คำรับรองว่ามาปฏิบัติราชการในวันดังกล่าวด้วย จักขอบคุณยิ่ง";
    $RTF->open_line();			
    $RTF->set_font($font,$FONTSIZE);
    $RTF->color("0");	// 0=BLACK
    $RTF->cell(str_repeat(" ", 15).convert2rtfascii($Title0), "100", "left", "0");  
    $RTF->close_line();
    ...
    $RTF->set_font($font, 16);
    $RTF->color("0");	// 0=BLACK
    $RTF->cell($RTF->bold(1).$RTF->underline(1)."ความเห็นผู้บังคับบัญชา".$RTF->underline(0).$RTF->bold(0).$RTF->color("0"), "100", "left", "0");
    $RTF->close_line();
    show_checkbox_Allow($ABS_REVIEW1_FLAG);
	...
	if($DATA_ALLOW_DATE){
		...
		$RTF->cell(convert2rtfascii("วันที่ ".str_repeat(".", 50)), "50", "center", "0");  
	}
    $RTF->close_line();
    ...
    $RTF->close_line();
    show_checkbox_Command($ABS_APPROVE_FLAG);
