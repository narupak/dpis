...
//	echo "main-->$cmd (laptime=$show_lap [$tdiff]) count_data=$count_data<br>";
	...
	$rpt_order_start_index = 1;	// order ที  0 = COUNTRY ยังไม่ได้คำนวณ เริ่ม order ที่ 1 (MINISTRY) ก่อน
while($data = $db_dpis->get_array()){
							...
							if($MINISTRY_ID != "" && $MINISTRY_ID!=0 && $MINISTRY_ID!=-1){
								...
								if ($f_all) {			
									if(($MINISTRY_NAME !="" && $MINISTRY_NAME !="-") || ($BKK_FLAG==1 && $MINISTRY_NAME !="" && $MINISTRY_NAME !="-")){
										...
										if($rpt_order_index==$rpt_order_start_index) $LEVEL_TOTAL[$tmp_level_no] += $arr_content[$data_count]["level_".$tmp_level_no];
										} // end for
	...
	//							$db_dpis2->show_error();
							...
							$arr_content[$data_count][name] = str_repeat(" ", (($rpt_order_index - $rpt_order_start_index) * 5)) . $EN_NAME;
							for($i=0; $i<count($ARR_LEVEL_NO); $i++){ 
								$tmp_level_no = $ARR_LEVEL_NO[$i];
								$arr_content[$data_count]["level_".$tmp_level_no] = count_person($tmp_level_no, $search_condition, $addition_condition);
								if($rpt_order_index==$rpt_order_start_index) $LEVEL_TOTAL[$tmp_level_no] += $arr_content[$data_count]["level_".$tmp_level_no];
	...
	//							$db_dpis2->show_error();
							...
							$arr_content[$data_count][name] = str_repeat(" ", (($rpt_order_index - $rpt_order_start_index) * 5)) . $EM_NAME;
							for($i=0; $i<count($ARR_LEVEL_NO); $i++){ 
								$tmp_level_no = $ARR_LEVEL_NO[$i];
								$arr_content[$data_count]["level_".$tmp_level_no] = count_person($tmp_level_no, $search_condition, $addition_condition);
								if($rpt_order_index==$rpt_order_start_index) $LEVEL_TOTAL[$tmp_level_no] += $arr_content[$data_count]["level_".$tmp_level_no];
