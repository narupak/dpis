...
//	$arr_rpt_order = array("MINISTRY", "DEPARTMENT", "ORG", "LINE"); 
	...
	function print_header(){
		...
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 2), "", set_format("xlsFmtTableHeader", "", "", "TL", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), "", set_format("xlsFmtTableHeader", "", "", "T", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), "", set_format("xlsFmtTableHeader", "", "", "TR", "0"));
		...
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 2), "รวม", set_format("xlsFmtTableHeader", "", "", "LR", "1"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), "", set_format("xlsFmtTableHeader", "", "", "LR", "1"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), "", set_format("xlsFmtTableHeader", "", "", "LR", "1"));
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				...
				if( trim($ARR_LEVEL_NAME[$LEVEL_NO][1]) ){
					...
					$index++;
					$worksheet->write($xlsRow, $index, "", set_format("xlsFmtTableHeader", "", "", "LBR", "0"));
					$index++;
					$worksheet->write($xlsRow, $index, "", set_format("xlsFmtTableHeader", "", "", "BR", "0"));
				} // if else
			} // loop for
		} // loop for		
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 2), "", set_format("xlsFmtTableHeader", "", "", "LB", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), "", set_format("xlsFmtTableHeader", "", "", "B", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), "", set_format("xlsFmtTableHeader", "", "", "BR", "0"));
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
				$index++;
				$worksheet->write($xlsRow, $index, "ครอง", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
				$index++;
				$worksheet->write($xlsRow, $index, "ว่าง", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
				$index++;
				$worksheet->write($xlsRow, $index, "กรอบ", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
			} // loop for
		} // loop for
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 2), "ครอง", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), "ว่าง", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), "กรอบ", set_format("xlsFmtTableHeader", "", "", "TLBR", "0"));
	} // function		
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		if($company_name){
			...
			for($j=1; $j<=(($TOTAL_LEVEL * 3) + 3); $j++) $worksheet->write($xlsRow, $j, "", set_format("xlsFmtSubTitle", "B", "L", "", "0"));
		} // end if
		...
		for($data_count=0; $data_count<count($arr_content); $data_count++){
			...
			if(array_search($REPORT_ORDER, $arr_rpt_order) !== false  && array_search($REPORT_ORDER, $arr_rpt_order) == 0 && count($arr_rpt_order) > 1){
				$xlsRow++;
				$index = 1;
				$worksheet->write_string($xlsRow, 0, (($NUMBER_DISPLAY==2)?convert2thaidigit($DISPLAY_ORDER):$DISPLAY_ORDER), set_format("xlsFmtTableDetail", "B", "L", "TLRB", 0));
				...
				for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
					...
					for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
						$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
						$index++;            
						$worksheet->write($xlsRow, $index, (${"COUNT_M_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_M_".$LEVEL_NO})):number_format(${"COUNT_M_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));     
						$index++;
						$worksheet->write($xlsRow, $index,  (${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO})):number_format(${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
						$index++;
						$worksheet->write($xlsRow, $index, (${"COUNT_F_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_F_".$LEVEL_NO})):number_format(${"COUNT_F_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
					} // loop for
				} // loop for
				$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 2),($TOTAL_M?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_M)):number_format($TOTAL_M)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));                                
				$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), ($TOTAL_F-$TOTAL_M?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_F-$TOTAL_M)):number_format($TOTAL_F-$TOTAL_M)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), ($TOTAL_F?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_F)):number_format($TOTAL_F)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
			}else{
				$xlsRow++;
				$index = 1;
				$worksheet->write_string($xlsRow, 0, (($NUMBER_DISPLAY==2)?convert2thaidigit($DISPLAY_ORDER):$DISPLAY_ORDER), set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));
				...
				for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
					...
					for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
						$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
						$index++;
						$worksheet->write_string($xlsRow, $index,  (${"COUNT_M_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_M_".$LEVEL_NO})):number_format(${"COUNT_M_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
						$index++;
						$worksheet->write_string($xlsRow, $index,  (${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO})):number_format(${"COUNT_F_".$LEVEL_NO}-${"COUNT_M_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
						$index++;
						$worksheet->write_string($xlsRow, $index, (${"COUNT_F_".$LEVEL_NO}?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format(${"COUNT_F_".$LEVEL_NO})):number_format(${"COUNT_F_".$LEVEL_NO})):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
					} // loop for
				} // loop for
				$worksheet->write_string($xlsRow, (($TOTAL_LEVEL * 3) + 2), ($TOTAL_M?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_M)):number_format($TOTAL_M)):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
				$worksheet->write_string($xlsRow, (($TOTAL_LEVEL * 3) + 3), ($TOTAL_F-$TOTAL_M?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_F-$TOTAL_M)):number_format($TOTAL_F-$TOTAL_M)):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
				$worksheet->write_string($xlsRow, (($TOTAL_LEVEL * 3) + 4), ($TOTAL_F?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($TOTAL_F)):number_format($TOTAL_F)):"-"), set_format("xlsFmtTableDetail", "", "R", "TLRB", 0));
			} // end if
		} // end for
		...
		for($i=0; $i<count($ARR_LEVEL_GROUP); $i++){
			...
			for($j=0; $j<count($ARR_LEVEL[$LEVEL_GROUP]); $j++){
				$LEVEL_NO = $ARR_LEVEL[$LEVEL_GROUP][$j];
				$index++;
				$worksheet->write_string($xlsRow, $index,  ($LEVEL_GRAND_TOTAL["M"][$LEVEL_NO]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($LEVEL_GRAND_TOTAL["M"][$LEVEL_NO])):number_format($LEVEL_GRAND_TOTAL["M"][$LEVEL_NO])):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				$index++;
				$worksheet->write_string($xlsRow, $index, ($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]-$LEVEL_GRAND_TOTAL["M"][$LEVEL_NO]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]-$LEVEL_GRAND_TOTAL["M"][$LEVEL_NO])):number_format($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]-$LEVEL_GRAND_TOTAL["M"][$LEVEL_NO])):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
				$index++;     
				$worksheet->write_string($xlsRow, $index, ($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO]?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO])):number_format($LEVEL_GRAND_TOTAL["F"][$LEVEL_NO])):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 0));
			} // loop for                
		} // loop for
		$worksheet->write_string($xlsRow,  ($GRAND_TOTAL?(($NUMBER_DISPLAY==2)?convert2thaidigit(number_format($GRAND_TOTAL)):number_format($GRAND_TOTAL)):"-"), set_format("xlsFmtTableDetail", "B", "R", "TLRB", 1));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 3), "", set_format("xlsFmtTableDetail", "B", "R", "TLRB", 1));
		$worksheet->write($xlsRow, (($TOTAL_LEVEL * 3) + 4), "", set_format("xlsFmtTableDetail", "B", "R", "TLRB", 1));
