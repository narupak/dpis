...
function list_person($search_condition, $addition_condition){
        ...
        global $search_condition_New ,$search_per_status;
            ...
            if ($DPISDB == "odbc") {
                ...
                if(1==1){
                    ...
                    $cmd="WITH PERSONLIST AS (
                            ...
                            SELECT DISTINCT P.PER_ID,P.LEVEL_NO,P.POH_EFFECTIVEDATE
                              ,(SELECT LEVEL_SEQ_NO FROM PER_LEVEL PL WHERE P.LEVEL_NO=PL.LEVEL_NO) LEVEL_SEQ_NO
                              ,(SELECT POSITION_LEVEL FROM PER_LEVEL PL WHERE P.LEVEL_NO=PL.LEVEL_NO) POSITION_LEVEL
                            FROM PER_POSITIONHIS P
                            WHERE  P.PER_ID IN (SELECT PER_ID FROM PERSONLIST) 
                              AND P.LEVEL_NO IS NOT NULL 
                    ...
                    $cmd="select * from (
                            select p.PER_ID,pn.PN_NAME,PER_NAME,PER_SURNAME
                              ,(select PL_NAME from PER_LINE pl where pl.pl_code=(select pp.pl_code from per_position pp where (pp.pos_id=p.pos_id))) PL_NAME
                              ,(select pl_code from per_line pl where pl.pl_code=(select pp.pl_code from per_position pp where (pp.pos_id=p.pos_id) )) PL_CODE
                              , (select min(x.poh_effectivedate) from per_positionhis x where x.per_id=p.per_id and x.level_no=p.level_no) DATE_CUR_LEVEL
                              ,lvl.POSITION_LEVEL,org.org_name AS SANG_GUD,pos.POS_NO,eduname.EN_SHORTNAME as EDU,maj.EM_NAME AS EDU_MAJOR
                              , (select x.poh_effectivedate from per_positionhis x where x.poh_id=his.poh_id_start) BEG_CUR_LEVEL_DATE
                              , (select x.level_no from per_positionhis x where x.poh_id=his.poh_id_start) BEG_CUR_LEVEL_NO
                              , (select x.poh_effectivedate from per_positionhis x where x.poh_id=his.poh_id_before) BEG_BEFORE_LEVEL_DATE
                              , (select x.level_no from per_positionhis x where x.poh_id=his.poh_id_before) BEG_BEFORE_LEVEL_NO
                              , (SELECT   pt_name from per_type pt where pt.pt_code=(select min(x.pt_code) from per_positionhis x where x.per_id=p.per_id and x.level_no=p.level_no )) AS PTCODE
                              , p.PER_SALARY
                              , (SELECT min(dc_order) from PER_DECORATION  where dc_code in 
                                  (select dc_code from PER_DECORATEHIS dh where dh.per_id=p.per_id) 
                                  and (DC_TYPE = 1 or DC_TYPE = 2)
                            ...
                            left join (
                            SELECT xt.* 
                              ,(select min(poh_id) from per_positionhis x1 
                                where x1.per_id=xt.per_id and
                                  level_no = (select level_no from per_positionhis x2 where x2.poh_id=xt.poh_id_start_tmp)
                                ) poh_id_start
                              ,(SELECT level_seq_no FROM per_level WHERE level_no=
                                (select level_no from per_positionhis where poh_id=xt.poh_id_start_tmp)) cur_level_seq_no

                            FROM
                            (
                            select PER_ID, 
                              --------------------------------------------------------------
                              case when exists (select poh_id from per_positionhis x 
                                          where x.per_id=p.per_id and x.LEVEL_NO=p.LEVEL_NO and poh_effectivedate='2008-12-11')
                              then /* current Level is transfered from the old C system, so use first date in C system */
                              
                                  (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                    (select max(x1.poh_effectivedate) from per_positionhis x1 
                                          where x2.per_id=x1.per_id and x1.poh_effectivedate < '2008-12-11'
                                    )
                                  )
                                  
                              else  /* current Level is promoted in new Tang system, so use first date in current Level */
                                  (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                    (select min(x1.poh_effectivedate) from per_positionhis x1 
                                          where x2.per_id=x1.per_id and x1.LEVEL_NO=p.LEVEL_NO
                                          )
                                  )
                              end poh_id_start_tmp,
                              --------------------------------------------------------------
                              case when exists (select poh_id from per_positionhis x 
                                          where x.per_id=p.per_id and x.LEVEL_NO=p.LEVEL_NO and poh_effectivedate='2008-12-11')
                              then /* current Level is transfered from the old C system, so use first date in C system */
                                  (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                    (select min(x3.poh_effectivedate) from per_positionhis x3 where x3.per_id=x2.per_id and x3.level_no = 
                                      (select max(x4.level_no) from per_positionhis x4 where p.per_id=x4.per_id and x4.poh_effectivedate=
                                        (select max(x1.poh_effectivedate) from per_positionhis x1 
                                              where x2.per_id=x1.per_id and x1.poh_effectivedate < '2008-12-11' and x1.level_no < (
                                                select min(level_no) from per_positionhis where poh_id= (select 
                                                  min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                                  (select max(x1.poh_effectivedate) from per_positionhis x1 
                                                        where x2.per_id=x1.per_id and x1.poh_effectivedate < '2008-12-11'
                                                        )
                                                ))
                                  ...
                                  case when exists (select poh_id from per_positionhis x 
                                              where x.per_id=p.per_id and x.LEVEL_NO=(
                                                (select min(LEVEL_NO) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                                        (select max(x1.poh_effectivedate) from per_positionhis x1 
                                                              where x2.per_id=x1.per_id and x1.LEVEL_NO < p.LEVEL_NO
                                                              )
                                                      )
                                              ) and poh_effectivedate='2008-12-11')
                                  then /* current Level is transfered from the old C system, so use first date in C system */
                                    (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                                      (select min(x3.poh_effectivedate) from per_positionhis x3 where x3.per_id=x2.per_id and x3.level_no = 
                                        (select max(x4.level_no) from per_positionhis x4 where p.per_id=x4.per_id and x4.poh_effectivedate=
                                          (select max(x1.poh_effectivedate) from per_positionhis x1 
                                                where x2.per_id=x1.per_id and x1.poh_effectivedate < '2008-12-11' and x1.level_no = (
                                                  select min(level_no) from per_positionhis where poh_id= 
                                                    (select min(poh_id) from per_positionhis x2 
                                                     where p.per_id=x2.per_id and x2.poh_effectivedate=
                                                          (select max(x1.poh_effectivedate) from per_positionhis x1 
                                                          where x2.per_id=x1.per_id and x1.poh_effectivedate < '2008-12-11'
                                                          )
                                                  )
                                  ...
                                  (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.level_no=
                                    (select max(level_no) from per_positionhis x3 where x3.per_id=x2.per_id and x3.poh_effectivedate =
                                        (select max(x1.poh_effectivedate) from per_positionhis x1 
                                              where x2.per_id=x1.per_id and x1.LEVEL_NO < p.LEVEL_NO
                                          )
                                    )
                              ...
                              ,(SELECT level_seq_no FROM per_level WHERE level_no=(select level_no from per_positionhis where poh_id=(
                              case when exists (select poh_id from per_positionhis x 
                                          where x.per_id=p.per_id and x.LEVEL_NO=p.LEVEL_NO and poh_effectivedate='2008-12-11')
                              then /* current Level is transfered from the old C system, so use first date in C system */
                                  (select min(poh_id) from per_positionhis x2 where p.per_id=x2.per_id and x2.poh_effectivedate=
                              ...
                              from per_personal p
                            )xt) his on (his.per_id=p.per_id)
                            
                            $search_condition_New

                            )
                            order by $oder_by_lis  ";
                }
