<?
	...
	for ($PT_GROUP_N=1;$PT_GROUP_N<=4;$PT_GROUP_N++){
		...
		if ($PT_GROUP_N==4) $PT_GROUP = "('O1','O2','O3','O4')";
		elseif ($PT_GROUP_N==3) $PT_GROUP = "('K1','K2','K3','K4','K5')";
		elseif ($PT_GROUP_N==2) $PT_GROUP = "('D1','D2')";
		elseif ($PT_GROUP_N==1) $PT_GROUP = "('M1','M2')";
		
		$cmd = " select LEVEL_NO, LEVEL_NAME from PER_LEVEL where trim(LEVEL_NO) in $PT_GROUP order by LEVEL_SEQ_NO DESC ";
...
//		$db_dpis2->show_error();
		...
		while($data_dpis2 = $db_dpis2->get_array()){
			...
			for($i=1; $i<=11; $i++){ 
				...
				$cmd = " select count(PER_ID) from PER_COMDTL where COM_ID = $COM_ID and CMD_LEVEL='$level_no' and LEVEL_NO='$LEVEL_NO' ";
				$db_dpis->send_cmd($cmd);
//				$db_dpis->show_error();
				$data = $db_dpis->get_data();
				$count_per_id = $data[0];
				
				$cmd = " select count(PER_ID) from PER_COMDTL where COM_ID = $COM_ID and CMD_LEVEL='$level_no' and LEVEL_NO='$LEVEL_NO' and per_id < 900000000 ";
				$db_dpis->send_cmd($cmd);
				$data = $db_dpis->get_data();
				$count_per = $data[0];
