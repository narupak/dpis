...
//	$arr_rpt_order = array("POSNO"); 
  if($cnt_age){
	...
	function print_header(){
        ...
        $worksheet->write($xlsRow, 26, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
        $worksheet->write($xlsRow, 27, "", set_format("xlsFmtTableHeader", "B", "C", "TLR", 0));
        ...
        $worksheet->write($xlsRow, 26, "(22)", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        $worksheet->write($xlsRow, 27, "(23)", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        ...
        $worksheet->write($xlsRow, 26, "", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        $worksheet->write($xlsRow, 27, "", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        ...
        $worksheet->write($xlsRow, 26, "อายุข้าราชการ", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        $worksheet->write($xlsRow, 27, "อายุราชการ(อายุงาน)", set_format("xlsFmtTableHeader", "B", "C", "LR", 0));
        ...
        $worksheet->write($xlsRow, 26, "", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
        $worksheet->write($xlsRow, 27, "", set_format("xlsFmtTableHeader", "B", "C", "LRB", 0));
	} // function		
...
//	$fname= "../../Excel/tmp/dpis_$token";
	$fname= "../../Excel/tmp/rpt_kpr7_xls";
	$fname1 = $fname.".xls";
	$fnum = 0; $fnumtext="";
				...
				case "POSNO" :
					if($POS_NO != trim($data[POS_NO_NAME]).trim($data[POS_NO])){
						...
						if($rpt_order_index == (count($arr_rpt_order) - 1)){	
                            ...
                            $PER_BIRTHDATE_DIF = date_difference($today,$PER_BIRTHDATE,"full");//อายุข้าราชการ
                            ...
                            $PER_STARTDATE_DIF = date_difference($today,$PER_STARTDATE,"full");//อายุราชการ(อายุงาน)
							...
							if($PER_ID){
								...
								if ($ed) { 
									...
									$EN_NAME =  trim($data2[EN_NAME]);		//ชื่อวุฒ
								} else{
									if($DPISDB=="odbc"){
										$cmd = " select 	a.EDU_TYPE,c.EL_NAME,d.EM_NAME,b.EN_SHORTNAME,b.EN_NAME
														from ( 	
...
//							$PER_MGTSALARY = ($PER_MGTSALARY?number_format($PER_MGTSALARY):"");
                            ...
                            $worksheet->write_string($xlsRow, 26, "$PER_BIRTHDATE_DIF", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));		
                            $worksheet->write_string($xlsRow, 27, "$PER_STARTDATE_DIF", set_format("xlsFmtTableDetail", "", "L", "TLRB", 0));		
...
//	echo "$data_count>>$xls_fname>>$fname1......END<br>";
