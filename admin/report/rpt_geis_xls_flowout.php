...
//	$arr_rpt_order = array("POSNO"); 
	...
	if($DPISDB=="odbc"){
		$cmd = " select		b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 	f.PV_NAME, 
				...
				where a.DEPARTMENT_ID = $DEPARTMENT_ID and PER_TYPE=3 and ((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
		...
		$cmd = " select		b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 	f.PV_NAME, 
											...
											d.LEVEL_NO, PER_SALARY, PER_MGTSALARY, PER_OFFNO, PER_CARDNO, 
											PN_CODE, PER_NAME, PER_SURNAME, MOV_CODE, PER_OCCUPYDATE, 
											PER_POSDATE, PER_RETIREDATE, PER_CONTACT_COUNT, PER_DISABILITY , d.PER_STATUS 
				...
				where		a.DEPARTMENT_ID = $DEPARTMENT_ID and a.ORG_ID=b.ORG_ID and a.EP_CODE=c.EP_CODE and a.POEMS_ID=d.POEMS_ID and PER_TYPE=3 and
									...
									((PER_POSDATE >= '$budget_year_from' and PER_POSDATE <= '$budget_year_to') or 
									(PER_RETIREDATE >= '$budget_year_from' and PER_RETIREDATE <= '$budget_year_to' and PER_POSDATE is NULL))
				order by		b.ORG_ID_REF, $order_by ";
	}elseif($DPISDB=="mysql"){
		$cmd = " select		b.ORG_ID_REF, $select_list, a.POEMS_ID, b.ORG_NAME, c.EP_NAME, 	f.PV_NAME, 
			...
			//		echo("POEMS_NO = ".$POEMS_NO."/data[POEMS_NO] = ".$data[POEMS_NO]."<BR>");
				...
				//		echo("rpt_order_index = ".$rpt_order_index."/arr_rpt_order = ".(count($arr_rpt_order)-1)."<BR>");
						...
						if($rpt_order_index == (count($arr_rpt_order) - 1)){	
                            ...
                            $cmd="select rownum ,MOV_CODE from (select MOV_CODE from PER_POSITIONHIS where per_id = $PER_ID 
                                and MOV_CODE in (select MOV_CODE from PER_MOVMENT where mov_sub_type in('90','91','92','93','94','95'))order by POH_EFFECTIVEDATE DESC) where rownum = 1";
                            $db_dpis3->send_cmd($cmd);
                            $data3 = $db_dpis3->get_array();
							$MOV_CODE = ($data3[MOV_CODE]);
...
<?
	...
	if (count($arr_file) > 0) {
		...
		for($i_file = 0; $i_file < count($arr_file); $i_file++) {
			$grp="3";
			echo "<tr><td width='18%'>data พนักงานราชการ Flow Out</td>";
			$num=1;
