...
//	$search_condition .= (trim($search_condition)?" and ":" where ") . "(trim(e.MOV_CODE) in ('104', '10420') and trim(e.LEVEL_NO) = '$search_level_no')";
	...
	$report_code = "R0308";
	include ("rpt_R003008_format.php");	// กำหนดหัวและตัวแปรสำหรับการเปลี่ยนรูปแบบรายงาน
	...
	function count_year($search_per_gender, $search_condition, $addition_condition){
		...
		if($DPISDB=="odbc"){
			...
			$cmd = " select			a.PER_ID, MIN(SUBSTR(trim(a.PER_STARTDATE), 1, 10)) as PER_STARTDATE, 
							 ...
							 from			PER_PERSONAL a, $position_table b, PER_ORG c, PER_POSITIONHIS e, PER_ORG g
							 where		$position_join(+) and b.ORG_ID=c.ORG_ID(+)
												and a.PER_ID=e.PER_ID(+) and a.DEPARTMENT_ID=g.ORG_ID(+)
												$search_condition
							 group by	a.PER_ID ";
		}elseif($DPISDB=="mysql"){
			$cmd = " select			a.PER_ID, MIN(LEFT(trim(a.PER_STARTDATE), 10)) as PER_STARTDATE, 
...
//	echo "<pre>"; print_r($arr_content); echo "</pre>";
	...
	if($count_data){
		...
		if ($FLAG_RTF)
			$result = $RTF->add_data_tab($arr_data, 7, "TRHBL", $data_align, "", "12", "b", $fmtTableDetail_col_idx, $fmtTableDetail_bgcol_idx);
		else
			$result = $pdf->add_data_tab($arr_data, 7, "RHL", $data_align, "", "12", "b", "000000", "");		//TRHBL
		if (!$result) echo "****** error ****** add data to table at record count = $data_count <br>";
	}else{
		if ($FLAG_RTF)
