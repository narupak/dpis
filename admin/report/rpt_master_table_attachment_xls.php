...
//	echo "<pre>";		print_r($field_list);		echo "</pre>";
	...
	if($DPISDB=="odbc"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], AT_SEQ_NO
							from		$table
							$search_condition
							order by IIF(ISNULL(AT_SEQ_NO), 9999, AT_SEQ_NO),$arr_fields[0] 
					   ";
	}elseif($DPISDB=="oci8"){
		$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], AT_SEQ_NO 
								from		$table
								$search_condition
						order by AT_SEQ_NO,$arr_fields[0] 
					   ";
		}elseif($DPISDB=="mysql"){
			$cmd = "	select		$arr_fields[0], $arr_fields[1], $arr_fields[2], $arr_fields[3], AT_SEQ_NO 
								from		$table
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		while($data = $db_dpis->get_array()){
			...
			$worksheet->insert_bitmap($xlsRow, 3, $$arr_fields[3], 35, 4, 1, 0.6);				
		} // end while
	}else{
		$xlsRow = 0;
