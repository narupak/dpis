...
//	$arr_search_condition[] = "(a.PER_TYPE in (". implode(", ", $search_per_type) .") and (a.POS_ID >= 0 or a.POEM_ID >= 0))";
	...
	//ผู้ดำรงตำแหน่งระดับสูง=>ตำแหน่งประเภทบริหาร (ระดับต้น, ระดับสูง)	/วิชาการ (ระดับเชี่ยวชาญ,ทรงคุณวุฒิ) /อำนวยการ (ระดับสูง)
...
//	echo "default font_id::".$RTF->dfl_FontID."<br>";	
	...
	if($DPISDB=="odbc"){
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								a.PER_SALARY, b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $select_list,m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL
				   from		
				   				(
									(
										(
		...
		$cmd = " select			a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
				   ...
				   group by		a.PER_ID, i.PN_NAME, a.PER_NAME, a.PER_SURNAME, a.LEVEL_NO, f.PL_NAME, h.PM_SEQ_NO, h.PM_NAME,
								...
								b.ORG_ID, b.ORG_ID_1, c.ORG_NAME, $group_by, m.LEVEL_NAME,m.POSITION_TYPE,m.POSITION_LEVEL, m.LEVEL_SEQ_NO
				   order by $order_by, m.LEVEL_SEQ_NO desc, h.PM_SEQ_NO, a.PER_NAME, a.PER_SURNAME, MIN(SUBSTR(trim(j.POH_EFFECTIVEDATE), 1, 10)), a.PER_SALARY desc, 
