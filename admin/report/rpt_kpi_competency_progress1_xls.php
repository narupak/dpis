<?
	...
	$report_title = "รายงานสถานะของการประเมินที่ยังไม่เสร็จของแต่ละบุคคล ของ $DEPARTMENT_NAME";
	$report_title1 = ($search_org_name?"$ORG_TITLE $search_org_name":"");
	$company_name = "ประจำรอบการประเมินครั้งที่ $KF_CYCLE พ.ศ. $KF_YEAR ";
	$report_code = "R_KPI_Competency_Progress2";
...
//	$db_dpis->show_error();
	...
	if($count_data){
		...
		//============ ระดับตำแหน่ง ===========	
			...
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$f_complete=($data2[CF_STATUS] == 1 ? true : false);
			...
			$db_dpis2->send_cmd($cmd);
			$data2 = $db_dpis2->get_array();
			$f_complete=($f_complete && (!$CHIEF_PER_ID || $data2[CF_STATUS] == 1) ? true : false);
			...
			$f_complete=($f_complete && $SUB_SUM_STAT1==$SUB_SUM_ALL ? true : false);
...
//				$cmd = " select PER_ID from PER_KPI_FORM where ORG_ID_KPI=$ORG_ID_KPI ";
			...
			//$cmd = str_replace("PER_ORG", "PER_ORG_ASS", $cmd);
				...
				$f_complete=($f_complete && $FRIEND_SUM_STAT1==$FRIEND_SUM_ALL ? true : false);
			...
			if (!$f_complete) { // ถ้า ไม่ complete ก็สั่งพิมพ์	คือพิมพ์ เฉพาะรายการที่ยังประเมินไม่เสร็จ
				$data_count++;
				$xlsRow = $data_count;
			...
			} // ถ้า ไม่ complete ก็สั่งพิมพ์	
			if ($data_count == $rowprinted) { // พิมพ์ head มาแล้ว $rowprinted บรรทัด
				$xlsRow = $rowprinted+1;
