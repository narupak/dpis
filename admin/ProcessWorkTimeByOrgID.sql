...
Update note:
	...
	...
	20190102 - V.50 Change Logic for use only date part (10 chars) of PER_HOLIDAR.HOL_DATE 
           - Add SystemConfig "P_EXPIATE"; N-ไม่ชดเวลา (หน่วยงานราชการ) ใช้+แสดงค่าตาม logic เดิม (ทอนเวลาเป็น 8.30 ใช้เวลา สิ้นสุดตามรอบ (15.30,16.30) Y-ชดเวลา (กรมธุรกิจพลังงาน) ให้ใช้+แสดงค่าที่สแกนจริง
*/
...
(
  ...
  select TO_DATE(HOL_DATE,'YYYY-MM-DD HH24:MI:SS') HOL_DATE 
    from PER_HOLIDAY where HOL_DATE >= substr(:BEGINDATEAT,1,10) and 
                           substr(HOL_DATE,1,10) < TO_CHAR(LAST_DAY(to_date(:TODATEAT, 'YYYY-MM-DD HH24:MI:SS'))+1,'YYYY-MM-DD')
  union
  select DayList from (
  ...
  select * from (
    ...
    where WORK_DATE not in (
      select to_date(HOL_DATE,'YYYY-MM-DD') from PER_HOLIDAY 
        where HOL_DATE >= substr(:BEGINDATEAT,1,10) and substr(HOL_DATE,1,10) <= substr(:TODATEAT,1,10)
    )
...
/* NOT USE  
  ,case when pwc.WorkCycle_Type=1 then
      TRUNC(asd.WORK_DATE)+((to_number(substr(pwc.WC_Start,1,2))*60)+to_number(substr(pwc.WC_Start,3,2))+pwc.MIN_LEAVEAFTER)/1440+(59/86400) 
              ...
              --NULL
              TRUNC(asd.WORK_DATE)+((to_number(substr(pwc.WC_Start,1,2))*60)+to_number(substr(pwc.WC_Start,3,2))+(3.5*60))/1440+(59/86400) 
        end
    end LEAVEAFTER 
                      ...
                      and (a.END_DATE is null or to_char(x.WORK_DATE,'YYYY-MM-DD') <= a.END_DATE)
                       ...
                       then (case when substr(x.ABSENT,1,1) = 1 then (--2  /* La in this morning */
                                                          case when x.SCAN_ENTTIME > nvl(LEAVEEARLY, 
                                                                    ...
                                                                    then --2
                                                                      case when exists (select substr(REMARK,1,100) from TA_SET_EXCEPTPER a where a.per_id=x.per_id and CANCEL_FLAG=1 and flag_print=1
                                                                                    and to_char(x.WORK_DATE,'YYYY-MM-DD') >= a.START_DATE
                                                                                    and (a.END_DATE is null or to_char(x.WORK_DATE,'YYYY-MM-DD') <= a.END_DATE)
                                                                                  ) then 5 else 2 end
                                                                else (--0
...
/*        
        ...
        ,x.EXIT_TIME
*/        
     ,(select substr(REMARK,1,100) from TA_SET_EXCEPTPER a where a.per_id=x.per_id and CANCEL_FLAG=1 and flag_print=1
                      and to_char(x.WORK_DATE,'YYYY-MM-DD') >= a.START_DATE
                      and (a.END_DATE is null or to_char(x.WORK_DATE,'YYYY-MM-DD') <= a.END_DATE)
                    ) REMARK
...
/*    2, null,null,null,null,null */
     ,(select substr(REMARK,1,100) from TA_SET_EXCEPTPER a where a.per_id=p.per_id and CANCEL_FLAG=1 and flag_print=1
                      and to_char(asd.WORK_DATE,'YYYY-MM-DD') >= a.START_DATE
                      and (a.END_DATE is null or to_char(asd.WORK_DATE,'YYYY-MM-DD') <= a.END_DATE)
                    ) REMARK
