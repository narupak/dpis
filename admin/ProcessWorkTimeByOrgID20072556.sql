/*  ProcessWorkTimeByOrgID.sql
  6 parameters:
	- :CONTROL_ID (fix control_id from PER_WORK_TIME_CONTROL)  e.g 3446 (TopMan)
	- :ORG_ID (First ORG_ID for list all PER_ID)  e.g 3446 (TopMan) Note: ORD_ID get from assignment structure (PER_ORG_ASS)
	- :BEGINDATEAT	(must be first date of each month; e.g.  '2016-04-01 00:00:00')
	- :TODATEAT	   (must be last date of each month; e.g.  '2016-04-30 23:59:59')
...
*/
  ...
  select tr.PER_ID, to_date(REQUEST_DATE, 'YYYY-MM-DD') WORK_DATE,
     case when (exists (select null from PER_ABSENTHIS pa where rownum=1 and pa.PER_ID=tr.per_id and trim(ab_code) not in ('10','13') and
      ...
      case when tr.wc_code='-1' then
        GREATEST(
          nvl((select to_date(tr.REQUEST_DATE, 'YYYY-MM-DD')+pwc.NextDay_LeaveAfter+
           ...
           (select min(TIME_STAMP)+(4/24) from AllScan where AllScan.per_id=tr.PER_ID and AllScan.WORK_DATE=to_date(REQUEST_DATE, 'YYYY-MM-DD'))
           )
           ,
           (select min(TIME_STAMP)+(4/24) from AllScan where AllScan.per_id=tr.PER_ID and AllScan.WORK_DATE=to_date(REQUEST_DATE, 'YYYY-MM-DD'))
         )
  ...
  where tr.per_id  in (select PER_ID from PersonInDepart) and APPROVE_FLAG=1 and CANCEL_FLAG=0 and Req_Flag=1 and
  ...
  from TA_REQUESTTIME tr
  where tr.per_id  in (select PER_ID from PersonInDepart) and APPROVE_FLAG=1 and CANCEL_FLAG=0 and End_Flag=1 and
    REQUEST_DATE between substr(:BEGINDATEAT,1,10) and substr(:TODATEAT,1,10)
  ...
  from TA_REQUESTTIME tr
  where tr.per_id  in (select PER_ID from PersonInDepart) and APPROVE_FLAG=1 and CANCEL_FLAG=0 and Start_Flag=1 and
    REQUEST_DATE between substr(:BEGINDATEAT,1,10) and substr(:TODATEAT,1,10)
)
,AllScanDay As
(
  select distinct PER_ID, WORK_DATE
  from AllScanAdjust 
...
(
  select * from (
    select x.*,TO_CHAR(x.WORK_DATE, 'DY', 'NLS_DATE_LANGUAGE=ENGLISH') DayListDay
    from (
...
,ProcessDayScan As
(
select asd.PER_ID, asd.WORK_DATE, asa.WC_CODE
...
/* NOT USE  
  ,case when pwc.WorkCycle_Type=1 then
      TRUNC(asd.WORK_DATE)+((to_number(substr(pwc.WC_Start,1,2))*60)+to_number(substr(pwc.WC_Start,3,2))+pwc.MIN_LEAVEAFTER)/1440+(59/86400) 
              ...
              --NULL
              TRUNC(asd.WORK_DATE)+((to_number(substr(pwc.WC_Start,1,2))*60)+to_number(substr(pwc.WC_Start,3,2))+(4*60))/1440+(59/86400) 
        end
    end LEAVEAFTER 
...
*/  
  ...
  ,((
        case when (exists (select null from PER_ABSENTHIS pa where pa.PER_ID=asd.per_id and trim(ab_code) not in ('10','13')and 
                      pa.abs_startdate < cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate > cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))) then 30
                else 
                    ((select nvl(sum(case when abs_startperiod=2 then 2 else abs_startperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=asd.per_id and trim(ab_code) not in ('10','13') and 
                    ...
                    +
                      (select nvl(sum(case when abs_startperiod=2 then 2 else abs_startperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=asd.per_id and trim(ab_code) not in ('10','13')and 
                       pa.abs_startdate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate > cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))
                    +
                      (select nvl(sum(case when abs_endperiod=2 then 2 else abs_endperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=asd.per_id and trim(ab_code) not in ('10','13')and 
                       pa.abs_startdate < cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))
                     )
           end  
...
where asd.per_id  in (select PER_ID from PersonInDepart) 
)
,FinalProcessDayScan As
(
  select distinct * from ProcessDayScan where (SCAN_ENTTIME is not NULL or SCAN_EXITTIME is not NULL)
    and not exists (select null from PER_WORK_TIME a
                        WHERE a.CONTROL_ID=:CONTROL_ID and a.PER_ID=ProcessDayScan.PER_ID and a.WORK_DATE=ProcessDayScan.WORK_DATE 
                          and a.WC_CODE=ProcessDayScan.WC_CODE
                    )
)

select * from (
  select :CONTROL_ID CONTROL_ID,x.PER_ID,x.WORK_DATE WORK_DATE,x.WC_CODE--,x.SCAN_ENTTIME ENTTIME
  ,(case when (x.HOLIDAY = 1 or x.SCAN_ENTTIME <= x.STAMP_TIME or x.SCAN_ENTTIME > 
    (case when /*WorkCycle_Type=2 and*/ WorkCycle_Type=2 and pwl.Late_Time is not null then x.WORK_DATE+((to_number(substr(pwl.Late_Time,1,2))*60)+
...
--  , x.SCAN_EXITTIME EXITTIME
  , case when x.SCAN_EXITTIME is not null and substr(x.ABSENT,1,1) = 1 and x.SCAN_EXITTIME < nvl(LEAVEEARLY, 
                                                                                          (case when pwl.Late_Time is not null then x.WORK_DATE+((to_number(substr(pwl.Late_Time,1,2))*60)+
                                                                                                    to_number(substr(pwl.Late_Time,3,2)))/1440+(59/86400)+(4/24)
                                                                                                else (x.ON_TIME +(4/24)) /* Count HOUR */
                                                                                           end)
                                                                                         )
                                                                              then x.EXIT_TIME
                                                                              else x.SCAN_EXITTIME end
     EXITTIME
  ...
  , case when x.HOLIDAY = 1 or substr(x.ABSENT,1,1) = 3 then 0 else 
      case when x.SCAN_ENTTIME is null then --4
        ...
        else (case when x.SCAN_ENTTIME > (case when /*WorkCycle_Type=2 and*/ WorkCycle_Type=2 and pwl.Late_Time is not null then x.WORK_DATE+((to_number(substr(pwl.Late_Time,1,2))*60)+
                                                          to_number(substr(pwl.Late_Time,3,2)))/1440+(59/86400) 
                                                                                    else x.ENDLATETIME 
                                                          ...
                                                          case when x.SCAN_ENTTIME > nvl(LEAVEEARLY, 
                                                                    ...
                                                                    case when x.SCAN_EXITTIME < trunc(x.EXIT_TIME,'MI')  and x.SCAN_EXITTIME > nvl(LEAVEEARLY, 
                                                                              ...
                                                                              then 3
                                                                                ...
                                                                                else (case when (x.WC_CODE=-1 and (trunc(x.SCAN_EXITTIME,'MI') - trunc(x.SCAN_ENTTIME,'MI'))<((4*60)/1440)) then 3 else 0 end) end)
                                                                    end
                                                                    )
                                                          end
                                                        )
                                          ...
                                          case when x.SCAN_EXITTIME < trunc(x.EXIT_TIME,'MI')
                                                  ...
                                                  case when x.WC_CODE=-1 then
                                                    case when x.EXIT_TIME < x.Stamp_Time then  -- Time_LeaveAfter = 0000
                                                        case when x.SCAN_EXITTIME < (x.SCAN_ENTTIME + (4/24)) 
                                                          then 3 
                                                          else 0 
                                                        end
                                                      else 
...
*/        
  ...
  union
  select :CONTROL_ID,p.PER_ID,asd.WORK_DATE
    ,nvl((select WC_CODE from PER_WORK_CYCLEHIS where PER_ID=p.PER_ID
            and asd.WORK_DATE between to_date(START_DATE, 'YYYY-MM-DD hh24:mi:ss') and 
                to_date(nvl(END_DATE,:TODATEAT),'YYYY-MM-DD hh24:mi:ss')),'02') 
    WC_CODE 
    ,null,null,0 HOLIDAY
    ,((
        case when (exists (select null from PER_ABSENTHIS pa where pa.PER_ID=p.per_id and trim(ab_code) not in ('10','13')and 
                    ...
                    ((select nvl(sum(case when abs_startperiod=2 then 2 else abs_startperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=p.per_id and trim(ab_code) not in ('10','13') and 
                      pa.abs_startdate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))
                    +
                      (select nvl(sum(case when abs_startperiod=2 then 2 else abs_startperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=p.per_id and trim(ab_code) not in ('10','13')and 
                       pa.abs_startdate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate > cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))
                    +
                      (select nvl(sum(case when abs_endperiod=2 then 2 else abs_endperiod*10 end),0) from PER_ABSENTHIS pa where pa.PER_ID=p.per_id and trim(ab_code) not in ('10','13')and 
                       pa.abs_startdate < cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)))
                     )
           end  
...
/*    2, null,null,null,null,null */
  ...
  left join AllWorkDay asd on not exists (select null from FinalProcessDayScan a where a.per_id=p.PER_ID and a.WORK_DATE=asd.WORK_DATE)
  where asd.WORK_DATE is not null and
	not exists (select null from TA_SET_EXCEPTPER a where a.per_id=p.per_id and CANCEL_FLAG=1 and flag_print=0
          ...
          and not exists (select null from PER_WORK_TIME a
                          WHERE a.CONTROL_ID=:CONTROL_ID and a.PER_ID=p.PER_ID and a.WORK_DATE=asd.WORK_DATE 
                            and a.WC_CODE=nvl((select WC_CODE from PER_WORK_CYCLEHIS where PER_ID=p.PER_ID
                                and asd.WORK_DATE between to_date(START_DATE, 'YYYY-MM-DD hh24:mi:ss') and 
                                    to_date(nvl(END_DATE,:TODATEAT),'YYYY-MM-DD hh24:mi:ss')),'02')
                      )
...
/*                    
          and (
          case when (exists (select null from PER_ABSENTHIS pa where rownum=1 and pa.PER_ID=p.PER_ID and trim(ab_code) not in ('10','13') and
                pa.abs_startdate <= cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate >= cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19))))  then 3
          else 
              nvl(
                (select abs_startperiod from PER_ABSENTHIS pa where rownum=1 and pa.PER_ID=p.PER_ID and trim(ab_code) not in ('10','13') and
                pa.abs_startdate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD')as char(19))),
                   (
                      nvl((select abs_startperiod from PER_ABSENTHIS pa where rownum=1 and pa.PER_ID=p.PER_ID and trim(ab_code) not in ('10','13') and
                            pa.abs_startdate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate > cast(to_char(asd.WORK_DATE,'YYYY-MM-DD')as char(19))),
                          nvl((select abs_endperiod from PER_ABSENTHIS pa where rownum=1 and pa.PER_ID=p.PER_ID and trim(ab_code) not in ('10','13') and
                            pa.abs_startdate < cast(to_char(asd.WORK_DATE,'YYYY-MM-DD') as char(19)) and pa.abs_enddate = cast(to_char(asd.WORK_DATE,'YYYY-MM-DD')as char(19))),
                            0)
                        )
                  )
