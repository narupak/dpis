...
//							$db_dpis2->show_error();
	...
	//$data = '<chart caption="Business Results 2005 v 2006" xAxisName="Month" yAxisName="Revenue" showValues="0" numberPrefix="$" useRoundEdges="1" legendBorderAlpha="100" showBorder="0" bgColor="FFFFFF,FFFFFF">';

	for($data_count=1; $data_count<count($arr_content); $data_count++) {
		$category_node .= "<category label='".$arr_content[$data_count][name]."' /> ";
		$data1_node .= "<set value='".$arr_content[$data_count][count_1]."' /> ";
		$data2_node .= "<set value='".$arr_content[$data_count][count_2]."' /> ";
		$data3_node .= "<set value='".$arr_content[$data_count][count_3]."' /> ";
	}
	// Close series node
	$data = "<categories>$category_node</categories>";
	$data .= "<dataset seriesName='ทั่วไป' color='0080C0' anchorBorderColor='0080C0'>$data1_node</dataset>";
	$data .= "<dataset seriesName='วิชาชีพเฉพาะ' color='008040' anchorBorderColor='008040'>$data2_node</dataset>";
	$data .= "<dataset seriesName='บริหาร' color='808080' anchorBorderColor='808080'>$data3_node</dataset>";

	$data .= "<chart palette='2' caption='กราฟแสดงจำนวนตำแหน่ง' subCaption='In Thousands' numdivlines='9' showValues='0' showAlternateVGridColor='1' useRoundEdges='1'>".$data."</chart>";
