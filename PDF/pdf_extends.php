...
//					echo "MthaiCut sub_s [$ii]=".$sub_s[$ii]." ($w::$str_w) $h,$txt,$border,$align,$fill, ".($f_print?"print":"")."<br>";
			...
			function thaiWrapPDF($dataIn, $delim){
				$specword = array("ราชการ", "รัฐบาล", "อธิบดี", "คณะบดี", "ตำแหน่ง", "อำเภอ","จังหวัด","ปกครอง","ปฏิบัติ","สัญญา","เจ้าหน้าที่", "อยู่", "และ", "เกียรติยศ","บรรจุ", "ดำรง", "บำเหน็ด", "โรงเรียน", "มหาบัณฑิต", "บัณฑิต", "บริหาร", "ชำนาญ", "ศาสน", "ศาสตร", "พัฒนา", "ภาพ", "วันที่", "เลขที่", "ประเภท", "นโยบาย", "นักวิเคราะห์", "กำหนด", "วัน", "เดือน", "ปี", "เข้า", "สู่", "อายุ"    ,"นักวิชาการ","ตรวจสอบ","ภายใน"); // เพิ่มคำเฉพาะได้ที่นี่
...
//				echo "head_text=$head_text<br>";
				...
				if (!$COLUMN_FORMAT) {	// ถ้าส่งค่าการปรับแต่ง รายงานเข้ามา
					...
					$this->arr_column_width = $heading_width;	// ความกว้าง
					$this->arr_column_align = $heading_align;		// align
				} else {
...
//				echo "map=".implode(",",$this->arr_column_map)."  sel=".implode(",",$this->arr_column_sel)."  width=".implode(",",$this->arr_column_width)."  align=".implode(",",$this->arr_column_align)."<br>";
...
//					echo "cnt_line_head=$cnt_line_head, cnt_column_head=$cnt_column_head  (f_no_head=".$this->f_no_head.")<br>";
					if ($cnt_column_head > 0) {	// ถ้า มีค่า column เข้ามาถูกต้อง ทำงานต่อ
						for($jjj = 0; $jjj < $cnt_column_head; $jjj++) {
							$head_t_w+=$this->arr_head_width[$jjj];
							$arr_setborder[$jjj] = "";
						}
...
//												echo "check last record grp($columngrp) $iii == $cnt_column_show-1<br>";
												if ($iii == $cnt_column_show-1) { // ตัวสุดท้ายที่แสดงเป็น merge group column
...
//						echo "fill:$r $g $b<br>";
	...
	//				echo "font=".$font."<br>";
					...
					$cnt_column_show = 0;
					for($jjj = 0; $jjj <= count($this->arr_tab_head); $jjj++)  if ($this->arr_column_sel[$jjj]==1) $cnt_column_show+=(int)$this->arr_column_width[$jjj];	
...
//										$arr_b = $this->MultiCellThaiCut($arr_msum_grp[$grpidx], (int)$line_height, $arr_text[$iii], $border, $data_align[$iii],$f_fill, false);
										$arr_b = $this->MultiCellThaiCut($arr_msum_grp[$grpidx], (int)$line_height, $arr_text[$iii], $border, $arr_column_align[$iii],$f_fill, false);
	...
	//									echo "$iii - image name=".$arr_rows_image[$iii].", text=$ntext<br>";
							...
							$arr_col_align[] = $this->arr_column_align[$iii];
							$cnt++;
//							echo "just show ($cnt)-$ntext | ".$this->arr_column_width[$iii]." | ".$this->arr_column_align[$iii]."<br>";
							$last_iii = $iii;
						} // if ($this->arr_column_sel[$iii]==1) {	// 1 = แสดง column นี้
