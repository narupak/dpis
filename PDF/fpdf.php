...
function FPDF($orientation='P', $unit='mm', $format='A4', $lang_code, $company_name, $report_title, $report_code,$heading,$heading_width,$heading_align,$heading_border,$report_footer,$NUMBER_DISPLAY)
{
	//Some checks
	$this->_dochecks();
	...
	// Additional properties - modified by developer
	$this->lang_code = $lang_code;
	$this->company_name =$company_name;
	$this->report_title = $report_title;
	$this->report_code = $report_code;
	$this->heading = $heading;
	$this->heading_width = $heading_width;
	$this->heading_align = $heading_align;
	$this->heading_border=$heading_border;
...
//	echo "addpage font = ".$this->FontFamily." size=".$this->FontSizePt."<br>";
	$lw=$this->LineWidth;
	$dc=$this->DrawColor;
	$fc=$this->FillColor;
	$tc=$this->TextColor;
...
function SetFont($family,$style='',$size=0,$loop=0)
{
	//	echo "family=$family, style=$style, size=$size, loop=$loop<br>";
	if ($family) {
		...
		if(!isset($this->fonts[$fontkey]))
		{
			...
			} else {	// ถ้า Undefined font คือไม่เจอ font ชื่อนี้ ให้เช็คกรณี เป็นชื่อ fontb
				if (!isset($loop) || !$loop) {	// ถ้าไม่มีการส่งค่า loop เข้ามา แปลว่าเป็นรอบแรก
					// อาจถูกส่งเข้ามาเป็น fontb (ชื่อติด bold) ร่วมกับ style "b" จะทำให้ชื่อ มี bb ซ้อนกัน กลายเป็นไม่มีชื่อ font นี้
					$style1 = str_replace("b","",$style);
					$this->SetFont($family,$style1,$size,1);	// ส่งชื่อตัด bold เข้าไป แล้วให้ style เป็น "b" แทน
				} else {
...
//		echo "set font=".$this->FontFamily." size=".$this->FontSizePt."<br>";
		$this->CurrentFont=&$this->fonts[$fontkey];
		if($this->page>0)
			$this->_out(sprintf('BT /F%d %.2f Tf ET',$this->CurrentFont['i'],$this->FontSizePt));
	}// else echo "**** not font name send in<br>";
}
	...
	while($i<$nb)
	{
		...
		if($c=="\n")
		{
			...
			if($this->ws>0)
			{
				$this->ws=0;
				$this->_out('0 Tw');
//				echo "1.pdf->y:".$pdf->y."<br>";
			}
			$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
//			echo "1.pdf->y:".$this->y."<br>";
//			$this->pagebreak1();
			$i++;
			$sep=-1;
			$j=$i;
		...
		if($l>$wmax)
		{
			...
			if($sep==-1)
			{
				...
				if($this->ws>0)
				{
					$this->ws=0;
					$this->_out('0 Tw');
//					echo "2.pdf->y:".$pdf->y."<br>";
				}
				$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
//				echo "2.pdf->y:".$this->y."<br>";
//				$this->pagebreak1();
			}
			else
			{
...
//					echo "3.pdf->y:".$pdf->y."<br>";
				}
				$this->Cell($w,$h,substr($s,$j,$sep-$j),$b,2,$align,$fill);
//				echo "3.pdf->y:".$this->y."<br>";
//				$this->pagebreak1();
				$i=$sep+1;
			}
			$sep=-1;
	...
	if($this->ws>0)
	{
		$this->ws=0;
		$this->_out('0 Tw');
//		echo "4.pdf->y:".$pdf->y."<br>";
	}
	if($border && strpos($border,'B')!==false)
		$b.='B';
	$this->Cell($w,$h,substr($s,$j,$i-$j),$b,2,$align,$fill);
//	echo "4.pdf->y:".$this->y."<br>";
//	$this->pagebreak1();
	$this->x=$this->lMargin;	
}

	function pagebreak1() {
			if(($this->h - $this->y - 10) < 10){ 
					$this->AddPage();
					$max_y = $this->y;
			} // end if
		...
		case 'I':
			...
			if(php_sapi_name()!='cli')
			{
				...
				header('Content-Type: application/pdf');
                                header('Cache-Control: must-revalidate, post-check=0, pre-check=0');//add new
				if(headers_sent())
...
function _putpages()
{
	global $NUMBER_DISPLAY;	// เพิ่มเพื่อการ check ว่าจะให้พิมพ์ตัวเลขไทยหรือไม่ ==2
	$nb=$this->page;
	if(!empty($this->AliasNbPages))
	{
		//Replace number of pages
		for($n=1;$n<=$nb;$n++) {
			// เพิ่มเพื่อให้สามารถพิมพ์ ตัวเลขไทยได้
			$nn=($NUMBER_DISPLAY==2?convert2thaidigit($nb):$nb);
			$this->pages[$n]=str_replace($this->AliasNbPages,$nn,$this->pages[$n]); // แก้ไขจาก $nb เป็น $nn
